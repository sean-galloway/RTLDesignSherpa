...
class A7DDRPHYStandalone(Module):
    ...
    def __init__(self, platform):
        ...
        # Clock domains the PHY consumes. nphases=2 -> sys2x/sys2x_dqs;
        # nphases=4 -> sys4x/sys4x_dqs (LiteDRAM ddr_clk = sys{2,4}x by nphases).
        self.clock_domains.cd_sys   = ClockDomain("sys")
        self.clock_domains.cd_sys2x = ClockDomain("sys2x")
        _hi = "sys2x" if NPHASES == 2 else "sys4x"
        setattr(self, "cd_hi",     ClockDomain(_hi))
        setattr(self, "cd_hi_dqs", ClockDomain(_hi + "_dqs"))
        self.clock_domains.cd_hi     = getattr(self, "cd_hi")
        self.clock_domains.cd_hi_dqs = getattr(self, "cd_hi_dqs")
...
def collect_ios(top):
    ios = set()
    for cd in (top.cd_sys, top.cd_sys2x, top.cd_hi, top.cd_hi_dqs):
        ios.add(cd.clk)
        ios.add(cd.rst)
...
def main():
    ...
    ap.add_argument("--nphases", type=int, default=4)
    ap.add_argument("--sys-clk", type=float, default=100e6)
    ...
    global NPHASES, SYS_CLK_FREQ
    NPHASES = args.nphases; SYS_CLK_FREQ = args.sys_clk
