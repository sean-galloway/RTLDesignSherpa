...
# Purpose: Testbench for apb4_slave_wavedrom
# Subsystem: framework
#
# Extracted from val/amba/test_apb4_slave_wavedrom.py so the runner holds only the parameter grid and the
...
class ComprehensiveAPBSlaveTB(TBBase):
    ...
    def apb_transaction_callback(self, transaction):
        ...
        self.apb4_monitor_queue.append(transaction)
    ...
    async def send_apb_transaction(self, is_write, addr, data=None, strobe=None):
        ...
        await self.apb4_master.send(xmit_transaction)
        await self.wait_for_queue_empty(self.apb4_master, timeout=10000)
        await self.wait_clocks('pclk', 3)  # Small delay for response processing

        return xmit_transaction

    def set_randomizer_config(self, apb4_master_config=None, apb4_slave_config=None, axi_config=None):
        """Set randomizer configurations for all components."""
        if apb4_master_config:
            self.apb4_master.set_randomizer(FlexRandomizer(apb4_master_config))
            self.test_stats['configurations_tested'].add(f"apb4_master_{apb4_master_config}")

        if apb4_slave_config:
            ...
            self.test_stats['configurations_tested'].add(f"apb4_slave_{apb4_slave_config}")
    ...
    async def generate_all_wavedrom_scenarios(self):
        ...
        self.set_randomizer_config(
            apb4_master_config=APB_MASTER_RANDOMIZER_CONFIGS['fixed'],
        ...
        for apb_config, axi_config, config_desc in test_matrix:
            for seq_name, seq_generator in test_sequences:
                ...
                try:
                    # Set configuration
                    self.set_randomizer_config(
                        apb4_master_config=APB_MASTER_RANDOMIZER_CONFIGS[apb_config],
    ...
    async def run_error_injection_tests(self):
        ...
        try:
            ...
            self.set_randomizer_config(
                apb4_master_config=APB_MASTER_RANDOMIZER_CONFIGS['slow_master'],
