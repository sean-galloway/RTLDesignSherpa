...
# Purpose: Testbench for apb4_slave_cdc
# Subsystem: framework
#
# Extracted from val/amba/test_apb4_slave_cdc.py so the runner holds only the parameter grid and the
...
class APBSlaveCDCTB(TBBase):
    ...
    async def send_apb_write_read_pair(self, addr, data):
        ...
        await self.wait_for_queue_empty(self.apb4_master)
    ...
    async def run_cdc_comprehensive_test(self):
        ...
        self.log.info(f"GAXI response generation (pclk): {'OK' if gaxi_rsp_working else 'FAIL'}")
