"""Generic testing for a skid buffer"""
import os
import random
...
class GenRegSliceTB(TBBase):
    def __init__(self, dut):
        super().__init__(dut)
        self.DATA_WIDTH = self.convert_to_int(os.environ.get('PARAM_DATA_WIDTH', '0'))
        self.SKID_DEPTH = self.convert_to_int(os.environ.get('PARAM_SKID_DEPTH', '2'))
    ...
    def clear_interface(self):
        self.dut.data_in_val.value = 0
        self.dut.data_in_dat.value = 0
        self.dut.data_out_rdy.value = 0
    ...
    async def write_fifo(self, data, count, pause_duration):
        ...
        while data_sent < count:
            ...
            if crand == 0:
                self.dut.data_in_val.value = 1
                idx = data_sent
                self.dut.data_in_dat.value = data[idx]
                await FallingEdge(self.dut.clk)

                while self.dut.data_in_rdy.value == 0:  # Wait until o_ready is asserted
                    await FallingEdge(self.dut.clk)
                    timeout_counter += 1

                    # Check for timeout
                    if timeout_counter >= self.TIMEOUT_CYCLES:
                        self.log.error("Timeout waiting for data_in_rdy to assert during write")
                        self.dut.data_in_val.value = 0  # Ensure i_valid is de-asserted on timeout
                        self.dut.data_in_dat.value = 0
                ...
                msg = f'---> data_in_val remains de-asserted for {crand} cycles'
                self.log.info(msg)
                self.dut.data_in_val.value = 0
                self.dut.data_in_dat.value = 0
                await self.wait_clocks('clk', crand)
        ...
        await self.wait_clocks('clk', pause_duration)
    ...
    async def read_fifo(self, count, pause_duration):
        ...
        while data_read < count:
            ...
            # Prepare the data and assert i_valid if crand permits
            if crand == 0:
                self.dut.data_out_rdy.value = 1
                await FallingEdge(self.dut.clk)
                while self.dut.data_out_val.value == 0:
                    await FallingEdge(self.dut.clk)
                    timeout_counter += 1
                    if timeout_counter >= self.TIMEOUT_CYCLES:
                        self.log.error("Timeout during read, waiting for data_out_rdy to assert")
                        return  read_values # Exit the method on timeout
                read_data = int(self.dut.data_out_dat.value)
                ...
                msg = f'---> data_out_rdy remains de-asserted for {crand} cycles'
                self.log.info(msg)
                self.dut.data_out_rdy.value = 0
                await self.wait_clocks('clk', crand)
