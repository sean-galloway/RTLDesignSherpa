...
Q32_ROOT = SCRIPT_DIR.parent
sys.path.insert(0, str(Q32_ROOT))
...
def generate_text_report(coverage_data: Dict, output_dir: str, project_name: str = "Project") -> str:
    """Generate enhanced text coverage report with FUB breakdown and missed scenarios"""
    import os
    os.makedirs(output_dir, exist_ok=True)
    ...
    # FUB-BY-FUB BREAKDOWN TABLE
    lines.append("FUB-BY-FUB COVERAGE SUMMARY")
    lines.append("-" * 80)
    lines.append(f"{'Module':<30} {'Covered':>8} {'Total':>8} {'Percent':>10} {'Status':<12}")
    lines.append("-" * 80)

    # Sort modules by coverage percentage (worst first)
    module_data = []
    for model_name, model_data in coverage_data.get('models', {}).items():
        pct = model_data.get('coverage_pct', 0)
        module_data.append({
            'name': model_name,
            'covered': model_data.get('covered_bins', 0),
    ...
    module_data.sort(key=lambda x: x['pct'])
    
    for mod in module_data:
        pct = mod['pct']
        status = "EXCELLENT" if pct >= 80 else "GOOD" if pct >= 60 else "FAIR" if pct >= 40 else "POOR"
        lines.append(f"{mod['name']:<30} {mod['covered']:>8} {mod['total']:>8} {pct:>9.1f}% {status:<12}")

    lines.append("=" * 80)
    lines.append(f"{'TOTAL':<30} {covered_bins:>8} {total_bins:>8} {overall_pct:>9.1f}%")
    lines.append("")
    ...
    lines.append("DETAILED MODULE BREAKDOWN")
    lines.append("=" * 80)
    ...
    for model_name, model_data in coverage_data.get('models', {}).items():
        ...
        if missed:
            lines.append(f"  MISSED ({len(missed)} bins):")
            for key, count in sorted(missed):
                lines.append(f"    - {key}")
            lines.append("")

        if covered:
            lines.append(f"  COVERED ({len(covered)} bins):")
...
Examples:
    ...
    # Resolve paths
    output_dir = os.path.join(Q32_ROOT, args.output)
    fub_dir = os.path.join(Q32_ROOT, args.fub_dir)
    macro_dir = os.path.join(Q32_ROOT, args.macro_dir)
    ...
    if args.merge_verilator:
        ...
        if os.path.exists(rtl_dir):
            ...
            if result:
                ...
                rtl_sources = args.rtl_sources or [os.path.join(Q32_ROOT, 'rtl')]
