...
#
# Module: AXIL4MasterMonitorTB
# Purpose: AXIL4 Master Monitor Integration Testbench
#
...
"""
AXIL4 Master Monitor Integration Testbench

Reusable testbench class for testing AXIL4 master modules with integrated monitors.
...
Usage:
    ...
    tb = AXIL4MasterMonitorTB(dut, is_write=False)  # For read master
...
class AXIL4MasterMonitorTB(TBBase):
    """
    Reusable testbench for AXIL4 Master with Monitor integration testing
    """
    ...
    def __init__(self, dut, is_write=False, aclk=None, aresetn=None):
        ...
        Args:
            dut: The DUT (axil4_master_rd_mon or axil4_master_wr_mon)
    ...
    async def initialize(self):
        """Initialize the testbench (clock, reset, BFMs, monitor config)"""

        # Start clock
        await self.start_clock('aclk', self.TEST_CLK_PERIOD, 'ns')

        # Create AXIL4 master (fub_axil_* signals)
        try:
            if self.is_write:
        ...
        # Create AXIL4 slave on m_axil_* interface
        try:
            if self.is_write:
    ...
    async def run_integration_tests(self, test_level='basic'):
        ...
        self.log.info("="*80)
        self.log.info(f"AXIL4 Master {'Write' if self.is_write else 'Read'} Monitor Integration Tests")
