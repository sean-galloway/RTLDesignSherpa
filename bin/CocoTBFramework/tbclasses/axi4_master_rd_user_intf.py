...
class Axi4MasterRdUserIntf(TBBase):
    ...
    def __init__(self, dut):
        ...
        self.split_slave = create_gaxi_slave(
            ...
            multi_sig=True,
            signal_map={
                'm2s_valid': 's_split_valid',
                's2m_ready': 's_split_ready'
            },
            optional_signal_map={
                'm2s_pkt_addr': 's_split_addr',
                'm2s_pkt_id': 's_split_id',
                'm2s_pkt_num_splits': 's_split_num_splits'
            }
        )
        ...
        self.error_slave = create_gaxi_slave(
            ...
            multi_sig=True,
            signal_map={
                'm2s_valid': 's_error_valid',
                's2m_ready': 's_error_ready'
            },
            optional_signal_map={
                'm2s_pkt_type': 's_error_type',
                'm2s_pkt_addr': 's_error_addr',
                'm2s_pkt_id': 's_error_id'
            }
        )
    ...
    def _handle_split_info(self, packet):
        ...
        """
        if hasattr(packet, 'addr') and hasattr(packet, 'id') and hasattr(packet, 'num_splits'):
            split_info = SplitInfo(
                addr=packet.addr,
                id=packet.id,
                num_splits=packet.num_splits
            )

            self.split_queue.append(split_info)
            self.log.info(f"Received split info @ {packet.start_time}ns: addr=0x{split_info.addr:08X}, id=0x{split_info.id:X}, num_splits={split_info.num_splits}")
            ...
            if self.check_splits_automatically and split_info.id in self.expected_splits:
                ...
                if expected != split_info.num_splits:
                    self.log.error(f"Split mismatch for ID=0x{split_info.id:X}: expected={expected}, actual={split_info.num_splits}")
                    self.total_errors += 1
                else:
                    self.log.debug(f"Split verification passed for ID=0x{split_info.id:X}: num_splits={split_info.num_splits}")
    ...
    async def reset_interfaces(self):
        ...
        self.log.info("User interfaces reset")
    ...
    def expect_split(self, id_value, num_splits):
        """
        ...
        """
        self.expected_splits[id_value] = num_splits
        self.log.info(f"Expecting {num_splits} splits for ID=0x{id_value:X}")
