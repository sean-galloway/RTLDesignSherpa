...
#
# Module: AXI5MasterWriteTB
# Purpose: AXI5 Write Master Testbench
#
...
"""
AXI5 Write Master Testbench

Simple testbench for testing AXI5 write master functionality using the CocoTB
...
class AXI5MasterWriteTB(TBBase):
    """
    Simple AXI5 Write Master testbench for baseline testing.

    Tests basic write functionality using AW, W, and B channels with the
    axi5_master_wr_stub.sv RTL module, including AXI5-specific features.
    """
    ...
    def __init__(self, dut, aclk=None, aresetn=None):
        ...
        self.MAX_TAG = (2**self.TEST_TAG_WIDTH) - 1

        # ATOP encodings
        self.ATOP_NONE = 0
        self.ATOP_LOAD = 0x10   # Atomic load
        self.ATOP_SWAP = 0x20   # Atomic swap
        self.ATOP_CMP = 0x30    # Atomic compare

        # Log configuration
        msg = '\n'
        msg += '='*80 + "\n"
        msg += ' AXI5 Write Master Test Configuration:\n'
        msg += '-'*80 + "\n"
        ...
        msg += f' TAG Width:    {self.TEST_TAG_WIDTH}\n'
        ...
        # Create AXI5 master (AW + W + B channels)
        try:
            ...
            self.log.info("AXI5 Master Write components created")
        ...
        self.log.info("AXI5 Write Master testbench initialized successfully")
    ...
    async def burst_write_sequence(self, burst_lengths=[2, 4, 8, 16]):
        ...
        for i, burst_len in enumerate(burst_lengths):
            ...
            data_list = [0x50000000 + (i * 16) + j for j in range(burst_len)]
    ...
    async def axi5_feature_test_sequence(self, count=10):
        ...
        for i in range(count):
            ...
            atop = random.choice([0, 0, 0, 0x10, 0x20, 0x30])  # Mostly none
            ...
            tag = random.randint(0, self.MAX_TAG) if tagop is not None else None
    ...
    async def atomic_operation_test(self, count=5):
        """Test AXI5 atomic operations"""
        self.log.info(f"Running atomic operation test ({count} atomics)...")
        ...
        atop_types = [
            (self.ATOP_LOAD, "Atomic Load"),
            (self.ATOP_SWAP, "Atomic Swap"),
            (self.ATOP_CMP, "Atomic Compare"),
        ]

        for i in range(count):
            atop_type, atop_name = atop_types[i % len(atop_types)]
            addr = base_addr + (i * (self.TEST_DATA_WIDTH // 8))
            data = 0x70000000 + i

            self.log.debug(f"Testing {atop_name} at 0x{addr:08X}")

            success, info = await self.single_write_test(
                addr, data,
                atop=atop_type,
                unique=1  # Atomic requires unique
            )
            if success:
                success_count += 1
        ...
        self.log.info(f"Atomic operation test result: {success_count}/{count} successful")
        return success_count == count
    ...
    async def stress_write_test(self, count=50):
        ...
        success_count = 0
        for i in range(count):
            base_addr = random.choice([0x4000, 0x5000, 0x6000])
