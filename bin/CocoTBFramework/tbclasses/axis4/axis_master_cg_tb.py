...
#
# Module: AXISMasterCGTB
# Purpose: AXIS Master Clock Gated Testbench
#
...
# Import base AXIS master testbench
from .axis_master_tb import AXISMasterTB


class AXISMasterCGTB(AXISMasterTB):
    """
    Clock-gated AXIS Master testbench.
    Extends the base AXIS master testbench with clock gating capabilities.
    """
    ...
    def setup_clock_gating_controller(self):
        ...
        try:
            self.cg_ctrl = AxiClockGateCtrl(
                ...
                user_valid_signals=["fub_axis_tvalid"],
                axi_valid_signals=["m_axis_tvalid"],
    ...
    async def validate_ready_signals_during_gating(self):
        ...
        if gating_active:
            ...
            if hasattr(self.dut, 'fub_axis_tready'):
                fub_tready_ok = not bool(self.dut.fub_axis_tready.value)
                ready_checks.append(('fub_axis_tready', fub_tready_ok))
    ...
    async def run_gating_stress_test(self, num_packets=20):
        ...
        packets_received = slave_stats.get('received_transactions', slave_stats.get('slave_stats', {}).get('received_transactions', slave_stats.get('packet_count', 0)))
