...
class MultiplierTB(TBBase):
    ...
    async def main_loop(self, count: int = 256) -> list:
        ...
        Tests various combinations of inputs using strategic sampling
        ...
        # Only use Cartesian product for very small widths (N <= 4, max 256 tests)
        if self.N <= 4:
            # Exhaustive testing for very small bit widths (N <= 4)
            self.log.info(f"Exhaustive testing: all {self.max_val}x{self.max_val} combinations")
            ...
            self.log.info(f"Strategic coverage: {len(strategic_values)} values for {self.N}-bit multiplier")
        ...
        for test_idx, (a, b) in enumerate(test_pairs):
