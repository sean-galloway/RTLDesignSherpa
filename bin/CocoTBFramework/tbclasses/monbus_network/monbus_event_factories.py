...
from .monbus_types import (
    ...
    APBErrorCode,
    # Network Protocol codes (corrected names - not NOC)
    MNOCErrorCode,
...
# =============================================================================
# Network EVENT FACTORIES (simplified - only error for now)
# =============================================================================

def create_network_error_event(error_code: MNOCErrorCode, channel_id: int = 0,
                        unit_id: int = 0, agent_id: int = 0, data: int = 0) -> Dict[str, Any]:
    """Create expected Network error event"""
    return {
        'pkt_type': PktType.ERROR.value,
        'protocol': ProtocolType.PROTOCOL_MNOC.value,
        'event_code': error_code.value,
        'channel_id': channel_id,
        'unit_id': unit_id,
        'agent_id': agent_id,
...
def create_test_event_suite() -> Dict[str, list]:
    ...
    suite = {
        ...
        'network_events': [
            create_network_error_event(MNOCErrorCode.PARITY, 7, data=0xFF00),
        ]
    }
