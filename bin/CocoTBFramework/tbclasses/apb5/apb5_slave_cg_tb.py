...
#
# Module: APB5SlaveCGTB
# Purpose: APB5 Slave Clock-Gated Testbench
#
...
"""
APB5 Slave Clock-Gated Testbench

Extends APB5SlaveTB with clock gating control testing.
Tests the apb5_slave_cg.sv module which wraps apb5_slave.sv
...
class APB5SlaveCGTB(APB5SlaveTB):
    """
    APB5 Slave Clock-Gated testbench.

    Extends APB5SlaveTB with clock gating specific tests.
    """
    ...
    def __init__(self, dut, pclk=None, presetn=None):
        ...
        self.log.info("APB5 Slave Clock-Gated testbench initialized")
    ...
    async def run_wakeup_with_clock_gating_test(self):
        """Test wakeup_request interaction with clock gating."""
        self.log.info("Starting wakeup with clock gating test")

        # Enable clock gating
        await self.enable_clock_gating(True)

        # Request wakeup if signal exists
        if hasattr(self.dut, 'wakeup_request'):
            self.dut.wakeup_request.value = 1
            await self.wait_clocks(self.pclk_name, 5)
            self.dut.wakeup_request.value = 0
    ...
    async def run_busy_during_clock_gating_test(self):
        """Test behavior when transactions are pending during clock gating."""
        ...
        # Start a transaction
        await self.single_write_test(addr=0xD0, data=0xD0000000)
