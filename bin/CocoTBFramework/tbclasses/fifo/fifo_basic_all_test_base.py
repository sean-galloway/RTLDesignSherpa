"""Enhanced base class for FIFO testbenches with common functionality for all buffer types"""
import os
...
@dataclass
class TestConfig:
    """Configuration parameters for FIFO testbenches"""
...
class FIFOBasicTestBase(TBBase):
    """Base class for all FIFO testbenches with common functionality"""
    ...
    RANDOMIZER_CONFIGS = {
        ...
        'fast': {
            'write': {
                'write_delay': ([[0, 0], [1, 8], [9, 20]], [5, 0, 0])
            },
            'read': {
                'read_delay': ([[0, 0], [1, 8], [9, 30]], [5, 0, 0])
            }
        },
        'backtoback': {
            'write': {
    ...
    def __init__(self, dut,
                ...
                field_mode='standard'):
        """
        Initialize the base FIFO testbench.
        ...
        self.scoreboard = FIFOScoreboard(f"{self.__class__.__name__} Scoreboard", self.field_config, self.log)
        ...
        self.sequence = FIFOSequence("test_sequence", self.field_config)
    ...
    async def send_packets(self, packets):
        ...
        Args:
            packets: List of FIFOPacket objects to send
        """
        # Queue all packets
    ...
    async def simple_incremental_loops(self, count, use_fast=True, delay_clks_after=20):
        ...
        sequence = FIFOSequence.create_sequential(
            "incremental", 
    ...
    async def random_payload_test(self, count, use_fast=True, delay_clks_after=20):
        ...
        sequence = FIFOSequence.create_random(
            "random", 
            num_transactions=count, 
            data_range=(0, self.config.max_data),
            field_config=self.field_config
        )
    ...
    async def back_to_back_test(self, count, delay_clks_after=20):
        ...
        sequence = FIFOSequence.create_burst(
            "burst", 
            num_values=count,
            field_config=self.field_config
        )
    ...
    async def burst_pause_test(self, burst_size=10, num_bursts=5, delay_clks_after=20):
        ...
        for burst in range(num_bursts):
            ...
            # Create a sequence for this burst
            sequence = FIFOSequence.create_sequential(
                f"burst_{burst+1}", 
                num_transactions=burst_size, 
                start_value=packet_count, 
                increment=1, 
                field_config=self.field_config
            )
            ...
            # Send all packets in this burst
            await self.send_packets(packets)
    ...
    async def full_empty_test(self, delay_clks_after=20):
        ...
        sequence = FIFOSequence.create_counting(
            "counting", 
            start_value=0, 
            count=count,
            field_config=self.field_config
    ...
    async def addr_ctrl_data_crossing_test(self, count, delay_clks_after=20):
        ...
        # Create a custom sequence with different patterns for each field
        sequence = FIFOSequence(f"field_crossing", self.field_config)
        
        # Add data with different field patterns
        for i in range(count):
            ...
            # Add to sequence with all fields
            sequence.add_data(data, addr)
    ...
    async def alternating_field_values_test(self, count, delay_clks_after=20):
        ...
        # Create a custom sequence for alternating patterns
        sequence = FIFOSequence(f"alternating_patterns", self.field_config)

        # Add data with alternating patterns
        for i in range(count):
            if i % 4 == 0:
    ...
    async def edge_value_test(self, count, delay_clks_after=20):
        ...
        # Create a data pattern sequence
        sequence = FIFOSequence.create_data_pattern(
            "edge_values",
            field_config=self.field_config
        )
        
        # If the number of packets is larger than our patterns, repeat as needed
        if count > 10:  # Default pattern sequence has 10 patterns
            # Add more patterns by repeating
            for i in range(count - 10):
                pattern_index = i % 10
                if pattern_index == 0:
                    sequence.add_data(0)  # All 0s
                elif pattern_index == 1:
                    sequence.add_data(self.config.max_data)  # All 1s
                elif pattern_index == 2:
                    sequence.add_data(self.generate_alternating_ones(self.config.data_width))  # Alternating 1/0
                elif pattern_index == 3:
                    sequence.add_data(self.invert_bits(self.generate_alternating_ones(self.config.data_width), self.config.data_width))  # Alternating 0/1
                elif pattern_index == 4:
                    sequence.add_data(0xA5A5A5A5 & self.config.max_data)  # Alternating nibbles
                elif pattern_index == 5:
                    sequence.add_data(0x5A5A5A5A & self.config.max_data)  # Alternating nibbles inverted
                elif pattern_index == 6:
                    sequence.add_data(0x00FF00FF & self.config.max_data)  # Alternating bytes
                elif pattern_index == 7:
                    sequence.add_data(0xFF00FF00 & self.config.max_data)  # Alternating bytes inverted
                elif pattern_index == 8:
                    sequence.add_data(0x0000FFFF & self.config.max_data)  # Half and half
                else:
                    sequence.add_data(0xFFFF0000 & self.config.max_data)  # Half and half inverted
        
        # Set randomizers for sequence
        sequence.set_master_randomizer(self.write_randomizer)
        sequence.set_slave_randomizer(self.read_randomizer)
        
        # Generate packets (adjust count to match what we have)
        actual_count = min(count, len(sequence.data_seq))
    ...
    async def async_cdc_test(self, count, delay_clks_after=20):
        ...
        # Create a sequence with incrementing values and varying delays
        sequence = FIFOSequence("cdc_test", self.field_config)
        
        # Add data with incrementing values and varying delays
        for i in range(count):
            if self.use_multi_field_packets:
                addr = i & self.config.max_addr
                sequence.add_data(i & self.config.max_data, addr)
            else:
                sequence.add_data(i & self.config.max_data)
            
            # Add a varying delay to stress CDC logic
            if i % 5 == 0:
    ...
    async def clock_ratio_test(self, count, delay_clks_after=20):
        ...
        for wr_period, rd_period, description in clock_ratios:
            ...
            # Add varying delays periodically
