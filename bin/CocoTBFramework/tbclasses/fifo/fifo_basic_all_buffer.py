"""Unified FIFO Buffer Testbench for all buffer types including async designs"""
from CocoTBFramework.components.fifo.fifo_components import FIFOMaster, FIFOSlave, FIFOMonitor
from CocoTBFramework.tbclasses.fifo.fifo_basic_all_test_base import FIFOBasicTestBase, TestConfig
from CocoTBFramework.scoreboards.fifo_scoreboard import FIFOScoreboard


class FifoBasicBufferAllTB(FIFOBasicTestBase):
    """
    Universal testbench for FIFO buffers that supports:
    ...
    def __init__(self, dut, wr_clk=None, wr_rstn=None, rd_clk=None, rd_rstn=None,
        ...
        """
        Initialize the unified FIFO buffer testbench.
        ...
        self.log.info(f"FifoBufferAllTB initialized for buffer type: {buffer_type}, mode: {self.config.mode}")
    ...
    def _setup_sync_multi_components(self):
        ...
        # Required signals (write/read) for master
        master_signal_map = {
            'i_write': 'i_write',
            'o_wr_full': 'o_wr_full'
        }
        ...
        # Required signals (read/empty) for slave
        slave_signal_map = {
            'i_read': 'i_read',
        ...
        if self.config.mode == 'fifo_mux':
            slave_optional_map['o_rd_addr'] = 'ow_rd_addr'
            slave_optional_map['o_rd_ctrl'] = 'ow_rd_ctrl'
            slave_optional_map['o_rd_data'] = 'ow_rd_data'
