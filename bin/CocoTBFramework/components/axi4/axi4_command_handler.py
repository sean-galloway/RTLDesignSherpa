...
from CocoTBFramework.components.axi4.axi4_response_sequence import AXI4ResponseSequence
from CocoTBFramework.components.axi4.axi4_protocol_sequence import AXI4ProtocolSequence
...
class AXI4CommandHandler:
    ...
    def _handle_write_response(self, response):
        ...
        if hasattr(response, 'bresp'):
            if response.bresp == 1:  # EXOKAY
                # This was an exclusive write - check if we have a monitor for it
...
class AXI4MasterWriteFUBHandler:
    """
    AXI4 Master Write Command Handler for FUB interface.
    
    This class provides a simplified interface for FUB testbenches to
    execute AXI4 write transactions using the enhanced sequence classes.
    """

    def __init__(self, command_handler, master_id=0):
        """
        Initialize the FUB handler.
        
        Args:
            command_handler: AXI4CommandHandler instance
            master_id: ID for this master (for multiple master systems)
        """
        self.command_handler = command_handler
        self.master_id = master_id
        self.log = command_handler.log
        self.id_counter = 0
        
        # Create sequences
        self.addr_sequence = AXI4AddressSequence(
            name=f"fub_aw_m{master_id}",
            channel="AW"
        )
        
        self.data_sequence = AXI4DataSequence(
            name=f"fub_w_m{master_id}",
            channel="W"
        )

    async def write(self, addr, data, size=2, burst=1, strobe=None, id_value=None):
        """
        ...
        if id_value is None:
            id_value = self.id_counter
            self.id_counter += 1
        
        # Convert data to list if it's a single value
        if not isinstance(data, (list, tuple, bytearray)):
            data = [data]
            
        # Calculate burst length based on data
        burst_len = len(data) - 1
        
        # Add address transaction
        self.addr_sequence.add_transaction(
            addr=addr,
            id_value=id_value,
            burst_len=burst_len,
            burst_size=size,
        ...
        if strobe is None:
            strb_mask = (1 << (1 << size)) - 1
            strobe = [strb_mask] * len(data)
        elif not isinstance(strobe, (list, tuple)):
            strobe = [strobe]
            
        # Add data burst
        for i, data_value in enumerate(data):
            is_last = (i == len(data) - 1)
            strb_value = strobe[i] if i < len(strobe) else strobe[-1]
            
            self.data_sequence.add_transaction(
                data=data_value,
                strb=strb_value,
                last=is_last
            )
        ...
        await self.command_handler.process_address_sequence(self.addr_sequence, is_read=False)
        await self.command_handler.process_data_sequence(self.data_sequence)
        
        return id_value

    async def exclusive_write(self, addr, data, size=2):
        """
        Execute an exclusive write transaction.
        ...
        # Add exclusive address transaction
        self.addr_sequence.add_transaction(
            addr=addr,
            id_value=id_value,
...
class AXI4MasterReadFUBHandler:
    """
    AXI4 Master Read Command Handler for FUB interface.
    
    This class provides a simplified interface for FUB testbenches to
    execute AXI4 read transactions using the enhanced sequence classes.
    """
    ...
    def __init__(self, command_handler, master_id=0):
        ...
        # Create sequences
        self.addr_sequence = AXI4AddressSequence(
            name=f"fub_ar_m{master_id}",
            channel="AR"
        )

    async def read(self, addr, size=2, burst=1, length=0, id_value=None):
        """
        ...
        await self.command_handler.process_address_sequence(self.addr_sequence, is_read=True)
        
        return id_value

    async def exclusive_read(self, addr, size=2):
        """
        Execute an exclusive read transaction.
