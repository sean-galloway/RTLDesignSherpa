...
class AXI4Master:
    ...
    def _handle_b_response(self, transaction):
        ...
        if hasattr(transaction, 'bid'):
            # Check if there's a pending write transaction for this ID
            if transaction.bid in self.write_responses:
    ...
    def _handle_r_response(self, transaction):
        ...
        if hasattr(transaction, 'rid'):
            # Check if there's a pending read transaction for this ID
