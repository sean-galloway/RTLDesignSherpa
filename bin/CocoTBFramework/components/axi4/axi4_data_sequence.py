...
class AXI4DataSequence:
    ...
    def _adjust_field_config(self, field_config: FieldConfig) -> FieldConfig:
        ...
        if self.channel == 'W':
            ...
            if adjusted_config.has_field('rid'):
                # Default ID width is 8, we'll keep it
                pass
    ...
    def set_randomizer(self, randomizer: FlexRandomizer) -> 'AXI4DataSequence':
        """
    ...
    def generate_packet(self) -> AXI4Packet:
        ...
        if self.channel == 'W':
            ...
            packet = AXI4Packet.create_w_packet(
                wdata=data,
                wstrb=strb,
                wlast=last,
                wuser=user
    ...
    def generate_packets(self, count: Optional[int] = None) -> List[AXI4Packet]:
        ...
        if count is None and self.data_sequence:
    ...
    def add_random_data(self, 
        ...
        for i in range(count):
            value = random.randint(0, self.data_mask)
            is_last = all_last or (i == count - 1)
    ...
    def add_varied_strobes(self, 
        ...
        if strobe_patterns is None:
            ...
            for i in range(self.strb_width):
                strobe_patterns.append(1 << i)
                
            # Every other byte
            strobe_patterns.append(0x55 & self.strb_mask)
            strobe_patterns.append(0xAA & self.strb_mask)
            
            # High half / low half
            if self.strb_width > 1:
                strobe_patterns.append((1 << (self.strb_width // 2)) - 1)
                strobe_patterns.append(((1 << (self.strb_width // 2)) - 1) << (self.strb_width // 2))
    ...
    def create_write_data(cls, 
                        data_values: List[int], 
                        data_width: int = 32) -> 'AXI4DataSequence':
        """
    ...
    @classmethod
    def create_strobe_test(cls, data_width: int = 32) -> 'AXI4DataSequence':
        """
        ...
        # All bytes enabled
        strobe_patterns.append((1 << strb_width) - 1)
        
        # Single byte patterns (one-hot)
        for i in range(strb_width):
            strobe_patterns.append(1 << i)
    ...
    @classmethod
    def create_error_response_test(cls, id_value: int = 0, data_width: int = 32) -> 'AXI4DataSequence':
        """
    ...
    @classmethod
    def create_comprehensive_test(cls, channel: str = 'W', data_width: int = 32) -> 'AXI4DataSequence':
        """
