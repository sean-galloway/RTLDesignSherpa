...
class AXI4ProtocolSequence(AXI4BaseSequence):
    """
    ...
    def register_protocol_violation(self, id_value: int, is_read: bool, message: str) -> 'AXI4ProtocolSequence':
        ...
        self.log_warning(f"Protocol violation for {key}: {message}")
        return self
    ...
    def cleanup(self) -> None:
        ...
        """
        # Clean up transaction sequence
        if self.transaction_sequence:
            self.transaction_sequence.cleanup()

        # Clean up specialized sequences
        for sequence in self.error_sequences.values():
            sequence.cleanup()
        for sequence in self.boundary_sequences.values():
            sequence.cleanup()
        for sequence in self.compliance_sequences.values():
            sequence.cleanup()
        for sequence in self.performance_sequences.values():
            sequence.cleanup()

        # Clear dictionaries
        self.error_sequences.clear()
        self.boundary_sequences.clear()
        self.compliance_sequences.clear()
        self.performance_sequences.clear()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Add usage from transaction sequence
        memory_usage += stats['transaction_stats'].get('memory_usage', 0)

        # Add sequence counts
        stats['sequence_counts'] = {
            'error': len(self.error_sequences),
            'boundary': len(self.boundary_sequences),
            'compliance': len(self.compliance_sequences),
            'performance': len(self.performance_sequences)
        }
    ...
    def create_max_burst_sequence(self) -> 'AXI4ProtocolSequence':
        ...
        # Create data for max length burst - limited to a practical size for simulation
        # Full-length burst would be too memory-intensive
        practical_len = min(max_burst_len, 31)  # 32 beats instead of 256
        data_values = [0xAA000000 + i for i in range(practical_len + 1)]

        # Add max length write
        sequence.add_write_transaction(
            addr=base_addr,
            data_values=data_values,
            burst_len=practical_len,
            burst_type=1  # INCR
        )
    ...
    def create_burst_type_sequence(self) -> 'AXI4ProtocolSequence':
        ...
        # 4. Invalid burst type (3) - protocol violation - this is no longer added
        # since we're improving validation. The TransactionSequence will reject this.
    ...
    def create_exclusive_sequence(self) -> 'AXI4ProtocolSequence':
        ...
        # 3. Exclusive write without matching exclusive read - protocol violation
    ...
    def create_throughput_sequence(self) -> 'AXI4ProtocolSequence':
        ...
        # Use a more practical transaction count for simulation
        transaction_count = 20  # Reduced from 100 to avoid excessive memory usage

        # Generate a series of back-to-back write transactions
        ...
        # Generate a series of back-to-back read transactions
        for i in range(transaction_count):
            addr = base_addr + (i * 4)
    ...
    def create_interleaved_sequence(self) -> 'AXI4ProtocolSequence':
        ...
        # Reduced transaction count for practical simulation
        transaction_count = 5  # Reduced from 10

        # Generate interleaved write transactions with different IDs
        for i, id_value in itertools.product(range(transaction_count), range(4)):
    ...
    def create_protocol_test_suite(cls,
                                    ...
                                    data_width: int = 32,
                                    user_width: int = 1,
                                    randomization_config: Optional[RandomizationConfig] = None) -> 'AXI4ProtocolSequence':
        """
