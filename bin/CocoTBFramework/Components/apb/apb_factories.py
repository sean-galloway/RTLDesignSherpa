...
"""
from CocoTBFramework.Componentsapb import APBMaster, APBSlave, APBMonitor, APBSequence, APBTransaction
from CocoTBFramework.Components.flex_randomizer import FlexRandomizer
from CocoTBFramework.Componentsmemory_model import MemoryModel
from Scoreboards.apb_scoreboard import APBScoreboard, APBtoGAXITransformer
...
def create_apb_command_handler(dut, memory_model, log=None):
    ...
    from CocoTBFramework.Componentsapb.command_handler import APBCommandHandler
...
def create_apb_sequence(name="basic", num_regs=10, base_addr=0, 
    ...
    if pattern == "alternating":
        ...
        for i in range(strb_width):
            test_strobes.append(1 << i)
            
        # Add some combinations (first half, second half, etc.)
        if strb_width >= 2:
            test_strobes.append(0x3)  # First two bytes
        if strb_width >= 4:
            test_strobes.append(0x5)  # First and third bytes
            test_strobes.append(0xA)  # Second and fourth bytes
