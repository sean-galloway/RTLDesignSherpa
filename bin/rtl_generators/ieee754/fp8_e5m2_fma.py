...
class FP8E5M2FMA(Module):
    ...
    def generate_result_assembly(self):
        ...
        self.instruction('    // Note: w_c_eff_zero (a*b + 0 = a*b) is handled by the default assignment')
        self.instruction('    // which uses the normalized/rounded sum path result (w_exp_final, w_mant_final).')
        self.instruction('    // A separate raw passthrough would bypass rounding and overflow/underflow')
        self.instruction('    // checks, producing invalid encodings in edge cases.')
        self.instruction('end')
        self.instruction('')
