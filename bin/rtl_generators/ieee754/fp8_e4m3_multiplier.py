...
class FP8E4M3Multiplier(Module):
    ...
    def verilog(self, file_path):
        ...
        self.comment('Also detect overflow when mantissa rounding pushes exp=15 to exp=16 (wraps to 0)')
        self.instruction("wire w_round_causes_overflow = w_mant_round_overflow & (w_exp_sum == 4'hF);")
        self.instruction('wire w_final_overflow = w_exp_overflow | w_round_causes_overflow |')
        ...
        self.instruction("wire w_exp_sum_was_zero = ({1'b0, w_exp_a} + {1'b0, w_exp_b} + {4'b0, w_needs_norm}) == 5'd7;")
