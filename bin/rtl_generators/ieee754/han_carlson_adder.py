...
# Purpose: Han-Carlson Prefix Adder Generator for IEEE 754-2008 FP32
#
...
# - O(log2(N)) logic levels
# - Approximately N prefix cells
# - Constant fanout of 2 (vs exponential for Sklansky)
# - Balanced wiring tracks
#
# This makes it optimal for advanced process nodes where wire delay
# dominates and routing congestion must be minimized.
#
# For FP32:
# - 32-bit adder for exponent paths
# - 48-bit adder for product CPA
# - 72-bit adder for FMA wide accumulator
#
...
class HanCarlsonAdder(Module):
    """
    Generates a Han-Carlson prefix adder for optimal CPA operations.

    Han-Carlson structure:
    - Stage 1: Kogge-Stone style (all even positions)
    - Stages 2-n: Brent-Kung style (parallel prefix)
    - Final stage: Ripple for odd positions from even neighbors

    For any width N:
    - log2(N) + 1 logic levels (depth)
    - ~N prefix cells
    - Fanout = 2 (constant)
    - log2(N) wiring tracks
    """

    module_str = 'math_adder_han_carlson'
    param_str = 'parameter int N=32'
    port_str = '''
    ...
    def generate_han_carlson_tree(self):
        ...
        For N-bit: stages work on even positions 0,2,4,...,N-2
        ...
        - Stage k: Span 2^k (combine with position -2^(k-1))
        - Final stage: Fill odd positions
        """
        N = self.width
    ...
    def verilog(self, file_path):
        ...
        header = generate_rtl_header(
            ...
            purpose=f'Han-Carlson {self.width}-bit prefix adder for IEEE 754-2008 FP32',
...
def generate_han_carlson_adder(width, output_path):
    ...
    Args:
        width: Bit width of the adder (32, 48, 72 for FP32)
...
if __name__ == '__main__':
    ...
    # Default to 32-bit for FP32 exponent
