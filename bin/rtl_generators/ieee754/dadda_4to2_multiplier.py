...
# Purpose: Dadda Multiplier with 4:2 Compressors for IEEE 754-2008 FP32
#
# This implements a Dadda reduction tree using 4:2 compressors instead of
# 3:2 CSA cells. For a 24x24 multiplier (FP32 mantissa with implicit bit):
# - 7 reduction stages (vs 10 with 3:2 CSA)
# - ~130 4:2 compressors
# - Critical path: 21 XOR delays (3 per stage)
#
...
#
# For FP32:
# - 24-bit mantissa (23 explicit + 1 implicit)
# - 576 partial products
# - 48-bit product
#
...
class Dadda4to2Multiplier(Module):
    ...
    For 24-bit operands (FP32 mantissa with implicit bit):
    - 576 partial products from AND gates
    - 7-stage Dadda reduction with 4:2 compressors
    - ~130 compressors total
    - Final CPA on 48-bit result
    """

    module_str = 'math_multiplier_dadda_4to2'
    param_str = 'parameter int N=24'
    port_str = '''
    ...
    def generate_dadda_heights(n):
        ...
        For 24-bit multiply: max column height is 24
        Target heights: 19 -> 13 -> 9 -> 6 -> 4 -> 3 -> 2
        """
        heights = [2]
        while heights[-1] < n:
    ...
    def generate_final_cpa(self, bit_groups):
        ...
        Use Han-Carlson adder for optimal 48-bit CPA.
        """
        N = self.buswidth
        max_col_digits = len(str(2 * N - 1))
    ...
    def verilog(self, file_path):
        ...
        header = generate_rtl_header(
            ...
            purpose=f'Dadda {self.buswidth}x{self.buswidth} multiplier with 4:2 compressors for FP32 mantissa',
...
def generate_dadda_4to2_multiplier(width, output_path):
    ...
    Args:
        width: Operand width (24 for FP32 mantissa with implicit bit)
...
if __name__ == '__main__':
    ...
    # Default to 24-bit for FP32 mantissa
    width = int(sys.argv[1]) if len(sys.argv) > 1 else 24
