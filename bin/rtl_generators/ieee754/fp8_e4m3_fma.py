...
class FP8E4M3FMA(Module):
    ...
    def generate_result_assembly(self):
        ...
        self.instruction('    // NOTE: The c_eff_zero shortcut branch has been removed. The default')
        self.instruction('    // assignment at the top of this always_comb already uses the properly')
        self.instruction('    // rounded and normalized sum-path result (w_exp_final, w_mant_final),')
        self.instruction('    // which correctly handles the a*b case when c=0 via the main pipeline.')
        self.instruction('    // The previous shortcut override used raw unrounded product bits and')
        self.instruction('    // produced invalid phantom-subnormal encodings. Bug found by formal.')
        self.instruction('end')
        self.instruction('')
