...
#
# Module: BF16Multiplier
# Purpose: Complete BF16 Multiplier Generator
#
# Implements a full BF16 (Brain Float 16) multiplier with:
# - Sign computation (XOR of input signs)
# - Exponent addition with bias handling
# - 8x8 mantissa multiplication (Dadda tree with 4:2 compressors)
# - Normalization and rounding (Round-to-Nearest-Even)
# - Special case handling (zero, inf, NaN)
# - Optional FP32 accumulation output
#
# BF16 Format:
#   [15]    - Sign bit
...
class BF16Multiplier(Module):
    ...
    def generate_field_extraction(self):
        """Extract sign, exponent, and mantissa from BF16 operands."""
        self.comment('BF16 field extraction')
        self.comment('Format: [15]=sign, [14:7]=exponent, [6:0]=mantissa')
        self.instruction('')
    ...
    def generate_special_case_detection(self):
        ...
        self.comment('Subnormal: exp=0, mant!=0 (flushed to zero per BF16 convention)')
    ...
    def generate_mantissa_multiplication(self):
        ...
        self.comment('Mantissa multiplication (8x8 with Dadda 4:2 tree)')
        self.instruction('wire [15:0] w_mant_product;')
        self.instruction('wire        w_needs_norm;')
        self.instruction('wire [6:0]  w_mant_mult_out;')
        self.instruction('wire        w_guard_bit;')
        self.instruction('wire        w_round_bit;')
        self.instruction('wire        w_sticky_bit;')
        self.instruction('')
        self.instruction('math_bf16_mantissa_mult u_mant_mult (')
    ...
    def generate_exponent_addition(self):
        ...
        self.instruction('')
        self.instruction('math_bf16_exponent_adder u_exp_add (')
    ...
    def generate_rounding(self):
        ...
        self.comment('w_sticky_bit arrives TRUE (unfolded) from mantissa_mult; the pre-MATH-001')
        self.comment('logic used round & (sticky|LSB) with a folded guard|sticky -- a different')
        self.comment('decision bit, diverging from RNE in 6/16 guard patterns, sim-verified.')
        self.instruction('')
        self.instruction('wire w_lsb = w_mant_mult_out[0];')
        self.instruction('wire w_round_up = w_guard_bit & (w_round_bit | w_sticky_bit | w_lsb);')
        self.instruction('')

        self.comment('Apply rounding to mantissa')
        self.instruction("wire [7:0] w_mant_rounded = {1'b0, w_mant_mult_out} + {7'b0, w_round_up};")
        self.instruction('')

        self.comment('Check for mantissa overflow from rounding (rare)')
        self.instruction('wire w_mant_round_overflow = w_mant_rounded[7];')
        self.instruction('')

        self.comment('Final mantissa (7 bits)')
        self.instruction('wire [6:0] w_mant_final = w_mant_round_overflow ? ')
        self.instruction("    7'h00 : w_mant_rounded[6:0];  // Overflow means 1.0 -> needs exp adjust")
        self.instruction('')
    ...
    def generate_result_assembly(self):
        ...
        self.instruction('    // IEEE 754: NaN > Zero > Infinity/Overflow > Underflow > Normal')
        self.instruction('    // Zero MUST be checked before overflow because when either input')
        self.instruction('    // is zero, the exponent adder produces a garbage value (e.g. 0xFF)')
        self.instruction('    // that falsely triggers the overflow path.')
        self.instruction('    if (w_any_nan | w_invalid_op) begin')
        self.instruction('        // NaN result: quiet NaN with sign preserved')
        self.instruction("        ow_result = {w_sign_result, 8'hFF, 7'h40};  // Canonical qNaN")
        self.instruction('        ow_invalid = w_invalid_op;')
        self.instruction('    end else if (w_result_zero) begin')
        self.instruction('        // Zero result: either input is zero/subnormal')
    ...
    def verilog(self, file_path):
        """Generate the complete BF16 multiplier."""
        ...
        header = generate_rtl_header(
            ...
            generator_script='bf16_multiplier.py'
        )
...
def generate_bf16_multiplier(output_path):
    """
    Generate complete BF16 multiplier.
...
if __name__ == '__main__':
    ...
    module_name = generate_bf16_multiplier(output_path)
