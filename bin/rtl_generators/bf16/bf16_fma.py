...
class BF16FMA(Module):
    ...
    def generate_product_computation(self):
        ...
        self.comment('Product exponent: exp_a + exp_b - bias (SIGNED to detect underflow!)')
        self.instruction("wire signed [10:0] w_prod_exp_raw = $signed({3'b0, w_exp_a}) + $signed({3'b0, w_exp_b}) - 11'sd127;")
        self.instruction('')
        ...
        self.instruction("wire signed [10:0] w_prod_exp_adj = w_prod_exp_raw + {10'b0, w_prod_needs_norm};")
        self.instruction('')

        self.comment('Detect product underflow: when product exponent is too small to represent')
        self.comment('Product is effectively zero if exp < 1 (or exp goes negative)')
        self.instruction("wire w_prod_underflow = w_prod_exp_adj[10] | (w_prod_exp_adj < 11'sd1);")
        self.instruction('')

        self.comment('Clamp product exponent for downstream use (will be overridden by special cases)')
        self.instruction("wire [9:0] w_prod_exp = w_prod_underflow ? 10'd0 : w_prod_exp_adj[9:0];")
        self.instruction('')
    ...
    def generate_rounding_and_packing(self):
        ...
        self.comment('When rounding overflows (mantissa goes from 0x7FFFFF to 0x800000), the value')
        self.comment('becomes exactly 2.0, which normalizes to 1.0 * 2^1. The mantissa for 1.0 is 0.')
        self.instruction("wire [22:0] w_mant_final = w_round_overflow ? 23'h0 : w_mant_rounded[22:0];")
        self.instruction('')
    ...
    def generate_special_case_handling(self):
        ...
        self.comment('Product is zero if either operand is zero/subnormal, OR if the product underflows')
        self.instruction('wire w_prod_is_zero = w_a_eff_zero | w_b_eff_zero | w_prod_underflow;')
    ...
    def generate_result_assembly(self):
        ...
        self.instruction('')
        self.instruction('    // Special cases - PRIORITY ORDER MATTERS!')
        self.instruction('    // 1. NaN/Invalid first (highest priority)')
        self.instruction('    // 2. Infinity cases')
        self.instruction('    // 3. Zero operand pass-through (BEFORE overflow/underflow check!)')
        self.instruction('    // 4. Overflow/underflow')
        self.instruction('    // 5. Normal result (default)')
        self.instruction('')
        ...
        self.instruction('        // 0 * 0 + 0 = 0 (sign from IEEE rules)')
        self.instruction("        ow_result = {w_prod_sign & w_sign_c, 8'h00, 23'h0};")
        self.instruction('    end else if (w_prod_is_zero) begin')
        self.instruction('        // 0 * X + C = C (pass-through addend when product is zero)')
        self.instruction('        ow_result = i_c;')
        self.instruction('    end else if (w_c_eff_zero) begin')
        self.instruction('        // A * B + 0 = A * B (product only, extend to FP32)')
        self.instruction('        // Must check product overflow/underflow here since the normal')
        self.instruction('        // overflow/underflow checks below operate on the addition result,')
        self.instruction('        // not the raw product exponent.')
        self.instruction("        if (w_prod_exp > 10'd254) begin")
        self.instruction("            ow_result = {w_prod_sign, 8'hFF, 23'h0};  // Overflow to inf")
        self.instruction("            ow_overflow = 1'b1;")
        self.instruction('        end else if (w_prod_underflow) begin')
        self.instruction("            ow_result = {w_prod_sign, 8'h00, 23'h0};  // Underflow to zero")
        ...
        self.instruction("    end else if (w_sum_abs == 48'h0) begin")
        self.instruction('        // Exact zero result: IEEE 754 round-to-nearest gives +0')
        self.instruction('        // (Exception: both operands negative gives -0, but that\'s handled by')
        self.instruction('        // the zero product cases above)')
        self.instruction("        ow_result = 32'h0;  // Positive zero")
        self.instruction('    end else if (w_overflow_cond) begin')
        self.instruction("        ow_result = {w_result_sign, 8'hFF, 23'h0};  // Overflow to inf")
        self.instruction("        ow_overflow = 1'b1;")
        self.instruction('    end else if (w_underflow_cond) begin')
