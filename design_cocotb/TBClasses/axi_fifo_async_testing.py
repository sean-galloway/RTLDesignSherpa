...
class FIFOAXIASyncTB(TBBase):
    ...
    async def write_fifo(self, data, count, pause_duration):
        ...
        hex_data = [format(num, '02x') for num in data]
        msg = f'Attempting to write {count=} packets: {len(data)=}\n{hex_data=}'
        self.log.info(msg)
        self.dut.i_wr_valid.value = 0
        data_sent = 0
        ...
        while data_sent < count:
            ...
            if crand == 0:
                ...
                msg = f"Writing data to fifo: {hex(data[idx])} pkt={data_sent:3} @ {current_time_ns}ns"
                self.log.info(msg)
                await self.wait_clocks('i_axi_wr_aclk', 1)
            else:
    ...
    async def read_fifo(self, count, pause_duration):
        ...
        while data_read < count:
            ...
            if crand == 0:
                ...
                data_read += 1
                current_time_ns = get_sim_time('ns')
                msg = f"Read data from fifo:  {hex(read_data)} pkt={data_read:3} @ {current_time_ns}ns"
                self.log.info(msg)
                await self.wait_clocks('i_axi_rd_aclk', 1)
            else:
