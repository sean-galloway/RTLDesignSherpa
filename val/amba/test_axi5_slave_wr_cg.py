...
#
# Module: AXI5SlaveWriteCGTB
# Purpose: AXI5 Slave Write Clock Gated Test Runner
#
...
"""
AXI5 Slave Write Clock Gated Test Runner

Test runner for the axi5_slave_wr_cg module using the CocoTB framework.
...
class AXI5SlaveWriteCGTB(AXI5SlaveWriteTB):
    """
    Clock-gated AXI5 Slave Write testbench.
    ...
    def setup_clock_gating_controller(self):
        ...
        try:
            self.cg_ctrl = AxiClockGateCtrl(
                ...
                axi_valid_signals=["fub_axi_awvalid", "fub_axi_wvalid", "fub_axi_bvalid"],
...
async def axi5_slave_write_cg_test(dut):
    ...
    tb = AXI5SlaveWriteCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXI5 slave write CG test with seed: {seed}')
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXI5 SLAVE WRITE CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        tb.log.info("=" * 80)
        tb.log.info("AXI5 SLAVE WRITE CG TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        tb.log.info("AXI5 SLAVE WRITE CG TEST PASSED")
...
def generate_axi5_cg_params():
    ...
    if reg_level == 'GATE':
        params = [(8, 32, 32, 1, 2, 4, 2, 'gate')]
    elif reg_level == 'FUNC':
        params = [
            (8, 32, 32, 1, 2, 4, 2, 'gate'),
            (8, 32, 32, 1, 4, 8, 4, 'func'),
            (8, 64, 64, 1, 2, 4, 2, 'func'),
        ]
    else:  # FULL
...
def test_axi5_slave_wr_cg(id_width, addr_width, data_width, user_width, aw_depth, w_depth, b_depth, test_level):
    """Test AXI5 slave write clock gated with specified parameters"""
