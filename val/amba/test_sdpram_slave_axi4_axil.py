...
#
# Module: test_sdpram_slave_axi4_axil
# Purpose: Coverage for rtl/amba/shared/sdpram_slave_axi4_axil.sv -- AXI4
#          write slave + AXIL4 read slave over the shared sdpram_core.
#
...
async def cocotb_test_sdpram_slave_axi4_axil(dut):
    test_level = os.environ.get('TEST_LEVEL', 'gate').lower()

    tb = SdpramSlaveMixedTB(dut, wr_protocol="AXI4", rd_protocol="AXIL",
                             aclk=dut.aclk, aresetn=dut.aresetn)
    await tb.setup_clocks_and_reset()

    tb.log.info(f"sdpram_slave_axi4_axil: TEST_LEVEL={test_level}")
...
def generate_sdpram_axi4_axil_params():
...
@pytest.mark.parametrize("test_level,data_width,mem_depth", generate_sdpram_axi4_axil_params())
def test_sdpram_slave_axi4_axil(request, test_level, data_width, mem_depth):
    """Pytest wrapper -- exercises sdpram_slave_axi4_axil (AXI4 wr / AXIL rd)."""
    worker_id = os.environ.get('PYTEST_XDIST_WORKER', 'gw0')
    module, repo_root, tests_dir, log_dir, _ = get_paths({})

    dut_name = "sdpram_slave_axi4_axil"
    reg_level = os.environ.get("REG_LEVEL", "FUNC").upper()
    ...
    try:
        run(
            ...
            toplevel=dut_name,
            module=module,
            testcase="cocotb_test_sdpram_slave_axi4_axil",
