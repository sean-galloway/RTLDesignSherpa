...
#
# axi_monitor_addr_check is documented as a "deliberate mirror" of
...
class AxiAddrCheckTB(TBBase):
    ...
    async def initialize_inputs(self):
        ...
        d.cfg_debug_enable.value = 1      # MATCH path
        d.cfg_error_enable.value = 1      # MISS path -- the preempting source
    ...
    async def send_cmd(self, addr, is_write):
        pkt = self.cmd_master.create_packet(addr=addr, id=(0 if is_write else 1))
        await self.cmd_master.send(pkt)


@cocotb.test(timeout_time=120, timeout_unit="sec")
async def axi_addr_check_payload_stability_test(dut):
    tb = AxiAddrCheckTB(dut)
...
def test_axi_monitor_addr_check(request, n_ranges):
    ...
    dut_name = "axi_monitor_addr_check"
    test_name = f"test_{worker_id}_axi_monitor_addr_check_n{n_ranges}"
    ...
    run(
        ...
        parameters={
            ...
            # range 3 = ERROR/allowlist -> addresses outside it emit MISS,
            # which preempts a stalled MATCH beat. That is the second defect.
            'ADDR_RANGE_IS_ERROR': '8',
            'ADDR_WIDTH': '32',
            'UNIT_ID': '1',
            'AGENT_ID': '10',
        },
