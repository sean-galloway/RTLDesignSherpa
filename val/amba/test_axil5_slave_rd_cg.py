...
#
# Module: AXIL5SlaveReadCGTB
# Purpose: AXIL5 Slave Read Clock Gated Test Runner
#
...
AXIL5 Slave Read Clock Gated Test Runner

Test runner for the axil5_slave_rd_cg module using the CocoTB framework.
Tests clock gating functionality while validating AXIL5 slave read response behavior
...
from TBClasses.axil5.axil5_slave_read_tb import AXIL5SlaveReadTB


class AXIL5SlaveReadCGTB(AXIL5SlaveReadTB):
    """
    Clock-gated AXIL5 Slave Read testbench.
    Extends the base AXIL5 slave read testbench with clock gating capabilities.
    """
    ...
    async def measure_power_efficiency(self, test_duration_cycles=1000):
        ...
        self.log.info(f"AXIL5 power efficiency: {efficiency:.1f}% ({gated_cycles}/{test_duration_cycles} cycles gated)")
...
async def axil5_slave_read_cg_test(dut):
    """AXIL5 slave read clock gated test using the enhanced framework"""

    # Create clock-gated testbench instance
    tb = AXIL5SlaveReadCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXIL5 slave read CG test with seed: {seed}')
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXIL5 SLAVE READ CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        if cg_test_mode in ['comprehensive', 'basic']:
            ...
            tb.log.info("=== Test 1: AXIL5 Functional Equivalence ===")
            
            equivalence, results = await tb.functional_equivalence_test(test_iterations=test_transactions)
            total_tests += 1
            if equivalence:
                passed_tests += 1
                tb.log.info("✅ AXIL5 functional equivalence: PASS")
            else:
                tb.log.error("❌ AXIL5 functional equivalence: FAIL")

            # Test 2: Power Efficiency at Different Idle Counts
            tb.log.info("=== Test 2: AXIL5 Power Efficiency Testing ===")
            
            for idle_count in idle_counts:
                tb.log.info(f"Testing AXIL5 idle count: {idle_count}")
                ...
                if efficiency_result['power_efficiency_percent'] > 10.0:  # Reasonable threshold
                    passed_tests += 1
                    tb.log.info(f"✅ AXIL5 efficiency test (idle={idle_count}): {efficiency_result['power_efficiency_percent']:.1f}%")
                else:
                    tb.log.warning(f"⚠️ AXIL5 low efficiency (idle={idle_count}): {efficiency_result['power_efficiency_percent']:.1f}%")

            # Test 3: Clock Gating Disable/Enable Transitions
            tb.log.info("=== Test 3: AXIL5 Enable/Disable Transitions ===")
            ...
            if transition_success >= 4:  # Allow one failure
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 transition test: {transition_success}/5 successful")
            else:
                tb.log.error(f"❌ AXIL5 transition test: {transition_success}/5 successful")

        # === EFFICIENCY-FOCUSED TESTS ===
        if cg_test_mode in ['efficiency']:
            tb.log.info("=== Efficiency Mode: Extended AXIL5 Power Testing ===")
            ...
            if efficiency_result['power_efficiency_percent'] > 15.0:  # Higher threshold for efficiency mode
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 extended efficiency: {efficiency_result['power_efficiency_percent']:.1f}%")
            else:
                tb.log.warning(f"⚠️ AXIL5 extended efficiency: {efficiency_result['power_efficiency_percent']:.1f}%")

        # Test 4: Register Access with Clock Gating (medium and full levels)
        if test_level in ['func', 'full']:
            tb.log.info("=== Test 4: AXIL5 Register Access with Clock Gating ===")
            ...
            if success:
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 register access with gating: {success_count}/{total_count} successful")
            else:
                tb.log.error(f"❌ AXIL5 register access with gating: {success_count}/{total_count} successful")

        # Test 5: Stress Testing with Clock Gating (full level)
        if test_level == 'full':
            tb.log.info("=== Test 5: AXIL5 Clock Gated Stress Testing ===")
            ...
            if stress_success >= int(stress_count * 0.9):  # Allow 10% failures in stress test
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 stress test: {stress_success}/{stress_count} responses successful")
            else:
                tb.log.error(f"❌ AXIL5 stress test: {stress_success}/{stress_count} responses successful")

        # === FINAL RESULTS ===
        tb.log.info("=" * 80)
        tb.log.info("CLOCK GATED AXIL5 SLAVE READ TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        if power_efficiency_results:
            tb.log.info("AXIL5 Power Efficiency Results:")
            for result in power_efficiency_results:
                tb.log.info(f"  Idle count {result['idle_count']}: {result['efficiency_percent']:.1f}% efficiency")
                
        if success_rate < 90:
            tb.log.error("❌ AXIL5 SLAVE READ CG TEST FAILED")
            raise Exception(f"Clock gated test failed with {success_rate:.1f}% success rate")
            
        tb.log.info("✅ AXIL5 SLAVE READ CG TEST PASSED")

    except Exception as e:
        tb.log.error(f"AXIL5 slave read CG test FAILED with exception: {str(e)}")
        raise
    finally:
...
def generate_axil5_cg_params():
    """Generate test parameters for clock-gated AXIL5 slave read testing"""
...
def test_axil5_slave_read_cg(addr_width, data_width, ar_depth, r_depth, test_level, cg_test_mode):
    """Test AXIL5 slave read clock gated with specified parameters"""
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXIL5 Slave Read CG test: {dut_name}")
    ...
    try:
        ...
        print(f"✅ {test_level.upper()} AXIL5 Slave Read CG test PASSED")
    except Exception as e:
        print(f"❌ {test_level.upper()} AXIL5 Slave Read CG test FAILED: {str(e)}")
