"""
AXI4 Slave Write Clock Gated Test Runner

Test runner for the axi4_slave_wr_cg module using the CocoTB framework.
Tests clock gating functionality while validating slave write response behavior.
...
class AXI4SlaveWriteCGTB(AXI4SlaveWriteTB):
    """
    Clock-gated AXI4 Slave Write testbench.
    Extends the base slave write testbench with clock gating capabilities.
    """
    ...
    def setup_clock_gating_controller(self):
        ...
        try:
            self.cg_ctrl = AxiClockGateCtrl(
                ...
                user_valid_signals=["s_axi_awvalid", "s_axi_wvalid", "s_axi_bready"],
                axi_valid_signals=["fub_axi_awvalid", "fub_axi_wvalid", "s_axi_bvalid"],
...
async def axi4_slave_write_cg_test(dut):
    """AXI4 slave write clock gated test using the enhanced framework"""

    # Create clock-gated testbench instance
    tb = AXI4SlaveWriteCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXI4 SLAVE WRITE CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        if cg_test_mode in ['comprehensive', 'basic']:
            ...
            for i in range(5):
                ...
                success2, _ = await tb.single_write_response_test(addr + 8, data + 0x1000)
        ...
        if test_level in ['medium', 'full']:
            ...
            for i in range(20):
                addr = 0x5000 + (i * 8)
                data = 0x5000000 + i  # Fixed hex literal
        ...
        # === FINAL RESULTS ===
        tb.log.info("=" * 80)
        tb.log.info("CLOCK GATED AXI4 SLAVE WRITE TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        if success_rate < 90:
            tb.log.error("❌ AXI4 SLAVE WRITE CG TEST FAILED")
            raise Exception(f"Clock gated test failed with {success_rate:.1f}% success rate")
            
        tb.log.info("✅ AXI4 SLAVE WRITE CG TEST PASSED")

    except Exception as e:
        tb.log.error(f"AXI4 slave write CG test FAILED with exception: {str(e)}")
        raise


def generate_axi4_cg_params():
    """Generate test parameters for clock-gated AXI4 slave write testing"""
...
def test_axi4_slave_write_cg(id_width, addr_width, data_width, user_width, aw_depth, w_depth, b_depth, test_level, cg_test_mode):
    """Test AXI4 slave write clock gated with specified parameters"""
    ...
    dut_name = "axi4_slave_wr_cg"
    
    id_str = f"ID{id_width}_ADDR{addr_width}_DATA{data_width}_USER{user_width}_AW{aw_depth}_W{w_depth}_B{b_depth}_{test_level}_{cg_test_mode}"
    # Create unique test name
    test_name_plus_params = f"test_axi4_slave_wr_cg_{id_str}"
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_axi4'], "axi4_slave_wr.sv"),  # Base module
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXI4 Slave Write CG test: {dut_name}")
    ...
    try:
        ...
        print(f"✓ {test_level.upper()} AXI4 Slave Write CG test PASSED")
    except Exception as e:
        print(f"✗ {test_level.upper()} AXI4 Slave Write CG test FAILED: {str(e)}")
