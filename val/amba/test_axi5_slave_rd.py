...
#
# Module: test_axi5_slave_rd
# Purpose: AXI5 Read Slave Test Runner
#
...
"""
AXI5 Read Slave Test Runner

Test runner for the AXI5 read slave using the CocoTB framework.
...
async def axi5_read_slave_test(dut):
    """AXI5 read slave test using the AXI5 framework components"""

    # Create testbench instance
    tb = AXI5SlaveReadTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use the seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXI5 read slave test with seed: {seed}')
    ...
    tb.log.info(f"Starting {test_level.upper()} AXI5 read slave test...")
    ...
    if success_rate >= 95.0:
        tb.log.info(f"ALL {test_level.upper()} AXI5 READ SLAVE TESTS PASSED!")
    else:
        tb.log.error(f"AXI5 READ SLAVE TESTS FAILED (success rate: {success_rate:.1f}%)")
...
def test_axi5_slave_rd(request, stub, id_width, addr_width, data_width, user_width,
                            ar_depth, r_depth, test_level):
    """Test AXI5 read slave with different parameter combinations"""
    ...
    if stub == 1:
        ...
        dut_name = "axi5_slave_rd_stub"
    else:
        dir_key = 'rtl_axi5'
        dut_name = "axi5_slave_rd"
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXI5 Read Slave test: {dut_name}")
    ...
    try:
        ...
        print(f"{test_level.upper()} AXI5 Read Slave test PASSED")
    except Exception as e:
        print(f"{test_level.upper()} AXI5 Read Slave test FAILED: {str(e)}")
