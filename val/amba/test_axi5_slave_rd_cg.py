...
#
# Module: AXI5SlaveReadCGTB
# Purpose: AXI5 Slave Read Clock Gated Test Runner
#
...
"""
AXI5 Slave Read Clock Gated Test Runner

Test runner for the axi5_slave_rd_cg module using the CocoTB framework.
...
from TBClasses.axi5.axi5_slave_read_tb import AXI5SlaveReadTB


class AXI5SlaveReadCGTB(AXI5SlaveReadTB):
    """
    Clock-gated AXI5 Slave Read testbench.
    ...
    def setup_clock_gating_controller(self):
        ...
        try:
            self.cg_ctrl = AxiClockGateCtrl(
                ...
                axi_valid_signals=["fub_axi_arvalid", "fub_axi_rvalid"],
...
async def axi5_slave_read_cg_test(dut):
    """AXI5 slave read clock gated test"""

    tb = AXI5SlaveReadCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXI5 SLAVE READ CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        tb.log.info("=" * 80)
        tb.log.info("AXI5 SLAVE READ CG TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        tb.log.info("AXI5 SLAVE READ CG TEST PASSED")

    except Exception as e:
        tb.log.error(f"AXI5 slave read CG test FAILED: {str(e)}")
        raise
...
@pytest.mark.parametrize(
    "id_width, addr_width, data_width, user_width, ar_depth, r_depth, test_level",
    generate_axi5_cg_params()
)
def test_axi5_slave_rd_cg(id_width, addr_width, data_width, user_width, ar_depth, r_depth, test_level):
    """Test AXI5 slave read clock gated with specified parameters"""
    ...
    dut_name = "axi5_slave_rd_cg"
    reg_level = os.environ.get("REG_LEVEL", "FUNC").upper()
    test_name = f"test_{worker_id}_{dut_name}_iw{id_width}_aw{addr_width}_dw{data_width}_ard{ar_depth}_rd{r_depth}_{test_level}_{reg_level}"
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path="rtl/amba/filelists/axi5_slave_rd_cg.f")
