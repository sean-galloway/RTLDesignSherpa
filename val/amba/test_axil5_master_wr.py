...
#
# Module: test_axil5_master_wr
# Purpose: AXIL5 Write Master Test Runner
#
...
AXIL5 Write Master Test Runner

Test runner for the AXIL5 write master using the CocoTB framework.
Tests various AXIL5 configurations and validates write transactions.

Based on the AXI4 master write test runner pattern but simplified for AXIL5 register access.
"""
...
async def axil5_write_master_test(dut):
    """AXIL5 write master test using the AXIL5 framework components"""

    # Create testbench instance
    tb = AXIL5MasterWriteTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use the seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXIL5 write master test with seed: {seed}')
    ...
    tb.log.info(f"Starting {test_level.upper()} AXIL5 write master test...")
    ...
    try:
        ...
        if success_rate >= 95.0:
            tb.log.info(f"ALL {test_level.upper()} AXIL5 WRITE MASTER TESTS PASSED!")
        else:
            tb.log.error(f"AXIL5 WRITE MASTER TESTS FAILED (success rate: {success_rate:.1f}%)")
            raise RuntimeError(f"Test failed with {success_rate:.1f}% success rate")

    except Exception as e:
        tb.log.error(f"AXIL5 write master test FAILED with exception: {str(e)}")
...
def generate_axil5_params():
    """
    Generate AXIL5 write parameter combinations based on REG_LEVEL.
...
def test_axil5_write_master(request, addr_width, data_width, aw_depth, w_depth, b_depth, test_level):
    """Test AXIL5 write master with different parameter combinations"""
    ...
    # AXIL5 module details (no stub versions)
    dut_name = "axil5_master_wr"
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXIL5 Write Master test: {dut_name}")
    ...
    try:
        ...
        print(f"✓ {test_level.upper()} AXIL5 Write Master test PASSED")
    except Exception as e:
        print(f"✗ {test_level.upper()} AXIL5 Write Master test FAILED: {str(e)}")
