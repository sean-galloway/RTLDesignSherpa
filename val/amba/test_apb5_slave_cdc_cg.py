...
#
# Module: test_apb5_slave_cdc_cg
# Purpose: Test runner for APB5 slave clock domain crossing with clock gating
#
...
"""
APB5 Slave CDC Clock-Gated Test Runner

Tests APB5 slave with clock domain crossing and clock gating.
"""
import os
import random
...
class APB5SlaveCDCCGBasicTB(TBBase):
    """Basic APB5 slave CDC clock-gated testbench."""
    ...
    def __init__(self, dut):
        ...
        self.log.info("="*60)
        self.log.info(" APB5 Slave CDC CG Testbench Configuration")
        self.log.info("-"*60)
...
async def cocotb_test_apb5_slave_cdc_cg_basic(dut):
    """Basic APB5 slave CDC clock-gated test."""
    tb = APB5SlaveCDCCGBasicTB(dut)
    ...
    tb.log.info("=== Test: CDC + Clock Gating Control ===")
    ...
    tb.log.info("=== APB5 Slave CDC CG Basic Test PASSED ===")


def generate_apb5_slave_cdc_cg_params():
    """Generate test parameters for APB5 slave CDC clock-gated."""
    return [
        (12, 32, 4, 0),  # Basic
...
@pytest.mark.parametrize(
    ...
    generate_apb5_slave_cdc_cg_params()
)
def test_apb5_slave_cdc_cg(request, addr_width, data_width, auser_width, enable_parity):
    """APB5 slave CDC clock-gated test runner."""
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_apb5'], "apb5_slave_cdc_cg.sv"),
    ]
    ...
    test_name_plus_params = f"test_{worker_id}_apb5_slave_cdc_cg_aw{aw_str}_dw{dw_str}_au{au_str}_{pr_str}"
    ...
    try:
        run(
            ...
            testcase="cocotb_test_apb5_slave_cdc_cg_basic",
            simulator="verilator",
        )
