...
#
# Module: test_axi4_slave_wr_mon_cg
# Purpose: AXI4 Slave Write Monitor CG Integration Test
#
...
"""
AXI4 Slave Write Monitor CG Integration Test

Thin wrapper that uses the reusable AXI4SlaveMonitorTB testbench class.
Tests the clock-gated version of the AXI4 slave write monitor.
...
def generate_axi4_write_monitor_cg_params():
    """
    Generate AXI4 write monitor CG parameter combinations based on REG_LEVEL.
    ...
    if reg_level == 'GATE':
        ...
        params = [(8, 32, 32, 1, 4, 16, 2, 4, 2, 'basic'), (8, 32, 32, 1, 4, 16, 4, 8, 4, 'medium'), (8, 32, 32, 1, 4, 32, 2, 4, 2, 'medium')]
    else:  # FULL
        test_levels = ['basic', 'medium', 'full']
        configs = [(8, 32, 32, 1, 4, 16, 2, 4, 2), (8, 32, 32, 1, 4, 16, 4, 8, 4), (8, 32, 32, 1, 4, 32, 2, 4, 2)]
        params = [(id_w, addr_w, data_w, user_w, wstrb_w, max_t, skid_aw, skid_w, skid_b, level) for (id_w, addr_w, data_w, user_w, wstrb_w, max_t, skid_aw, skid_w, skid_b) in configs for level in test_levels]
...
async def axi4_slave_wr_mon_cg_test(dut):
    """AXI4 slave write monitor CG integration test"""
...
@pytest.mark.parametrize(
    ...
    generate_axi4_write_monitor_cg_params()
)
def test_axi4_slave_wr_mon_cg(id_width, addr_width, data_width, user_width, wstrb_width, max_trans, skid_aw, skid_w, skid_b, test_level):
    """
    Integration test runner for AXI4 slave write monitor CG.
