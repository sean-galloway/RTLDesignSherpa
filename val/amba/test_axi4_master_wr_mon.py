...
#
# Module: test_axi4_master_wr_mon
# Purpose: AXI4 Master Write Monitor Integration Test
#
...
def generate_axi4_write_monitor_params():
    ...
    if reg_level == 'GATE':
        ...
        configs = [
            ...
            (8, 32, 32, 1, 4, 32, 2, 4, 2),  # Many transactions
        ]
        params = [
...
async def axi4_master_wr_mon_wavedrom_test(dut):
    """
    AXI4 master write monitor WaveDrom timing diagram generation.
    ...
    # Setup WaveDrom with constraint solver (no field config needed for manual bindings)
    ...
    # Manually bind all AXI write channel signals
    wave_solver.add_signal_binding('aclk', 'aclk')
    wave_solver.add_signal_binding('aresetn', 'aresetn')
    # AW channel
    wave_solver.add_signal_binding('m_axi_awvalid', 'm_axi_awvalid')
    wave_solver.add_signal_binding('m_axi_awready', 'm_axi_awready')
    wave_solver.add_signal_binding('m_axi_awid', 'm_axi_awid')
    wave_solver.add_signal_binding('m_axi_awaddr', 'm_axi_awaddr')
    wave_solver.add_signal_binding('m_axi_awlen', 'm_axi_awlen')
    # W channel
    wave_solver.add_signal_binding('m_axi_wvalid', 'm_axi_wvalid')
    wave_solver.add_signal_binding('m_axi_wready', 'm_axi_wready')
    wave_solver.add_signal_binding('m_axi_wdata', 'm_axi_wdata')
    wave_solver.add_signal_binding('m_axi_wlast', 'm_axi_wlast')
    wave_solver.add_signal_binding('m_axi_wstrb', 'm_axi_wstrb')
    # B channel
    wave_solver.add_signal_binding('m_axi_bvalid', 'm_axi_bvalid')
    wave_solver.add_signal_binding('m_axi_bready', 'm_axi_bready')
    wave_solver.add_signal_binding('m_axi_bid', 'm_axi_bid')
    wave_solver.add_signal_binding('m_axi_bresp', 'm_axi_bresp')
    # MonBus
    ...
    complete_signals = [
        'aclk', 'aresetn', '|',
        ['AW Channel', 'm_axi_awvalid', 'm_axi_awready', 'm_axi_awid', 'm_axi_awaddr', 'm_axi_awlen'],
        '|',
        ['W Channel', 'm_axi_wvalid', 'm_axi_wready', 'm_axi_wdata', 'm_axi_wlast', 'm_axi_wstrb'],
        '|',
        ['B Channel', 'm_axi_bvalid', 'm_axi_bready', 'm_axi_bid', 'm_axi_bresp'],
        '|',
        ['Monitor Bus', 'monbus_valid', 'monbus_ready']
    ]
    ...
    single_write_constraint = TemporalConstraint(
        ...
        events=[
            TemporalEvent("aw_handshake", SignalTransition("m_axi_awvalid", 0, 1))
        ],
    ...
    second_write_constraint = TemporalConstraint(
        ...
        events=[
            TemporalEvent("aw_handshake_2", SignalTransition("m_axi_awvalid", 0, 1))
        ],
        temporal_relation=TemporalRelation.SEQUENCE,
        max_window_size=80,
        required=False,
    ...
    dut._log.info(f"WaveDrom configured with {len(wave_solver.constraints)} AXI4 write constraints")
...
def test_axi4_master_wr_mon(id_width, addr_width, data_width, user_width, wstrb_width, max_trans, skid_aw, skid_w, skid_b, test_level):
    """
    Integration test runner for AXI4 master write monitor.
