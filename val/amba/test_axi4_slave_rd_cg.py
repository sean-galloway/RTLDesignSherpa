...
#
# Module: AXI4SlaveReadCGTB
# Purpose: AXI4 Slave Read Clock Gated Test Runner
#
...
"""
AXI4 Slave Read Clock Gated Test Runner

Test runner for the axi4_slave_rd_cg module using the CocoTB framework.
Tests clock gating functionality while validating slave read response behavior.
...
class AXI4SlaveReadCGTB(AXI4SlaveReadTB):
    """
    Clock-gated AXI4 Slave Read testbench.
    Extends the base slave read testbench with clock gating capabilities.
    """
    ...
    def setup_clock_gating_controller(self):
        ...
        try:
            self.cg_ctrl = AxiClockGateCtrl(
                ...
                user_valid_signals=["s_axi_arvalid", "s_axi_rready"],
                axi_valid_signals=["fub_axi_arvalid", "fub_axi_rvalid", "s_axi_rvalid"],
...
async def axi4_slave_read_cg_test(dut):
    """AXI4 slave read clock gated test using the enhanced framework"""

    # Create clock-gated testbench instance
    tb = AXI4SlaveReadCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXI4 slave read CG test with seed: {seed}')
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXI4 SLAVE READ CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        if cg_test_mode in ['comprehensive', 'basic']:
            ...
            for i in range(5):
                ...
                success2, _, _ = await tb.single_read_response_test(addr + 8)
        ...
        # === FINAL RESULTS ===
        tb.log.info("=" * 80)
        tb.log.info("CLOCK GATED AXI4 SLAVE READ TEST SUMMARY")
        tb.log.info("=" * 80)
...
def generate_axi4_cg_params():
    """Generate test parameters for clock-gated AXI4 slave read testing"""
...
def test_axi4_slave_read_cg(id_width, addr_width, data_width, user_width, ar_depth, r_depth, test_level, cg_test_mode):
    """Test AXI4 slave read clock gated with specified parameters"""
    ...
    test_name_plus_params = f"test_{worker_id}_axi4_slave_rd_cg_{id_str}"
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_axi4'], "axi4_slave_rd.sv"),  # Base module
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXI4 Slave Read CG test: {dut_name}")
