...
#
# Module: test_axi5_master_wr
# Purpose: AXI5 Write Master Test Runner
#
...
"""
AXI5 Write Master Test Runner

Test runner for the AXI5 write master using the CocoTB framework.
Tests various AXI5 configurations and validates write transactions with
AXI5-specific features like ATOP (atomics), NSAID, TRACE, MPAM, MECID,
TAGOP, POISON, etc.
...
from TBClasses.axi5.axi5_master_write_tb import AXI5MasterWriteTB


@cocotb.test(timeout_time=30, timeout_unit="sec")
async def axi5_write_master_test(dut):
    """AXI5 write master test using the AXI5 framework components"""

    # Create testbench instance
    tb = AXI5MasterWriteTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)
    ...
    tb.log.info(f"Starting {test_level.upper()} AXI5 write master test...")
    ...
    if test_level == 'gate':
        ...
        timing_profiles = ['normal', 'fast', 'slow', 'backtoback', 'stress', 'atomic', 'mte', 'secure']
    ...
    success, info = await tb.single_write_test(0x4000, 0xDEADBEEF)
    if not success:
        tb.log.error("Basic connectivity test failed!")
        raise RuntimeError("Basic connectivity failed")

    tb.log.info(f"Basic connectivity test passed")
    ...
    if test_level in ['func', 'full']:
        ...
        if result:
            tb.log.info(f"Atomic operation tests passed ({atomic_count} tests)")
        else:
    ...
    if test_level in ['func', 'full']:
        ...
        for i in range(mixed_total):
            if i % 2 == 0:
                ...
                # Short burst write
                addr = 0x5000 + (i * 16)
                data_list = [0x50000000 + (i * 4) + j for j in range(4)]
                success, _ = await tb.burst_write_test(addr, data_list)
    ...
    # Test 8: Security and MTE features (full level)
    if test_level == 'full':
        tb.log.info("=== Test 8: Security and MTE Features ===")
        ...
        for i in range(security_count):
            ...
            success, _ = await tb.single_write_test(
                ...
                poison=random.randint(0, 1) if random.random() > 0.8 else 0
            )
            if not success:
    ...
    if success_rate >= 95.0:
        tb.log.info(f"ALL {test_level.upper()} AXI5 WRITE MASTER TESTS PASSED!")
    else:
        tb.log.error(f"AXI5 WRITE MASTER TESTS FAILED (success rate: {success_rate:.1f}%)")
...
def generate_axi5_params():
    ...
    if reg_level == 'GATE':
        ...
        for stub, id_w, addr_w, (aw_d, w_d, b_d), level in product(
                stubs, id_widths, addr_widths, depths, test_levels):
...
def test_axi5_write_master(request, stub, id_width, addr_width, data_width, user_width,
                            aw_depth, w_depth, b_depth, test_level):
    """Test AXI5 write master with different parameter combinations"""
    ...
    if stub == 1:
        ...
        dut_name = "axi5_master_wr_stub"
    else:
        dir_key = 'rtl_axi5'
        dut_name = "axi5_master_wr"
    ...
    aw_size = (id_width + addr_width + 8 + 3 + 2 + 1 + 4 + 3 + 4 + user_width +
               atop_width + nsaid_width + 1 + mpam_width + mecid_width + 1 +
               tagop_width + (tag_width * num_tags))
    w_size = data_width + wstrb_width + 1 + user_width + 1 + (tag_width * num_tags) + num_tags
    b_size = id_width + 2 + user_width + 1 + (tag_width * num_tags) + 1
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXI5 Write Master test: {dut_name}")
    print(f"AXI5 Config: ID={id_width}, ADDR={addr_width}, DATA={data_width}, USER={user_width}")
    print(f"AXI5 Features: ATOP={atop_width}, NSAID={nsaid_width}, MPAM={mpam_width}")
    ...
    try:
        ...
        print(f"{test_level.upper()} AXI5 Write Master test PASSED")
    except Exception as e:
        print(f"{test_level.upper()} AXI5 Write Master test FAILED: {str(e)}")
