...
#
# Module: test_axis5_slave_cg
# Purpose: Test runner for AXIS5 slave clock-gated variant
#
...
"""
AXIS5 Slave Clock-Gated Test Runner

Tests AXIS5 slave with clock gating for power management.
"""
import os
import random
...
async def cocotb_test_axis5_slave_cg_basic(dut):
    """Basic AXIS5 slave clock-gated test."""
    tb = AXIS5SlaveCGBasicTB(dut)
    ...
    tb.log.info("=== AXIS5 Slave CG Basic Test PASSED ===")


def generate_axis5_slave_cg_params():
    """Generate test parameters for AXIS5 slave clock-gated."""
    return [
...
@pytest.mark.parametrize(
    ...
    generate_axis5_slave_cg_params()
)
def test_axis5_slave_cg(request, skid_depth, data_width, enable_wakeup, enable_parity):
    """AXIS5 slave clock-gated test runner."""
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_axis5'], "axis5_slave_cg.sv"),
    ]
    ...
    test_name_plus_params = f"test_{worker_id}_axis5_slave_cg_sd{sd_str}_dw{dw_str}_{wk_str}"
    ...
    try:
        run(
            ...
            testcase="cocotb_test_axis5_slave_cg_basic",
            simulator="verilator",
        )
