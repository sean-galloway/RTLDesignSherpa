...
#
# Module: AXI4MasterReadCGTB
# Purpose: AXI4 Master Read Clock Gated Test Runner
#
...
"""
AXI4 Master Read Clock Gated Test Runner

Test runner for the axi4_master_rd_cg module using the CocoTB framework.
...
class AXI4MasterReadCGTB(AXI4MasterReadTB):
    """
    Clock-gated AXI4 Master Read testbench.
    ...
    def setup_clock_gating_controller(self):
        ...
        try:
            self.cg_ctrl = AxiClockGateCtrl(
                ...
                user_valid_signals=["fub_axi_arvalid", "fub_axi_rready"],
    ...
    async def validate_ready_signals_during_gating(self):
        ...
        if gating_active:
            ...
            if hasattr(self.dut, 'fub_axi_arready'):
                arready_ok = not bool(self.dut.fub_axi_arready.value)
                ready_checks.append(('fub_axi_arready', arready_ok))
                
            if hasattr(self.dut, 'm_axi_rready'):
                rready_ok = not bool(self.dut.m_axi_rready.value)
                ready_checks.append(('m_axi_rready', rready_ok))
...
async def axi4_master_read_cg_test(dut):
    """AXI4 master read clock gated test using the enhanced framework"""

    # Create clock-gated testbench instance
    tb = AXI4MasterReadCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXI4 master read CG test with seed: {seed}')
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXI4 MASTER READ CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        if cg_test_mode in ['comprehensive', 'basic'] or test_level != 'efficiency':
            ...
            async def basic_read_test():
                ...
                for i in range(5):
                    address = 0x1000 + (i * 4)
                    success, data, info = await tb.single_read_test(address)
            ...
            if gating_achieved:
                ...
                success, data, info = await tb.single_read_test(0x3000)
            ...
            tb.log.info("=== Power Efficiency Measurement ===")
        ...
        # === FINAL RESULTS ===
        tb.log.info("=" * 80)
        tb.log.info("CLOCK GATED AXI4 MASTER READ TEST SUMMARY")
        tb.log.info("=" * 80)
...
def generate_axi4_cg_params():
    """Generate test parameters for clock-gated AXI4 master read testing"""
    ...
    if debug_mode:
        return [
            (8, 32, 32, 1, 2, 4, 'full', 'comprehensive'),
            # (8, 32, 32, 1, 2, 4, 'basic', 'efficiency'),
        ]
    
    return list(product(id_widths, addr_widths, data_widths, user_widths,
                        ar_depths, r_depths, test_levels, cg_test_modes))


@pytest.mark.parametrize("id_width, addr_width, data_width, user_width, ar_depth, r_depth, test_level, cg_test_mode",
                        generate_axi4_cg_params())
def test_axi4_master_read_cg(id_width, addr_width, data_width, user_width, ar_depth, r_depth, test_level, cg_test_mode):
    """Test AXI4 master read clock gated with specified parameters"""
    ...
    test_name_plus_params = f"test_{worker_id}_axi4_master_rd_cg_{id_str}"
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_axi4'], "axi4_master_rd.sv"),  # Base module
