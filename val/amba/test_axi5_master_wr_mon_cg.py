...
#
# Module: test_axi5_master_wr_mon_cg
# Purpose: AXI5 Master Write Monitor with Clock Gating Integration Test
#
...
"""
AXI5 Master Write Monitor with Clock Gating Integration Test

Tests the clock-gated version of the AXI5 master write monitor.
"""
...
async def axi5_master_wr_mon_cg_test(dut):
    """AXI5 master write monitor clock-gated integration test"""

    test_level = os.environ.get('TEST_LEVEL', 'basic').lower()

    tb = AXI5MasterMonitorTB(dut, is_write=True, aclk=dut.aclk, aresetn=dut.aresetn)
    await tb.initialize()
...
def generate_axi5_monitor_cg_params():
    ...
    if reg_level == 'GATE':
        params = [(8, 32, 32, 1, 16, 2, 4, 2, 'basic')]
    elif reg_level == 'FUNC':
        params = [
            (8, 32, 32, 1, 16, 2, 4, 2, 'basic'),
            (8, 32, 32, 1, 16, 4, 8, 4, 'medium'),
            (8, 32, 32, 1, 32, 2, 4, 2, 'medium'),
        ]
    else:  # FULL
        test_levels = ['basic', 'medium', 'full']
        configs = [(8, 32, 32, 1, 16, 2, 4, 2), (8, 32, 32, 1, 16, 4, 8, 4), (8, 32, 32, 1, 32, 2, 4, 2)]
        params = [
...
def test_axi5_master_wr_mon_cg(id_width, addr_width, data_width, user_width, max_trans, skid_aw, skid_w, skid_b, test_level):
    """Integration test runner for AXI5 master write monitor with clock gating."""
    ...
    dut_name = "axi5_master_wr_mon_cg"
    reg_level = os.environ.get("REG_LEVEL", "FUNC").upper()
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_axi5'], "axi5_master_wr_cg.sv"),
        ...
        os.path.join(rtl_dict['rtl_axi5'], "axi5_master_wr_mon.sv"),
    ...
    print(f"\n{'='*80}")
    print(f"AXI5 Master Write Monitor CG Integration Test")
