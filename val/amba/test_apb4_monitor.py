...
async def simple_apb4_monitor_test(dut):
...
def test_apb4_monitor():
    ...
    try:
        run(
            ...
            # Pin the testcase: this module also holds apb4_monitor_addr_range_test,
...
async def apb4_monitor_addr_range_test(dut):
    ...
    apb4_monitor already carries the correct 128-bit addr_check path. This test
    ...
    for idx, (lo, _hi) in enumerate(ranges):
        for is_write in (True, False):
            ...
            tb.log.info(f"apb4_monitor range {idx} {'WR' if is_write else 'RD'} "
                        f"addr=0x{addr:X} -> {f}")
...
def test_apb4_monitor_addr_range():
    ...
    try:
        run(
            ...
            plus_args=(['--trace'] if enable_waves else []),
            compile_args=compile_args,
            testcase="apb4_monitor_addr_range_test",
        )
    except Exception as e:
