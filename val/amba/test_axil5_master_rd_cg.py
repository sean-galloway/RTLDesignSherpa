...
#
# Module: AXIL5MasterReadCGTB
# Purpose: AXIL5 Master Read Clock Gated Test Runner
#
...
AXIL5 Master Read Clock Gated Test Runner

Test runner for the axil5_master_rd_cg module using the CocoTB framework.
...
class AXIL5MasterReadCGTB(AXIL5MasterReadTB):
    """
    Clock-gated AXIL5 Master Read testbench.
...
async def axil5_master_read_cg_test(dut):
    """AXIL5 master read clock gated test using the enhanced framework"""

    # Create clock-gated testbench instance
    tb = AXIL5MasterReadCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXIL5 master read CG test with seed: {seed}')
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXIL5 MASTER READ CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        # === FINAL RESULTS ===
        tb.log.info("=" * 80)
        tb.log.info("CLOCK GATED AXIL5 MASTER READ TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        if success_rate < 90:
            tb.log.error("AXIL5 MASTER READ CG TEST FAILED")
            raise Exception(f"Clock gated test failed with {success_rate:.1f}% success rate")
            
        tb.log.info("AXIL5 MASTER READ CG TEST PASSED")

    except Exception as e:
        tb.log.error(f"AXIL5 master read CG test FAILED with exception: {str(e)}")
        raise


def generate_axil5_cg_params():
    """Generate test parameters for clock-gated AXIL5 master read testing"""
...
def test_axil5_master_read_cg(addr_width, data_width, ar_depth, r_depth, test_level, cg_test_mode):
    """Test AXIL5 master read clock gated with specified parameters"""
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path="rtl/amba/filelists/axil5_master_rd_cg.f")
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXIL5 Master Read Clock Gated test: {dut_name}")
    ...
    try:
        ...
        print(f"✓ {test_level.upper()} AXIL5 Master Read Clock Gated test PASSED")
    except Exception as e:
        print(f"✗ {test_level.upper()} AXIL5 Master Read Clock Gated test FAILED: {str(e)}")
