...
#
# Module: test_axi_master_rd_splitter
# Purpose: AXI Master Read Splitter Test Runner
#
...
"""
AXI Master Read Splitter Test Runner

Comprehensive test runner for the AXI master read splitter module following
the patterns established in the test_axi4_gen_addr.py test runner.
...
async def axi_read_splitter_test(dut):
    """Main test function for AXI read splitter"""
    tb = AxiReadSplitterTB(dut)

    # Use the seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXI Read Splitter test with seed: {seed}')
    ...
    tb.log.info(f"Starting {test_level.upper()} AXI read splitter test...")
    ...
    # Verify test result
    assert passed, f"AXI read splitter test failed at level {test_level}"

    tb.log.info(f"✓ ALL {test_level.upper()} AXI READ SPLITTER TESTS PASSED!")
...
def test_axi_read_splitter(request, iw, aw, dw, uw, fifo_depth, alignment_mask, test_level):
    """Test AXI read splitter with parametrized configurations"""
    ...
    dut_name = "axi_master_rd_splitter"
    ...
    test_name_plus_params = (f"test_axi_rd_splitter_"
    ...
    total_complexity = complexity_factor * data_complexity * fifo_complexity * id_complexity
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXI Read Splitter test")
    ...
    try:
        ...
        print(f"✗ {test_level.upper()} AXI Read Splitter test FAILED: {str(e)}")
