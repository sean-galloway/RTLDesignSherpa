...
#
# Module: test_axil5_slave_wr
# Purpose: AXIL5 Slave Write Test Runner
#
...
AXIL5 Slave Write Test Runner

Test runner for the AXIL5 slave write module using the CocoTB framework.
Tests various AXIL5 configurations and validates write response behavior
...
async def axil5_slave_write_test(dut):
    """AXIL5 slave write test using the AXIL5 framework components"""

    # Create testbench instance
    tb = AXIL5SlaveWriteTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use the seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXIL5 slave write test with seed: {seed}')
    ...
    tb.log.info(f"Starting {test_level.upper()} AXIL5 slave write test...")
    ...
    try:
        ...
        tb.log.info("=== Test 1: Basic AXIL5 Slave Write Connectivity ===")
        total_tests += 1
        ...
        tb.log.info("=== Test 7: AXIL5 Slave Write Stress Testing ===")
        total_tests += 1
        tb.set_timing_profile('stress')

        tb.log.info(f"Running AXIL5 slave write stress test with {stress_count} register writes...")
        success = await tb.stress_register_write_test(stress_count)
        if not success:
            error_msg = "AXIL5 slave write stress test failed"
            tb.log.error(error_msg)
            raise Exception(error_msg)
        ...
        tb.log.info("="*80)
        tb.log.info("AXIL5 SLAVE WRITE TEST RESULTS")
        tb.log.info("="*80)
        ...
        tb.log.error(f"AXIL5 slave write test failed with exception: {str(e)}")
        final_stats = tb.get_test_stats()
...
def generate_axil5_params():
    """
    Generate AXIL5 slave write parameter combinations based on REG_LEVEL.
...
def test_axil5_slave_write(request, addr_width, data_width, aw_depth, w_depth, b_depth, test_level):
    """Test AXIL5 slave write with different parameter combinations"""
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXIL5 Slave Write test: {dut_name}")
