...
#
# Module: AXIL5SlaveWriteCGTB
# Purpose: AXIL5 Slave Write Clock Gated Test Runner
#
...
the TB class is the AXIL4 one with the component factories swapped and the
AXI5-Lite optional groups enabled, so every phase, check and randomizer
sweep here is the AXI4-Lite flow running against AXI5-Lite RTL.

What differs is the DUT and what rides with it. The AXI5-Lite modules default
every ENABLE_* to 1, so this exercises AxUSER/AxTRACE/AxLOOP/AxMPAM/AxMECID/
AxNSAID/AxLOCK and W/R POISON on real transport RTL -- which nothing did
before: the only AXI5-Lite DUT was a behavioural test slave, and
test_axil5_master_rd.py drives AXI4-Lite RTL by design, to pin the
groups-off equivalence.
AXIL5 Slave Write Clock Gated Test Runner

Test runner for the axil5_slave_wr_cg module using the CocoTB framework.
Tests clock gating functionality while validating AXIL5 slave write response behavior
for single transfer register write patterns.

Based on the AXI4 clock gated test runner but simplified for AXIL5 specification.
"""
...
from TBClasses.axil5.axil5_slave_write_tb import AXIL5SlaveWriteTB


class AXIL5SlaveWriteCGTB(AXIL5SlaveWriteTB):
    """
    Clock-gated AXIL5 Slave Write testbench.
    Extends the base AXIL5 slave write testbench with clock gating capabilities.
    """
    ...
    def setup_clock_gating_controller(self):
        ...
        try:
            ...
            self.log.info("✅ AXIL5 Clock gating controller initialized")
    ...
    async def configure_clock_gating(self, enable=True, idle_count=8):
        ...
        self.log.info(f"AXIL5 clock gating configured: enable={enable}, idle_count={idle_count}")
    ...
    async def measure_power_efficiency(self, test_duration_cycles=1000):
        ...
        self.log.info(f"AXIL5 write power efficiency: {efficiency:.1f}% ({gated_cycles}/{test_duration_cycles} cycles gated)")
...
async def axil5_slave_write_cg_test(dut):
    """AXIL5 slave write clock gated test using the enhanced framework"""

    # Create clock-gated testbench instance
    tb = AXIL5SlaveWriteCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXIL5 slave write CG test with seed: {seed}')
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXIL5 SLAVE WRITE CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    tb.log.info(f"CG Test Mode: {cg_test_mode.upper()}")
    tb.log.info(f"AXIL5 widths: ADDR={tb.TEST_ADDR_WIDTH}, DATA={tb.TEST_DATA_WIDTH}")
    ...
    try:
        ...
        if cg_test_mode in ['comprehensive', 'basic']:
            ...
            tb.log.info("=== Test 1: AXIL5 Write Functional Equivalence ===")
            
            equivalence, results = await tb.functional_equivalence_test(test_iterations=test_transactions)
            total_tests += 1
            if equivalence:
                passed_tests += 1
                tb.log.info("✅ AXIL5 write functional equivalence: PASS")
            else:
                tb.log.error("❌ AXIL5 write functional equivalence: FAIL")

            # Test 2: Power Efficiency at Different Idle Counts
            tb.log.info("=== Test 2: AXIL5 Write Power Efficiency Testing ===")
            
            for idle_count in idle_counts:
                tb.log.info(f"Testing AXIL5 write idle count: {idle_count}")
                ...
                if efficiency_result['power_efficiency_percent'] > 10.0:  # Reasonable threshold
                    passed_tests += 1
                    tb.log.info(f"✅ AXIL5 write efficiency test (idle={idle_count}): {efficiency_result['power_efficiency_percent']:.1f}%")
                else:
                    tb.log.warning(f"⚠️ AXIL5 write low efficiency (idle={idle_count}): {efficiency_result['power_efficiency_percent']:.1f}%")

            # Test 3: Clock Gating Disable/Enable Transitions
            tb.log.info("=== Test 3: AXIL5 Write Enable/Disable Transitions ===")
            ...
            if transition_success >= 4:  # Allow one failure
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 write transition test: {transition_success}/5 successful")
            else:
                tb.log.error(f"❌ AXIL5 write transition test: {transition_success}/5 successful")

        # === EFFICIENCY-FOCUSED TESTS ===
        if cg_test_mode in ['efficiency']:
            tb.log.info("=== Efficiency Mode: Extended AXIL5 Write Power Testing ===")
            ...
            if efficiency_result['power_efficiency_percent'] > 15.0:  # Higher threshold for efficiency mode
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 write extended efficiency: {efficiency_result['power_efficiency_percent']:.1f}%")
            else:
                tb.log.warning(f"⚠️ AXIL5 write extended efficiency: {efficiency_result['power_efficiency_percent']:.1f}%")

        # Test 4: Register Write with Clock Gating (medium and full levels)
        if test_level in ['func', 'full']:
            tb.log.info("=== Test 4: AXIL5 Register Write with Clock Gating ===")
            ...
            if success:
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 register write with gating: {success_count}/{total_count} successful")
            else:
                tb.log.error(f"❌ AXIL5 register write with gating: {success_count}/{total_count} successful")

        # Test 5: Strobe Pattern with Clock Gating (full level)
        if test_level == 'full':
            tb.log.info("=== Test 5: AXIL5 Strobe Pattern with Clock Gating ===")
            ...
            if success:
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 strobe pattern with gating: {success_count}/{total_patterns} successful")
            else:
                tb.log.error(f"❌ AXIL5 strobe pattern with gating: {success_count}/{total_patterns} successful")

        # Test 6: Stress Testing with Clock Gating (full level)
        if test_level == 'full':
            tb.log.info("=== Test 6: AXIL5 Clock Gated Write Stress Testing ===")
            ...
            if stress_success >= int(stress_count * 0.9):  # Allow 10% failures in stress test
                passed_tests += 1
                tb.log.info(f"✅ AXIL5 write stress test: {stress_success}/{stress_count} responses successful")
            else:
                tb.log.error(f"❌ AXIL5 write stress test: {stress_success}/{stress_count} responses successful")

        # === FINAL RESULTS ===
        tb.log.info("=" * 80)
        tb.log.info("CLOCK GATED AXIL5 SLAVE WRITE TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        if power_efficiency_results:
            tb.log.info("AXIL5 Write Power Efficiency Results:")
            for result in power_efficiency_results:
                tb.log.info(f"  Idle count {result['idle_count']}: {result['efficiency_percent']:.1f}% efficiency")
                
        if success_rate < 90:
            tb.log.error("❌ AXIL5 SLAVE WRITE CG TEST FAILED")
            raise Exception(f"Clock gated test failed with {success_rate:.1f}% success rate")
            
        tb.log.info("✅ AXIL5 SLAVE WRITE CG TEST PASSED")

    except Exception as e:
        tb.log.error(f"AXIL5 slave write CG test FAILED with exception: {str(e)}")
        raise
    finally:
...
def generate_axil5_cg_params():
    """Generate test parameters for clock-gated AXIL5 slave write testing"""
...
@pytest.mark.parametrize("addr_width, data_width, aw_depth, w_depth, b_depth, test_level, cg_test_mode",
                        generate_axil5_cg_params())
def test_axil5_slave_write_cg(addr_width, data_width, aw_depth, w_depth, b_depth, test_level, cg_test_mode):
    """Test AXIL5 slave write clock gated with specified parameters"""
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXIL5 Slave Write CG test: {dut_name}")
    print(f"AXIL5 Config: ADDR={addr_width}, DATA={data_width}")
    ...
    try:
        ...
        print(f"✅ {test_level.upper()} AXIL5 Slave Write CG test PASSED")
    except Exception as e:
        print(f"❌ {test_level.upper()} AXIL5 Slave Write CG test FAILED: {str(e)}")
