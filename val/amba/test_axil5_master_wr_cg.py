...
#
# Module: AXIL5MasterWriteCGTB
# Purpose: AXIL5 Master Write Clock Gated Test Runner
#
...
AXIL5 Master Write Clock Gated Test Runner

Test runner for the axil5_master_wr_cg module using the CocoTB framework.
Tests clock gating functionality while validating write transactions.

Based on the AXI4 clock gated test runner pattern but simplified for AXIL5 register access.
"""
...
from TBClasses.axil5.axil5_master_write_tb import AXIL5MasterWriteTB


class AXIL5MasterWriteCGTB(AXIL5MasterWriteTB):
    """
    Clock-gated AXIL5 Master Write testbench.
...
async def axil5_master_write_cg_test(dut):
    """AXIL5 master write clock gated test using the enhanced framework"""

    # Create clock-gated testbench instance
    tb = AXIL5MasterWriteCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXIL5 master write CG test with seed: {seed}')
    ...
    tb.log.info("=" * 80)
    tb.log.info(f"AXIL5 MASTER WRITE CLOCK GATED TEST - {test_level.upper()} LEVEL")
    tb.log.info("=" * 80)
    ...
    try:
        ...
        # === FINAL RESULTS ===
        tb.log.info("=" * 80)
        tb.log.info("CLOCK GATED AXIL5 MASTER WRITE TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        if success_rate < 90:
            tb.log.error("AXIL5 MASTER WRITE CG TEST FAILED")
            raise Exception(f"Clock gated test failed with {success_rate:.1f}% success rate")
            
        tb.log.info("AXIL5 MASTER WRITE CG TEST PASSED")

    except Exception as e:
        tb.log.error(f"AXIL5 master write CG test FAILED with exception: {str(e)}")
        raise


def generate_axil5_cg_params():
    """Generate test parameters for clock-gated AXIL5 master write testing"""
...
def test_axil5_master_write_cg(addr_width, data_width, aw_depth, w_depth, b_depth, test_level, cg_test_mode):
    """Test AXIL5 master write clock gated with specified parameters"""
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path="rtl/amba/filelists/axil5_master_wr_cg.f")
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXIL5 Master Write Clock Gated test: {dut_name}")
    ...
    try:
        ...
        print(f"✓ {test_level.upper()} AXIL5 Master Write Clock Gated test PASSED")
    except Exception as e:
        print(f"✗ {test_level.upper()} AXIL5 Master Write Clock Gated test FAILED: {str(e)}")
