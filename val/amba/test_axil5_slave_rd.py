...
#
# Module: test_axil5_slave_rd
# Purpose: AXIL5 Slave Read Test Runner
#
...
AXIL5 Slave Read Test Runner

Test runner for the AXIL5 slave read module using the CocoTB framework.
Tests various AXIL5 configurations and validates read response behavior
for single transfer register access patterns.

Based on AXI4 test runner but simplified for AXIL5 specification:
- No stub version (AXIL5 only has full RTL)
- No ID width parameter (AXIL5 doesn't use IDs)  
...
async def axil5_slave_read_test(dut):
    """AXIL5 slave read test using the AXIL5 framework components"""

    # Create testbench instance
    tb = AXIL5SlaveReadTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    # Use the seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXIL5 slave read test with seed: {seed}')
    ...
    tb.log.info(f"Starting {test_level.upper()} AXIL5 slave read test...")
    ...
    try:
        ...
        tb.log.info("=== Test 1: Basic AXIL5 Slave Connectivity ===")
        total_tests += 1
        ...
        tb.log.info("=== Test 6: AXIL5 Slave Stress Testing ===")
        total_tests += 1
        tb.set_timing_profile('stress')

        tb.log.info(f"Running AXIL5 slave stress test with {stress_count} register reads...")
        success = await tb.stress_register_access_test(stress_count)
        if not success:
            error_msg = "AXIL5 slave stress test failed"
            tb.log.error(error_msg)
            raise Exception(error_msg)
        ...
        tb.log.info("="*80)
        tb.log.info("AXIL5 SLAVE READ TEST RESULTS")
        tb.log.info("="*80)
        ...
        tb.log.error(f"AXIL5 slave read test failed with exception: {str(e)}")
        final_stats = tb.get_test_stats()
...
def generate_axil5_params():
    """
    Generate AXIL5 slave read parameter combinations based on REG_LEVEL.
    ...
    if reg_level == 'GATE':
        params = [(32, 32, 2, 4, 'gate')]
    elif reg_level == 'FUNC':
        params = [(32, 32, 2, 4, 'gate'), (32, 32, 4, 8, 'func'), (64, 64, 2, 4, 'func')]
    else:  # FULL
...
@pytest.mark.parametrize("addr_width, data_width, ar_depth, r_depth, test_level",
                        generate_axil5_params())
def test_axil5_slave_read(request, addr_width, data_width, ar_depth, r_depth, test_level):
    """Test AXIL5 slave read with different parameter combinations"""
    ...
    # RTL parameters (simplified for AXIL5)
    rtl_parameters = {
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXIL5 Slave Read test: {dut_name}")
