...
#
# Module: test_axi5_slave_wr
# Purpose: AXI5 Write Slave Test Runner
#
...
"""
AXI5 Write Slave Test Runner

Test runner for the AXI5 write slave using the CocoTB framework.
Tests various AXI5 configurations and validates write transactions with
AXI5-specific features like ATOP, NSAID, TRACE, MPAM, MECID, TAGOP, TAG,
POISON, and TAGMATCH.
...
async def axi5_write_slave_test(dut):
    """AXI5 write slave test using the AXI5 framework components"""

    # Create testbench instance
    tb = AXI5SlaveWriteTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)
    ...
    tb.log.info(f"Starting {test_level.upper()} AXI5 write slave test...")
    ...
    test_data = random.randint(0, (1 << tb.TEST_DATA_WIDTH) - 1)
    success, response_info = await tb.single_write_test(0x1000, test_data)
    if not success:
        tb.log.error("Basic connectivity test failed!")
        raise RuntimeError("Basic connectivity failed")

    tb.log.info(f"Basic connectivity test passed: data=0x{test_data:08X}, response={response_info}")

    # Test 2: Single write sequences with different timing profiles
    tb.log.info("=== Test 2: Single Write Sequences ===")

    for i, (profile, count) in enumerate(zip(timing_profiles, single_write_counts)):
        tb.log.info(f"[{i+1}/{len(timing_profiles)}] Single writes with '{profile}' timing ({count} writes)")
        tb.set_timing_profile(profile)

        result = await tb.basic_write_sequence(count)
        if not result:
            tb.log.error(f"Single write sequence failed with '{profile}' timing")
        else:
            tb.log.info(f"Single write sequence passed with '{profile}' timing")

    # Test 3: Burst write sequences
    tb.log.info("=== Test 3: Burst Write Sequences ===")

    for i, (profile, lengths) in enumerate(zip(timing_profiles, burst_lengths)):
        tb.log.info(f"[{i+1}/{len(timing_profiles)}] Burst writes with '{profile}' timing: {lengths}")
        tb.set_timing_profile(profile)

        result = await tb.burst_write_sequence(lengths)
        if not result:
            tb.log.error(f"Burst write sequence failed with '{profile}' timing")
        else:
            tb.log.info(f"Burst write sequence passed with '{profile}' timing")
    ...
    # Test 5: Atomic operations (AXI5 ATOP feature)
    ...
    if test_level in ['medium', 'full']:
        ...
        for i in range(mixed_total):
            if i % 2 == 0:
                # Single write with AXI5 features
                addr = 0x1000 + (i * 4)
                data = random.randint(0, (1 << tb.TEST_DATA_WIDTH) - 1)
                nsaid = random.randint(0, 15)
                success, _ = await tb.single_write_test(addr, data, nsaid=nsaid, trace=1)
            else:
                # Short burst write
                addr = 0x2000 + (i * 16)
                burst_data = [random.randint(0, (1 << tb.TEST_DATA_WIDTH) - 1) for _ in range(4)]
                success, _ = await tb.burst_write_test(addr, burst_data)
    ...
    # Test 8: Poison and MTE features (full level)
    if test_level == 'full':
        tb.log.info("=== Test 8: Poison and MTE Features ===")

        tb.set_timing_profile('mte')
        mte_count = 20

        for i in range(mte_count):
            addr = 0x3000 + (i * 4)
            data = random.randint(0, (1 << tb.TEST_DATA_WIDTH) - 1)
            success, _ = await tb.single_write_test(
                addr, data,
                nsaid=random.randint(0, 15),
                mpam=random.randint(0, 2047),
                mecid=random.randint(0, 65535),
                tagop=random.randint(0, 3),
                poison=random.choice([0, 1])
            )
            if not success:
                tb.log.warning(f"Poison/MTE feature test {i} failed")

        tb.log.info(f"Poison and MTE tests completed")

    # Test 9: Security features (full level)
    if test_level == 'full':
        tb.log.info("=== Test 9: Security Features ===")
        ...
        for i in range(security_count):
            ...
            success, _ = await tb.single_write_test(
                ...
                mpam=random.randint(0, 2047),
                mecid=random.randint(0, 65535),
                unique=random.choice([0, 1])
            )
            if not success:
                tb.log.warning(f"Security feature test {i} failed")

        tb.log.info(f"Security tests completed")
    ...
    tb.log.info(f"  Single writes: {stats['details']['single_writes']}")
    tb.log.info(f"  Burst writes: {stats['details']['burst_writes']}")
    tb.log.info(f"  AXI5 feature tests: {stats['axi5_features']['total_axi5_tests']}")
    tb.log.info(f"  ATOP tests: {stats['axi5_features']['atop_tests']}")
    tb.log.info(f"  NSAID tests: {stats['axi5_features']['nsaid_tests']}")
    tb.log.info(f"  TRACE tests: {stats['axi5_features']['trace_tests']}")
    tb.log.info(f"  POISON tests: {stats['axi5_features']['poison_tests']}")
    ...
    if success_rate >= 95.0:
        tb.log.info(f"ALL {test_level.upper()} AXI5 WRITE SLAVE TESTS PASSED!")
    else:
...
def generate_axi5_params():
    ...
    if reg_level == 'GATE':
        ...
        for stub, id_w, addr_w, (aw_d, w_d, b_d), level in product(
                stubs, id_widths, addr_widths, aw_w_b_depths, test_levels):
...
def test_axi5_slave_wr(request, stub, id_width, addr_width, data_width, user_width,
    ...
    """Test AXI5 write slave with different parameter combinations"""
    ...
    if stub == 1:
        ...
        dut_name = "axi5_slave_wr_stub"
    else:
        dir_key = 'rtl_axi5'
        dut_name = "axi5_slave_wr"
    ...
    tw = tag_width * num_tags

    # Calculate packet sizes for AXI5
    aw_size = (id_width + addr_width + 8 + 3 + 2 + 1 + 4 + 3 + 4 + user_width +
               atop_width + nsaid_width + 1 + mpam_width + mecid_width + 1 + tagop_width + tw)
    w_size = data_width + wstrb_width + 1 + user_width + 1 + tw + num_tags
    b_size = id_width + 2 + user_width + 1 + tw + 1
    ...
    extra_env = {
        ...
        'TEST_ATOP_WIDTH': str(atop_width),
    ...
    print(f"\n{'='*80}")
    print(f"Running {test_level.upper()} AXI5 Write Slave test: {dut_name}")
    print(f"AXI5 Config: ID={id_width}, ADDR={addr_width}, DATA={data_width}, USER={user_width}")
    print(f"AXI5 Features: ATOP={atop_width}, NSAID={nsaid_width}, MPAM={mpam_width}, MECID={mecid_width}")
    ...
    try:
        ...
        print(f"{test_level.upper()} AXI5 Write Slave test FAILED: {str(e)}")
