...
#
# Module: AXI5MasterReadCGTB
# Purpose: AXI5 Master Read Clock Gated Test Runner
#
...
"""
AXI5 Master Read Clock Gated Test Runner

Test runner for the axi5_master_rd_cg module using the CocoTB framework.
...
class AXI5MasterReadCGTB(AXI5MasterReadTB):
    """
    Clock-gated AXI5 Master Read testbench.
...
async def axi5_master_read_cg_test(dut):
    """AXI5 master read clock gated test"""

    tb = AXI5MasterReadCGTB(dut, aclk=dut.aclk, aresetn=dut.aresetn)

    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'AXI5 master read CG test with seed: {seed}')
    ...
    try:
        ...
        tb.log.info("=" * 80)
        tb.log.info("AXI5 MASTER READ CG TEST SUMMARY")
        tb.log.info("=" * 80)
        ...
        tb.log.info("AXI5 MASTER READ CG TEST PASSED")

    except Exception as e:
        tb.log.error(f"AXI5 master read CG test FAILED: {str(e)}")
        raise
...
def test_axi5_master_rd_cg(id_width, addr_width, data_width, user_width, ar_depth, r_depth, test_level):
    """Test AXI5 master read clock gated with specified parameters"""
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path="rtl/amba/filelists/axi5_master_rd_cg.f")
    ...
    print(f"\n{'='*80}")
    print(f"AXI5 Master Read CG Test")
