...
def generate_test_params():
    ...
    return [
        # (num_timers, vendor_id, revision_id, test_level, description)
        (2, 0x8086, 0x01, 'full', "2-timer Intel-like"),
        (3, 0x1022, 0x02, 'full', "3-timer AMD-like"),
        (8, 0xABCD, 0x10, 'full', "8-timer custom"),
    ]


@pytest.mark.parametrize("num_timers, vendor_id, revision_id, test_level, description",
                        generate_test_params())
def test_hpet(request, num_timers, vendor_id, revision_id, test_level, description):
    ...
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({
        ...
        'rtl_hpet':          'rtl/amba/components/hpet',
    })
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict.get('rtl_hpet', 'rtl/hpet'), "hpet_config_regs.sv"),
        os.path.join(rtl_dict.get('rtl_hpet', 'rtl/hpet'), f"{dut_name}.sv")
    ]

    # RTL parameters - NUM_TIMERS, VENDOR_ID, and REVISION_ID are parameterized
    rtl_parameters = {
