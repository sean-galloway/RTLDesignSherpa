...
from CocoTBFramework.components.apb.apb_scoreboard import APBCrossbarScoreboard
    ...
    async def create_components(self):
        ...
        for i in range(self.M):
            ...
            def master_monitor_callback(transaction, master_id=i):
                transaction.pprot = master_id  # Set the master ID in pprot field
        ...
        if self.model_type == 'feedthru' or self.model_type == 'thin':
            # Fast response for simple models
            slave_constraints = {
                'ready': ([[0, 0], [1, 1]], [5, 0]),
                'error': ([[0, 0], [1, 1]], [10, 0]),
