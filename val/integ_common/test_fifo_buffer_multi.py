...
async def fifo_multi_test(dut):
    ...
    tb = FifoMultiBufferTB(dut, dut.clk, dut.rst_n, super_debug=False)
...
def generate_params():
    ...
    return [(4, 5, 8, 4, 10, 10, 0, 'func'), (4, 5, 8, 4, 10, 10, 1, 'func')]
    # return list(product(addr_widths, ctrl_widths, data_widths, depths, wr_clk_periods, rd_clk_periods, registered, test_levels))
...
def test_fifo_buffer_multi(request, addr_width, ctrl_width, data_width, depth, wr_clk_period, rd_clk_period, registered, test_level):
    """
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path="rtl/integ_common/filelists/fifo_sync_multi.f")
