"""
Validation Test Configuration for pytest - val/cdc

Configures the test environment for clock-domain-crossing module tests
(rtl/cdc). Supports Verilator line coverage collection.

The area name is DERIVED from this file's directory rather than typed. The
val/common conftest was copied to val/math during the arithmetic split and kept
announcing "val/common test session finished" while running math tests, and
pointed coverage reports at --dir val/common. A derived name cannot do that.

Coverage Features:
- Set COVERAGE=1 to enable Verilator line coverage collection
- Coverage reports are aggregated at session end
- Use `python bin/cov_utils/generate_coverage_report.py --dir val/cdc` for reports
...
AREA = os.path.basename(_AREA_DIR)
...
def pytest_configure(config):
    ...
    bin_path = os.path.abspath(os.path.join(_AREA_DIR, '../../bin'))
    if bin_path not in sys.path:
        sys.path.insert(0, bin_path)

    # Create logs directory if it doesn't exist
    log_dir = os.path.join(_AREA_DIR, "logs")
    os.makedirs(log_dir, exist_ok=True)

    # Create coverage directory if coverage is enabled
    if os.environ.get('COVERAGE', '0') == '1':
        coverage_dir = os.path.join(_AREA_DIR, "coverage_data")
...
def pytest_sessionfinish(session, exitstatus):
    ...
    """
    logging.info(f"val/{AREA} test session finished. Preserving all logs and build artifacts.")
...
def _aggregate_verilator_coverage():
    ...
    # test_bin2gray_w8_GATE/ not just test_bin2gray/
    dat_files = []
    sim_build_root = os.path.join(_AREA_DIR, "local_sim_build")
