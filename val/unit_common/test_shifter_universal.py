...
class ShifterUniversalTB(TBBase):
    ...
    async def drive_and_check(self, pdata, select, sdata_lt=0, sdata_rt=0, expected_pdata=None, 
        ...
        time_ns = get_sim_time('ns')
        self.log.info(f"Testing @ {time_ns}ns: pdata=0x{pdata:x}, select={select}, " + 
        ...
        # Wait a little more for outputs to stabilize
    ...
    async def run_all_tests(self):
        ...
        # 1. Hold operation test
        self.log.info("1. Testing hold operation")
        hold_passed = await self.test_hold_operation()
        if not hold_passed:
            self.log.error("Hold operation test failed")
            all_passed = False
            if self.TEST_LEVEL == 'basic':
                return all_passed

        # 2. Right shift operation test
        self.log.info("2. Testing right shift operation")
        right_shift_passed = await self.test_right_shift_operation()
        if not right_shift_passed:
            self.log.error("Right shift operation test failed")
            all_passed = False
            if self.TEST_LEVEL == 'basic':
                return all_passed

        # 3. Left shift operation test
        self.log.info("3. Testing left shift operation")
        left_shift_passed = await self.test_left_shift_operation()
        if not left_shift_passed:
            self.log.error("Left shift operation test failed")
            all_passed = False
            if self.TEST_LEVEL == 'basic':
                return all_passed

        # 4. Load operation test
        self.log.info("4. Testing load operation")
        load_passed = await self.test_load_operation()
        if not load_passed:
            self.log.error("Load operation test failed")
            all_passed = False
            if self.TEST_LEVEL == 'basic':
                return all_passed

        # 5. Serial I/O test (medium and full only)
        if self.TEST_LEVEL != 'basic':
            self.log.info("5. Testing serial I/O functionality")
            serial_io_passed = await self.test_serial_io()
            if not serial_io_passed:
                self.log.error("Serial I/O test failed")
                all_passed = False
                if self.TEST_LEVEL == 'medium':
                    return all_passed

        # 6. Shift pattern test
        self.log.info("6. Testing shift patterns")
        shift_pattern_passed = await self.test_shift_pattern()
        if not shift_pattern_passed:
            self.log.error("Shift pattern test failed")
            all_passed = False
