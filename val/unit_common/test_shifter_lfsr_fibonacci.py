...
class ShifterLFSRFibonacciTB(TBBase):
    ...
    async def reset_dut(self):
        """Reset the DUT"""
        self.log.debug(f'Starting reset_dut{self.get_time_ns_str()}s')
    ...
    def simulate_xor_lfsr(self, seed, taps, cycles):
        """
        Simulate a Fibonacci LFSR (XOR-based) in software for reference
        ...
        for _ in range(cycles - 1):
            # Calculate feedback bit (XOR of tapped bits)
            feedback = 0
    ...
    async def test_seed_loading(self):
        ...
        for seed in seeds:
            ...
            cycles = 5
            expected_sequence = self.simulate_xor_lfsr(seed, [self.WIDTH, self.WIDTH//2, 2, 1], cycles)
