"""Universal test runner for all types of FIFO buffers"""
import os
import random
...
from CocoTBFramework.tbclasses.fifo.fifo_basic_all_buffer import FifoBasicBufferAllTB
from CocoTBFramework.tbclasses.utilities import get_paths, create_view_cmd


@dataclass
class BufferTestParams:
    """Parameters for FIFO buffer tests"""
    # RTL parameters
    dut_name: str
    buffer_type: str
    data_width: int
    ...
    def __post_init__(self):
        ...
        # For async buffers, ensure clock periods are different
...
async def buffer_test(dut):
    """Universal test for all FIFO buffer types"""
...
def generate_standard_buffer_params():
    ...
    params.extend(
        ...
        for width, depth, mode in product(
            [8, 16, 32], [4, 8, 16], ['fifo_mux', 'fifo_flop']
        )
    )
    return params
...
def generate_multi_buffer_params():
    ...
    params.extend(
        ...
        for addr_width, ctrl_width, data_width, depth, mode in product(
            [4, 8], [3, 7], [8, 16], [4, 8], ['fifo_mux', 'fifo_flop']
        )
    )
    return params
...
def generate_field_buffer_params():
    ...
    params.extend(
        BufferTestParams(
            dut_name="fifo_sync",  # Uses standard FIFO with field mapping
            buffer_type="field",
            addr_width=addr_width,
            ctrl_width=ctrl_width,
            data_width=data_width,
        ...
        for addr_width, ctrl_width, data_width, depth, mode in product(
            [6], [9], [8, 16], [4, 8], ['fifo_mux', 'fifo_flop']
        )
    )
    return params
...
def generate_async_buffer_params():
    ...
    params.extend(
        ...
        for width, depth, mode, wr_clk, rd_clk in product(
            [8, 16], [4, 8], ['fifo_mux', 'fifo_flop'], [10, 15], [10, 15]
        )
        if wr_clk != rd_clk  # Only keep cases where clocks differ
    )
    
    # Async multi-signal FIFO tests
    params.extend(
        BufferTestParams(
            dut_name="fifo_async_multi",
            buffer_type="async",
            addr_width=addr_width,
            ctrl_width=ctrl_width,
            data_width=width,
        ...
        for addr_width, ctrl_width, width, depth, mode, wr_clk, rd_clk in product(
            [4], [3], [8], [4, 8], ['fifo_mux', 'fifo_flop'], [10, 15], [10, 15]
        )
        if wr_clk != rd_clk  # Only keep cases where clocks differ
    )
...
def generate_all_test_params(include_standard=True, include_multi=True, 
    ...
    if include_standard:
        ...
        if short_list:
            # Just pick representative examples
            params.extend([
                std_params[0],  # FIFO mux mode
                std_params[-1]  # FIFO flop mode
            ])
        else:
    ...
    if include_multi:
        ...
        if short_list:
            # Just pick representative examples
            params.extend([
                multi_params[0],  # Multi FIFO mux mode
                multi_params[-1]  # Multi FIFO flop mode
            ])
        else:
    ...
    if include_async:
        ...
        if short_list:
            # Just pick representative examples
            params.extend([
                async_params[0],  # Async standard FIFO
                async_params[-1]  # Async multi FIFO 
            ])
        else:
...
test_params = [BufferTestParams(dut_name='fifo_sync', buffer_type='standard', data_width=8, depth=4, mode='fifo_mux', short_test=True)]

@pytest.mark.parametrize("params", test_params)
def test_fifo_buffer(request, params):
    """Universal parametrized test for all FIFO buffer types"""
...
def get_verilog_sources(rtl_dict, dut_name):
    ...
    if dut_name == 'fifo_sync':
        return [
            os.path.join(rtl_dict['rtl_cmn'], "counter_bin.sv"),
            os.path.join(rtl_dict['rtl_cmn'], "fifo_control.sv"),
            os.path.join(rtl_dict['rtl_fifo'], f"{dut_name}.sv"),
        ]
    elif dut_name == 'fifo_sync_multi':
        return [
            os.path.join(rtl_dict['rtl_cmn'], "counter_bin.sv"),
            os.path.join(rtl_dict['rtl_cmn'], "fifo_control.sv"),
            os.path.join(rtl_dict['rtl_fifo'], "fifo_sync.sv"),
            os.path.join(rtl_dict['rtl_fifo'], f"{dut_name}.sv"),
        ]
    elif dut_name == 'fifo_async':
        return [
            os.path.join(rtl_dict['rtl_cmn'], "counter_bin.sv"),
    ...
    elif dut_name == 'fifo_async_multi':
        return [
            os.path.join(rtl_dict['rtl_cmn'], "counter_bin.sv"),
            os.path.join(rtl_dict['rtl_cmn'], "counter_bingray.sv"),
            os.path.join(rtl_dict['rtl_cmn'], "gray2bin.sv"),
            os.path.join(rtl_dict['rtl_cmn'], "glitch_free_n_dff_arn.sv"),
            os.path.join(rtl_dict['rtl_cmn'], "fifo_control.sv"),
            os.path.join(rtl_dict['rtl_fifo'], "fifo_async.sv"),
            os.path.join(rtl_dict['rtl_fifo'], f"{dut_name}.sv"),
        ]
    else:
