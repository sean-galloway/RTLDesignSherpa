...
#
# Module: test_math_bf16_goldschmidt_div
# Purpose: Test for the BF16 Goldschmidt division module.
#
...
"""
Test for the BF16 Goldschmidt division module.

Tests the iterative Goldschmidt division (a/b) including:
- LUT-based initial scaling
- 1 or 2 Goldschmidt iterations
...
from CocoTBFramework.tbclasses.common.bf16_testing import BF16GoldschmidtDivTB


def get_bf16_goldschmidt_params():
    """Generate BF16 Goldschmidt division test parameters based on REG_LEVEL."""
    reg_level = os.environ.get('REG_LEVEL', 'FUNC').upper()

    if reg_level == 'GATE':
        return [
            {'test_level': 'simple', 'iterations': 1, 'pipelined': 1, 'lut_depth': 32},
        ]
    elif reg_level == 'FUNC':
        return [
            {'test_level': 'basic', 'iterations': 1, 'pipelined': 1, 'lut_depth': 32},
        ]
    else:  # FULL
        return [
            {'test_level': 'basic', 'iterations': 1, 'pipelined': 1, 'lut_depth': 32},
            {'test_level': 'basic', 'iterations': 2, 'pipelined': 1, 'lut_depth': 32},
            {'test_level': 'basic', 'iterations': 1, 'pipelined': 0, 'lut_depth': 32},
            {'test_level': 'medium', 'iterations': 2, 'pipelined': 1, 'lut_depth': 64},
            {'test_level': 'full', 'iterations': 1, 'pipelined': 1, 'lut_depth': 32},
        ]


@cocotb.test(timeout_time=100, timeout_unit="ms")
async def bf16_goldschmidt_div_test(dut):
    ...
    # Setup clocks for pipelined mode
    if pipelined:
...
@pytest.mark.parametrize("params", get_bf16_goldschmidt_params())
def test_math_bf16_goldschmidt_div(request, params):
    """PyTest function to run the cocotb test for BF16 Goldschmidt division."""
    # Get all of the directory and module information
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({
        'rtl_cmn': 'rtl/common'
    })

    dut_name = "math_bf16_goldschmidt_div"
    toplevel = dut_name
    ...
    pipe_str = "pipe" if pipelined else "comb"
    test_name_plus_params = f"test_{dut_name}_{t_name}_iter{iterations}_{pipe_str}_lut{lut_depth}_{reg_level}"
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_cmn'], "math_bf16_goldschmidt_div.sv"),
    ]
