...
#
# Module: test_fifo_buffer_async_div2
# Purpose: Asynchronous FIFO Buffer Div2 Test with Parameterized Test Levels
#
...
def generate_params():
    ...
    REG_LEVEL=GATE: 2 tests (depth 6, both clocks, both modes, gate)
    REG_LEVEL=FUNC: ~8 tests (depths 6, 10, both clocks, both modes, gate+func) - default
    REG_LEVEL=FULL: ~48 tests (depths 4-14, both clocks, both modes, all levels)

    Examples for quick debugging:
        # Single test case:
        return [(8, 6, 10, 8, 0, 'gate')]
    """
    import os
    ...
    if reg_level == 'GATE':
        ...
        depths = [6, 10]  # Even depths for Johnson counters
        rd_clk_periods = [8, 12]
        registered = [0, 1]
        test_levels = ['gate', 'func']
    else:  # FULL
        # Comprehensive validation
        depths = [4, 6, 10, 14]  # Any even depths for div2 async FIFOs
        rd_clk_periods = [8, 12]
        registered = [0, 1]
...
def test_fifo_async(request, data_width, depth, wr_clk_period, rd_clk_period, registered, test_level):
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='rtl/common/filelists/fifo_async_div2.f'
    )
    ...
    reg_level = os.environ.get("REG_LEVEL", "FUNC").upper()
    test_name_plus_params = f"test_fifo_async_div2_w{w_str}_d{d_str}_wcl{wcl_str}_rcl{rcl_str}_{mode}_{test_level}_{reg_level}"
