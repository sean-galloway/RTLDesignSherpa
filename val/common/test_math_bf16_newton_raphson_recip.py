...
#
# Module: test_math_bf16_newton_raphson_recip
# Purpose: Test for the BF16 Newton-Raphson reciprocal module.
#
...
"""
Test for the BF16 Newton-Raphson reciprocal module.

Tests the iterative Newton-Raphson reciprocal (1/x) including:
- LUT-based initial estimate
- 1 or 2 Newton-Raphson iterations for increased accuracy
- Zero/infinity handling
- NaN handling
- Random value testing with iteration-dependent ULP tolerance
"""
import os
import random
import pytest
...
def get_bf16_newton_raphson_params():
    """Generate BF16 Newton-Raphson reciprocal test parameters based on REG_LEVEL."""
...
async def bf16_newton_raphson_recip_test(dut):
    """Test the BF16 Newton-Raphson reciprocal"""
    iterations = int(os.environ.get('ITERATIONS', '1'))
    lut_depth = int(os.environ.get('LUT_DEPTH', '32'))

    tb = BF16NewtonRaphsonRecipTB(dut, iterations=iterations, lut_depth=lut_depth)
...
@pytest.mark.parametrize("params", get_bf16_newton_raphson_params())
def test_math_bf16_newton_raphson_recip(request, params):
    """PyTest function to run the cocotb test for BF16 Newton-Raphson reciprocal."""
    # Get all of the directory and module information
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({
        'rtl_cmn': 'rtl/common'
    })

    dut_name = "math_bf16_newton_raphson_recip"
    toplevel = dut_name
    ...
    test_name_plus_params = f"test_{dut_name}_{t_name}_iter{iterations}_lut{lut_depth}_{reg_level}"
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_cmn'], "math_bf16_newton_raphson_recip.sv"),
    ]
