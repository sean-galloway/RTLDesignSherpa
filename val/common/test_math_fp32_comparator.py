...
#
# Module: test_math_fp32_comparator
# Purpose: Test for the FP32 comparator module
#
...
"""
Test for the FP32 comparator module.
"""
import os
import random
import pytest
...
def get_fp32_comparator_params():
...
async def fp32_comparator_test(dut):
    """Test the FP32 comparator"""
    tb = FPComparatorTB(dut, FORMATS['fp32'])
...
@pytest.mark.parametrize("params", get_fp32_comparator_params())
def test_math_fp32_comparator(request, params):
    """PyTest wrapper for FP32 comparator."""
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({'rtl_cmn': 'rtl/common'})
    dut_name = "math_fp32_comparator"
    ...
    verilog_sources = [
        os.path.join(rtl_dict['rtl_cmn'], "math_fp32_comparator.sv"),
    ]
