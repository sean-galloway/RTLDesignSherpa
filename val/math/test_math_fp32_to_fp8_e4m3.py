...
#
# Module: test_math_fp32_to_fp8_e4m3
# Purpose: Test for FP32 to FP8_E4M3 conversion
#
...
"""
Test for FP32 to FP8_E4M3 format conversion.
"""
import os
import random
import pytest
...
def get_fp32_to_fp8_e4m3_params():
...
async def fp32_to_fp8_e4m3_test(dut):
    tb = FPConversionTB(dut, FORMATS['fp32'], FORMATS['fp8_e4m3'])
...
@pytest.mark.parametrize("params", get_fp32_to_fp8_e4m3_params())
def test_math_fp32_to_fp8_e4m3(request, params):
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({'rtl_cmn': 'rtl/common',
        'rtl_math': 'rtl/math'})
    dut_name = "math_fp32_to_fp8_e4m3"
    ...
    verilog_sources = [os.path.join(rtl_dict['rtl_math'], "math_fp32_to_fp8_e4m3.sv")]
