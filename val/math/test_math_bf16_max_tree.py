# SPDX-License-Identifier: MIT
# SPDX-FileCopyrightText: 2024-2025 sean galloway
#
# RTL Design Sherpa - Industry-Standard RTL Design and Verification
# https://github.com/sean-galloway/RTLDesignSherpa
#
# Module: test_math_bf16_max_tree
# Purpose: Test for the BF16 max tree module (tree-based max finder).
#
# Documentation: BF16_ARCHITECTURE.md
# Subsystem: tests
#
# Author: sean galloway
# Created: 2025-12-25

"""
Test for the BF16 max tree module.

Tests the tree-based maximum magnitude finder including:
- Finding max across N BF16 values
- Correct index tracking
- NaN handling (NaN propagates)
- All-zero detection
"""
import os
import random
import pytest
import cocotb
from cocotb_test.simulator import run

# Add repo root to path for CocoTBFramework imports
from TBClasses.shared.utilities import get_paths, create_view_cmd

# Import the BF16 testbench class
from TBClasses.common.bf16_testing import BF16MaxTreeTB

def get_bf16_max_tree_params():
    """Generate BF16 max tree test parameters based on REG_LEVEL."""
    reg_level = os.environ.get('REG_LEVEL', 'FUNC').upper()

    if reg_level == 'GATE':
        return [
            {'test_level': 'gate', 'num_inputs': 8},  # GATE: Minimal test
        ]
    elif reg_level == 'FUNC':
        return [
            {'test_level': 'func', 'num_inputs': 8},  # FUNC: Basic coverage
        ]
    else:  # FULL
        return [
            {'test_level': 'gate', 'num_inputs': 4},
            {'test_level': 'func', 'num_inputs': 8},
            {'test_level': 'func', 'num_inputs': 16},
            {'test_level': 'full', 'num_inputs': 32},
        ]

@cocotb.test(timeout_time=10, timeout_unit="ms")
async def bf16_max_tree_test(dut):
    """Test the BF16 max tree"""
    tb = BF16MaxTreeTB(dut)

    # Use the seed for reproducibility
    seed = int(os.environ.get('SEED', '0'))
    random.seed(seed)
    tb.log.info(f'seed changed to {seed}')

    # Print testbench settings
    tb.print_settings()

    # Clear and initialize interface
    await tb.clear_interface()
    await tb.wait_time(1, 'ns')

    # Run the comprehensive test suite
    await tb.run_comprehensive_tests()

@pytest.mark.parametrize("params", get_bf16_max_tree_params())
def test_math_bf16_max_tree(request, params):
    """PyTest function to run the cocotb test for BF16 max tree."""
    # Get all of the directory and module information
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({
        'rtl_cmn': 'rtl/common',
        'rtl_math': 'rtl/math'
    })

    dut_name = "math_bf16_max_tree"
    toplevel = dut_name
    t_name = params['test_level']
    num_inputs = params['num_inputs']

    # Get REG_LEVEL before creating test name
    reg_level = os.environ.get('REG_LEVEL', 'FUNC').upper()

    # Create a human-readable test identifier
    test_name_plus_params = f"test_{dut_name}_{t_name}_n{num_inputs}_{reg_level}"

    # Add worker ID for pytest-xdist parallel execution
    worker_id = os.environ.get('PYTEST_XDIST_WORKER', '')
    if worker_id:
        test_name_plus_params = f"{test_name_plus_params}_{worker_id}"

    log_path = os.path.join(log_dir, f'{test_name_plus_params}.log')

    # BF16 max tree is self-contained with inline logic
    verilog_sources = [
        os.path.join(rtl_dict['rtl_math'], "math_bf16_max_tree.sv"),
    ]

    # RTL parameters
    rtl_parameters = {
        'NUM_INPUTS': num_inputs,
    }

    # Define simulation build and log paths
    sim_build = os.path.join(tests_dir, 'local_sim_build', test_name_plus_params)
    enable_waves = bool(int(os.environ.get('WAVES', '0')))
    os.makedirs(sim_build, exist_ok=True)

    # Define log path
    os.makedirs(log_dir, exist_ok=True)
    log_path = os.path.join(log_dir, f'{test_name_plus_params}.log')
    results_path = os.path.join(log_dir, f'results_{test_name_plus_params}.xml')

    # Set up environment variables
    seed = int(os.environ.get('SEED', str(random.randint(0, 100000))))

    extra_env = {
        'TRACE_FILE': f"{sim_build}/dump.fst",
        'VERILATOR_TRACE': '1',
        'DUT': dut_name,
        'LOG_PATH': log_path,
        'COCOTB_LOG_LEVEL': 'INFO',
        'COCOTB_RESULTS_FILE': results_path,
        'SEED': str(seed),
        'TEST_LEVEL': params['test_level'],
        'NUM_INPUTS': str(num_inputs),
    }

    # Add coverage compile args if COVERAGE=1
    extra_args = [
        '--trace-fst',
        '--trace-structs',
        '-Wno-TIMESCALEMOD',
    ]

    sim_args = ['--trace'] if enable_waves else []

    if enable_waves:
        extra_env['COCOTB_TRACE_FILE'] = os.path.join(sim_build, 'dump.fst')

    cmd_filename = create_view_cmd(log_dir, log_path, sim_build, module, test_name_plus_params)

    try:
        run(
            python_search=[tests_dir],
            verilog_sources=verilog_sources,
            includes=[],
            toplevel=toplevel,
            module=module,
            parameters=rtl_parameters,
            sim_build=sim_build,
            extra_env=extra_env,
            extra_args=extra_args,
            plus_args=sim_args,

            waves=enable_waves,
        )
    except Exception as e:
        print(f"Test failed: {str(e)}")
        print(f"Logs preserved at: {log_path}")
        print(f"To view the Waveforms run this command: {cmd_filename}")
        raise
