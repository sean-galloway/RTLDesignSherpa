...
#
# Module: test_math_ieee754_2008_fp32_adder
# Purpose: Test for the FP32 floating-point adder module
#
...
"""
Test for the FP32 floating-point adder module.
"""
import os
import random
import pytest
...
async def fp32_adder_test(dut):
    """Test the FP32 adder"""
    tb = FPAdderTB(dut, FORMATS['fp32'])
...
@pytest.mark.parametrize("params", get_fp32_adder_params())
def test_math_ieee754_2008_fp32_adder(request, params):
    """PyTest wrapper for FP32 adder."""
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({'rtl_cmn': 'rtl/common',
        'rtl_math': 'rtl/math'})
    dut_name = "math_ieee754_2008_fp32_adder"
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_math'], "math_adder_han_carlson_032.sv"),
        os.path.join(rtl_dict['rtl_math'], "math_ieee754_2008_fp32_adder.sv"),
    ]
