...
# Exhaustive test for rtl/common/math_mod_3_compress.sv (d_in mod 3 over all 16-bit
...
@cocotb.test()
async def math_mod_3_compress_test(dut):
    """Check rem_out == d_in % 3 across the 16-bit input space."""
    tb = MathMod3CompressTB(dut)
...
def test_math_mod_3_compress(request, test_level):
    module, repo_root_l, tests_dir, log_dir, rtl_dict = get_paths({
        'rtl_common': 'rtl/common',
        'rtl_math': 'rtl/math',
    })
    dut_name = "math_mod_3_compress"
