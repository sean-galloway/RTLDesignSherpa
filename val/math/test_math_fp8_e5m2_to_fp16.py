...
#
# Module: test_math_fp8_e5m2_to_fp16
# Purpose: Test for FP8_E5M2 to FP16 conversion
#
...
"""
Test for FP8_E5M2 to FP16 format conversion.
"""
import os
import random
import pytest
...
def get_fp8_e5m2_to_fp16_params():
...
async def fp8_e5m2_to_fp16_test(dut):
    tb = FPConversionTB(dut, FORMATS['fp8_e5m2'], FORMATS['fp16'])
...
@pytest.mark.parametrize("params", get_fp8_e5m2_to_fp16_params())
def test_math_fp8_e5m2_to_fp16(request, params):
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({'rtl_cmn': 'rtl/common',
        'rtl_math': 'rtl/math'})
    dut_name = "math_fp8_e5m2_to_fp16"
    ...
    verilog_sources = [os.path.join(rtl_dict['rtl_math'], "math_fp8_e5m2_to_fp16.sv")]
