...
def get_int_to_bf16_params():
    ...
    if reg_level == 'GATE':
        return [
            {'test_level': 'gate', 'int_width': 32},  # GATE: Minimal test
        ]
    elif reg_level == 'FUNC':
        return [
            {'test_level': 'func', 'int_width': 32},  # FUNC: Basic coverage
        ]
    else:  # FULL
        return [
            {'test_level': 'gate', 'int_width': 8},
            {'test_level': 'func', 'int_width': 16},
            {'test_level': 'func', 'int_width': 32},
...
def test_math_int_to_bf16(request, params):
    ...
    verilog_sources = [
        ...
        os.path.join(rtl_dict['rtl_math'], "math_int_to_bf16.sv"),
    ]
