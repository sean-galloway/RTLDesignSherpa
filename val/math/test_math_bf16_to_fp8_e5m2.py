...
#
# Module: test_math_bf16_to_fp8_e5m2
# Purpose: Test for BF16 to FP8_E5M2 conversion
#
...
"""
Test for BF16 to FP8_E5M2 format conversion.
"""
import os
import random
import pytest
...
def get_bf16_to_fp8_e5m2_params():
...
async def bf16_to_fp8_e5m2_test(dut):
    tb = FPConversionTB(dut, FORMATS['bf16'], FORMATS['fp8_e5m2'])
...
@pytest.mark.parametrize("params", get_bf16_to_fp8_e5m2_params())
def test_math_bf16_to_fp8_e5m2(request, params):
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({'rtl_cmn': 'rtl/common',
        'rtl_math': 'rtl/math'})
    dut_name = "math_bf16_to_fp8_e5m2"
    ...
    verilog_sources = [os.path.join(rtl_dict['rtl_math'], "math_bf16_to_fp8_e5m2.sv")]
