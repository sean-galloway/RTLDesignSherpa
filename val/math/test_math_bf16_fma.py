...
#
# Module: test_math_bf16_fma
# Purpose: Test for the BF16 Fused Multiply-Add module with FP32 accumulator.
#
...
"""
Test for the BF16 FMA (Fused Multiply-Add) module.

Tests the FMA with BF16 inputs (a, b) and FP32 accumulator (c):
    result = (a * b) + c

Key features tested:
- Normal FMA operations with RNE rounding
- Special value handling (zero, infinity, NaN, subnormal)
- Accumulation scenarios typical of AI training workloads
- Overflow and underflow detection
- Invalid operation detection (0 * inf, inf - inf)
"""
import os
import random
import pytest
...
# Import the BF16 FMA testbench class
from TBClasses.common.bf16_testing import BF16FMATB

def get_bf16_fma_params():
    """Generate BF16 FMA test parameters based on REG_LEVEL."""
...
@pytest.mark.parametrize("params", get_bf16_fma_params())
def test_math_bf16_fma(request, params):
    """PyTest function to run the cocotb test for BF16 FMA."""
    ...
    verilog_sources = [
        ...
        # Generated BF16/math modules
        os.path.join(rtl_dict['rtl_math'], "math_adder_han_carlson_016.sv"),
        os.path.join(rtl_dict['rtl_math'], "math_adder_han_carlson_048.sv"),
        os.path.join(rtl_dict['rtl_math'], "math_multiplier_dadda_4to2_008.sv"),
        os.path.join(rtl_dict['rtl_math'], "math_bf16_fma.sv"),
    ]
