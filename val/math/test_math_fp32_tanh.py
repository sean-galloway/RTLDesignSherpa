...
#
# Module: test_math_fp32_tanh
# Purpose: Test for the FP32 tanh module
#
...
async def fp32_tanh_test(dut):
    """Test the FP32 tanh"""
    tb = FPTanhTB(dut, FORMATS['fp32'])
...
@pytest.mark.parametrize("params", get_fp32_tanh_params())
def test_math_fp32_tanh(request, params):
    """PyTest wrapper for FP32 tanh."""
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({'rtl_cmn': 'rtl/common',
        'rtl_math': 'rtl/math'})
    dut_name = "math_fp32_tanh"
    ...
    verilog_sources = [
        os.path.join(rtl_dict['rtl_math'], "math_fp32_tanh.sv"),
    ]
