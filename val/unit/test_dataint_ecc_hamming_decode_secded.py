...
async def hamming_decode_repair_test(dut):
    ...
    width = int(os.environ.get('PARAM_WIDTH', '0'))
    parity_bits = int(dut.ParityBits)
    total_width = int(dut.TotalWidth)

    log.info('-------------------------------------------')
    log.info(f'Data Width   {width}')
...
# tf = TestFactory(hamming_decode_repair_test)
# tf.generate_tests()
...
def test_dataint_ecc_hamming_decode_secded(request, width):
    ...
    verilog_sources = [
        os.path.join(rtl_dir, "dataint_ecc_hamming_decode_secded.sv"),
