...
def reduce_partial_products(bit_groups, N, f):
    ...
    for bit_idx in range(2 * N - 1):
        while len(bit_groups[bit_idx]) > 2:
            # If there are three or more bits, use a CSA
            ...
            f.write(f"math_adder_carry_save CSA_{bit_idx}_{len(bit_groups[bit_idx])}(.i_a({a}), .i_b({b}), .i_c({c}), .ow_sum({sum_name}), .ow_c({carry_name}));\n")
...
def generate_wallace_tree(N):
    filename = f"math_multiplier_wallace_tree_csa_{N}.sv"

    with open(filename, 'w') as f:
        f.write(f"`timescale 1ns / 1ps\n\n")
        # Module header
        f.write(f"module math_multiplier_wallace_tree_csa_{N} (\n")
    ...
    initial begin
        $dumpfile("dump.vcd");
        $dumpvars(0, math_multiplier_wallace_tree_csa_{N});
    end
                ''')
        # End of module
        f.write(f"\nendmodule : math_multiplier_wallace_tree_csa_{N}\n")
