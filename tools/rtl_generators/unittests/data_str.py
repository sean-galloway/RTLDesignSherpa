...
module spstore
     (
       ...
       input  logic                             resetn,
       // Common Address Port
       input  logic [log2d-1:0]                 addr,
       // Read Port
       input  logic                             rd_en,       // active high
       output logic [w-1:0]                     rd_data,
       // Write Port
       input  logic                             wr_en,       // active high
       input  logic [(w/p)-1:0]                 wr_segsel,   // active high
       input  logic [w-1:0]                     wr_data,
       // DFX
       input  logic [spstore_in(w, p, d)-1:0]   dfx_in,
       input  logic                             tst_scan_mode,
       output logic [spstore_out(w, p, d)-1:0]  dfx_out
     );
"""
...
#(parameter add_extra_instr  = 1,  //use extra instructions (rotl, ...)
  ...
  parameter shorten_pipeline = 3)  //remove two cycles latency    def test_compiler_directive_code(self):
