...
module arbiter_rr_pwm_monbus #(
    ...
    always_comb begin
        ...
        for (int i = 0; i < CLIENTS; i++) begin
            if (request[i] && !block_arb_internal) begin
    ...
    always_comb begin
        ...
        if (cfg_mon_enable && !block_arb_internal && cfg_mon_starvation > 16'h0) begin
            for (int i = 0; i < CLIENTS; i++) begin
                if (request[i] && r_req_timers[i] >= cfg_mon_starvation) begin
    ...
    always_comb begin
        ...
        if (cfg_mon_enable && !block_arb_internal && cfg_mon_latency > 16'h0) begin
            for (int i = 0; i < CLIENTS; i++) begin
                if (request[i] && r_req_timers[i] == cfg_mon_latency) begin  // Edge Detection: == instead of >=
