...
//
// Module: axi_monitor_trans_mgr  (legacy in-place revision, parked here)
...
// Parked here:  2026-06-08 (superseded by CAM-backed version in
//               rtl/amba/shared/axi_monitor_trans_mgr.sv)
//
// ============================================================================
// LEGACY VERSION -- not on the synth path
// ----------------------------------------------------------------------------
// This is the pre-CAM revision of axi_monitor_trans_mgr.sv. The shared
// monitor_trans_cam module did not exist when this was authored; ID
// match vectors, free-slot vector, priority-encoded alloc, and per-slot
// storage were all inlined here. The 2026-04-23 refactor cut synth
// logic-level depth on the trans_table update path but kept the
// in-place storage model.
//
// The successor in rtl/amba/shared/axi_monitor_trans_mgr.sv delegates
// the keying + storage to monitor_trans_cam and is bit-exact equivalent
// to this file (proven by val/amba/test_axi_monitor_trans_mgr_equiv.py
// across 6 parameter configs).
//
// Kept here so any future timing investigation can swap back to the
// in-place version via filelist or env-var knob, and so the synth
// shapes can be compared on real silicon if needed.
// ============================================================================
