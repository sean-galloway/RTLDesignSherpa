...
// Purpose: Timeout-packet detection cone (split out of axi_monitor_reporter
//          so integrators can drop it with ENABLE_TIMEOUT_LOGIC=0).
//
// Scans for unreported ERROR slots that the timeout module has flagged
// via timeout_detected[idx], priority-encodes the first match, and emits
// the packet fields. The error sub-block masks these via the same
// timeout_detected vector so there's no double-reporting.
//
...
#(
    ...
    always_comb begin
        ...
        for (int idx = 0; idx < MAX_TRANSACTIONS; idx++) begin
            if (trans_table[idx].valid && !event_reported[idx] &&
                trans_table[idx].state == TRANS_ERROR && cfg_timeout_enable &&
                timeout_detected[idx]) begin
                w_events[idx] = 1'b1;
            end
        end
    ...
    assign pkt_type       = PktTypeTimeout;
...
endmodule : axi_monitor_reporter_timeout
