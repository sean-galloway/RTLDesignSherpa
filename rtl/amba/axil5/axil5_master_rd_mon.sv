 ...
 *
 * This module combines the standard axil5_master_rd module with an axi_monitor_filtered
 * to provide comprehensive transaction monitoring for AXI4-Lite read transactions.
 *
 * Features:
 * - Instantiates axil5_master_rd for core AXIL4 functionality
...
module axil5_master_rd_mon
    import monitor_pkg::*;
#(
    // AXIL4 Master parameters (passed through to axil5_master_rd)
...
(
    ...
    // told "not ready", so the master held the command on the bus and
    // the core accepted it AGAIN every cycle until the table drained.
    // Backpressure became replay -- 49 commands in, 367 accepted on
    // the Genesys 2 STREAM build.
    logic w_gated_arvalid;
    assign w_gated_arvalid = fub_axil_arvalid & (w_block_ready | ~cfg_monitor_enable);
    ...
    // -------------------------------------------------------------------------
    // Instantiate AXI Monitor with Filtering (configured for AXIL4, optional)
    // -------------------------------------------------------------------------
    // -------------------------------------------------------------------------
    ...
    assign fub_axil_arready = w_core_fub_axil_arready &
