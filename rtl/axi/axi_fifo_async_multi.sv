...
module axi_fifo_async_multi #(
    ...
    /////////////////////////////////////////////////////////////////////////
    // local wires
    logic [PAW-1:0] r_wr_addr, r_rd_addr;
    ...
    logic [PAW:0] r_wr_ptr_bin, w_wdom_rd_ptr_bin, r_rd_ptr_bin, w_rdom_wr_ptr_bin;
    logic [PAW:0] w_wr_ptr_bin_next, w_rd_ptr_bin_next;
    logic         r_wr_full, r_wr_almost_full, r_rd_empty, r_rd_almost_empty;

    // The flop storage registers
    logic [AW+CW+DW-1:0] r_mem[0:((1<<PAW)-1)];  // verilog_lint: waive unpacked-dimensions-range-ordering
    ...
    assign r_wr_addr = r_wr_ptr_bin[PAW-1:0];
    assign r_rd_addr = r_rd_ptr_bin[PAW-1:0];

    /////////////////////////////////////////////////////////////////////////
    // Memory Flops
    always_ff @(posedge i_axi_wr_aclk) begin
        if (w_write && !r_wr_full) r_mem[r_wr_addr] <= {i_wr_addr, i_wr_ctrl, i_wr_data};
    end

    // Flop stage for the flopped data
    always_ff @(posedge i_axi_rd_aclk or negedge i_axi_rd_aresetn) begin
        if (!i_axi_rd_aresetn) {o_rd_addr, o_rd_ctrl, o_rd_data} <= 'b0;
        else {o_rd_addr, o_rd_ctrl, o_rd_data} <= r_mem[r_rd_addr];
    end

    /////////////////////////////////////////////////////////////////////////
    // Read Port
    assign {ow_rd_addr, ow_rd_ctrl, ow_rd_data} = r_mem[r_rd_addr];
...
endmodule : axi_fifo_async_multi
