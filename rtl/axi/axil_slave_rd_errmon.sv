...
(
    ...
    assign w_addr_fifo_wr_valid = fub_arvalid && fub_arready;
    assign w_addr_fifo_wr_data = fub_araddr;
    
    // Read from the address FIFO when a read response is received
    assign w_addr_fifo_rd_ready = fub_rvalid && fub_rready;
        ...
        if (!aresetn) begin
            ...
            if (w_arto_error) begin
                ...
                r_error_fifo_wr_data <= r_ar_timeout ?
                    {ErrorARTimeout, fub_araddr} : r_error_flag_arto_data;
                r_error_flag_arto <= 'b0;
                r_error_flag_arto_data <= 'b0;
            end else if (r_ar_timeout) begin
                r_error_flag_arto <= 'b1;
                r_error_flag_arto_data <= {ErrorARTimeout, fub_araddr};
            end
...
endmodule : axil_slave_rd_errmon