...
(
    ...
    assign w_addr_fifo_wr_valid = fub_awvalid && fub_awready;
    assign w_addr_fifo_wr_data = fub_awaddr;
    
    // Read from the address FIFO when a write response is received
    assign w_addr_fifo_rd_ready = fub_bvalid && fub_bready;
        ...
        if (!aresetn) begin
            ...
            if (w_awto_error) begin
                ...
                r_error_fifo_wr_data <= r_aw_timeout ?
                    {ErrorAWTimeout, fub_awaddr} : r_error_flag_awto_data;
                r_error_flag_awto <= 'b0;
                r_error_flag_awto_data <= 'b0;
            end else if (r_aw_timeout) begin
                r_error_flag_awto <= 'b1;
                r_error_flag_awto_data <= {ErrorAWTimeout, fub_awaddr};
            end
