# Filelist for apb4_xbar_monitored
# Location: rtl/integ_amba/filelists/apb4_xbar_monitored.f
#
# Integration example: apb4_xbar_thin with a monitor on every port.
#
# NOTE ON LAYERING: apb4_xbar_thin lives in projects/components/apb4_xbar, so this
...
# apb4_xbar_thin instantiates arbiter_round_robin_weighted but its own filelist
# does not carry it, so it cannot be compiled from that list alone. Declared
# here so this example builds; the real fix belongs in apb4_xbar_thin.f, which
# is in the projects tree and out of scope for this change.
-f $REPO_ROOT/rtl/common/filelists/arbiter_round_robin_weighted.f
-f $REPO_ROOT/projects/components/apb4_xbar/rtl/filelists/core/apb4_xbar_thin.f

$REPO_ROOT/rtl/integ_amba/examples/apb4_xbar_monitored.sv
