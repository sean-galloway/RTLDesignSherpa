...
module counter_bin #(
    ...
    always_comb begin
        if (enable) begin
            if (counter_bin[WIDTH-2:0] == w_max_val)
                counter_bin_next = {~counter_bin[WIDTH-1], {(WIDTH - 1){1'b0}}};
            else
                counter_bin_next = counter_bin + 1;
        end else begin
            counter_bin_next = counter_bin;
        end
    end

    // Flop stage for the counter
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) counter_bin <= 'b0;
        else counter_bin <= counter_bin_next;
    end
