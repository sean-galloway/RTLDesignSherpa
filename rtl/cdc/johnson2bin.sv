...
//
// Documentation: docs/markdown/rtl-cdc/johnson2bin.md
// Subsystem: cdc
//
...
//   - Used by fifo_async / gaxi_fifo_async (USE_JOHNSON=1) for pointer conversion
//
//------------------------------------------------------------------------------
...
//   Example config           | USE_JOHNSON=1     | USE_JOHNSON=0
//
...
//   - fifo_async.sv uses Johnson counters (USE_JOHNSON=1) for non-power-of-2 depths
...
//     4. johnson2bin converts to binary (this module)
...
//
//   fifo_async with USE_JOHNSON=1 (any depth):
//   - Uses counter_johnson → sync → johnson2bin (this module)
...
//   - Set USE_JOHNSON=1 if depth must be odd or non-power-of-2 (10, 12, 20, etc.)
...
//   - johnson2bin:        ~60 LUTs (priority encoders + mux logic)
//   - gray2bin (WIDTH=4): ~8 LUTs (simple XOR tree)
//   - Ratio: johnson2bin is ~7.5x larger than gray2bin for same address width
...
//   - fifo_async.sv / gaxi_fifo_async.sv (USE_JOHNSON=1) - primary users
//
//------------------------------------------------------------------------------
// Test:
//------------------------------------------------------------------------------
