...
|-----------|------|---------|-------------|
| `WIDTH` | `int` | `5` | Total width of the counter in bits |
| `MAX` | `int` | `10` | Maximum count value before special wrap behavior |

`WIDTH` must be ≥ 2 for proper MSB inversion operation, and it determines the counter's bit width and maximum representable value.

`MAX` ranges from `2` to `2^(WIDTH-1)` **inclusive**. The RTL computes
...
exactly at `MAX = 2^(WIDTH-1)`. The one hard constraint: `MAX-1` must fit within the lower `WIDTH-1` bits.
