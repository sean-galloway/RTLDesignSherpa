# rtl-amba Block Diagram Generation Plan
...
This plan outlines the approach for generating Mermaid block diagrams for the 94 rtl-amba markdown files that contain ASCII block diagrams needing conversion to SVG format.
...
- **Target location:** `/mnt/data/github/rtldesignsherpa/docs/markdown/assets/rtl-amba/`
...
![gaxi_fifo_async Block Diagram](gaxi_fifo_async.png)
```
...
├── rtl-amba/
│   ├── gaxi/
│   │   ├── gaxi_fifo_async.md      # References ../assets/rtl-amba/gaxi_fifo_async.svg
│   │   └── ...
│   └── ...
└── assets/
    └── rtl-amba/
...
|------|-------------|------------|--------|
| index.md | rtl-amba main index | Low | [ ] Pending |
...
```bash
#!/bin/bash
# Regenerate all rtl-amba block diagrams from Mermaid sources

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

echo "Regenerating rtl-amba diagrams..."
...
```bash
cd /mnt/data/github/rtldesignsherpa/docs/markdown/assets/rtl-amba
for f in *.mmd; do
  svg="${f%.mmd}.svg"
  echo "Rendering $f..."
