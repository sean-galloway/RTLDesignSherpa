...
**Module:** `apbx_xbar_monitored.sv`
**Location:** `rtl/integ_amba/examples/`
**Status:** Integration example -- elaborates clean; no test yet

## Overview

`apbx_xbar_thin` with full observability: a monitor on every master port and
...
Traffic passes through `apbx_xbar_thin` unchanged -- the crossbar is not modified
...
`apbx_xbar_thin` is raw APB on both sides, so each monitored port converts the
