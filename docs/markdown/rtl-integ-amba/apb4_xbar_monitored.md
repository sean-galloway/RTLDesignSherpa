...
**Module:** `apb4_xbar_monitored.sv`
**Location:** `rtl/integ_amba/examples/`
**Status:** Integration example -- elaborates clean; no test yet

## Overview

`apb4_xbar_thin` with full observability: a monitor on every master port and
...
Traffic passes through `apb4_xbar_thin` unchanged -- the crossbar is not modified
...
`apb4_xbar_thin` is raw APB on both sides, so each monitored port converts the
...
- **Monitor count scales with ports.** Seven monitors at `MAX_TRANSACTIONS=8`
  ...
  reflex.
- **Layering.** `apb4_xbar_thin` lives in `projects/components/apb4_xbar`, so this
  module under `rtl/` depends on a project area. That direction is backwards and
  is noted in `rtl/integ_amba/filelists/apb4_xbar_monitored.f`.

## Testing

None, in `val/` or anywhere else. See the note on
[apb4_peripheral_subsystem](apb4_peripheral_subsystem.md#testing).
...
- [apb4_peripheral_subsystem](apb4_peripheral_subsystem.md) - the same pattern, smaller
