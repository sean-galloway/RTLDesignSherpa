...
# AXIL4 Master Read Interface (Clock-Gated)
...
-- so functional behaviour is identical to `axil4_master_rd` and the wrapper adds no
latency of its own.
...
`val/amba/test_axil4_master_rd_cg.py` drives this module with the AXI4-Lite BFMs from `TBClasses/axil4`. It collects 1 parameter cases at the default `REG_LEVEL`. Run it with:
