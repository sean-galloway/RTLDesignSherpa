...
# AXIL5 Slave Read Monitor, Clock-Gated

**Module:** `axil5_slave_rd_mon_cg.sv`
**Location:** `rtl/amba/axil5/`
**AXI4-Lite counterpart:** [`axil4_slave_rd_mon_cg`](../axil4/axil4_slave_rd_mon_cg.md)
...
- [`axil4_slave_rd_mon_cg`](../axil4/axil4_slave_rd_mon_cg.md) -- the AXI4-Lite counterpart
...
`val/amba/test_axil5_slave_rd_mon_cg.py` drives this module with the AXI5-Lite BFMs and **every optional group enabled** -- `TBClasses/axil5` sets the group widths in `COMPONENT_KWARGS` to mirror the RTL defaults. A BFM configured differently from its DUT is a bind failure, which is the loud version of the mistake.
