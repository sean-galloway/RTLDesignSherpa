...
# AXIL5 Slave Read, Clock-Gated

**Module:** `axil5_slave_rd_cg.sv`
**Location:** `rtl/amba/axil5/`
**AXI4-Lite counterpart:** [`axil4_slave_rd_cg`](../axil4/axil4_slave_rd_cg.md)

---

## Overview

The AXIL5 Slave Read, Clock-Gated provides a buffered AXI5-Lite read interface for slave devices.
...
axil5_slave_rd_cg #(
    ...
    // ... `s_axil_arprot`, `s_axil_arlock`, `s_axil_aruser`, +7 more
    // fub_ar
    ...
    // ... `s_axil_rresp`, `s_axil_ruser`, `s_axil_rtrace`, +4 more
    // status
    .cg_gating           (cg_gating),
    .cg_idle             (cg_idle)
);
...
entirely, silently, because function is unaffected. This wrapper wakes on
VALIDs and pending work only. `val/amba/test_cg_peer_ready.py` parks the peer
READY high, holds every VALID low, and requires `cg_gating` -- see
`vault/handbook/design/clock-gating-activity-terms.md`.

**`busy` is not brought out here.** The plain `_cg` wrappers consume the base
module's `busy` internally as a wake term; use `cg_idle` or `cg_gating` for
idle detection. The `_mon_cg` variants do expose `busy`.

**`cfg_cg_enable` is not a kill switch.** It arms gating and reaches
`amba_clock_gate_ctrl` only. With it low the clock free-runs and the module
...
- [`axil4_slave_rd_cg`](../axil4/axil4_slave_rd_cg.md) -- the AXI4-Lite counterpart
...
`val/amba/test_axil5_slave_rd_cg.py` drives this module with the AXI5-Lite BFMs and **every optional group enabled** -- `TBClasses/axil5` sets the group widths in `COMPONENT_KWARGS` to mirror the RTL defaults. A BFM configured differently from its DUT is a bind failure, which is the loud version of the mistake.
