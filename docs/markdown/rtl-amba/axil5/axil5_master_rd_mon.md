...
# AXIL5 Master Read Monitor

**Module:** `axil5_master_rd_mon.sv`
**Location:** `rtl/amba/axil5/`
**AXI4-Lite counterpart:** [`axil4_master_rd_mon`](../axil4/axil4_master_rd_mon.md)
...
axil5_master_rd_mon #(
    ...
    .ENABLE_POISON       (1),
    .ENABLE_LOCK         (1)
) u_axil5_master_rd_mon (
    // clock/reset
    .aclk                (aclk),
    .aresetn             (aresetn),
    // status
    .cam_clear           (cam_clear),
    // ... `fub_axil_araddr`, `fub_axil_arprot`, `fub_axil_arlock`, +31 more
    // m_axil_ar
...
- [`axil4_master_rd_mon`](../axil4/axil4_master_rd_mon.md) -- the AXI4-Lite counterpart
...
`val/amba/test_axil5_master_rd_mon.py` drives this module with the AXI5-Lite BFMs and **every optional group enabled** -- `TBClasses/axil5` sets the group widths in `COMPONENT_KWARGS` to mirror the RTL defaults. A BFM configured differently from its DUT is a bind failure, which is the loud version of the mistake.
