...
**Module:** `apb4_xbar_thin.sv`
**Location:** `projects/components/apb4_xbar/rtl/`
...
Earlier revisions of this document described a module named `apb4_xbar` with
...
`apb4_xbar_thin`, and it has a materially different architecture -- it is
combinational and does no protocol conversion at all. This document has been
rewritten against the actual RTL.

The other family, the fixed-configuration generated crossbars (`apb4_xbar_1to1`,
`apb4_xbar_2to1`, `apb4_xbar_1to4`, `apb4_xbar_2to4`), *does* use `apb4_slave` +
`apb4_master` conversion and is specified separately in
[apb_crossbar.md](apb_crossbar.md).

### Choosing Between the Two Families

| | `apb4_xbar_thin` | Generated crossbars |
|---|---|---|
...
`apb4_xbar_thin` adds **zero cycles** of latency. The master's `PSEL`, `PENABLE`,
...
of any design instantiating a large `apb4_xbar_thin`. If timing does not close,
...
SymbiYosys proofs are checked in at `formal/apb4_xbar/apb4_xbar_thin/`, with
`prove` and `cover` tasks.

### Simulation

`apb4_xbar_thin` has no dedicated CocoTB testbench. The four generated variants in
`projects/components/apb4_xbar/dv/tests/` do, but they exercise the *other*
...
- **apb4_xbar_1to1 / 2to1 / 1to4 / 2to4**: fixed-configuration generated crossbars ([apb_crossbar.md](apb_crossbar.md))
