...
Earlier revisions of this document described a module named `apbx_xbar` with
...
`apbx_xbar_thin`, and it has a materially different architecture -- it is
...
| | `apbx_xbar_thin` | Generated crossbars |
|---|---|---|
...
`apbx_xbar_thin` adds **zero cycles** of latency. The master's `PSEL`, `PENABLE`,
