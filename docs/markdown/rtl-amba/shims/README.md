...
None of these three modules is in `rtl/amba` — all three belong to the
converters component, and their documentation now lives with it. The two
axi4-to-apb4 shims are specified in that component's MAS chapter 3.4; this
book carried a second full copy of each until 2026-08-31.
`peakrdl_to_cmdrsp` had no MAS chapter, so its 515-line reference page was
MOVED rather than deleted — losing it would have destroyed the only real
documentation that module has. This table is kept as a pointer so the shim
family stays discoverable from the AMBA book.
