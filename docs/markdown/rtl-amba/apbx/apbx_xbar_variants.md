...
The APB crossbar family provides scalable interconnect solutions for connecting multiple APB masters to multiple APB slaves. All crossbar variants are generated using the `apbx_xbar_generator.py` tool and share a common architecture based on `apb4_slave` and `apb4_master` building blocks.

**Where things live.** The crossbar RTL, generator, and testbenches are in the
`apbx_xbar` component area, not under `rtl/amba/apb4/`:
...
1. **Master Side**: `apb4_slave` modules convert incoming APB transactions to cmd/rsp interface
2. **Crossbar Core**: Address decoding and arbitration logic routes commands to slaves
3. **Slave Side**: `apb4_master` modules convert cmd/rsp back to APB transactions
...
**Write Transaction:**
```
1. Master APB PWRITE=1 → apb4_slave
2. apb4_slave generates cmd_valid with write data
3. Address decoder selects target slave
4. Arbiter grants access (if available)
5. apb4_master converts to APB write
6. Slave APB completes transaction
7. apb4_master generates rsp_valid
8. apb4_slave routes response back to master
9. Master APB sees PREADY=1
```

**Read Transaction:**
```
1. Master APB PWRITE=0 → apb4_slave
2. apb4_slave generates cmd_valid with address
3. Address decoder selects target slave
4. Arbiter grants access (if available)
5. apb4_master converts to APB read
6. Slave APB returns data
7. apb4_master generates rsp_valid with read data
8. apb4_slave routes response back to master
...
`apb4_slave` (master side) → decode/arbitration → `apb4_master` (slave side). None
of them is a wire.
...
| Stage | Cost |
|-------|------|
| `apb4_slave` capture to `cmd_valid` | ≥1 cycle (registered FSM) |
| Address decode | 0 cycles (combinational) |
| Arbitration | 1 cycle when contended (`WAIT_GNT_ACK` grant handshake) |
| `apb4_master` SETUP + ACCESS on the downstream slave | ≥2 cycles, plus that slave's wait states |
| Response return through `apb4_slave` | ≥1 cycle |
...
|------|-------------|--------|
| `test_apbx_xbar_1to1` | Basic passthrough, protocol conversion | ✅ PASS |
...
```
test_apbx_xbar_1to1.py::test_apbx_xbar_1to1   PASSED
test_apbx_xbar_2to1.py::test_apbx_xbar_2to1   PASSED
test_apbx_xbar_1to4.py::test_apbx_xbar_1to4   PASSED
test_apbx_xbar_2to4.py::test_apbx_xbar_2to4   PASSED  (350 transactions)
...
**Generator Tutorial:**
- `docs/markdown/GeneratorTutorial/apbx_xbar_generator_tutorial.md` - How to use generator
...
| 1.1 | 2026-07-19 | RTL Design Sherpa | Corrected module/test paths to the `apbx_xbar` component area; APB5 claim reduced to APB4; removed unsubstantiated latency, LUT, MHz and transaction-count figures; documented that unmapped addresses stall rather than returning `PSLVERR`/`0xDEADBEEF`; added `BASE_ADDR` alignment requirement |
