# RTL Clock Domain Crossing

<!-- Auto-generated link index for PDF build. Regenerate via generate_rtl_pdfs.sh -->

- [Clock Domain Crossing (CDC)](cdc.md)
- [Binary-to-Gray converter (`bin2gray.sv`)](bin2gray.md)
- [Gray-to-binary converter (`gray2bin.sv`)](gray2bin.md)
- [Johnson-to-binary converter (`johnson2bin.sv`)](johnson2bin.md)
- [Binary-Gray counter (`counter_bingray.sv`)](counter_bingray.md)
- [Johnson counter (`counter_johnson.sv`)](counter_johnson.md)
- [Asynchronous FIFO (`fifo_async.sv`)](fifo_async.md)
- [GAXI asynchronous FIFO (`gaxi_fifo_async.sv`)](gaxi_fifo_async.md)
- [gaxi_skid_buffer_async](gaxi_skid_buffer_async.md)
- [glitch_free_n_dff_arn](glitch_free_n_dff_arn.md)
- [clock_pulse](../rtl-common/clock_pulse.md)
- [apb4_slave_cdc](../rtl-amba/apb4/apb4_slave_cdc.md)
- [apb4_slave_cdc_cg](../rtl-amba/apb4/apb4_slave_cdc_cg.md)
- [APB5 Slave CDC](../rtl-amba/apb5/apb5_slave_cdc.md)
- [APB5 Slave CDC (Clock-Gated)](../rtl-amba/apb5/apb5_slave_cdc_cg.md)
