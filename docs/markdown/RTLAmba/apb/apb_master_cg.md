...
The `apb_master_cg` module adds power optimization to `apb_master` through
activity-based clock gating. It is a thin wrapper: an `amba_clock_gate_ctrl`
instance produces `gated_pclk`, which feeds an otherwise unmodified `apb_master`.
...
In addition to all [apb_master](./apb_master.md) ports:
...
family. The first gated-clock rising edge available to the wrapped `apb_master`
