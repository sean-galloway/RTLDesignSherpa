...
# Harness CSRs are resolved BY NAME through rapids_harness_csr_regmap.py
# (generated by bin/gen_rapids_harness_regmap.py from the table mirroring
# rapids_char_harness.sv). The kick sequencer's registers used to have no
# by-name entry, so this file hardcoded their offsets -- TASK-057. They are in
# the regmap now, so the literals are gone.
...
def _stage_and_go(io, half_is_snk: int, mask: int):
    ...
    io.csr_write_reg("KICK_CFG", HALF=(1 if half_is_snk else 0))
    io.csr_write_reg("KICK_MASK", VALUE=mask)
    io.csr_write_reg("KICK_BASE_LO", VALUE=DESC_BASE & 0xFFFF_FFFF)
    io.csr_write_reg("KICK_BASE_HI", VALUE=(DESC_BASE >> 32) & 0xFFFF_FFFF)
    io.csr_write_reg("KICK_STRIDE", VALUE=KICK_STRIDE)
    io.csr_write_reg("GO", GO=1)
