...
def configure_monitors(bridge, A):
    """Enable the three in-core monitors to EMIT packets: allow every type
    (PKT_MASK=0), clear ALL per-event-code masks, enable completion+IRQ,
    ERR_CFG=0 (BULK_TRACE -> tally ingest). Match-all DEBUG address range on
    rd+wr so every AR/AW emits AddrMatch; open the perf windows so PERF flows.

    TWO LAYERS OF MASK, and both must be cleared:

      PKT_MASK          drops by PACKET TYPE      (bit = pkt_type)
      MASK1/2/3/4       drops by EVENT CODE       (bit = event_code[3:0]),
                        one 16-bit field per class

    Their RDL defaults are NOT uniform, which is the trap this function walked
    into for months:

      TIMEOUT_MASK 0xFFFF  drop all      COMPL_MASK 0x0000  allow all
      THRESH_MASK  0xFFFF  drop all      PERF_MASK  0x0000  allow all
      ADDR_MASK    0xFFFF  drop all      DEBUG_MASK 0x0000  allow all

    This used to clear PKT_MASK and MASK3 only. So AddrMatch, Completion and
    Perf appeared on the board while TIMEOUT and THRESHOLD could NEVER appear --
    monbus_group_core drops them on the event-code mask before they reach the
    tally, and the group has no drop counter, so the loss looks exactly like
    "the monitor never fired". Weeks were spent looking at the monitors; the
    cones were firing the whole time (dv macro + top tests prove it).

    Clear MASK1..MASK4 unconditionally. mon_matrix.py already did.
    """
    for pk, en, err, mbase in (
        ("DAXMON_PKT_MASK", "DAXMON_ENABLE", "DAXMON_ERR_CFG", "DAXMON"),
        ("RDMON_PKT_MASK",  "RDMON_ENABLE",  "RDMON_ERR_CFG",  "RDMON"),
        ("WRMON_PKT_MASK",  "WRMON_ENABLE",  "WRMON_ERR_CFG",  "WRMON"),
    ):
        bridge.write(A(pk),  0x0000_0000)   # drop nothing -> allow every type
        for mi in (1, 2, 3, 4):             # every event-code drop mask
            name = f"{mbase}_MASK{mi}"
            try:
                bridge.write(A(name), 0x0)
            except KeyError:
                pass                        # MASK4 absent on some blocks
