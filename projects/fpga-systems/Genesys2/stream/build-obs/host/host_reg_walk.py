...
# Registers whose WRITE launches hardware. A walk writes 0xFFFFFFFF to prove a
# register takes a value -- and to prove a read-only register does NOT -- so it
# writes to everything it can reach. For these that is not an observation:
#
#   KICK_ENABLE reads back 0, so it takes the read-only branch, which writes to
#   it. That write kicked all eight channels at whatever CHx_CTRL_LOW had just
#   been walked to (0xFFFFFFFF). The descriptor fetch never returned, the
#   channels stayed busy forever, and every functional test after the walk
#   failed -- DESC_ENGINE_IDLE 0xFE, CH_STATE 1, no error bit, the same shape as
#   a real DE hang. Only reprogramming the FPGA recovered it: CTRL.SOFT_RESET
#   does not abort an AXI transaction already in flight.
#
#   CHx_CTRL_LOW/HIGH are the descriptor address that kick consumes. Harmless to
#   write on their own, but leaving 0xFFFFFFFF there arms the trap for anything
#   that kicks afterwards, so they are read-only to the walk too.
#
# Their reset values are still checked; only the write is skipped.
NO_WRITE = {
    "stream": (
        ["KICK_ENABLE"]
        + [f"CH{i}_CTRL_LOW" for i in range(8)]
