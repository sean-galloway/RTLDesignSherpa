...
# the tally memories live one level up in stream_harness, so this TB checks
...
class DmaSlaveMonitorsTB(TBBase):
    ...
    async def setup_clocks_and_reset(self):
        ...
        # Monitor configuration is no longer a set of ports: the block owns an
        # APB regblock (slvmon_regs @ slvmon_apb) whose RESET DEFAULTS are all
        # seven classes enabled and timeout 0xFFFF -- exactly what this TB used
        # to drive by hand. So idle the APB and let the defaults stand; a test
        # that wants a non-default config drives the APB instead.
