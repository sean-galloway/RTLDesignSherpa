"""
STREAM harness testbench -- the shared UART transport.

Drives the stream_harness module through its UART interface using
...
SHARED BY BOTH BUILDS, which is why it sits at COMPONENT level rather than in
either build's dv/. build-mon and build-perf elaborate the same stream_harness
with different parameters; the UART command protocol they speak to it is
identical, so there is one transport class and no copy to drift.

(It was previously reachable only from the pre-migration perf tree, and
build-mon's test loaded it by absolute path through a block commented
TRANSITIONAL. That hook is gone -- see [[flow-migration]].)
...
# projects/NexysA7/.../flows-stream-bridge/host, which is precisely the kind of
# reach that survives a relocation and then compiles the wrong tree.
...
# =========================================================================
# Memory map constants (must match stream_harness parameters)
# =========================================================================
...
# STREAM APB registers — addresses from projects/components/dmas/stream/rtl/stream_regmap.py
...
APB_SCHED_TIMEOUT_CYC   = A("SCHED_TIMEOUT_CYCLES")
APB_SCHED_CONFIG        = A("SCHED_CONFIG")
APB_DESCENG_CONFIG      = A("DESCENG_CONFIG")
APB_DESCENG_ADDR0_BASE  = A("DESCENG_ADDR0_BASE")
APB_DESCENG_ADDR0_LIMIT = A("DESCENG_ADDR0_LIMIT")
...
# Monitor CSRs were RELOCATED to the 0x1000+ MON regfile (STREAM APB space is
# now 8 KB / APB_ADDR_WIDTH=13). Addresses from stream_regmap.py; the char
# harness stream_apb window was widened to 8 KB to reach them (previously 4 KB,
# which truncated these and made every perf read return 0).
APB_DAXMON_ENABLE       = A("DAXMON_ENABLE")
APB_DAXMON_PKT_MASK     = A("DAXMON_PKT_MASK")   # [15:0] per-type drop mask
APB_DAXMON_ERR_CFG      = A("DAXMON_ERR_CFG")    # [3:0]=ERR_SELECT, [15:8]=ERR_MASK
APB_RDMON_ENABLE        = A("RDMON_ENABLE")
APB_RDMON_PKT_MASK      = A("RDMON_PKT_MASK")
APB_RDMON_ERR_CFG       = A("RDMON_ERR_CFG")
...
APB_DAXMON_PERF_CTRL          = A("DAXMON_PERF_CTRL")    # RUN[0]
APB_DAXMON_PERF_STATUS        = A("DAXMON_PERF_STATUS")  # WIN_ACTIVE[0]
APB_DAXMON_PERF_WINDOW_CYCLES = A("DAXMON_PERF_WINDOW_CYCLES")
APB_DAXMON_PERF_PROD_CYCLES   = A("DAXMON_PERF_PROD_CYCLES")
APB_DAXMON_PERF_BP_CYCLES     = A("DAXMON_PERF_BP_CYCLES")
APB_DAXMON_PERF_STARV_CYCLES  = A("DAXMON_PERF_STARV_CYCLES")
APB_DAXMON_PERF_IDLE_CYCLES   = A("DAXMON_PERF_IDLE_CYCLES")
APB_DAXMON_PERF_BEAT_COUNT    = A("DAXMON_PERF_BEAT_COUNT")
APB_DAXMON_PERF_BYTE_COUNT_LO = A("DAXMON_PERF_BYTE_COUNT_LO")
APB_DAXMON_PERF_BYTE_COUNT_HI = A("DAXMON_PERF_BYTE_COUNT_HI")
APB_DAXMON_PERF_BURST_COUNT   = A("DAXMON_PERF_BURST_COUNT")
...
APB_RDMON_PERF_BASE = A("RDMON_PERF_CTRL")   # perf-window block base = CTRL reg
APB_WRMON_PERF_BASE = A("WRMON_PERF_CTRL")
...
# 0x180) was retired in RFC Stage E.4. The 0x100-0x128 region has been reclaimed
# by the RFC Stage E external DMA observer readback (see below) -- the in-core
# monitor CSRs further down remain the primary datapath-utilization source, with
# the observer providing an independent, host-readable cross-check.
#
# RFC Stage E external DMA observer readback (host-readable over CSR). The
...
# PERF_CH_SEL / PERF_CONFIG stay in the main regfile (0x2B0/0x35C); the
# per-channel READOUTS relocated to the 0x1000+ MON block.
APB_PERF_CH_SEL              = A("PERF_CH_SEL")              # CH_SEL[2:0]
APB_RDMON_PERF_CH_PROD_BP    = A("RDMON_PERF_CH_PROD_BP")   # {bp[31:16], prod[15:0]}
APB_RDMON_PERF_CH_STARV_IDLE = A("RDMON_PERF_CH_STARV_IDLE")  # {idle[31:16], starv[15:0]}
APB_WRMON_PERF_CH_PROD_BP    = A("WRMON_PERF_CH_PROD_BP")
APB_WRMON_PERF_CH_STARV_IDLE = A("WRMON_PERF_CH_STARV_IDLE")
APB_RDMON_PERF_CH_OVERFLOW   = A("RDMON_PERF_CH_OVERFLOW")
APB_WRMON_PERF_CH_OVERFLOW   = A("WRMON_PERF_CH_OVERFLOW")
...
APB_HIST_SEL   = A("HIST_SEL")
APB_HIST_DATA  = A("HIST_DATA")
APB_HIST_TOTAL = A("HIST_TOTAL")
HIST_NUM_BINS  = 16
...
class StreamHarnessTB(TBBase):
    """
    Testbench for stream_harness.
    ...
    def __init__(self, dut):
        ...
        # backdoor.
        #
        # None = RESOLVE IT (see _dma_slaves). A hardcoded default is what broke
        # here: the default was ('u_dma_slaves',), correct when the perf harness
        # instantiated the slaves bare, and the monitor harness overrode it to go
        # one level deeper through its dma_slave_monitors wrapper. Unifying on
        # ONE harness made the wrapper unconditional, so the un-overriding perf
        # tests silently probed a handle that was not the slaves -- every beat
        # count read as garbage and five DMA tests failed with no hint that a
        # hierarchy path was the cause.
        #
        # Resolving instead of defaulting means the next re-wrap costs nothing.
        # Set explicitly only to force a path.
        self.dma_slaves_path = None
        ...
        self.log.info(
            f"StreamHarnessTB: TEST_LEVEL={self.TEST_LEVEL} "
    ...
    async def run_apb_config_test(self) -> bool:
        ...
        val = await self.uart_read(A("GLOBAL_CTRL"))
        if val is None:
            self.log.error("Failed to read STREAM APB GLOBAL_CTRL")
            return False
        self.log.info(f"STREAM GLOBAL_CTRL = 0x{val:08X}")
        # Write and readback
        ok = await self.uart_write(A("GLOBAL_CTRL"), 0x01)  # GLOBAL_EN=1
        if not ok:
            return False
        rb = await self.uart_read(A("GLOBAL_CTRL"))
        if rb is None:
            return False
    ...
    async def run_ext_char_test(self, sizes, out_path: str) -> bool:
        ...
        import json
        from stream_device import Stream
        # The ext_char sweep logic lives in the perf build's host program, so
        # the SAME code drives sim and board. Renamed to host_ext_char when
        # entry points gained the host_ prefix; the build's host/ is on
        # python_search (see the wrapper).
        import host_ext_char as ec
        ...
        for (W, H) in sizes:
            for case in ec.CASES:
                ...
                self.log.info(
                    ...
                    f"rd(beats={rec['rd']['beats']} bursts={rec['rd']['bursts']} "
                    f"bpc={rec['rd']['bytes_per_cycle']:.2f}) "
                    f"wr(beats={rec['wr']['beats']} bpc={rec['wr']['bytes_per_cycle']:.2f})")
    ...
    # Candidate hierarchy paths to axi4_dma_slaves, most-wrapped first. The
    # shared harness instantiates it inside dma_slave_monitors (also named
    # u_dma_slaves), so the nested path is the live one; the bare path is kept
    # for any harness that instantiates the slaves directly.
    _DMA_SLAVES_CANDIDATES = (('u_dma_slaves', 'u_dma_slaves'), ('u_dma_slaves',))

    def _dma_slaves(self):
        """Handle to the axi4_dma_slaves instance (LFSR pattern-gen + CRC-check
        beat counters).

        Resolved by PROBING for the beat counters rather than trusting a path,
        and cached. Verified by reaching the counters the callers actually read,
        so a handle that merely exists cannot pass -- `dma_slave_monitors` and
        the `axi4_dma_slaves` it wraps share the instance name `u_dma_slaves`,
        so a name check alone would accept the wrapper and read nonsense.
        """
        cached = getattr(self, '_dma_slaves_h', None)
        if cached is not None:
            return cached

        paths = ([tuple(self.dma_slaves_path)] if self.dma_slaves_path
                 else list(self._DMA_SLAVES_CANDIDATES))
        tried = []
        for path in paths:
            h = self.dut
            try:
                ...
                continue
            self._dma_slaves_h = h
            self.log.info("dma_slaves backdoor resolved to dut.%s", '.'.join(path))
            return h

        raise AttributeError(
            "could not reach axi4_dma_slaves beat counters from the DUT; tried: "
            + "; ".join(tried)
    ...
    async def run_dma_test(self, num_channels: int,
        ...
        for name, addr in [
            ...
            ("CHANNEL_IDLE",      A("CHANNEL_IDLE")),
            ("DESC_ENGINE_IDLE",  A("DESC_ENGINE_IDLE")),
            ("SCHEDULER_IDLE",    A("SCHEDULER_IDLE")),
            ("SCHED_ERROR",       A("SCHED_ERROR")),
        ]:
        ...
        if measure_rw_perf:
            ...
            # DIAG: did the RUN write actually land? Read the CTRL regs back.
            _rd_ctrl = await self.uart_read(APB_RDMON_PERF_BASE + PERF_OFF_CTRL)
            _wr_ctrl = await self.uart_read(APB_WRMON_PERF_BASE + PERF_OFF_CTRL)
            self.log.info(f"  [DIAG] RDMON_PERF_CTRL @ 0x{APB_RDMON_PERF_BASE:04X} = "
                          f"{('0x%08X' % _rd_ctrl) if _rd_ctrl is not None else 'READ FAILED'} "
                          f"| WRMON_PERF_CTRL @ 0x{APB_WRMON_PERF_BASE:04X} = "
                          f"{('0x%08X' % _wr_ctrl) if _wr_ctrl is not None else 'READ FAILED'}")
        ...
        for name, addr in [
            ...
            ("SCHEDULER_IDLE",    A("SCHEDULER_IDLE")),
            ("SCHED_ERROR",       A("SCHED_ERROR")),
            ("AXI_RD_COMPLETE",   A("AXI_RD_COMPLETE")),
            ("AXI_WR_COMPLETE",   A("AXI_WR_COMPLETE")),
        ]:
            val = await self.uart_read(addr)
        ...
        if measure_rw_perf:
            ...
            # Drain length is a KNOB so 'the in-core total is short' can be
            # told apart from 'the in-core total is not drained yet'.
            await self.wait_clocks(self.clk_name,
                                   int(os.environ.get('PERF_DRAIN_CLK', '2000')))
