...
# paths of monbus_axil4_axil4_group:
#
...
# monbus_axil4_axil4_group; every record is now 24 bytes (3 × 64-bit beats) in
...
def parse_records(words32: Iterable[int], *, validate: bool = False) -> List:
    ...
    parsed = list(parse_stream(beats, stride_bytes=RECORD_BYTES, ts_mode=1))
    if not validate:
        return parsed
    out = []
    for rec in parsed:
        pkt = rec.packet if hasattr(rec, "packet") else rec
        if hasattr(pkt, "is_valid") and not pkt.is_valid():
            continue
        out.append(rec)
    return out
...
def dump_json(
    ...
    meta = {
        ...
        "source_format": "monbus_axil4_axil4_group_24B_ts_mode_1",
