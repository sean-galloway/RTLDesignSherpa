...
`Stream` API so the SAME program runs on the board (pyserial `UARTAxiBridge`)
...
def main(argv=None) -> int:
    ...
    # stream_env locates the shared FPGA layer (UARTAxiBridge) by SEARCHING
    # UPWARD for a marker file. This used to try `".." * up` for up in 3..8 --
    # a level count that happens to work from one directory depth and silently
    # picks nothing from another, which is why the area has one path authority.
