...
module stream_harness #(
    ...
    stream_top_ch8 #(
        ...
        // Compression is a MON-FLAVOUR feature and must not reach perf or obs.
        // The compressor sits on the IN-CORE monbus path, which only exists when
        // USE_AXI_MONITORS=1, so gate it on exactly that: mon gets the harness
        // parameter (default CFG_USE_MON_COMPRESSION = 1), perf and obs get 0 and
        // are bit-identical to before this change.
        //
        // These were hardcoded 0 for EVERY flavour in c454d64c, so the compressor
        // was compiled out even in mon while the parameter above advertised it as
        // on. The compression test still passed because the capture path carries
        // RAW records happily -- 96 slots for 32 packets, exactly the 3:1 raw
        // ratio. With the compressor actually built the same workload measures
        // 1.12 slots/packet (62.5% smaller).
        .USE_MON_COMPRESSION((USE_AXI_MONITORS == 1) ? USE_MON_COMPRESSION : 0),
        .USE_MON_HALFBEAT   ((USE_AXI_MONITORS == 1) ? USE_MON_HALFBEAT    : 0),
        .CDC_ENABLE         (0),
