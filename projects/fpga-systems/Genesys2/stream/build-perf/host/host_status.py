#!/usr/bin/env python3
# Quick status dump for a hung STREAM run on the FPGA.
# Usage:  make host-status ARGS="--port /dev/ttyUSB1"
#
...
#
# This MUST precede the library imports below. It did not have to when
# harness_addrs sat in the same directory as this script -- Python puts a
# script's own directory on sys.path -- but those libraries are shared and now
# live in bin/, so the bootstrap is what makes them reachable.
...
# Harness CSR offsets (by name through harness_addrs.H -- never hardcoded)
CSR_CTRL          = H("CTRL")
CSR_STATUS        = H("STATUS")
