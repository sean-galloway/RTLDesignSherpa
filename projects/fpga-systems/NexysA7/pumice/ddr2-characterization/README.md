...
- **2026-09-10** — The DUT-agnostic half of `ddr2_char_macro` (chargen regs + generator arrays + crossbars + perf taps) extracted into `ddr2_char_framework/rtl/char_engine_block.sv`; the macro now wraps it around pumice. `flows-litedram-uart/rtl/char_engine_harness.sv` rewritten as `build-perf`'s `ddr2_char_harness` minus the controller, on the same block and the same bridge address map, so the LiteDRAM A/B measures through identical RTL (PUMICE-026).
