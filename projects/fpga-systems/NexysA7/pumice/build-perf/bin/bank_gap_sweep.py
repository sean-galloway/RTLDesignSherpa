...
Shape of the experiment:
  ...
  1. Fill the WHOLE device, then stop. Every measurement phase is read-only,
     so the numbers describe the read path against a known memory image
     instead of a write and a read fighting each other.
...
REFILL says how often the fill is repeated: `loop` (default) re-fills at the
start of each generator-count block, `once` fills a single time for the whole
run, `point` re-fills before every measurement.

Re-filling is not needed for correctness -- reads do not modify memory, so one
fill is sufficient and the single-seed rule below is what makes it safe. It is
worth the time anyway at `loop`: a full fill is about 0.24 s at the measured
write rate, so four of them cost roughly a second, and in exchange a mismatch
anywhere in a block is bounded by a fill you just watched succeed rather than
by one from the top of the run. `point` costs about 46 s over the full matrix
and buys only a tighter bound on the same question.

One seed for the entire run REGARDLESS, not one per scenario. The expected data
is a function of address AND seed, so a per-scenario seed would invalidate
whatever fill preceded it -- including at `point`, because the cacheline family
marches past its own region and col_major wraps at the device boundary, so a
reader reaches addresses the immediately preceding fill did not write with that
seed. That is the whole reason pumice_char.measure()'s
write-then-read-per-scenario flow is not reused here.
    ...
    REFILL=point python3 bin/bank_gap_sweep.py                   # paranoid
    REFILL=once  python3 bin/bank_gap_sweep.py                   # fastest
"""
import os
import sys
...
REFILL = os.environ.get("REFILL", "loop").lower()
if REFILL not in ("loop", "once", "point"):
    raise SystemExit(f"REFILL must be loop|once|point, got {REFILL!r}")
...
def main() -> int:
    ...
    # The fill always uses every generator the bitstream has, whatever the
    # sweep is currently exercising: it is a fill, and more writers finish it
    # sooner. Only the READ phase varies n_gen.
    def refill(why):
        if not _fill_all(drv, geom, n_built, timeout_s=120.0):
            print(f"fill ({why}) did not complete -- the read numbers would be "
                  f"measured against an incomplete image; stopping.")
            return False
        return True

    print(f"refill policy: {REFILL}\n")
    if REFILL == "once" and not refill("once"):
        return 1

    bad = 0
    for n_gen in gens:
        if REFILL == "loop" and not refill(f"{n_gen} generators"):
            return 1
        ...
        for gap in GAPS:
            cells = []
            for _, fam in ORDERS:
                if REFILL == "point" and not refill(f"{fam} g{n_gen} gap{gap}"):
                    return 1
