...
## ---------------------------------------------------------------------------
## Clock-buffer cascade placement overrides (Place 30-120 / rule_cascaded_bufg)
##
## The per-counter glitchless mux tree is a 3-level BUFGCTRL cascade
## (u_m0/u_m1 -> u_m4 -> u_mO), plus a BUFG->BUFGCTRL cascade from each
## divided clock. The placer's "cascaded bufg must be adjacent and cyclic"
## rule cannot be satisfied for four such trees at once on the xc7a100t, and
## place_design fails outright. ANY_CMT_COLUMN keeps every route on
## clock-capable resources and only relaxes the adjacency requirement --
## acceptable here: the muxed clocks are <= 72.7 MHz demo domains, every
## crossing out of them is Gray-coded or handshaken, and each domain's
## internal timing is still fully analyzed. These lines are the overrides
## Vivado itself prints in the Place 30-120 message.
## ---------------------------------------------------------------------------
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets clk_mmcm_72m]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets clk_mmcm_27m]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets clk_mmcm_12m]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets clk_mmcm_6m]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[0].m0}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[0].m1}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[0].m4}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[1].m0}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[1].m1}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[1].m4}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[2].m0}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[2].m1}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[2].m4}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[3].m0}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[3].m1}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets {g_clkmux[3].m4}]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets w_clk_div_bufg_0]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets w_clk_div_bufg_1]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets w_clk_div_bufg_2]
set_property CLOCK_DEDICATED_ROUTE ANY_CMT_COLUMN [get_nets w_clk_div_bufg_3]

## ---------------------------------------------------------------------------
## Human-visible outputs are not timing endpoints.
##
## The set_output_delay lines above pull LED / 7-seg pins into timing analysis
## against sys_clk_pin, which also captures the ctr_clk-domain value flops
## driving SEG[*] as 10 ns cross-domain paths -- 19 failing endpoints at
## WNS -5.2 ns on a display a human reads at ~10 Hz. Nothing samples these
## pins synchronously; false-path them.
## ---------------------------------------------------------------------------
set_false_path -to [get_ports {LED[*] AN[*] SEG[*] DP}]
