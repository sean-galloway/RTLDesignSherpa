...
"""host_cdc_demo.py — Host CLI for the cdc_demo_top FPGA bitstream (`make host-cdc_demo ARGS=...`).
...
Bitstream: cdc_demo_top.  Port resolution: shared board layer (--board picks
the registry entry; --port auto narrows by USB serial then probes BUILD_ID).
...
# cdc_demo puts the shared board+UART layer (projects/fpga-systems/bin) on
# sys.path; the registry knows each board's JTAG/UART serial and baud.
...
def main():
    ...
    p.add_argument("--port", default="auto",
                   help="Serial device. Default 'auto' probes this board's "
                        "ports for the harness (BUILD_ID magic); pass an "
                        "explicit path to try it first.")
    p.add_argument("--board", default="nexys_a7_100t",
                   help="board registry name (see fpga-systems/bin/boards/)")
    ...
    args = p.parse_args()
    # Resolve the port through the shared board layer: narrow to THIS board's
    # ports by USB serial, then probe for the harness (BUILD_ID magic) --
    # ttyUSB numbering drifts, and a second board may be on the chain.
    board = get_board(args.board)
    port = board.find_uart_port(probe=cd.harness_probe(), want=args.port,
                                baudrate=args.baud, label="cdc_demo harness")
