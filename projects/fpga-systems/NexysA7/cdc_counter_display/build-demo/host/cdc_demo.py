...
with by-name register access via `UartRegisterMap`, backed by the PeakRDL-generated
...
if not _REPO_ROOT:
    raise RuntimeError(
        ...
        "importing cdc_demo.")
# Shared board+UART layer (uart_axi_bridge, uart_link, boards). The old
# converters/bin location is a re-export shim for un-migrated flows -- never
# import through it. See vault/handbook/fpga/cmn-infra/host-stack.md.
...
CDC_REGMAP = os.path.join(
    _REPO_ROOT, "projects/fpga-systems/NexysA7/cdc_counter_display/build-demo/"
    "dv/tbclasses/cdc_demo_csr_regmap.py")
...
def harness_probe():
    """Predicate answering "is this link the cdc_demo harness?".

    Reads BUILD_ID by name (never a hardcoded offset) and compares it to the
    'CDC1' magic. Read-only, so it is safe against a board running someone
    else's bitstream. Hand it to `Board.find_uart_port(probe=harness_probe())`
    so the scan also narrows to one board's ports by USB serial.
    """
    def probe(link) -> bool:
        return CdcDemoDriver(bridge=link.bridge()).build_id() == EXPECTED_BUILD_ID
    return probe
...
def autodetect_port(baud: int = 115200, want: str = None) -> str:
    ...
    Thin wrapper over the shared `uart_link.find_port` probe loop (which
    replaced this flow's hand-rolled scan); the cdc-specific part is only
    `harness_probe()`. Prefer `Board.find_uart_port(probe=harness_probe())` in
    new code -- it also narrows to one board's ports by USB serial, which
    matters when the Nexys A7 and the Genesys 2 are both attached."""
    return find_port(probe=harness_probe(), want=want, baudrate=baud,
                     label="cdc_demo harness")
