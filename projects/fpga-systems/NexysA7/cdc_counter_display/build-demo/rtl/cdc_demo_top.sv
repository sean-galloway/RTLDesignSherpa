...
module cdc_demo_top (
    ...
    `ALWAYS_FF_RST(sys_clk, sys_rstn,
        if (`RST_ASSERTED(sys_rstn)) begin r_btn_sync0 <= 4'h0; r_btn_sync1 <= 4'h0; end
    ...
    //
    //   Mux tree (4 muxes; sel[2] short-circuits straight to the divided
    //   clock, so the dup-input L0/L1 muxes a full binary tree would carry
    //   for inputs 4..7 never exist):
    //     L0: M0 = BUFGMUX(72m, 27m), sel = sel[0]
    //         M1 = BUFGMUX(12m, 6m),  sel = sel[0]
    //     L1: M4 = BUFGMUX(M0, M1),   sel = sel[1]
    //     L2: ctr_clk = BUFGMUX(M4, div), sel = sel[2]
    //
    //   So sel=000..011 -> MMCM outputs 0..3; sel=100..111 -> divided clock.
    //
    //   The earlier full binary tree (7 muxes, three of them with both
    //   inputs tied to the divided clock) needed 28 BUFGCTRL sites across
    //   the four counters and, with the 9 BUFGs, blew past the xc7a100t's
    //   32 clock-buffer sites: place_design failed DRC UTLZ-1 (37 > 32).
    //   4 muxes x 4 counters + 9 BUFGs = 25 fits.
    // ----------------------------------------------------------------
    ...
    generate
        for (mi = 0; mi < NUM_COUNTERS; mi = mi + 1) begin : g_clkmux
            ...
            BUFGMUX_CTRL u_mO (.I0(m4), .I1(w_clk_div_bufg[mi]), .S(sel[2]), .O(w_ctr_clk[mi]));
        end
    endgenerate
