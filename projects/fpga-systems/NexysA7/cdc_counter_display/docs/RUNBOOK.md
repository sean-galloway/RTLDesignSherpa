...
**Bitstream:** `cdc_demo_top` (build-demo, `make bitstream`)
...
| **`01`** | 1 | **STRETCH** | [`cdc_open_loop`](../../../../../rtl/cdc/cdc_open_loop.sv) with `STRETCH_CYCLES=1`, `SYNC_STAGES=3` | Source pulse-stretches data+valid for 1 ctr_clk; dst synchronizes through 3 flops. **Tuned to work up to ~20–25 MHz `ctr_clk`, lose values above** (dst misses pulses that don't outlast the synchronizer chain). |
| **`02`** | 2 | **SYNC FIFO** | [`fifo_async`](../../../../../rtl/cdc/fifo_async.sv), depth 16, Gray-pointer | Push every press in ctr_clk, pop continuously in sys_clk. Gray-coded pointer CDC is always safe — clean across the whole pickoff range. The "robust" reference path. |
| **`03`** | 3 | **2-PHASE** | [`cdc_2_phase_handshake`](../../../../../rtl/cdc/cdc_2_phase_handshake.sv) | Toggle (NRZ) handshake; periodic snapshots every 256 ctr_clk. Never garbage; value lags by the handshake round-trip. Faster than 4-phase. |
| **`04`** | 4 | **4-PHASE** | [`cdc_4_phase_handshake`](../../../../../rtl/cdc/cdc_4_phase_handshake.sv) | RTZ req/ack handshake; periodic snapshots every 256 ctr_clk. Most conservative, slowest. Same "safe but lagged" character as `03`. |
...
When a laptop is connected, the same controls are available via CSR. See [`HARNESS.md`](HARNESS.md) for the register map and [`build-demo/host/host_cdc_demo.py`](../build-demo/host/host_cdc_demo.py) for the runner. The port autodetects through the board registry (probes for the `CDC1` BUILD_ID); pass `--port /dev/ttyUSBn` only to force a device.

```bash
make program                                   # one-time flash
make host-cdc_demo ARGS=smoke                  # link check
make host-cdc_demo ARGS="watch-fail --counter 2"   # the headline demo, scripted
make host-cdc_demo ARGS=monitor                # live view of all 4 counters
```
