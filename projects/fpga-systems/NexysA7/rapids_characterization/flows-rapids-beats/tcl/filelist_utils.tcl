...
#   - Environment-variable substitution ($REPO_ROOT, ...)
...
#
# Ported verbatim from the STREAM characterization flow (flows-stream-bridge)
# so the filelist->Vivado read flow stays identical across board targets.
#==============================================================================
...
namespace eval filelist {
    ...
    proc _expand_env {line} {
        # Expand ${NAME} first (so e.g. "${REPO_ROOT}foo" works), then plain
        # $NAME. Each match is replaced with the env value directly.
    ...
    # Absolute paths come back normalized unchanged.
