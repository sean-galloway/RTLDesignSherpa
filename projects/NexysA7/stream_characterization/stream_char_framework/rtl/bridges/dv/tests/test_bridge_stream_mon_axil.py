...
from projects.NexysA7.stream_characterization.stream_char_framework.rtl.bridges.dv.tbclasses.bridge_stream_mon_axil_tb import BridgeStreamMonAxilTB
...
async def cocotb_test_bridge_stream_mon_axil_basic_connectivity(dut):
    ...
    tb.log.info(f"Configuration: 4M x 10S, RW channels")
    tb.log.info("=" * 80)
    ...
    # +0x100 is always safely inside the slave's window.
    test_addr = 0x00040100
    expected = tb.slave_mem_read(4, test_addr, master_idx=2)
    tb.log.info(f"  R slave=4 addr=0x{test_addr:08x} expect=0x{expected:08x}")
    actual = await tb.master_read(2, test_addr)
    assert actual == expected, (
        f"Read mismatch master 2 ← slave 4 at 0x{test_addr:08x}: "
        f"got 0x{actual:08x}, expected 0x{expected:08x} (seeded pattern)")
    ...
    # +0x100 is always safely inside the slave's window.
    test_addr = 0x000c0100
    expected = tb.slave_mem_read(7, test_addr, master_idx=3)
    tb.log.info(f"  R slave=7 addr=0x{test_addr:08x} expect=0x{expected:08x}")
    actual = await tb.master_read(3, test_addr)
    assert actual == expected, (
        f"Read mismatch master 3 ← slave 7 at 0x{test_addr:08x}: "
        f"got 0x{actual:08x}, expected 0x{expected:08x} (seeded pattern)")
...
async def cocotb_test_bridge_stream_mon_axil_boundary_probe(dut):
    ...
    for page_idx, page_base in enumerate(pages_1_2):
        for probe_idx, probe_off in enumerate(in_page_1_2):
            ...
            # path, framework just drops OOR memory writes silently).
            if tb.is_seeded(2, addr):
                got = tb.slave_mem_read(2, addr, master_idx=1)
                assert got == d, (
                    f"M1→S2 data mismatch at "
                    f"0x{addr:08x} (page=0x{page_base:08x}, off=0x{probe_off:x}): "
                    f"got 0x{got:08x}, expected 0x{d:08x}")
    tb.log.info(f"Master 2 (monbus_wr)")
    # Slave 4 (stream_tally): 0x00040000-0x0007ffff
    pages_2_4 = tb.slave_probe_pages(4, mode=mode)
    in_page_2_4 = tb.page_probe_offsets(4, master_idx=2)
    tb.log.info(f"  slave 4: {len(pages_2_4)} pages x 3 probes/page")
    for page_idx, page_base in enumerate(pages_2_4):
        for probe_idx, probe_off in enumerate(in_page_2_4):
            addr = page_base + probe_off
            # Tag data with (master, slave, page_idx, probe_idx) — every
            # probe within an (M,S) pair gets a unique 16-bit ID so a
            # misroute (write lands at wrong slave / wrong offset) is
            # visible at a glance in the failure message.
            d = (0xDE000000 | (2 << 20) | (4 << 16)
                 | ((page_idx & 0xFFF) << 4) | (probe_idx & 0xF))
            ...
            # path, framework just drops OOR memory writes silently).
            if tb.is_seeded(4, addr):
                got = tb.slave_mem_read(4, addr, master_idx=2)
                assert got == d, (
                    f"M2→S4 data mismatch at "
                    f"0x{addr:08x} (page=0x{page_base:08x}, off=0x{probe_off:x}): "
                    f"got 0x{got:08x}, expected 0x{d:08x}")
    tb.log.info(f"Master 3 (slave_monbus_wr)")
    # Slave 7 (slave_tally): 0x000c0000-0x000fffff
    pages_3_7 = tb.slave_probe_pages(7, mode=mode)
    in_page_3_7 = tb.page_probe_offsets(7, master_idx=3)
    tb.log.info(f"  slave 7: {len(pages_3_7)} pages x 3 probes/page")
    for page_idx, page_base in enumerate(pages_3_7):
        for probe_idx, probe_off in enumerate(in_page_3_7):
            addr = page_base + probe_off
            # Tag data with (master, slave, page_idx, probe_idx) — every
            # probe within an (M,S) pair gets a unique 16-bit ID so a
            # misroute (write lands at wrong slave / wrong offset) is
            # visible at a glance in the failure message.
            d = (0xDE000000 | (3 << 20) | (7 << 16)
                 | ((page_idx & 0xFFF) << 4) | (probe_idx & 0xF))
            ...
            # path, framework just drops OOR memory writes silently).
            if tb.is_seeded(7, addr):
                got = tb.slave_mem_read(7, addr, master_idx=3)
                assert got == d, (
                    f"M3→S7 data mismatch at "
                    f"0x{addr:08x} (page=0x{page_base:08x}, off=0x{probe_off:x}): "
                    f"got 0x{got:08x}, expected 0x{d:08x}")
...
def test_bridge_stream_mon_axil_basic_connectivity(request):
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/NexysA7/stream_characterization/stream_char_framework/rtl/bridges/filelists/bridge_stream_mon_axil.f'
    )
