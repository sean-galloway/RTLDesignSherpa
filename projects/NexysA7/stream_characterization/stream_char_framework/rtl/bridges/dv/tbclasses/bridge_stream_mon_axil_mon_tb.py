...
class BridgeStreamMonAxilMonTB(TBBase):
    ...
    def __init__(self, dut):
        ...
        self.slave_info = {
            ...
            4: ('axil', 0x00040000, 0x00040000, 64),  # stream_tally
        ...
        self._setup_slave_4_stream_tally()
    ...
    def _setup_slave_4_stream_tally(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 4: stream_tally (protocol: axil)"""
        # AXIL slave — own MemoryModel pre-seeded with the slave-specific
        # pattern; AXIL4SlaveRead/Write auto-responds from it honoring
        # whatever ARADDR / AWADDR the bridge forwards (after burst
        # decomposition by axi4_to_axil4 shims, if any).
    ...
    def _setup_slave_6_slave_err(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 6: slave_err (protocol: axil)"""
        # AXIL slave — own MemoryModel pre-seeded with the slave-specific
        # pattern; AXIL4SlaveRead/Write auto-responds from it honoring
        # whatever ARADDR / AWADDR the bridge forwards (after burst
        # decomposition by axi4_to_axil4 shims, if any).
        ...
        # Cap MemoryModel size — see SLAVE_MEM_CAP_BYTES comment.
        mem_bytes = min(addr_range, self.SLAVE_MEM_CAP_BYTES)
        preset = self._build_preset(6, addr_range, 32)
    ...
    def _setup_slave_7_slave_tally(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 7: slave_tally (protocol: axil)"""
        # AXIL slave — own MemoryModel pre-seeded with the slave-specific
        # pattern; AXIL4SlaveRead/Write auto-responds from it honoring
        # whatever ARADDR / AWADDR the bridge forwards (after burst
        # decomposition by axi4_to_axil4 shims, if any).
        ...
        # Cap MemoryModel size — see SLAVE_MEM_CAP_BYTES comment.
        mem_bytes = min(addr_range, self.SLAVE_MEM_CAP_BYTES)
        preset = self._build_preset(7, addr_range, 64)
