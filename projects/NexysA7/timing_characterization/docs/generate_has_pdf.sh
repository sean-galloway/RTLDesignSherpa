...
# ------------------------------------------------------------
# Timing Characterization Hardware Architecture Specification PDF Generator
# ------------------------------------------------------------
# Usage:
#   ./generate_has_pdf.sh [--rev <version>] [--help]
#
# Example:
#   ./generate_has_pdf.sh --rev 0.5
#
# This script builds the Timing Characterization HAS document
# (DOCX and PDF) from Markdown sources using md_to_docx.py.
# ------------------------------------------------------------

# Default values
REV="0.5"
ASSETS="timing_char_has/assets"
HAS_INDEX="timing_char_has/timing_char_has_index.md"
...
Description:
  This script generates a DOCX and PDF version of the Timing Characterization HAS
...
# ------------------------------------------------------------
OUTPUT_BASENAME="Timing_Characterization_HAS_v${REV}"
...
# ------------------------------------------------------------
echo "------------------------------------------------------------"
echo " Generating Timing Characterization Hardware Architecture Specification"
echo "------------------------------------------------------------"
echo "  Version:     ${REV}"
...
python3 "${REPO_ROOT}/bin/md_to_docx.py" \
  ...
  --style "timing_char_has/timing_char_has_styles.yaml" \
  --expand-index \
  --skip-index-content \
  --toc \
  --number-sections \
