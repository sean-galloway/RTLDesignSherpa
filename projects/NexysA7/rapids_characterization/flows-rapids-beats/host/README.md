...
|--------|------|----------|
| `0x0_0000` DUT-REG | APB byte addr `addr[12:0]` | AXIL -> `apb4_master` -> harness `s_apb`. SRC config @ `0x0000`, SNK config @ `0x1000` (half = APB `addr[12]`), plus the per-half `apbtodescr` kick windows (`base + ch*8` = LOW, `+4` = HIGH). |
