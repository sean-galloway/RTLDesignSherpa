...
# Sync the live sweep plots into docs/assets/png so the doc is a
# self-contained bundle. The graphviz PNGs already live there; we
# overlay the freshest plot PNGs from ../plots/ on top, keeping
# whichever the user has just regenerated.
if [[ -d "${PLOTS_SRC}" ]]; then
  echo "Syncing plots:  ${PLOTS_SRC}/*.png  →  ${ASSETS}/png/"
  cp -u "${PLOTS_SRC}"/*.png "${ASSETS}/png/" 2>/dev/null || true
else
  echo "WARNING: ${PLOTS_SRC} not found — using whatever PNGs already live in assets/png/"
fi
