"""
STREAM Macro-Level Test Configuration for pytest

Configures the test environment for STREAM macro-level integration tests.
...
def pytest_configure(config):
    ...
    config.option.log_file = os.path.join(log_dir, "pytest_stream_macro.log")
...
def pytest_sessionfinish(session, exitstatus):
    ...
    """
    logging.info("STREAM macro test session finished. Preserving all logs and build artifacts.")
...
def _generate_coverage_report(data: dict, reports_dir: str, timestamp: str):
    ...
    report_lines = []
    report_lines.append("# STREAM Macro-Level Coverage Report")
    report_lines.append("")
    ...
    report_lines.append("")
    report_lines.append("---")
    report_lines.append("*Report generated by STREAM macro-level coverage infrastructure*")
