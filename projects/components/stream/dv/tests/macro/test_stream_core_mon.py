"""
================================================================================
Stream Core Monitor Integration Test
================================================================================

Test suite for stream_core_mon - STREAM DMA engine with AXI monitoring.

Tests the full integration of:
- scheduler_group_array (task scheduling)
- axi_read_engine → axi4_master_rd_mon (AXI read with monitoring)
- axi_write_engine → axi4_master_wr_mon (AXI write with monitoring)
...
def test_stream_core_mon_single_channel(request, params):
    ...
    # Get sources from filelist (monitor version)
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/stream/rtl/filelists/macro/stream_core_mon.f'
    )
...
@pytest.mark.parametrize("params", generate_test_params())
def test_stream_core_mon_multi_channel(request, params):
