...
# ------------------------------------------------------------
# APB Crossbar Hardware Architecture Specification PDF Generator
# ------------------------------------------------------------
# Usage:
#   ./generate_has_pdf.sh [--rev <version>] [--help]
#
# Example:
#   ./generate_has_pdf.sh --rev 1.0
#
# This script builds the APB Crossbar HAS document
# (DOCX and PDF) from Markdown sources using md_to_docx.py.
# ------------------------------------------------------------

# Default values
REV="1.0"
ASSETS="apb4_xbar_has/assets"
HAS_INDEX="apb4_xbar_has/apb4_xbar_has_index.md"
...
Description:
  This script generates a DOCX and PDF version of the APB Crossbar HAS
...
# ------------------------------------------------------------
OUTPUT_BASENAME="APB_Crossbar_HAS_v${REV}"
...
# ------------------------------------------------------------
echo "------------------------------------------------------------"
echo " Generating APB Crossbar Hardware Architecture Specification"
echo "------------------------------------------------------------"
echo "  Version:     ${REV}"
...
python3 "${REPO_ROOT}/bin/md_to_docx.py" \
  ...
  --style "apb4_xbar_has/apb4_xbar_has_styles.yaml" \
  --pdf-engine=lualatex \
  --mainfont "Noto Serif" \
