...
|---------|--------|-------------|
| [apb4_xbar_architecture](apb4_xbar_spec/assets/graphviz/apb4_xbar_architecture.gv) | Graphviz | 2x4 crossbar showing master-side slaves, arbitration, slave-side masters |
| [address_decode_flow](apb4_xbar_spec/assets/graphviz/address_decode_flow.gv) | Graphviz | Step-by-step address decode example (0x10023456 → Slave 2) |
...
| Diagram | Format | Description |
|---------|--------|-------------|
| [arbitration_round_robin](apb4_xbar_spec/assets/wavedrom/arbitration_round_robin.json) | WaveJSON | 2 masters competing for Slave 0 with round-robin arbitration |
...
**Need a Crossbar?**
- 1 master, N slaves → Use `apb4_xbar_1toN` or generate
- 2 masters, N slaves → Use `apb4_xbar_2toN` or generate
...
docs/apb4_xbar_mas/
├── README.md                              ← This file
├── apb4_xbar_mas_index.md                  ← Book index (PDF build)
├── chapters/
...
- **Core Generator:** `bin/apb4_xbar_generator.py` (library implementation)
...
**Decision Tree:**
```
1 master, 1 slave  → apb4_xbar_1to1 (passthrough)
1 master, 4 slaves → apb4_xbar_1to4 (address decode only)
2 masters, 1 slave → apb4_xbar_2to1 (arbitration only)
2 masters, 4 slaves → apb4_xbar_2to4 (full crossbar)
...
# Generate SVG
dot -Tsvg apb4_xbar_architecture.gv -o ../svg/apb4_xbar_architecture.svg
