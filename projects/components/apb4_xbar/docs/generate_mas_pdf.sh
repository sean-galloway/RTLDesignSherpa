...
# ------------------------------------------------------------
# APB Crossbar Micro-Architecture Specification PDF Generator
# ------------------------------------------------------------
# Usage:
#   ./generate_mas_pdf.sh [--rev <version>] [--help]
#
# Example:
#   ./generate_mas_pdf.sh --rev 1.0
#
# This script builds the APB Crossbar MAS document
# (DOCX and PDF) from Markdown sources using md_to_docx.py.
# ------------------------------------------------------------

# Default values
REV="1.0"
ASSETS="apb4_xbar_mas/assets"
MAS_INDEX="apb4_xbar_mas/apb4_xbar_mas_index.md"
...
Description:
  This script generates a DOCX and PDF version of the APB Crossbar MAS
...
# ------------------------------------------------------------
OUTPUT_BASENAME="APB_Crossbar_MAS_v${REV}"
...
# ------------------------------------------------------------
echo "------------------------------------------------------------"
echo " Generating APB Crossbar Micro-Architecture Specification"
echo "------------------------------------------------------------"
echo "  Version:     ${REV}"
...
python3 "${REPO_ROOT}/bin/md_to_docx.py" \
  ...
  --style "apb4_xbar_mas/apb4_xbar_mas_styles.yaml" \
  --pdf-engine=lualatex \
  --mainfont "Noto Serif" \
