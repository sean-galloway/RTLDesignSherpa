...
|--------|-------------|---------|--------|----------|
| `apb4_xbar_1to1.sv` | Simple passthrough | 1 | 1 | Protocol conversion, testing |
| `apb4_xbar_2to1.sv` | Arbitration only | 2 | 1 | Multi-master to single slave |
| `apb4_xbar_1to4.sv` | Address decode only | 1 | 4 | Single master to multi-slave |
| `apb4_xbar_2to4.sv` | Full crossbar | 2 | 4 | Multi-master to multi-slave |
...
```bash
cd projects/components/apb4_xbar/bin/
python apb4_xbar_generator.py --masters 2 --slaves 4 --output ../rtl/apb4_xbar_2to4.sv
```
...
- `test_apb4_xbar_1to1.py` - 100+ transactions, variable delay profiles
- `test_apb4_xbar_2to1.py` - 130+ transactions, arbitration stress tests
- `test_apb4_xbar_1to4.py` - 200+ transactions, address decode validation
- `test_apb4_xbar_2to4.py` - 350+ transactions, full crossbar stress

Run tests:
```bash
pytest projects/components/apb4_xbar/dv/tests/test_apb4_xbar_2to4.py -v
pytest projects/components/apb4_xbar/dv/tests/ -v  # All variants
```
...
For variants beyond 16x16, modify generator limits in `apb4_xbar_generator.py`:
...
- `apb4_xbar_1to1.sv` - 1-to-1 passthrough
- `apb4_xbar_2to1.sv` - 2-to-1 with arbitration
- `apb4_xbar_1to4.sv` - 1-to-4 with address decode
- `apb4_xbar_2to4.sv` - 2-to-4 full crossbar
- `README.md` - This file
