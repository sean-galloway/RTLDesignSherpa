...
Usage:
    python apb4_xbar_generator.py --masters 2 --slaves 4 --output apb4_xbar_2to4.sv
    python apb4_xbar_generator.py --masters 1 --slaves 1 --output apb4_xbar_1to1.sv
...
def generate_apb4_xbar(num_masters, num_slaves, base_addr=0x10000000,
    ...
    Args:
        ...
        output_file: Output filename (default apb4_xbar_MtoN.sv)
    ...
    if output_file is None:
        output_file = f"apb4_xbar_{M}to{N}.sv"

    module_name = f"apb4_xbar_{M}to{N}"
...
Examples:
  Generate 2-to-4 crossbar:
    %(prog)s --masters 2 --slaves 4 --output rtl/amba/apb4/xbar/apb4_xbar_2to4.sv

  Generate 1-to-1 passthrough:
    %(prog)s --masters 1 --slaves 1 --output rtl/amba/apb4/xbar/apb4_xbar_1to1.sv

  Generate 4-to-8 crossbar with custom base address:
    %(prog)s --masters 4 --slaves 8 --base-addr 0x80000000 --output apb4_xbar_4to8.sv
        """
    )
    ...
    parser.add_argument('--output', '-o', type=str,
                        help='Output filename (default apb4_xbar_MtoN.sv)')

    args = parser.parse_args()

    try:
        code = generate_apb4_xbar(
        ...
        output_file = args.output if args.output else f"apb4_xbar_{args.masters}to{args.slaves}.sv"
