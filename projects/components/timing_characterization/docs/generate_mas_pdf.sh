...
# ------------------------------------------------------------
# Timing Characterization Micro-Architecture Specification PDF Generator
# ------------------------------------------------------------
# Usage:
#   ./generate_mas_pdf.sh [--rev <version>] [--help]
#
# Example:
#   ./generate_mas_pdf.sh --rev 0.5
#
# This script builds the Timing Characterization MAS document
# (DOCX and PDF) from Markdown sources using md_to_docx.py.
# ------------------------------------------------------------

# Default values
REV="0.5"
ASSETS="timing_char_mas/assets"
MAS_INDEX="timing_char_mas/timing_char_mas_index.md"
...
Description:
  This script generates a DOCX and PDF version of the Timing Characterization MAS
...
# ------------------------------------------------------------
OUTPUT_BASENAME="Timing_Characterization_MAS_v${REV}"
...
# ------------------------------------------------------------
echo "------------------------------------------------------------"
echo " Generating Timing Characterization Micro-Architecture Specification"
echo "------------------------------------------------------------"
echo "  Version:     ${REV}"
...
python3 "${REPO_ROOT}/bin/md_to_docx.py" \
  ...
  --style "timing_char_mas/timing_char_mas_styles.yaml" \
  --expand-index \
  --skip-index-content \
  --toc \
  --number-sections \
