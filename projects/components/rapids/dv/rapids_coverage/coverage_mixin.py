...
"""
Coverage Mixin for RAPIDS Testbenches
...
class CoverageHelper:
    ...
    Legal Coverage Mode:
        Set COVERAGE_LEGAL=1 to measure against RAPIDS-specific legal points only.
        This gives realistic coverage numbers instead of measuring against the
        full AXI4 protocol space that RAPIDS doesn't fully implement.
    ...
    def __init__(self, test_name: str, log: Optional[logging.Logger] = None,
        ...
        Args:
            ...
            legal_config: Name of legal config to load (e.g., 'rapids')
                         Or set COVERAGE_LEGAL=1 env var to auto-load 'rapids'
        """
        ...
        if legal_config:
            ...
            self.legal_config = LegalCoverageConfig.load('rapids')
...
def get_coverage_env(test_name: str, sim_build: str = None) -> dict:
    ...
    # Walk up from current file to find rapids component root
...
def register_bfm_coverage(bfms: list, coverage_helper: 'CoverageHelper') -> None:
    ...
    This connects the GAXI BFM coverage hook infrastructure to the rapids
