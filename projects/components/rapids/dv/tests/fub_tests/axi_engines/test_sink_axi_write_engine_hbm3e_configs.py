...
#
# Module: test_sink_axi_write_engine_hbm3e_configs
# Purpose: RAPIDS Sink AXI Write Engine HBM3e Transfer Size Configuration Test
#
...
"""
RAPIDS Sink AXI Write Engine HBM3e Transfer Size Configuration Test

A test to validate the new HBM3e transfer size configurations:
- 256B (cfg_transfer_size = 0)
- 512B (cfg_transfer_size = 1)
- 1KB  (cfg_transfer_size = 2)
- 2KB  (cfg_transfer_size = 3)
- 4KB  (cfg_transfer_size = 4)

This test validates that the calc_transfer_size_beats function works correctly
and that the engine generates appropriate AXI burst lengths for each setting.
"""
...
@cocotb.test()
async def test_hbm3e_transfer_size_configs(dut):
    """Test all HBM3e transfer size configurations."""
    ...
    cocotb.log.info("=== Starting HBM3e Transfer Size Configuration Test ===")
    ...
    for cfg_value, size_name, expected_beats in test_configs:
        cocotb.log.info(f"Testing {size_name} transfers (cfg={cfg_value}, expected_beats={expected_beats})")
        ...
        # Provide SRAM read data to trigger streaming
        ...
        if aw_valid:
            ...
            # AXI AWLEN is (burst_length - 1)
            if aw_len == (expected_beats - 1):
                cocotb.log.info(f"✓ {size_name}: Correct AXI AWLEN = {aw_len} (burst_len={expected_beats})")
            else:
                cocotb.log.error(f"✗ {size_name}: Wrong AXI AWLEN = {aw_len}, expected {expected_beats-1}")
        else:
            cocotb.log.info(f"⚠ {size_name}: No AXI AW activity (may need different stimulus)")
    ...
    cocotb.log.info("=== HBM3e Transfer Size Configuration Test Complete ===")
...
async def init_signals(dut):
    ...
    # Configuration for HBM3e transfer sizes (will be changed in test)
    dut.cfg_transfer_size.value = 1  # Start with 512B transfers
...
@pytest.mark.parametrize("num_channels,addr_width,data_width,axi_id_width", [
    (2, 32, 512, 4),  # Basic configuration for HBM3e testing
])
def test_sink_axi_write_engine_hbm3e_configs_rtl(request, num_channels, addr_width, data_width, axi_id_width):
    """RTL test for sink AXI write engine HBM3e configurations."""
    ...
    test_name_plus_params = f"test_sink_axi_write_engine_hbm3e_nc{nc_str}_aw{aw_str}_dw{dw_str}_id{id_str}"
    ...
    try:
        ...
        print(f"✓ Sink AXI Write Engine HBM3e configuration test completed!")
        print(f"Logs: {log_path}")
        print(f"Waveforms: {cmd_filename}")
        print(f"Validated: 256B/512B/1K/2K/4K HBM3e transfer sizes")

    except Exception as e:
        print(f"❌ Sink AXI Write Engine HBM3e test failed: {str(e)}")
...
if __name__ == "__main__":
    print("RAPIDS Sink AXI Write Engine HBM3e Configuration Test")
    print("Run with: pytest val/rapids/fub_tests/axi_engines/test_sink_axi_write_engine_hbm3e_configs.py -v")