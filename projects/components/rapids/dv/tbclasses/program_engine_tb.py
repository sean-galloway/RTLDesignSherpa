...
#
# Module: DelayProfile
# Purpose: Program Engine Testbench
#
...
"""
Program Engine Testbench

Reusable testbench class for RAPIDS Program Engine validation using CocoTB Framework components.

The program engine manages post-descriptor write operations with:
...
Test Scenarios:
1. Basic program write - Single 32-bit write to aligned address
...
See: ../../../../docs/rapids_spec/ch02_blocks/01_03_program_engine.md
"""
...
class TestScenario(Enum):
    """Test scenario types"""
    BASIC_WRITE = "basic_write"              # Simple program write
...
class ProgramEngineTB(TBBase):
    """
    Reusable testbench for Program Engine validation.
    ...
    def __init__(self, dut):
        ...
        # GAXI Master for program interface (will be created after clock starts)
        self.program_master = None
    ...
    async def setup_clocks_and_reset(self):
        ...
        # Signal: program_pkt_addr → prefix="program" + "_" + field_name="pkt_addr"
        program_field_config = FieldConfig()
        program_field_config.add_field(FieldDefinition(
            name='pkt_addr',  # prefix + _ + pkt_addr → program_pkt_addr
            bits=64,
            format='hex',
            description='Program write address'
        ))
        program_field_config.add_field(FieldDefinition(
            name='pkt_data',  # prefix + _ + pkt_data → program_pkt_data
            bits=32,
            format='hex',
            description='Program write data'
        ))

        self.program_master = create_gaxi_master(
            dut=self.dut,
            title="ProgramMaster",
            prefix="program",  # Will combine: program + _ + field_name
            clock=self.clk,
            field_config=program_field_config,
            log=self.log,
            mode='skid',
            multi_sig=True  # Program interface uses separate pkt_addr and pkt_data signals
        )
        ...
        # Program engine is AXI master, so these are inputs from slave perspective
    ...
    async def initialize_axi_slave(self, profile: DelayProfile):
        ...
        # Program engine is AXI4 write master, so we need slave write interface
        ...
        self.axi_slave = create_axi4_slave_wr(
            ...
            addr_width=64,   # Program engine has 64-bit addresses
    ...
    async def send_program_request(self, addr: int, data: int,
                                   profile: DelayProfile) -> bool:
        """
        Send single program request and wait for completion.

        Args:
            addr: Program write address (64-bit)
            data: Program write data (32-bit)
        ...
        self.log.info(f"📤 Program Request: addr=0x{addr:X}, data=0x{data:08X}")

        # Use GAXI Master to send program request
        # Field names: pkt_addr and pkt_data (prefix gets added automatically)
        packet = self.program_master.create_packet(
            pkt_addr=addr,
            pkt_data=data
        )

        try:
            await self.program_master.send(packet)
            self.log.info(f"  ✓ Program request sent successfully")
        except Exception as e:
            self.log.error(f"❌ Failed to send program request: {str(e)}")
        ...
        while cycles < timeout:
            ...
            # Check for error
            if int(self.dut.program_error.value) == 1:
                self.log.warning(f"⚠️  Program error detected")
                self.errors_detected.append("program_error")
                return False

            # Check for idle (operation complete)
            if int(self.dut.program_engine_idle.value) == 1:
        ...
        if not operation_complete:
            ...
            try:
                ...
                fifo_valid = int(self.dut.w_prog_req_skid_valid_out.value)
                fifo_ready = int(self.dut.w_prog_req_skid_ready_out.value)
                idle_signal = int(self.dut.program_engine_idle.value)
    ...
    async def test_basic_write(self, profile: DelayProfile) -> bool:
        ...
        """
        self.log.info("=" * 70)
        self.log.info(f"TEST: Basic Program Write (Profile: {profile.value})")
        self.log.info("=" * 70)
        ...
        success = await self.send_program_request(test_addr, test_data, profile)
    ...
    async def test_back_to_back(self, num_operations: int,
                                profile: DelayProfile) -> bool:
        """
        Test multiple back-to-back program operations.
        ...
        for i in range(num_operations):
            ...
            success = await self.send_program_request(addr, data, profile)
            if not success:
    ...
    async def test_misaligned_address(self, profile: DelayProfile) -> bool:
        ...
        Program engine requires 4-byte alignment for 32-bit writes.
        ...
        for addr, data in test_cases:
            ...
            for cycle in range(max_monitor_cycles):
                ...
                try:
                    state = int(self.dut.r_current_state.value)
                    error_flag = int(self.dut.program_error.value)
                    addr_error = int(self.dut.w_address_error.value)
                    latched_addr = int(self.dut.r_program_addr.value)
            ...
            if not error_detected:
                self.log.error(f"❌ Expected program_error for misaligned address 0x{addr:X}")
                # Final state dump
                try:
                    state = int(self.dut.r_current_state.value)
                    error_flag = int(self.dut.program_error.value)
        ...
        # The fact that program_error was asserted is sufficient verification
    ...
    async def run_test_suite(self, scenario: TestScenario,
        ...
        """
        self.log.info("=" * 70)
        self.log.info(f"PROGRAM ENGINE TEST SUITE")
