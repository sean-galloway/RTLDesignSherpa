...
#
# Module: test_axil4_to_axi4_rd
# Purpose: AXIL4 to AXI4 Read Converter Test Runner (READ-ONLY)
#
...
"""
AXIL4 to AXI4 Read Converter Test Runner - READ-ONLY

Test runner for AXIL4→AXI4 READ protocol upgrade converter (AR, R channels only).
Imports testbench class from projects/components/converters/dv/tbclasses/axil4_to_axi4_rd_tb.py

Test Levels:
- basic: Quick smoke test (5 single-beat read transactions)
- medium: Multiple read transactions with different addresses (20 reads)
- full: Comprehensive read coverage (50 reads, various parameters)

Tests ONLY the read path. For write path, see test_axil4_to_axi4_wr.py.
"""
...
from projects.components.converters.dv.tbclasses.axil4_to_axi4_rd_tb import AXIL4ToAXI4ReadTB
from CocoTBFramework.tbclasses.shared.filelist_utils import get_sources_from_filelist


@cocotb.test(timeout_time=30, timeout_unit="ms")
async def axil4_to_axi4_rd_test(dut):
    """
    AXIL4 to AXI4 Read Converter test - READ-ONLY.

    Tests ONLY read path (AR, R channels) protocol upgrade.
    Test intelligence resides here, infrastructure in TB class.
    """
    tb = AXIL4ToAXI4ReadTB(dut)
    ...
    tb.log.info(f"Running {test_level.upper()} AXIL4→AXI4 Read Converter test suite")
    ...
    try:
        ...
        tb.log.info("=" * 80)
        tb.log.info(f"Reads sent:        {final_stats['reads_sent']}")
        tb.log.info(f"Reads received:    {final_stats['reads_received']}")
...
def test_axil4_to_axi4_rd(request, params):
    """
    AXIL4 to AXI4 Read Converter test with protocol upgrade validation.

    Features:
    - Protocol upgrade (AXI4-Lite → AXI4)
    - Field addition (ID, LEN=0, SIZE, BURST, etc.)
    ...
    dut_name = "axil4_to_axi4_rd"  # Read-only converter
    toplevel = dut_name
    ...
    test_name_plus_params = (f"test_axil4_to_axi4_rd_"
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/converters/rtl/filelists/axil4_to_axi4_rd.f'
    )
    ...
    print(f"\n{'='*80}")
    print(f"AXIL4→AXI4 Read Converter Test: {test_level.upper()}")
