...
from CocoTBFramework.components.apb.apb_factories import \
    create_apb4_slave, create_apb4_monitor
...
async def axi2apb4_shim_test(dut):
    """Main test function"""
    tb = Axi2ApbTB(dut)
...
@pytest.mark.parametrize("id_width, addr_width, data_width, user_width, apb_addr_width, apb_data_width",
                            [
                                # Original: 32b AXI / 8b APB (axi2apb4ratio=4 — narrow-APB decomposition)
                                (8, 32, 32, 1, 12, 8),
                                # Bridge-matched: 32b AXI / 32b APB (axi2apb4ratio=1 — same-width).
