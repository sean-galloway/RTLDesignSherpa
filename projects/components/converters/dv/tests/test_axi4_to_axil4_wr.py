...
#
# Module: test_axi4_to_axil4_wr
# Purpose: AXI4 to AXIL4 Write Converter Test Runner (WRITE-ONLY)
#
...
"""
AXI4 to AXIL4 Write Converter Test Runner - WRITE-ONLY

Test runner for AXI4→AXIL4 WRITE protocol downgrade converter with burst decomposition.
...
async def axi4_to_axil4_wr_test(dut):
    """
    AXI4 to AXIL4 Write Converter test - WRITE-ONLY with burst decomposition.

    Tests write path (AW, W, B channels) with:
    ...
    try:
        ...
        tb.log.info(f"AXIL writes received:  {final_stats['axil_writes_received']}")
        ...
        tb.log.info(f"Data errors:           {final_stats['data_errors']}")
...
def generate_test_params():
    ...
    all_params = [
        ...
        # partial strobes, narrow-within-wide, max burst)
...
def test_axi4_to_axil4_wr(request, params):
    enable_waves = bool(int(os.environ.get('WAVES', '0')))
    """
    AXI4 to AXIL4 Write Converter test with burst decomposition validation.
    ...
    dut_name = "axi4_to_axil4_wr"  # Write-only burst decomposition converter
    toplevel = dut_name
    ...
    test_name_plus_params = (f"test_axi4_to_axil4_wr_"
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/converters/rtl/filelists/axi4_to_axil4_wr.f'
    )
    ...
    print(f"\n{'='*80}")
    print(f"AXI4→AXIL4 Write Converter Test: {test_level.upper()}")
    print(f"Configuration: DW={data_width}, AW={addr_width}, ID={id_width}")
    print(f"Expected Duration: {timeout_ms/1000:.1f}s")
    print(f"Features: Burst decomposition, address incrementing, data integrity")
    print(f"{'='*80}")
