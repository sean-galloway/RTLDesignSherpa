...
#
# Module: test_apb5_to_axi4
# Purpose: apb5_to_axi4 (APB5 completer in, AXI4 requester out) test runner
#
# Author: sean galloway
# Created: 2026-09-11

"""APB5 -> AXI4 converter: data round trip, single-beat shape, PPROT/PSTRB,
error folding, USER sideband (APB5), backpressure. The testbench is
dv/tbclasses/apb_to_axi4_tb.py, shared with the APB4 twin.
...
async def apb5_to_axi4_test(dut):
    tb = APBToAXI4TB(dut)
...
def test_apb5_to_axi4(request, params):
    """APB5 completer -> AXI4 requester converter."""
    ...
    print(f"\n{'='*80}")
    print(f"APB5 -> AXI4: {test_level.upper()} DW={data_width} "
