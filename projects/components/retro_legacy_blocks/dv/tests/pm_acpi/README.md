...
- `test_apb_pm_acpi.py` - Main test runner
- `conftest.py` - Pytest configuration

## Testbench Classes

Testbench classes are located in:
`dv/tbclasses/pm_acpi/`

- `pm_acpi_tb.py` - Main testbench
- `pm_acpi_tests_basic.py` - Basic test suite
- `pm_acpi_tests_medium.py` - Medium test suite
- `pm_acpi_tests_full.py` - Full test suite

## Running Tests

```bash
# Run all tests
pytest dv/tests/pm_acpi/ -v

# Run basic tests only
pytest dv/tests/pm_acpi/ -v -k "basic"

# With waveforms
WAVES=1 pytest dv/tests/pm_acpi/ -v
```
