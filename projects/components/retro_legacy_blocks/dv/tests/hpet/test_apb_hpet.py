...
# Subsystem: retro_legacy_blocks/hpet
#
...
# From test file: hpet -> tests -> dv -> retro_legacy_blocks -> components -> projects -> repo_root
repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../../../..'))
import sys
sys.path.insert(0, repo_root)

# Import from PROJECT AREA (not framework!)
from projects.components.retro_legacy_blocks.dv.tbclasses.hpet.hpet_tb import HPETTB, HPETRegisterMap
...
def test_hpet(request, num_timers, vendor_id, revision_id, cdc_enable, test_level, description):
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/retro_legacy_blocks/rtl/hpet/filelists/integration/apb_hpet.f'
    )
