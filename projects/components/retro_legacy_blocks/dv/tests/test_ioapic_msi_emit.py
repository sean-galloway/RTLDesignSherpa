...
"""System-context test for ioapic_msi_emit.

The DUT is ioapic_msi_emit_tb_top (dv/tb/), which wires the emitter onto a real
apb4_ioapic delivery channel and a real apb4_master_stub. The emitter's own
mapping is proved by formal; this covers the SEAM -- see ioapic_msi_tests.py
for what is deliberately not re-tested here.
...
from projects.components.retro_legacy_blocks.dv.tbclasses.ioapic.ioapic_msi_tb import IOAPICMsiTB
from projects.components.retro_legacy_blocks.dv.tbclasses.ioapic.ioapic_msi_tests import IOAPICMsiTests


@cocotb.test(timeout_time=500, timeout_unit="us")
async def cocotb_test_msi_seam(dut):
    ...
    random.seed(seed)
    tb.log.info(f'IOAPIC MSI seam test with seed: {seed}')
    ...
    tests = IOAPICMsiTests(tb)

    gate_methods = [
        ('Programmed address and data reach the bus',
         tests.test_programmed_values_reach_the_bus),
    ]
    func_methods = [
        ('Message fields overwrite the template',
         tests.test_message_fields_overwrite_the_template),
        ('Reprogramming retargets the write',
         tests.test_reprogramming_retargets_the_write),
    ]
    full_methods = [
        ('PSLVERR reaches the emitter as retry',
         tests.test_pslverr_reaches_the_emitter_as_retry),
    ]
    ...
    tb.log.info(f"Starting {test_level.upper()} IOAPIC MSI seam test "
    ...
    if passed != total:
        assert False, f"IOAPIC MSI seam test failed: {passed}/{total} passed"
    tb.log.info("\nAll IOAPIC MSI seam tests PASSED!")


def generate_test_params():
    """REG_LEVEL selects the grid; TEST_LEVEL gates the depth."""
    return [(lvl, f"IOAPIC MSI seam {lvl}") for lvl in reg_level_grid()]


@pytest.mark.parametrize("test_level, description", generate_test_params())
def test_ioapic_msi_emit(request, test_level, description):
    """Pytest wrapper -- calls cocotb_test_msi_seam."""
    module, repo_root_local, tests_dir, log_dir, rtl_dict = get_paths({})

    dut_name = "ioapic_msi_emit_tb_top"
    test_name_plus_params = f"test_ioapic_msi_emit_{test_level}"
