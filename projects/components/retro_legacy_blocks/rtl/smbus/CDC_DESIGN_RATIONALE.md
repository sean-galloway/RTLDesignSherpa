...
System Clock (pclk) - e.g., 100 MHz
        │
        ├─→ APB Interface (runs at pclk)
        │
        ├─→ SMBus Registers (runs at pclk)
        │
        └─→ SMBus Core & Physical Layer (runs at pclk)
                │
                └─→ Clock Divider (generates SCL timing from pclk)
                        │
                        └─→ SCL Output: 100 kHz or 400 kHz
```
...
|--------|-----------|------------|----------------|------------|
| **HPET** | pclk (50MHz) | hpet_clk (14.318MHz) | Precise timestamps | ✅ YES |
| **PIT** | pclk (variable) | pit_clk (1.193MHz) | Independent timing | ✅ YES |
| **RTC** | pclk | rtc_clk (32.768kHz) **OR** pclk | Battery backup option | Optional |
| **SMBus** | pclk | pclk | Bus speed derived from pclk | ❌ NO |
| **PIC** | pclk | pclk | Synchronous interrupts | ❌ NO |
...
**Example:** 100 MHz pclk → 100 kHz SCL
- Clock divider = 249
- SCL frequency = 100 MHz / (4 × (249 + 1)) = 100 kHz
...
❌ "SMBus doesn't need CDC because it's slow (100kHz)"

### Actual Reason
✅ "SMBus doesn't need CDC because it uses **single clock domain design** - SCL is an output derived from pclk, not an independent clock input"
...
✅ Simpler design - no CDC complexity  
✅ Lower latency - no synchronization delay  
✅ Easier verification - single clock domain  
✅ Smaller area - no CDC FIFOs needed  

### When This Works
✅ Derived timing (bus protocols: I2C, SPI, UART)  
✅ I/O where speed is controlled via dividers  
✅ When core doesn't need independent operation  

### When CDC Is Needed
🔄 Independent clock sources (crystals, PLLs)  
🔄 Battery-backed operation  
🔄 Precise timing independent of system  
🔄 Multiple clock domains by requirement  
...
1. ✅ **Architectural:** Designed as single clock domain (pclk only)
2. ✅ **SCL Generation:** SCL derived from pclk via divider, not separate input
3. ✅ **Synchronous Operation:** All registers/FSMs clocked by pclk
4. ✅ **Design Choice:** SMBus spec doesn't require independent clock

**Not because:**
-❌ SMBus is "slow" (speed helps but isn't the reason)
