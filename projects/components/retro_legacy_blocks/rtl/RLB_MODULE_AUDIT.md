...
**Features:**
- Uses `apb4_slave_cdc` with CDC_ENABLE parameter (supports both CDC and non-CDC)
...
**Features:**
- Uses `apb4_slave` (single clock domain - appropriate for SMBus)
...
**When to use apb4_slave_cdc vs apb4_slave:**
...
│                                                              │
│  ┌────────────────┐                                         │
│  │  apb4_slave_cdc │ (or apb4_slave if no CDC)               │
│  │  (APB → CMD/RSP)│                                         │
│  └────────┬───────┘                                         │
│           │ cmd/rsp signals                                  │
│           ▼                                                  │
...
### Key Files Needed
1. `apb_<module>.sv` - Top level with apb4_slave(_cdc)
