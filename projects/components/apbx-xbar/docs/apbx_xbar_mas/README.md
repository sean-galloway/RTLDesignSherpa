...
**Need a Crossbar?**
- 1 master, N slaves → Use `apbx_xbar_1toN` or generate
- 2 masters, N slaves → Use `apbx_xbar_2toN` or generate
...
docs/apbx_xbar_mas/
├── README.md                              ← This file
├── apbx_xbar_mas_index.md                  ← Book index (PDF build)
├── chapters/
...
- **Core Generator:** `bin/apbx_xbar_generator.py` (library implementation)
...
**Decision Tree:**
```
1 master, 1 slave  → apbx_xbar_1to1 (passthrough)
1 master, 4 slaves → apbx_xbar_1to4 (address decode only)
2 masters, 1 slave → apbx_xbar_2to1 (arbitration only)
2 masters, 4 slaves → apbx_xbar_2to4 (full crossbar)
