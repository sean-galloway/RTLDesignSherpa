...
| Back-to-Back | Consecutive transactions with no master-side idle cycles. Note this crossbar does not OVERLAP them, and cadence is one cycle longer than single-transfer latency: 10 PREADY-to-PREADY against 9 SETUP-to-PREADY with one master, 11 against 10 when arbitrated (see 5.2) |
