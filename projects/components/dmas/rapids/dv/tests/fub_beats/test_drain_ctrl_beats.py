...
This test file imports the reusable DrainCtrlBeatsTB class from:
  projects/components/dmas/rapids/dv/tbclasses/drain_ctrl_beats_tb.py
...
from projects.components.dmas.rapids.dv.tbclasses.drain_ctrl_beats_tb import DrainCtrlBeatsTB
...
def generate_beats_drain_ctrl_test_params():
    ...
    # GAXI BFM delay-profile sweep on the primary config (others stay 'default'
    # = backtoback). Drives the wr/rd drain masters via GAXI_TIMING_PROFILE.
...
def test_basic_write_drain(request, depth, almost_wr_margin, almost_rd_margin, timing_profile):
