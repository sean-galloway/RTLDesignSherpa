...
This test file imports the reusable DescriptorEngineTB class from:
  projects/components/dmas/rapids/dv/tbclasses/descriptor_engine_tb.py (PROJECT AREA)
...
def _run_descriptor_engine_test(request, testcase_name, channel_id, num_channels, axi_id_width,
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/dmas/rapids/rtl/filelists/fub_beats/descriptor_engine_beats.f'
    )
    ...
    # BFM delay-profile sweep: drive both the AXI read slave (TIMING_PROFILE)
    # and the apb GAXI master (GAXI_TIMING_PROFILE). 'default' leaves the TB
    # defaults in place.
