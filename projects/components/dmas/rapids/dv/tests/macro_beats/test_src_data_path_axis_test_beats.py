...
#
# Module: test_source_data_path_axis_test
# Purpose: RAPIDS Source Data Path AXIS Test Wrapper Validation
#
...
"""
RAPIDS Source Data Path AXIS Test Wrapper Validation

Test suite for the source_data_path_axis_test module which wraps:
- 8x Scheduler instances
- source_data_path_axis (AXI read -> SRAM -> AXIS master)

Data Flow:
  8x Descriptor Packets (GAXI) -> 8x Schedulers -> AXI Read -> Source Data Path -> AXIS Master

Features tested:
- Descriptor injection via GAXI masters (one per channel)
- Scheduler processing of descriptors
- AXI4 read operations from memory
- SRAM buffering
- AXIS packet transmission
...
def generate_source_axis_test_params():
    """Generate test parameters for source_data_path_axis_test tests.
...
def _run_source_axis_test(request, testcase_name, num_channels, addr_width, data_width, axi_id_width, sram_depth,
    ...
    dut_name = "src_data_path_axis_test_beats"

    # Get Verilog sources from file list
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/dmas/rapids/rtl/filelists/macro_beats/src_data_path_axis_test_beats.f'
    )
...
if __name__ == "__main__":
    ...
    print("Running basic source_data_path_axis_test test...")
