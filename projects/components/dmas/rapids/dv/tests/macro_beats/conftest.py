"""
RAPIDS Macro-Beats Level Test Configuration for pytest

Configures the test environment for RAPIDS macro-level integration tests (beats architecture).
Follows the established repository patterns (same as STREAM).
...
def pytest_configure(config):
    ...
    config.option.log_file = os.path.join(log_dir, "pytest_rapids_macro_beats.log")
    ...
    config.addinivalue_line("markers", "macro_beats: Macro-level beats architecture tests")
...
def pytest_sessionfinish(session, exitstatus):
    ...
    """
    logging.info("RAPIDS Macro-Beats test session finished. Preserving all logs and build artifacts.")
...
def _generate_coverage_report(data: dict, reports_dir: str, timestamp: str):
    ...
    report_lines = []
    report_lines.append("# RAPIDS Macro-Beats Coverage Report")
    report_lines.append("")
    ...
    report_lines.append("")
    report_lines.append("---")
    report_lines.append("*Report generated by RAPIDS coverage infrastructure*")
...
def coverage_config():
    ...
    try:
        from projects.components.dmas.rapids.dv.rapids_coverage import CoverageConfig
