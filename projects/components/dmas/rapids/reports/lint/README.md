...
```
projects/components/dmas/rapids/reports/lint/
...
```bash
cat projects/components/dmas/rapids/reports/lint/SUMMARY.txt
```
...
cat projects/components/dmas/rapids/reports/lint/verilator/scheduler.txt
...
```bash
grep -l "Error" projects/components/dmas/rapids/reports/lint/verilator/*.txt
```

### Count Issues Per Module
```bash
for f in projects/components/dmas/rapids/reports/lint/verible/*.txt; do
...
- `rm -rf projects/components/dmas/rapids/reports/lint`
...
./bin/lint_rapids.sh
cp -r projects/components/dmas/rapids/reports/lint /tmp/lint_before
...
# After changes
./bin/lint_rapids.sh
cp -r projects/components/dmas/rapids/reports/lint /tmp/lint_after
...
grep "^%Error" projects/components/dmas/rapids/reports/lint/verilator/*.txt

# Verible - only line length violations (easiest to fix)
grep "line-length" projects/components/dmas/rapids/reports/lint/verible/*.txt

# Yosys - real syntax errors (skip include warnings)
grep "ERROR:" projects/components/dmas/rapids/reports/lint/yosys/*.txt | grep -v "Can't open include"
```
