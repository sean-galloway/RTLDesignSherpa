...
# ------------------------------------------------------------
# RAPIDS Beats Micro-Architecture Specification PDF Generator
# ------------------------------------------------------------
# Usage:
...
#
# This script builds the RAPIDS Beats MAS document
# (DOCX and PDF) from Markdown sources using md_to_docx.py.
# ------------------------------------------------------------

# Default values
REV="0.6"
ASSETS="rapids_beats_mas/assets"
SPEC_INDEX="rapids_beats_mas/rapids_beats_mas_index.md"

# Detect repository root (go up from docs/ to project root)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git -C "${SCRIPT_DIR}" rev-parse --show-toplevel)"  # move-proof (was ../../../.. before dmas/ relocation)
...
Description:
  This script generates a DOCX and PDF version of the RAPIDS Beats MAS
...
# ------------------------------------------------------------
OUTPUT_BASENAME="RAPIDS_Beats_MAS_v${REV}"
...
# ------------------------------------------------------------
echo "------------------------------------------------------------"
echo " Generating RAPIDS Beats Micro-Architecture Specification"
echo "------------------------------------------------------------"
echo "  Version:     ${REV}"
...
python3 "${REPO_ROOT}/bin/md_to_docx.py" \
  ...
  --style "rapids_beats_mas/rapids_beats_mas_styles.yaml" \
  --expand-index \
  --skip-index-content \
  --toc \
  --number-sections \
