...
# ------------------------------------------------------------
# RAPIDS Beats Hardware Architecture Specification PDF Generator
# ------------------------------------------------------------
# Usage:
#   ./generate_has_pdf.sh [--rev <version>] [--help]
#
# Example:
#   ./generate_has_pdf.sh --rev 0.90
#
# This script builds the RAPIDS Beats HAS document
# (DOCX and PDF) from Markdown sources using md_to_docx.py.
# ------------------------------------------------------------

# Default values
REV="0.6"
ASSETS="rapids_beats_has/assets"
...
Description:
  This script generates a DOCX and PDF version of the RAPIDS Beats HAS
...
# ------------------------------------------------------------
OUTPUT_BASENAME="RAPIDS_Beats_HAS_v${REV}"
...
# ------------------------------------------------------------
echo "------------------------------------------------------------"
echo " Generating RAPIDS Beats Hardware Architecture Specification"
echo "------------------------------------------------------------"
echo "  Version:     ${REV}"
...
python3 "${REPO_ROOT}/bin/md_to_docx.py" \
  ...
  --style "rapids_beats_has/rapids_beats_has_styles.yaml" \
  --expand-index \
  --skip-index-content \
  --toc \
  --number-sections \
