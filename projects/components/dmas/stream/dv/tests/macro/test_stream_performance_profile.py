...
# ============================================================================
# Focused in-core perf-window regression
# ============================================================================
# The char harness (test_stream_char.py rw_perf/desc_perf/obs_equiv) reads the
# in-core datapath perf windows all-zero on the board. Addressing was ruled out
# (A() resolves DAXMON/RDMON/WRMON_PERF_CTRL to the correct 0x1150/0x1180/0x11B0),
# so the suspect is stream_core's HW perf-window controller (r_perf_win_active):
# it opens only when a RUN bit AND w_perf_dma_busy (=|(~scheduler_idle &
# cfg_channel_enable)) coincide. This test drives those stream_core ports
# DIRECTLY (no APB/UART) -- arm RUN, run one DMA, and assert the windows opened
# and counted. If it reproduces the zero, the bug is in the controller; if it
# passes, the bug is char-harness-side (arm/kick timing or the APB route).

@cocotb.test()
async def cocotb_test_perf_window_arm(dut):
    """Arm the in-core RD/WR datapath perf windows, run one DMA, verify they
    opened and counted productive cycles (isolates r_perf_win_active)."""
    config = next(c for c in PERF_TEST_CONFIGS if c['name'] == 'single_ch_dw128')
    tb = StreamCoreTB(dut, num_channels=config['num_channels'],
                      data_width=config['data_width'], fifo_depth=config['fifo_depth'])
    await tb.setup_clocks_and_reset()          # sets cfg_channel_enable = all
    await tb.wait_clocks('clk', 10)

    channel = config['test_channels'][0]
    # Default to a LARGE transfer: the char harness moves ~4096 beats/descriptor.
    # With a big transfer the scheduler dispatches early and goes idle while the
    # datapath keeps streaming -- the exact condition that makes the char harness
    # read the perf window all-zero (dma_busy keys on scheduler_idle). A small
    # transfer (128) hides it because the scheduler stays busy the whole time.
    transfer_size = int(os.environ.get('PERF_WIN_BEATS', '2048'))

    # One descriptor + its source data.
    desc_addr = tb.desc_mem_base + (channel * 0x1000)
    src_addr  = tb.src_mem_base
    dst_addr  = tb.dst_mem_base
    for beat in range(transfer_size):
        data = int.from_bytes(bytes([beat & 0xFF]) * tb.data_bytes, 'little')
    ...
    tb.write_descriptor(addr=desc_addr, src_addr=src_addr, dst_addr=dst_addr,
                        length=transfer_size, next_ptr=0, priority=0, last=True)

    # Arm the perf windows BEFORE the kick. RUN is a level: the window opens on
    # the first DMA activity after arm (w_perf_begin = run & dma_busy).
    dut.cfg_desc_mon_perf_run.value  = 1
    dut.cfg_rdeng_mon_perf_run.value = 1
    dut.cfg_wreng_mon_perf_run.value = 1
    await tb.wait_clocks('clk', 2)

    await tb.kick_off_channel(channel, desc_addr)
    await tb.wait_for_channel_idle(channel, timeout_us=100_000)
    await tb.wait_clocks('clk', 64)            # let the HW settle-close fire

    keys = ('window_active', 'window_cycles', 'prod_cycles', 'beat_count',
            'burst_count', 'byte_count')
    rd = {k: int(getattr(dut, f'rdmon_perf_{k}').value) for k in keys}
    wr = {k: int(getattr(dut, f'wrmon_perf_{k}').value) for k in keys}
    tb.log.info(f"RD perf window: {rd}")
    tb.log.info(f"WR perf window: {wr}")

    # These must hold REGARDLESS of USE_AXI_MONITORS -- the bus meters +
    # beat/byte/burst counters are always-on (only the heavy mons + histograms
    # are gated). This is the whole point of the refactor.
    for name, d in (('RD', rd), ('WR', wr)):
        assert d['prod_cycles'] > 0, f"{name} perf window never opened/counted: {d}"
        assert d['beat_count'] == d['prod_cycles'], f"{name} beats != productive: {d}"
        assert d['burst_count'] > 0, f"{name} no AR/AW bursts counted: {d}"
        assert d['byte_count'] > 0, f"{name} no bytes counted: {d}"
    um = os.environ.get('PERF_USE_AXI_MONITORS', '1')
    tb.log.info(f"in-core perf always-on OK (USE_AXI_MONITORS={um})")
...
def test_stream_perf_window_arm(request):
    """Focused regression for the in-core perf-window controller (r_perf_win_active)."""
    module, repo_root, tests_dir, log_dir, rtl_dict = get_paths({})
    dut_name = "stream_core"
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
    ...
    cfg = next(c for c in PERF_TEST_CONFIGS if c['name'] == 'single_ch_dw128')
    use_axi_monitors = int(os.environ.get('PERF_USE_AXI_MONITORS', '1'))
    rtl_parameters = {
    ...
    os.environ['PERF_CONFIG_NAME'] = cfg['name']
    os.environ['TIMING_PROFILE'] = 'fast'
    enable_waves = bool(int(os.environ.get('WAVES', '0')))
    extra_args = ["-Wno-WIDTH", "-Wno-CASEINCOMPLETE", "-Wno-TIMESCALEMOD", "-Wno-SELRANGE"]
    if enable_waves:
        extra_args.extend(["--trace-fst", "--trace-structs"])
    run(
        python_search=[tests_dir],
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_perf_window_arm",
        parameters=rtl_parameters,
        simulator="verilator",
        sim_build=f"local_sim_build/perf_window_arm_um{use_axi_monitors}",
        waves=enable_waves,
        extra_args=extra_args,
        compile_args=["-j", "4"],
        plus_args=['--trace'] if enable_waves else [],
