...
from projects.components.dmas.stream.dv.tbclasses.descriptor_packet_builder import DescriptorPacketBuilder
...
class DatapathWrTestTB(TBBase):
    ...
    async def setup_clocks_and_reset(self, xfer_beats=16):
        ...
        self.dut.cfg_axi_wr_xfer_beats.value = xfer_beats - 1
        self.xfer_beats = xfer_beats  # Store for test use
        self.log.info(f"Configured AXI write transfer size: {xfer_beats} beats (ARLEN={xfer_beats-1}){self.get_time_ns_str()}")
        ...
        self.axi_slave = AXI4SlaveWrite(
            ...
            # No optional_fields opt-out here: this macro's AW channel omits
            # the qualifier ports, and those are optional at the PROTOCOL-config
            # level now (axi4_aw_master in signal_mapping_helper), so every
