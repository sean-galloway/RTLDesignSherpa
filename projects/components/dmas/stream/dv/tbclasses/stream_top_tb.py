...
from projects.components.dmas.stream.rtl.stream_helper import StreamHelper
...
class StreamTopTB(TBBase):
    ...
    async def _init_bfm_components(self, rd_xfer_beats=16, wr_xfer_beats=16):
        ...
        # Apply per-channel AXI timing profiles (env-driven). Defaults to
        # 'fixed' so behavior is unchanged unless a profile is requested.
