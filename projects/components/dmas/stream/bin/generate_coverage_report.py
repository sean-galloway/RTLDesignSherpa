...
# Purpose: Generate comprehensive coverage report for STREAM component
#
...
"""
STREAM Coverage Report Generator

This script aggregates coverage data from all tests and generates:
...
Usage:
    ...
    # Specify coverage data directory
    python generate_coverage_report.py --coverage-dir /path/to/coverage_data

    # Generate HTML report
    python generate_coverage_report.py --html

    # Show detailed per-point coverage
    python generate_coverage_report.py --detailed
"""
...
def get_default_coverage_dir() -> str:
    """Get the default coverage data directory."""
    script_dir = get_script_dir()
    return os.path.join(script_dir, '..', 'dv', 'tests', 'fub', 'coverage_data')


def find_coverage_files(coverage_dir: str) -> Dict[str, List[str]]:
    """Find all coverage files in the coverage directory."""
    per_test_dir = os.path.join(coverage_dir, 'per_test')

    # Find sim_build directories (parent of coverage_data)
    tests_dir = os.path.dirname(coverage_dir)
    sim_build_dir = os.path.join(tests_dir, 'local_sim_build')

    files = {
        'summaries': glob.glob(os.path.join(per_test_dir, '*_summary.json')),
        'protocols': glob.glob(os.path.join(per_test_dir, '*_protocol.json')),
        'verilator': glob.glob(os.path.join(per_test_dir, '*.dat')),
        'sim_build_coverage': [],  # coverage.dat files in sim_build dirs
    }

    # Also look for coverage.dat files in sim_build directories
    # These are created by Verilator after simulation completes
    if os.path.exists(sim_build_dir):
        for test_dir in glob.glob(os.path.join(sim_build_dir, 'test_*')):
            ...
            if os.path.exists(coverage_dat):
                files['sim_build_coverage'].append(coverage_dat)
        ...
        with tempfile.TemporaryDirectory() as annotate_dir:
            ...
            for line in result.stdout.split('\n'):
                if 'Total coverage' in line:
                    # Extract covered/total from "(X/Y)"
                    import re
...
def parse_verilator_coverage(coverage_files: List[str]) -> Dict[str, Dict]:
    """Parse Verilator coverage.dat files and return merged coverage stats.
...
def merge_coverage_data(coverage_files: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    # First, parse Verilator coverage.dat files from sim_build directories
    # This gives us accurate line coverage for each test
    ...
    for summary_file in coverage_files.get('summaries', []):
        ...
        # The summary may have 0% if coverage.dat wasn't available when test saved
        ...
        # Aggregate code coverage from sim_build (more accurate than summary)
        if test_name in sim_build_coverage:
            ...
            code_cov = data.get('code_coverage', {})
            for metric in ['line', 'toggle', 'branch']:
                if metric in code_cov:
                    merged['code_coverage'][metric]['covered'] += code_cov[metric].get('covered', 0)
                    merged['code_coverage'][metric]['total'] += code_cov[metric].get('total', 0)

    # Calculate final percentages
    # For protocol coverage, use average per test
    ...
    # This gives accurate coverage accounting for overlapping lines across tests
    ...
    if sim_build_files:
        ...
        if lines_total > 0:
            ...
            # Fallback to average if merge failed
...
def generate_markdown_report(data: Dict[str, Any], detailed: bool = False) -> str:
    """Generate a markdown coverage report."""
    lines = []

    lines.append("# STREAM Component Coverage Report")
    lines.append("")
    ...
    if uncovered:
        ...
        lines.append("")
        for item in uncovered[:30]:  # Limit to first 30
    ...
    if not uncovered and protocol_pct >= 80 and line_pct >= 80:
        lines.append("- Coverage goals met! Consider adding edge case tests for robustness.")

    lines.append("")
    lines.append("---")
    lines.append("*Report generated by STREAM coverage infrastructure*")
...
<html>
<head>
    <title>STREAM Coverage Report</title>
    <style>
...
<body>
    <h1>STREAM Component Coverage Report</h1>
...
"""
    ...
    <table>
        <tr><th>Test Name</th><th>Protocol</th><th>Line</th></tr>
"""
    ...
    <footer>
        <p>Report generated by STREAM coverage infrastructure</p>
    </footer>
</body>
</html>
"""
...
def load_legal_config():
    """Load the legal coverage configuration for STREAM."""
    try:
        # Add the repo to path to import coverage config
        script_dir = get_script_dir()
        repo_root = os.path.abspath(os.path.join(script_dir, '..', '..', '..', '..'))
        sys.path.insert(0, os.path.join(repo_root, 'bin'))
        sys.path.insert(0, repo_root)

        from projects.components.dmas.stream.dv.stream_coverage.coverage_config import LegalCoverageConfig
        return LegalCoverageConfig.load('stream')
    except Exception as e:
        print(f"Warning: Could not load legal config: {e}")
        return None


def calculate_legal_coverage(data: Dict[str, Any], legal_config) -> Dict[str, Any]:
    ...
    # Category mapping to coverage groups
    category_to_groups = {
    ...
    # Overall legal coverage percentage
...
def generate_legal_coverage_report(data: Dict[str, Any], legal_result: Dict[str, Any]) -> str:
    """Generate markdown report section for legal coverage."""
    lines = []

    lines.append("## Legal Coverage (STREAM-Specific)")
    lines.append("")
    lines.append("This section shows coverage against **legal variations only** - the AXI/APB")
    lines.append("transaction types that STREAM actually uses, rather than the full protocol space.")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description='Generate STREAM coverage report')
    parser.add_argument('--coverage-dir', '-c',
                        help='Path to coverage data directory',
                        default=get_default_coverage_dir())
    ...
    parser.add_argument('--legal', '-l',
                        action='store_true',
                        help='Include legal coverage analysis (STREAM-specific points only)')

    args = parser.parse_args()

    # Resolve paths
    coverage_dir = os.path.abspath(args.coverage_dir)
    output_dir = args.output_dir or os.path.join(coverage_dir, 'reports')
    os.makedirs(output_dir, exist_ok=True)

    print("=" * 70)
    print("STREAM Coverage Report Generator")
    print("=" * 70)
    ...
    # Find coverage files
    files = find_coverage_files(coverage_dir)

    if not files['summaries']:
        print("No coverage data found!")
        print(f"Looking in: {os.path.join(coverage_dir, 'per_test')}")
        print("\nTo collect coverage, run tests with:")
        print("  COVERAGE=1 pytest projects/components/dmas/stream/dv/tests/fub/ -v")
        return 1

    print(f"Found {len(files['summaries'])} test coverage files")
    print(f"Found {len(files.get('sim_build_coverage', []))} sim_build coverage.dat files")
    ...
    md_report = generate_markdown_report(merged, detailed=args.detailed)

    # Add legal coverage analysis if requested
    legal_result = None
    if args.legal:
        legal_config = load_legal_config()
        if legal_config:
            legal_result = calculate_legal_coverage(merged, legal_config)
            legal_section = generate_legal_coverage_report(merged, legal_result)
            # Insert legal coverage section after executive summary
            lines = md_report.split('\n')
            insert_idx = None
            for i, line in enumerate(lines):
                if line.startswith('## Protocol Coverage by Group'):
                    insert_idx = i
                    break
            if insert_idx:
    ...
    # Generate HTML report
    if args.html:
        html_report = generate_html_report(merged)
    ...
    if legal_result:
        print()
        print(f"Legal Coverage:     {legal_result['legal_coverage_pct']:.1f}% (STREAM-specific points)")
        print(f"  Covered: {legal_result['legal_covered']}/{legal_result['legal_total']} legal points")
    print()

    protocol_ok = merged['overall']['protocol_pct'] >= 80
    ...
    if protocol_ok and line_ok:
        ...
        print("  (Full protocol coverage low, but all STREAM-specific points covered)")
        return 0
    else:
        print("Status: FAIL - Coverage below target (80%)")
        if legal_result and not legal_ok:
            print("  Missing STREAM-specific legal coverage points!")
        return 1
