...
**Module:** `monbus_axil4_axil4_group.sv` (documented here under its historical STREAM name `monbus_axil_group`)
...
The `monbus_axil_group` module is the canonical shared monitor bus processing block (RTL source: `rtl/amba/monitor/monbus_axil4_axil4_group.sv`; formerly `rtl/amba/shared/monbus_axil_group.sv`) instantiated by STREAM for packet filtering and routing. It receives 128-bit monitor bus packets paired with 64-bit timestamps (192-bit atomic group through AXI-Lite skid), filters them per protocol/packet type, and routes to either error/interrupt FIFO or external memory.
...
- **Shared Building Block:** Source of truth at `rtl/amba/monitor/monbus_axil4_axil4_group.sv` (module renamed `monbus_axil4_axil4_group` in the monitor-path move); STREAM instantiates this with zero modifications (drift area 1, commit 13706d10)
