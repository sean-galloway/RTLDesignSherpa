...
# ------------------------------------------------------------
# STREAM Hardware Architecture Specification PDF Generator
# ------------------------------------------------------------
# Usage:
...
#
# This script builds the STREAM HAS document
# (DOCX and PDF) from Markdown sources using md_to_docx.py.
# ------------------------------------------------------------

# Default values
REV="0.92"
ASSETS="stream_has/assets"
HAS_INDEX="stream_has/stream_has_index.md"
...
Description:
  This script generates a DOCX and PDF version of the STREAM HAS
...
# ------------------------------------------------------------
OUTPUT_BASENAME="STREAM_HAS_v${REV}"
...
# ------------------------------------------------------------
echo "------------------------------------------------------------"
echo " Generating STREAM Hardware Architecture Specification"
echo "------------------------------------------------------------"
echo "  Version:     ${REV}"
...
python3 "${REPO_ROOT}/bin/md_to_docx.py" \
  ...
  --style "stream_has/stream_has_styles.yaml" \
  --expand-index \
  --skip-index-content \
  --toc \
  --number-sections \
