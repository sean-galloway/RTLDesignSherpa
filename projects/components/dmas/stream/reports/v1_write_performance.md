...
# V1 Write Engine Performance Baseline
...
V1 write engine uses a **single outstanding transaction** architecture:
- Blocks on B response before accepting next request
- Simple control logic (r_aw_inflight, r_aw_valid, B response tracking)
...
1. **Latency Impact:** V1 write performance limited by B response latency
   - Blocks on B response before accepting next AW/W
   - No latency hiding capability

2. **Burst Length Effect:** Longer bursts improve efficiency
   - W channel can stream data during burst
   - Amortizes AW channel handshake latency
...
V1 write architecture is limited by:
- Sequential operation (no W channel pipelining)
- B response latency blocks next command
- No command queue to hide latency

**Path to Improvement (V2/V3):**

- V2 adds command queue + W drain FSM + B scoreboard → 6.7x improvement target
- V3 adds OOO W drain selection → 7.0x improvement target

---

## Test Execution Details

**Test Script:** `test_performance_baseline_v1_write.py`
**DUT:** `axi_write_engine.sv` with `ENABLE_CMD_PIPELINE = 0`
**Testbench:** `DatapathWrTestTB` with performance measurement extensions

**Measurement Methodology:**
1. Issue 1000 consecutive write commands (back-to-back when ready)
2. Measure total cycles from first AW handshake to last B response
3. Calculate sustained bandwidth = total_beats / total_cycles
4. Report efficiency = (sustained_bw / ideal_bw) * 100%

**Memory Model Configuration:**
- AXI slave with parameterizable write latency
- B response delay configurable independently
