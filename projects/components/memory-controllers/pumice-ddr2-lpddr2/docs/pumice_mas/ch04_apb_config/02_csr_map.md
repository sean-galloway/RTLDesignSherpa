...
The CSR map is generated from `regs/pumice_csrs.yaml` (PeakRDL-style YAML). The generator produces:

| File                                  | Consumer                                           |
|---------------------------------------|----------------------------------------------------|
| `regs/pumice_csr_pkg.sv`         | SV package: offsets, field masks, reset values     |
| `regs/pumice_csr_block.sv`       | The wrapped register file (used inside `csr_apb_fub`) |
| `regs/pumice_csrs.h`             | C header for SoC software                          |
| `regs/pumice_csr_block_uvm.sv`   | UVM register model for verification                |
| `regs/pumice_csrs.json`          | Machine-readable spec for documentation tools      |
...
peakrdl-csr-gen.py    (or equivalent)
        |
        +--> pumice_csr_pkg.sv
        +--> pumice_csr_block.sv      <-- instantiated in csr_apb_fub
        +--> pumice_csrs.h            <-- consumed by SoC firmware
        +--> pumice_csr_block_uvm.sv  <-- consumed by UVM tests
        +--> pumice_csrs.json         <-- consumed by doc-gen
```

The generator is invoked by the FUB filelist build (`rtl/filelists/fub/csr_apb.f`); the `pumice_csr_block.sv` is a build artifact, not a checked-in source file. Changes to the YAML trigger regeneration on next build.
