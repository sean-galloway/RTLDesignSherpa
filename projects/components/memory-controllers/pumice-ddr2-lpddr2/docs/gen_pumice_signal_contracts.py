...
def build_index(wb):
    ...
    # ---- READ THIS FIRST -------------------------------------------------
    # The spec pages were written 2026-09-07, mid-debug, while the design WAS
    # wedging, and they are phrased in that moment's voice: "today", "the
    # wedge", "the deadlock". ELEVEN RTL commits landed after them and closed
    # those failures. Without this banner the workbook reads as a bug list for
    # a controller that is in fact meeting its targets.
    banner = [
        ("RTL STATUS 2026-09-10: the controller MEETS ITS TARGETS. This workbook is not a bug list.", True),
        ("  Board (Nexys A7, 75 MHz, DDR2-300, BL4 x16, 600 MB/s peak): write 570.3 MB/s, "
         "read 571.3 MB/s -- both ~95% of peak, 14/14 characterization points integrity-clean.", False),
        ("  Concurrent read+write in one window: 570.1 MB/s total, 2.00x LiteDRAM through the "
         "identical harness (LiteDRAM 285.6).  Timing +0.285 ns, 0 failing of 94060 endpoints.", False),
        ("  Regression: 219 pumice tests green; char-framework board gate 31 passed.", False),
        ("", False),
        ("How to read the SPEC pages: they were authored 2026-09-07 DURING a debugging campaign, "
         "when reads sat at ~15% of peak and the write path wedged. They are written in that "
         "moment's present tense -- 'today', 'the wedge', 'the deadlock'. Eleven RTL commits "
         "landed afterwards and closed those failures.", False),
        ("  Trust the dated 'RTL STATUS 2026-09-10' lines on the individual sheets, not the "
         "surrounding present tense. Where a sheet carries no such line, it describes the "
         "failure a mechanism EXISTS TO PREVENT, not a live defect.", False),
        ("", False),
        ("Genuinely still open against the spec (neither costs measurable bandwidth today):", True),
        ("  1. AP columns are still one-per-bank in flight. Deliberate -- the mask guards a real "
         "stale-row hazard. Non-AP columns are unrestricted.  (SAME_BANK_OUTSTANDING)", False),
        ("  2. The read return is positional with no length check, so a short or lost burst would "
         "desync later reads. A ROBUSTNESS gap under a fault that does not occur in normal "
         "operation, not a defect in running hardware.  (RETURN_TAGGING)", False),
    ]
    r = 3
    for text, bold in banner:
        c = ws.cell(r, 1, text)
        c.alignment = IX_WRAP
        if bold:
    ...
    for rr, vals in enumerate(rows, hdr_row + 1):
