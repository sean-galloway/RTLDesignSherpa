...
The top module is `pumice_ctrl` and has four external interfaces:
...
The full SystemVerilog port list — with every parameter substituted — is generated by the build system into `regs/pumice_ctrl_ports.svh` and is the canonical source. This section is the **integrator's overview**, not the build-time canonical list.
