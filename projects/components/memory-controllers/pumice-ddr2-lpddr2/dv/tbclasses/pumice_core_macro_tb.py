...
"""Test-bench harness for pumice_core_macro (AXI-to-DFI macro).
...
class DDR2LPDDR2CoreMacroTB:
    """End-to-end TB for pumice_core_macro."""
    ...
    def __init__(self, dut, *, mc_period_ns: int = 7,
        ...
        self.dut = dut
        self.log = logging.getLogger("pumice_core_macro_tb")
    ...
    async def wait_for_init_done(self, timeout_cycles: int = 10_000) -> None:
        for _ in range(timeout_cycles):
            ...
            if int(self.dut.status_init_done.value):
                ...
                await self.wait_clocks('mc_clk', 40)
                return
        raise TimeoutError(
