...
the **fresh-read data-path hang** (see `pumice_top_testplan.yaml`).
...
**Status: wired.** `dv/pumice_coverage/` ships
...
"covers_lines" side of the testplan format) still needs a pumice
...
- Suspect: rd_op_id (= `'0` in `pumice_top.sv`) or an FSM state
...
- `pumice_csr_slave.hole_read_returns_zero` shows holes return
...
- `pumice_csr_slave.refresh_tuning_rw` proves the field
...
- [x] Author `dv/pumice_coverage/` helper module (mirror stream's
...
- [ ] Port `bin/update_testplan_coverage.py` for pumice (the
