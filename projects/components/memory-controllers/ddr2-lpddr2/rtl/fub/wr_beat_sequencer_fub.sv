...
#(
    ...
    `ALWAYS_FF_RST(mc_clk, mc_rst_n, begin
        if (`RST_ASSERTED(mc_rst_n)) begin
            ...
            unique case (r_state)
                ...
                S_PULL: begin
                    // beat_pull_strb_o is asserted while in S_PULL (see outputs
                    // below). Latch this cycle's beat and advance.
    ...
    assign beat_pull_strb_o = (r_state == S_PULL);
    assign beat_pull_slot_o = r_cur_slot;

    // DFI write-data packing: for each DFI cycle, pack DFI_RATE consecutive
    // DRAM beats from the staging buffer. Out-of-burst slots (tail of last
    // DFI cycle when len % DFI_RATE != 0) are masked off.
    localparam int MAX_LOG2 = $clog2(MAX_BURST_LEN);

    // Precompute the per-lane beat indices and the validity mask outside
    // the always_comb. Drops the latch-warning Verilator throws on local
    // variables conditionally assigned inside a for loop.
    ...
    always_comb begin
        dfi_wrdata_o      = '0;
        dfi_wrdata_mask_o = '1;   // mask=1 → don't write
        dfi_wrdata_en_o   = '0;
        if (r_state == S_DRIVE) begin
            dfi_wrdata_en_o = '1;
            for (int unsigned r = 0; r < DFI_RATE; r++) begin
                if (w_beat_in_burst[r]) begin
                    dfi_wrdata_o[r*DRAM_BEAT_WIDTH +: DRAM_BEAT_WIDTH]
                        = r_stage_data[w_beat_idx[r]];
                    // AXI wstrb=1 (write) ↔ DFI mask=0 (don't mask).
                    dfi_wrdata_mask_o[r*DRAM_STRB_WIDTH +: DRAM_STRB_WIDTH]
                        = ~r_stage_strb[w_beat_idx[r]];
                end
                // else: leave mask=1, dfi_wrdata=0 — past the burst tail.
            end
        end
    end

    assign b_complete_strb_o = (r_state == S_COMPLETE);
    assign b_complete_slot_o = r_cur_slot;
