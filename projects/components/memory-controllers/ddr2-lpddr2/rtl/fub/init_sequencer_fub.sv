...
(
    ...
    // Outputs
    //=========================================================================
    assign dfi_init_start_o = (r_state != S_RESET);

    // MR-sequenced writes — one MR per cycle while in the MR_* states.
    always_comb begin
        mr_seq_we_o    = 1'b0;
        mr_seq_index_o = 5'd0;
        mr_seq_data_o  = 16'd0;
        unique case (r_state)
            S_MR_MR2: begin
                mr_seq_we_o    = 1'b1;
                mr_seq_index_o = 5'd2;
                mr_seq_data_o  = (memtype_i == MEMTYPE_DDR2)
    ...
    assign zqcl_req_o  = (r_state == S_ZQCL);
    assign init_busy_o = (r_state != S_DONE);
    assign init_done_o = (r_state == S_DONE);
