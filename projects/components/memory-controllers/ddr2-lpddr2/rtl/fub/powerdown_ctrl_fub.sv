...
#(
    ...
    // pdn_req high only while in S_REQ.
    assign pdn_req_o = (r_state == S_REQ);

    // CKE low for ALL ranks while asleep; high otherwise. Per-rank
    // power-down lands later.
    assign dfi_cke_o = (r_state == S_ASLEEP) ? '0 : '1;
