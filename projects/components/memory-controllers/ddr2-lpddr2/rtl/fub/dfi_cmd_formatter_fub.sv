...
#(
    ...
    //=========================================================================
    // Pack phases into the multi-phase bus.
    // Phase 0 = decoded command. Phases 1..DFI_RATE-1 = NOP.
    //=========================================================================
    always_comb begin
        // Start with the NOP default for the whole bus (every phase).
        dfi_address_o = '0;
        dfi_bank_o    = '0;
        dfi_cas_n_o   = '1;
        dfi_ras_n_o   = '1;
        ...
        // Overwrite phase 0 with the decoded command.
        dfi_address_o[0 +: DFI_ADDR_WIDTH] = w_p0_addr;
        dfi_bank_o   [0 +: DFI_BANK_WIDTH] = w_p0_bank;
        dfi_cas_n_o  [0 +: DFI_CTRL_WIDTH] = w_p0_cas_n;
        dfi_ras_n_o  [0 +: DFI_CTRL_WIDTH] = w_p0_ras_n;
        dfi_we_n_o   [0 +: DFI_CTRL_WIDTH] = w_p0_we_n;
        dfi_cs_n_o   [0 +: DFI_CS_WIDTH]   = w_p0_cs_n;
        dfi_odt_o    [0 +: DFI_CS_WIDTH]   = w_p0_odt;

        // TODO (multi-cmd-per-cycle): when the scheduler can issue 2+
        //   commands per cycle, drive phases 1..DFI_RATE-1 from the
        //   additional decode results here.
    end

    // v1: always ready to accept the next command (single combinational
    // formatter; the next stage's pipeline register absorbs timing).
    assign cmd_ready_o = 1'b1;

    wire unused_v1 = |{ mc_clk, mc_rst_n, cmd_len_i };
