...
"""Unit-test runner for `pumice_csr_slave`.
...
pumice_csr_tb. obs_words readback lives in the F1f core_macro
...
from tbclasses.pumice_csr_tb import (  # noqa: E402
...
async def cocotb_test_pumice_csr_slave(dut):
...
def test_pumice_csr_slave(request, test_type):
    module, repo_root, tests_dir, log_dir, _ = get_paths({})
    dut_name = "pumice_csr_slave"
    test_name = f"test_pumice_csr_slave_{test_type}"

    filelist_path = ("projects/components/memory-controllers/pumice/"
                     "rtl/filelists/macro/pumice_csr_slave.f")
    ...
    run(python_search=[tests_dir],
        ...
        testcase="cocotb_test_pumice_csr_slave",
