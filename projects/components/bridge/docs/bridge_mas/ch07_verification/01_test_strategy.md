...
| `bridge_2x2_rw_pipe` | as `bridge_2x2_rw` with `xbar_pipeline = true` | generated suite plus `test_bridge_2x2_rw_pipe_latency` (BRIDGE-017): asserts the registered crossbar adds exactly the documented stages on AW, AR and B/R (HAS Table 5.7a); `test_bridge_2x2_rw_perf` runs on it too (`BRIDGE_PERF_DUT`) to show the throughput floors still hold |
| `bridge_2x2_rw_qos` | as `bridge_2x2_rw` with `arbitration = "qos"`, `qos_aging_shift = 4` | generated suite plus `test_bridge_2x2_rw_qos_arb` (BRIDGE-017): under saturation the higher-AxQOS master takes >= 0.75 of a shared port in both orientations, the QoS-0 master's worst wait stays under the aging bound (`15 * 2**shift` plus a burst), equal QoS splits fairly, and the port stays >= 0.95 beat/cycle (MAS 2.3 policy paragraph) |
| `bridge_2x2_rw_qos_pipe` | `bridge_2x2_rw_qos` on the registered crossbar (`xbar_pipeline = true` as well) | generated suite; exists for the synthesis sweep (HAS 6.4), which showed the QoS compare, grant, mux and CAM allocate in one path and the crossbar's slave-side skid as the register that splits it |
| `bridge_2x2_axi5_native` | two 128-bit AXI5 masters, two 128-bit AXI5 slaves, every port nsaid/trace/unique/poison/mte/chunking | generated suite with the AXI5 compliance checkers, plus `test_bridge_2x2_axi5_native_mte_chunk` (BRIDGE-018): Transfer writes from both masters land their per-beat tags in the slave BFM's tag store, Transfer reads return them, Match writes report BTAGMATCH right for matching and mismatching tags, Update writes follow WTAGUPDATE, chunked reads carry RCHUNKV/RCHUNKNUM per beat and unchunked reads do not; with the slave BFM emitting each burst's chunks in reverse, every burst still completes and reassembles by RCHUNKNUM -- the fabric is order-agnostic |
| `bridge_1x2_rd_axi5c` | 128-bit AXI5 chunking master, one AXI5 chunking slave and one AXI4 slave | generated suite plus `test_bridge_1x2_rd_axi5c_chunk` (BRIDGE-018): the chunk enable reaches the AXI5 port and its beats come back chunked; to the AXI4 slave the enable terminates and the same read returns ordered data with RCHUNKV low |
| `bridge_2x2_rw_cdc` | as `bridge_2x2_rw`, `ddr` on its own clock (`cdc = true`), `sram` same-clock | generated suite on the CDC port at `BRIDGE_CDC_PERIOD_NS` (7 ns); `test_bridge_2x2_rw_cdc_ratio` (BRIDGE-017) sweeps 3/10/23 ns: both masters' streams land and read back, the port's beats are counted on the slave clock, the rate follows the slower clock (>= 0.85 beat/cycle), the same-clock neighbour is unaffected (HAS 4.5a) |
...
The generated TB picks the BFM family from each port's protocol: AXI4 ports get the AXI4 BFMs, `axi5` ports the AXI5 BFMs (which declare every AMBA5 sideband field as optional, so one BFM binds to any feature subset), `axil5` ports the AXIL5 BFMs (`AXIL5Master*` on a master port), `apb5` slave ports `APB5Slave` and `apb`/`apb5` master ports `APBMaster`/`APB5Master`, all at the generator's 1-bit USER widths, and `wb4` ports `WB4Slave` (base-relative memory, out-of-range answers ERR) / `WB4Master`. Every AXI5 master port also gets an `AXI5ComplianceChecker` on the same prefix, armed in `setup_clocks_and_reset` and read by `tb.assert_compliance()`, which every generated test calls before it declares PASSED -- a protocol violation on the AXI5 boundary fails the test that caused it even when the data still round-tripped.
