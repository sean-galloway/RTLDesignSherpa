// Crossbar: bridge_1x2_rd_axi5w_xbar
...
module bridge_1x2_rd_axi5w_xbar
...
#(
    ...
    wire cpu_rd_64b_ar_to_ddr_rd = (cpu_rd_64b_ar.addr <= 32'h7fffffff);
    ...
    assign sram_rd_axi_arsize   = cpu_rd_32b_ar_to_sram_rd ? cpu_rd_32b_ar.size : '0;
    assign sram_rd_axi_arburst  = cpu_rd_32b_ar_to_sram_rd ? cpu_rd_32b_ar.burst : '0;
    ...
    assign sram_rd_axi_arnsaid  = cpu_rd_32b_ar_to_sram_rd ? cpu_rd_32b_ar.nsaid : '0;  // AXI5 sideband
    ...
    // Rready (master → slave) — gated on rid_valid so the path stays
    // open through the entire R handshake, not just the AR phase.
    assign sram_rd_axi_rready = ((sram_rd_axi_rid_bridge_id == 0) && sram_rd_axi_rid_valid) ? cpu_rd_32b_rready : '0;

    // Bridge ID (master → slave)
    assign sram_rd_axi_bridge_id_ar = cpu_rd_32b_ar_to_sram_rd ? cpu_rd_bridge_id_ar : '0;
    ...
    // AR channel (gated by address re-decode -- see _addr_decode_expr)
    wire cpu_rd_64b_ar_to_subtractive = !(((cpu_rd_64b_ar.addr <= 32'h7fffffff)) || ((cpu_rd_64b_ar.addr >= 32'h80000000)));
    wire cpu_rd_64b_ar_gnt_subtractive = cpu_rd_64b_ar_to_subtractive;
    assign subtractive_axi_arid     = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.id : '0;
    assign subtractive_axi_araddr   = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.addr : '0;
    assign subtractive_axi_arlen    = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.len : '0;
    assign subtractive_axi_arsize   = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.size : '0;
    assign subtractive_axi_arburst  = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.burst : '0;
    assign subtractive_axi_arlock   = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.lock : '0;
    assign subtractive_axi_arcache  = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.cache : '0;
    assign subtractive_axi_arprot   = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.prot : '0;
    assign subtractive_axi_arqos    = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.qos : '0;
    assign subtractive_axi_arregion = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.region : '0;
    assign subtractive_axi_aruser   = cpu_rd_64b_ar_to_subtractive ? cpu_rd_64b_ar.user : '0;
    assign subtractive_axi_arvalid  = cpu_rd_64b_ar_to_subtractive && cpu_rd_64b_arvalid;

    // Rready (master → slave) — gated on rid_valid so the path stays
    // open through the entire R handshake, not just the AR phase.
    assign subtractive_axi_rready = ((subtractive_axi_rid_bridge_id == 0) && subtractive_axi_rid_valid) ? cpu_rd_64b_rready : '0;

    // Bridge ID (master → slave)
    assign subtractive_axi_bridge_id_ar = cpu_rd_64b_ar_to_subtractive ? cpu_rd_bridge_id_ar : '0;
...
`ifndef SYNTHESIS
    ...
    always_ff @(posedge aclk) begin
        if (aresetn && $countones({((sram_rd_axi_rid_bridge_id == 0) && sram_rd_axi_rid_valid)}) > 1) begin
            $error("%m: response mux for master cpu_rd (32b) has %0d slaves selected at once; ",
                   ...
                   $countones({((sram_rd_axi_rid_bridge_id == 0) && sram_rd_axi_rid_valid)}));
        end
    end
...
`endif
    ...
    assign cpu_rd_64b_arready = 
        ...
        (cpu_rd_64b_ar_gnt_subtractive ? subtractive_axi_arready : '0);
...
`ifndef SYNTHESIS
    // synthesis translate_off
    always_ff @(posedge aclk) begin
        if (aresetn && $countones({((ddr_rd_axi_rid_bridge_id == 0) && ddr_rd_axi_rid_valid), ((subtractive_axi_rid_bridge_id == 0) && subtractive_axi_rid_valid)}) > 1) begin
            $error("%m: response mux for master cpu_rd (64b) has %0d slaves selected at once; ",
                   "the single-outstanding-target invariant is broken and the R payload is OR-merged",
                   $countones({((ddr_rd_axi_rid_bridge_id == 0) && ddr_rd_axi_rid_valid), ((subtractive_axi_rid_bridge_id == 0) && subtractive_axi_rid_valid)}));
        end
    end
    // synthesis translate_on
`endif

    assign cpu_rd_64b_r.trace = 
        '0;


endmodule : bridge_1x2_rd_axi5w_xbar
