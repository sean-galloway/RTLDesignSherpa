// Slave Adapter: sram_rd_adapter
// Generated by: SlaveAdapterGenerator
//
// Provides timing isolation and protocol conversion for slave: sram_rd
// Protocol: AXI5

`timescale 1ns / 1ps

`include "reset_defs.svh"


module sram_rd_adapter
    import bridge_1x2_rd_axi5c_pkg::*;
#(
    parameter int ID_WIDTH = 4
) (
    input  logic aclk,
    input  logic aresetn,

    // Crossbar interface (AXI4 from crossbar)
    input  logic [3:0]  xbar_sram_rd_axi_arid,
    input  logic [31:0]  xbar_sram_rd_axi_araddr,
    input  logic [7:0]  xbar_sram_rd_axi_arlen,
    input  logic [2:0]  xbar_sram_rd_axi_arsize,
    input  logic [1:0]  xbar_sram_rd_axi_arburst,
    input  logic         xbar_sram_rd_axi_arlock,
    input  logic [3:0]  xbar_sram_rd_axi_arcache,
    input  logic [2:0]  xbar_sram_rd_axi_arprot,
    input  logic [3:0]  xbar_sram_rd_axi_arqos,
    input  logic [3:0]  xbar_sram_rd_axi_arregion,
    input  logic         xbar_sram_rd_axi_aruser,
    input  logic         xbar_sram_rd_axi_arvalid,
    output  logic         xbar_sram_rd_axi_arready,
    output  logic [3:0]  xbar_sram_rd_axi_rid,
    output  logic [127:0]  xbar_sram_rd_axi_rdata,
    output  logic [1:0]  xbar_sram_rd_axi_rresp,
    output  logic         xbar_sram_rd_axi_rlast,
    output  logic         xbar_sram_rd_axi_ruser,
    output  logic         xbar_sram_rd_axi_rvalid,
    input  logic         xbar_sram_rd_axi_rready,
    // AXI5 native sideband (A5-2 slice 2)
    input  logic         xbar_sram_rd_axi_artrace,
    input  logic         xbar_sram_rd_axi_archunken,
    output logic         xbar_sram_rd_axi_rtrace,
    output logic         xbar_sram_rd_axi_rchunkv,
    output logic [3:0]  xbar_sram_rd_axi_rchunknum,
    output logic         xbar_sram_rd_axi_rchunkstrb,

    // Bridge ID tracking signals
    input  logic [BRIDGE_ID_WIDTH-1:0] xbar_bridge_id_ar,
    output logic [BRIDGE_ID_WIDTH-1:0] rid_bridge_id,
    output logic                       rid_valid,

    // External slave interface (AXI5)
    output  logic [3:0]  sram_rd_axi_arid,
    output  logic [31:0]  sram_rd_axi_araddr,
    output  logic [7:0]  sram_rd_axi_arlen,
    output  logic [2:0]  sram_rd_axi_arsize,
    output  logic [1:0]  sram_rd_axi_arburst,
    output  logic         sram_rd_axi_arlock,
    output  logic [3:0]  sram_rd_axi_arcache,
    output  logic [2:0]  sram_rd_axi_arprot,
    output  logic [3:0]  sram_rd_axi_arqos,
    output  logic         sram_rd_axi_aruser,
    output  logic         sram_rd_axi_arvalid,
    input  logic         sram_rd_axi_arready,
    output  logic         sram_rd_axi_artrace,
    output  logic         sram_rd_axi_archunken,
    input  logic [3:0]  sram_rd_axi_rid,
    input  logic [127:0]  sram_rd_axi_rdata,
    input  logic [1:0]  sram_rd_axi_rresp,
    input  logic         sram_rd_axi_rlast,
    input  logic         sram_rd_axi_ruser,
    input  logic         sram_rd_axi_rvalid,
    output  logic         sram_rd_axi_rready,
    input  logic         sram_rd_axi_rtrace,
    input  logic         sram_rd_axi_rchunkv,
    input  logic [3:0]  sram_rd_axi_rchunknum,
    input  logic         sram_rd_axi_rchunkstrb
);

    // ================================================================
    // Internal Signals
    // ================================================================

    // FIFO tracking signals (in-order mode)
    logic cam_rd_allocate;
    logic cam_rd_deallocate;
    logic [ID_WIDTH-1:0] cam_rd_allocate_tag;
    logic [BRIDGE_ID_WIDTH-1:0] cam_rd_allocate_data;
    logic [ID_WIDTH-1:0] cam_rd_deallocate_tag;

    // ================================================================
    // Bridge ID Tracking - FIFO Mode (In-Order)
    // ================================================================

    // Read Channel FIFO (In-Order) - AXI4 Protocol
    // BRIDGE-011 not-full gating -- see the write channel.
    logic rd_trk_full;
    logic w_sub_arready;
    localparam RD_FIFO_DEPTH = 16;
    logic [BRIDGE_ID_WIDTH-1:0] rd_fifo [RD_FIFO_DEPTH];
    logic [$clog2(RD_FIFO_DEPTH):0] ar_ptr, r_ptr;

    // Push on AR (crossbar → adapter)
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
            ar_ptr <= '0;
        end else if (xbar_sram_rd_axi_arvalid && xbar_sram_rd_axi_arready) begin
            rd_fifo[ar_ptr[$clog2(RD_FIFO_DEPTH)-1:0]] <= xbar_bridge_id_ar;
            ar_ptr <= ar_ptr + 1'b1;
        end
    )

    // Pop on R response (xbar_sram_rd_axi_rvalid && xbar_sram_rd_axi_rready && xbar_sram_rd_axi_rlast)
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
            r_ptr <= '0;
        end else if (xbar_sram_rd_axi_rvalid && xbar_sram_rd_axi_rready && xbar_sram_rd_axi_rlast) begin
            r_ptr <= r_ptr + 1'b1;
        end
    )

    // rid_bridge_id / rid_valid drive the crossbar's response mux,
    // which gates R going BACK to the master on rid_valid. Earlier
    // versions registered these on the handshake completing — but
    // the handshake CAN'T complete until the master sees rvalid,
    // and the master can't see rvalid until rid_valid is high.
    // Result: deadlock. Drive these combinationally so the route
    // is open from the moment an R arrives.
    assign rid_bridge_id = rd_fifo[r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]];
    assign rid_valid     = (ar_ptr != r_ptr);

    // BRIDGE-011, read side -- see the write comment above.
    assign rd_trk_full = (ar_ptr[$clog2(RD_FIFO_DEPTH)] != r_ptr[$clog2(RD_FIFO_DEPTH)]) &&
                         (ar_ptr[$clog2(RD_FIFO_DEPTH)-1:0] == r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]);
    assign xbar_sram_rd_axi_arready = w_sub_arready && !rd_trk_full;

    // BRIDGE-010, read side -- see the write channel. Checked on the
    // LAST beat, since that is when the FIFO entry is retired.
`ifndef SYNTHESIS
    // synthesis translate_off
    logic [4-1:0] rd_id_fifo [RD_FIFO_DEPTH];
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
        end else begin
            if (xbar_sram_rd_axi_arvalid && xbar_sram_rd_axi_arready)
                rd_id_fifo[ar_ptr[$clog2(RD_FIFO_DEPTH)-1:0]] <= xbar_sram_rd_axi_arid;
            if (xbar_sram_rd_axi_rvalid && xbar_sram_rd_axi_rready && xbar_sram_rd_axi_rlast) begin
                if (xbar_sram_rd_axi_rid !== rd_id_fifo[r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]]) begin
                    $error({"BRIDGE-010: slave returned R out of AR order -- ",
                            "got RID=%0h, expected %0h. This bridge routes ",
                            "responses by FIFO position and does not support ",
                            "ID-based reordering; the data has gone to the ",
                            "wrong master."}, xbar_sram_rd_axi_rid,
                           rd_id_fifo[r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]]);
                end
            end
        end
    )
    // synthesis translate_on
`endif

    // AXI5 Master Read Timing Wrapper
    axi5_master_rd #(
        .SKID_DEPTH_AR(2),
        .SKID_DEPTH_R(2),
        .AXI_ID_WIDTH(4),
        .AXI_ADDR_WIDTH(32),
        .AXI_DATA_WIDTH(128),
        .AXI_USER_WIDTH(1),
        .ENABLE_NSAID(1'b0),
        .ENABLE_TRACE(1'b1),
        .ENABLE_MPAM(1'b0),
        .ENABLE_MECID(1'b0),
        .ENABLE_UNIQUE(1'b0),
        .ENABLE_CHUNKING(1'b1),
        .ENABLE_MTE(1'b0),
        .ENABLE_POISON(1'b0)
    ) u_master_rd (
        .aclk(aclk),
        .aresetn(aresetn),

        // Bridge-internal side (fub_axi)
        .fub_axi_arid(xbar_sram_rd_axi_arid),
        .fub_axi_araddr(xbar_sram_rd_axi_araddr),
        .fub_axi_arlen(xbar_sram_rd_axi_arlen),
        .fub_axi_arsize(xbar_sram_rd_axi_arsize),
        .fub_axi_arburst(xbar_sram_rd_axi_arburst),
        .fub_axi_arlock(xbar_sram_rd_axi_arlock),
        .fub_axi_arcache(xbar_sram_rd_axi_arcache),
        .fub_axi_arprot(xbar_sram_rd_axi_arprot),
        .fub_axi_arqos(xbar_sram_rd_axi_arqos),
        .fub_axi_aruser(xbar_sram_rd_axi_aruser),
        .fub_axi_arvalid(xbar_sram_rd_axi_arvalid && !rd_trk_full),
        .fub_axi_arready(w_sub_arready),
        .fub_axi_arnsaid('0),
        .fub_axi_artrace(xbar_sram_rd_axi_artrace),
        .fub_axi_armpam('0),
        .fub_axi_armecid('0),
        .fub_axi_arunique('0),
        .fub_axi_archunken(xbar_sram_rd_axi_archunken),
        .fub_axi_artagop('0),
        .fub_axi_rid(xbar_sram_rd_axi_rid),
        .fub_axi_rdata(xbar_sram_rd_axi_rdata),
        .fub_axi_rresp(xbar_sram_rd_axi_rresp),
        .fub_axi_rlast(xbar_sram_rd_axi_rlast),
        .fub_axi_ruser(xbar_sram_rd_axi_ruser),
        .fub_axi_rvalid(xbar_sram_rd_axi_rvalid),
        .fub_axi_rready(xbar_sram_rd_axi_rready),
        .fub_axi_rtrace(xbar_sram_rd_axi_rtrace),
        .fub_axi_rpoison(),
        .fub_axi_rchunkv(xbar_sram_rd_axi_rchunkv),
        .fub_axi_rchunknum(xbar_sram_rd_axi_rchunknum),
        .fub_axi_rchunkstrb(xbar_sram_rd_axi_rchunkstrb),
        .fub_axi_rtag(),
        .fub_axi_rtagmatch(),

        // External side (m_axi)
        .m_axi_arid(sram_rd_axi_arid),
        .m_axi_araddr(sram_rd_axi_araddr),
        .m_axi_arlen(sram_rd_axi_arlen),
        .m_axi_arsize(sram_rd_axi_arsize),
        .m_axi_arburst(sram_rd_axi_arburst),
        .m_axi_arlock(sram_rd_axi_arlock),
        .m_axi_arcache(sram_rd_axi_arcache),
        .m_axi_arprot(sram_rd_axi_arprot),
        .m_axi_arqos(sram_rd_axi_arqos),
        .m_axi_aruser(sram_rd_axi_aruser),
        .m_axi_arvalid(sram_rd_axi_arvalid),
        .m_axi_arready(sram_rd_axi_arready),
        .m_axi_arnsaid(),
        .m_axi_artrace(sram_rd_axi_artrace),
        .m_axi_armpam(),
        .m_axi_armecid(),
        .m_axi_arunique(),
        .m_axi_archunken(sram_rd_axi_archunken),
        .m_axi_artagop(),
        .m_axi_rid(sram_rd_axi_rid),
        .m_axi_rdata(sram_rd_axi_rdata),
        .m_axi_rresp(sram_rd_axi_rresp),
        .m_axi_rlast(sram_rd_axi_rlast),
        .m_axi_ruser(sram_rd_axi_ruser),
        .m_axi_rvalid(sram_rd_axi_rvalid),
        .m_axi_rready(sram_rd_axi_rready),
        .m_axi_rtrace(sram_rd_axi_rtrace),
        .m_axi_rpoison('0),
        .m_axi_rchunkv(sram_rd_axi_rchunkv),
        .m_axi_rchunknum(sram_rd_axi_rchunknum),
        .m_axi_rchunkstrb(sram_rd_axi_rchunkstrb),
        .m_axi_rtag('0),
        .m_axi_rtagmatch('0),

        // Status (empty connector = unconnected tie-off)
        .busy()
    );

endmodule : sram_rd_adapter
