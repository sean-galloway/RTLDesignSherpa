...
#(
    ...
    assign xbar_periph_wr_axi_awready = w_sub_awready && !wr_trk_full;
...
`ifndef SYNTHESIS
    ...
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
            ...
            if (xbar_periph_wr_axi_awvalid && xbar_periph_wr_axi_awready)
                wr_id_fifo[wr_ptr[$clog2(WR_FIFO_DEPTH)-1:0]] <= xbar_periph_wr_axi_awid;
            if (xbar_periph_wr_axi_bvalid && xbar_periph_wr_axi_bready) begin
                if (xbar_periph_wr_axi_bid !== wr_id_fifo[rd_ptr[$clog2(WR_FIFO_DEPTH)-1:0]]) begin
                    $error({"BRIDGE-010: slave returned B out of AW order -- ",
                            ...
                            "wrong master."}, xbar_periph_wr_axi_bid,
...
`endif
    ...
    axi4_master_wr #(
        ...
        .fub_axi_awvalid(xbar_periph_wr_axi_awvalid && !wr_trk_full),
