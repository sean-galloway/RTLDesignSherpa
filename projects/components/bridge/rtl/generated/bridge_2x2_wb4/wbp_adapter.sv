// Slave Adapter: wbp_adapter
// Generated by: SlaveAdapterGenerator
//
// Provides timing isolation and protocol conversion for slave: wbp
// Protocol: WB4

`timescale 1ns / 1ps

`include "reset_defs.svh"


module wbp_adapter
    import bridge_2x2_wb4_pkg::*;
#(
    parameter int ID_WIDTH = 5
) (
    input  logic aclk,
    input  logic aresetn,

    // Crossbar interface (AXI4 from crossbar)
    input  logic [4:0]  xbar_wbp_axi_awid,
    input  logic [31:0]  xbar_wbp_axi_awaddr,
    input  logic [7:0]  xbar_wbp_axi_awlen,
    input  logic [2:0]  xbar_wbp_axi_awsize,
    input  logic [1:0]  xbar_wbp_axi_awburst,
    input  logic         xbar_wbp_axi_awlock,
    input  logic [3:0]  xbar_wbp_axi_awcache,
    input  logic [2:0]  xbar_wbp_axi_awprot,
    input  logic [3:0]  xbar_wbp_axi_awqos,
    input  logic [3:0]  xbar_wbp_axi_awregion,
    input  logic         xbar_wbp_axi_awuser,
    input  logic         xbar_wbp_axi_awvalid,
    output  logic         xbar_wbp_axi_awready,
    input  logic [31:0]  xbar_wbp_axi_wdata,
    input  logic [3:0]  xbar_wbp_axi_wstrb,
    input  logic         xbar_wbp_axi_wlast,
    input  logic         xbar_wbp_axi_wuser,
    input  logic         xbar_wbp_axi_wvalid,
    output  logic         xbar_wbp_axi_wready,
    output  logic [4:0]  xbar_wbp_axi_bid,
    output  logic [1:0]  xbar_wbp_axi_bresp,
    output  logic         xbar_wbp_axi_buser,
    output  logic         xbar_wbp_axi_bvalid,
    input  logic         xbar_wbp_axi_bready,
    input  logic [4:0]  xbar_wbp_axi_arid,
    input  logic [31:0]  xbar_wbp_axi_araddr,
    input  logic [7:0]  xbar_wbp_axi_arlen,
    input  logic [2:0]  xbar_wbp_axi_arsize,
    input  logic [1:0]  xbar_wbp_axi_arburst,
    input  logic         xbar_wbp_axi_arlock,
    input  logic [3:0]  xbar_wbp_axi_arcache,
    input  logic [2:0]  xbar_wbp_axi_arprot,
    input  logic [3:0]  xbar_wbp_axi_arqos,
    input  logic [3:0]  xbar_wbp_axi_arregion,
    input  logic         xbar_wbp_axi_aruser,
    input  logic         xbar_wbp_axi_arvalid,
    output  logic         xbar_wbp_axi_arready,
    output  logic [4:0]  xbar_wbp_axi_rid,
    output  logic [31:0]  xbar_wbp_axi_rdata,
    output  logic [1:0]  xbar_wbp_axi_rresp,
    output  logic         xbar_wbp_axi_rlast,
    output  logic         xbar_wbp_axi_ruser,
    output  logic         xbar_wbp_axi_rvalid,
    input  logic         xbar_wbp_axi_rready,

    // Bridge ID tracking signals
    input  logic [BRIDGE_ID_WIDTH-1:0] xbar_bridge_id_aw,
    output logic [BRIDGE_ID_WIDTH-1:0] bid_bridge_id,
    output logic                       bid_valid,

    input  logic [BRIDGE_ID_WIDTH-1:0] xbar_bridge_id_ar,
    output logic [BRIDGE_ID_WIDTH-1:0] rid_bridge_id,
    output logic                       rid_valid,

    // External slave interface (WB4)
    output logic wbp_wb_CYC,
    output logic wbp_wb_STB,
    output logic wbp_wb_WE,
    output logic [31:0] wbp_wb_ADR,
    output logic [31:0] wbp_wb_DAT_W,
    output logic [3:0] wbp_wb_SEL,
    output logic [2:0] wbp_wb_CTI,
    output logic [1:0] wbp_wb_BTE,
    input  logic wbp_wb_STALL,
    input  logic wbp_wb_ACK,
    input  logic wbp_wb_ERR,
    input  logic wbp_wb_RTY,
    input  logic [31:0] wbp_wb_DAT_R
);

    // ================================================================
    // Internal Signals
    // ================================================================

    // Protocol converter intermediate signals
    // (FIFO tracking monitors these instead of crossbar signals)
    logic converter_bvalid;
    logic converter_bready;

    logic converter_rvalid;
    logic converter_rready;
    logic converter_rlast;

    // FIFO tracking signals (in-order mode)
    logic cam_wr_allocate;
    logic cam_wr_deallocate;
    logic [ID_WIDTH-1:0] cam_wr_allocate_tag;
    logic [BRIDGE_ID_WIDTH-1:0] cam_wr_allocate_data;
    logic [ID_WIDTH-1:0] cam_wr_deallocate_tag;

    logic cam_rd_allocate;
    logic cam_rd_deallocate;
    logic [ID_WIDTH-1:0] cam_rd_allocate_tag;
    logic [BRIDGE_ID_WIDTH-1:0] cam_rd_allocate_data;
    logic [ID_WIDTH-1:0] cam_rd_deallocate_tag;

    // ================================================================
    // Bridge ID Tracking - FIFO Mode (In-Order)
    // ================================================================

    // Write Channel FIFO (In-Order) - WB4 Protocol
    // NOTE: Monitors converter output (converter_bvalid), not crossbar input
    //       This ensures FIFO pops when converter actually produces response
    // BRIDGE-011 not-full gating: w_sub_awready is the sub-block's
    // own ready, masked before it reaches the crossbar.
    logic wr_trk_full;
    logic w_sub_awready;
    localparam WR_FIFO_DEPTH = 16;
    logic [BRIDGE_ID_WIDTH-1:0] wr_fifo [WR_FIFO_DEPTH];
    logic [$clog2(WR_FIFO_DEPTH):0] wr_ptr, rd_ptr;

    // Push on AW (crossbar → adapter)
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
            wr_ptr <= '0;
        end else if (xbar_wbp_axi_awvalid && xbar_wbp_axi_awready) begin
            wr_fifo[wr_ptr[$clog2(WR_FIFO_DEPTH)-1:0]] <= xbar_bridge_id_aw;
            wr_ptr <= wr_ptr + 1'b1;
        end
    )

    // Pop on B response (converter_bvalid && converter_bready)
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
            rd_ptr <= '0;
        end else if (converter_bvalid && converter_bready) begin
            rd_ptr <= rd_ptr + 1'b1;
        end
    )

    // bid_bridge_id / bid_valid drive the crossbar's response mux,
    // which gates B going BACK to the master on bid_valid. Earlier
    // versions registered these on the handshake completing — but
    // the handshake CAN'T complete until the master sees bvalid,
    // and the master can't see bvalid until bid_valid is high.
    // Result: deadlock. Drive these combinationally so the route
    // is open from the moment a B arrives.
    assign bid_bridge_id = wr_fifo[rd_ptr[$clog2(WR_FIFO_DEPTH)-1:0]];
    assign bid_valid     = (wr_ptr != rd_ptr);

    // BRIDGE-011: this FIFO routes B by POSITION, so overrunning it
    // misroutes responses -- past WR_FIFO_DEPTH a live entry is
    // overwritten and its B goes to the wrong master; at twice the
    // depth the pointers lap, (wr_ptr != rd_ptr) reads EMPTY and the
    // response is never routed at all. Gate the AW handshake on
    // not-full in BOTH directions. Draining never depends on
    // accepting a further AW, so this cannot deadlock.
    assign wr_trk_full = (wr_ptr[$clog2(WR_FIFO_DEPTH)] != rd_ptr[$clog2(WR_FIFO_DEPTH)]) &&
                         (wr_ptr[$clog2(WR_FIFO_DEPTH)-1:0] == rd_ptr[$clog2(WR_FIFO_DEPTH)-1:0]);
    assign xbar_wbp_axi_awready = w_sub_awready && !wr_trk_full;

    // BRIDGE-010: this port routes B by FIFO POSITION, so it REQUIRES
    // the slave to return B in AW order across all IDs. AXI4 permits a
    // slave to reorder between IDs; such a slave silently misroutes
    // here. Nothing detected that, so record the AWID alongside the
    // master id and check the returned BID against the head. Sim-only:
    // it is a contract check on the attached slave, not logic the
    // bridge needs, and it must cost no gates.
`ifndef SYNTHESIS
    // synthesis translate_off
    logic [5-1:0] wr_id_fifo [WR_FIFO_DEPTH];
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
        end else begin
            if (xbar_wbp_axi_awvalid && xbar_wbp_axi_awready)
                wr_id_fifo[wr_ptr[$clog2(WR_FIFO_DEPTH)-1:0]] <= xbar_wbp_axi_awid;
            if (xbar_wbp_axi_bvalid && xbar_wbp_axi_bready) begin
                if (xbar_wbp_axi_bid !== wr_id_fifo[rd_ptr[$clog2(WR_FIFO_DEPTH)-1:0]]) begin
                    $error({"BRIDGE-010: slave returned B out of AW order -- ",
                            "got BID=%0h, expected %0h. This bridge routes ",
                            "responses by FIFO position and does not support ",
                            "ID-based reordering; the response has gone to the ",
                            "wrong master."}, xbar_wbp_axi_bid,
                           wr_id_fifo[rd_ptr[$clog2(WR_FIFO_DEPTH)-1:0]]);
                end
            end
        end
    )
    // synthesis translate_on
`endif

    // Read Channel FIFO (In-Order) - WB4 Protocol
    // NOTE: Monitors converter output (converter_rvalid), not crossbar input
    //       This ensures FIFO pops when converter actually produces response
    // BRIDGE-011 not-full gating -- see the write channel.
    logic rd_trk_full;
    logic w_sub_arready;
    localparam RD_FIFO_DEPTH = 16;
    logic [BRIDGE_ID_WIDTH-1:0] rd_fifo [RD_FIFO_DEPTH];
    logic [$clog2(RD_FIFO_DEPTH):0] ar_ptr, r_ptr;

    // Push on AR (crossbar → adapter)
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
            ar_ptr <= '0;
        end else if (xbar_wbp_axi_arvalid && xbar_wbp_axi_arready) begin
            rd_fifo[ar_ptr[$clog2(RD_FIFO_DEPTH)-1:0]] <= xbar_bridge_id_ar;
            ar_ptr <= ar_ptr + 1'b1;
        end
    )

    // Pop on R response (converter_rvalid && converter_rready && converter_rlast)
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
            r_ptr <= '0;
        end else if (converter_rvalid && converter_rready && converter_rlast) begin
            r_ptr <= r_ptr + 1'b1;
        end
    )

    // rid_bridge_id / rid_valid drive the crossbar's response mux,
    // which gates R going BACK to the master on rid_valid. Earlier
    // versions registered these on the handshake completing — but
    // the handshake CAN'T complete until the master sees rvalid,
    // and the master can't see rvalid until rid_valid is high.
    // Result: deadlock. Drive these combinationally so the route
    // is open from the moment an R arrives.
    assign rid_bridge_id = rd_fifo[r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]];
    assign rid_valid     = (ar_ptr != r_ptr);

    // BRIDGE-011, read side -- see the write comment above.
    assign rd_trk_full = (ar_ptr[$clog2(RD_FIFO_DEPTH)] != r_ptr[$clog2(RD_FIFO_DEPTH)]) &&
                         (ar_ptr[$clog2(RD_FIFO_DEPTH)-1:0] == r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]);
    assign xbar_wbp_axi_arready = w_sub_arready && !rd_trk_full;

    // BRIDGE-010, read side -- see the write channel. Checked on the
    // LAST beat, since that is when the FIFO entry is retired.
`ifndef SYNTHESIS
    // synthesis translate_off
    logic [5-1:0] rd_id_fifo [RD_FIFO_DEPTH];
    `ALWAYS_FF_RST(aclk, aresetn,
        if (`RST_ASSERTED(aresetn)) begin
        end else begin
            if (xbar_wbp_axi_arvalid && xbar_wbp_axi_arready)
                rd_id_fifo[ar_ptr[$clog2(RD_FIFO_DEPTH)-1:0]] <= xbar_wbp_axi_arid;
            if (xbar_wbp_axi_rvalid && xbar_wbp_axi_rready && xbar_wbp_axi_rlast) begin
                if (xbar_wbp_axi_rid !== rd_id_fifo[r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]]) begin
                    $error({"BRIDGE-010: slave returned R out of AR order -- ",
                            "got RID=%0h, expected %0h. This bridge routes ",
                            "responses by FIFO position and does not support ",
                            "ID-based reordering; the data has gone to the ",
                            "wrong master."}, xbar_wbp_axi_rid,
                           rd_id_fifo[r_ptr[$clog2(RD_FIFO_DEPTH)-1:0]]);
                end
            end
        end
    )
    // synthesis translate_on
`endif

    // AXI4-to-Wishbone B4 converter shim
    axi4_to_wb4 #(
        .AXI_ID_WIDTH(5),
        .AXI_ADDR_WIDTH(32),
        .AXI_DATA_WIDTH(32),
        .AXI_USER_WIDTH(1),
        .SKID_DEPTH_AW(2),
        .SKID_DEPTH_W(2),
        .SKID_DEPTH_B(2),
        .SKID_DEPTH_AR(2),
        .SKID_DEPTH_R(2),
        .CMD_DEPTH(4),
        .RSP_DEPTH(4),
        .SIDE_DEPTH(8),
        .CLASSIC(0)
    ) u_wbp_wb4_converter (
        // Clock and reset
        .aclk(aclk),
        .aresetn(aresetn),

        // AXI4 write channels (from crossbar)
        .s_axi_awid(xbar_wbp_axi_awid),
        .s_axi_awaddr(xbar_wbp_axi_awaddr),
        .s_axi_awlen(xbar_wbp_axi_awlen),
        .s_axi_awsize(xbar_wbp_axi_awsize),
        .s_axi_awburst(xbar_wbp_axi_awburst),
        .s_axi_awlock(xbar_wbp_axi_awlock),
        .s_axi_awcache(xbar_wbp_axi_awcache),
        .s_axi_awprot(xbar_wbp_axi_awprot),
        .s_axi_awqos(xbar_wbp_axi_awqos),
        .s_axi_awregion(xbar_wbp_axi_awregion),
        .s_axi_awuser(xbar_wbp_axi_awuser),
        .s_axi_awvalid(xbar_wbp_axi_awvalid && !wr_trk_full),
        .s_axi_awready(w_sub_awready),
        .s_axi_wdata(xbar_wbp_axi_wdata),
        .s_axi_wstrb(xbar_wbp_axi_wstrb),
        .s_axi_wlast(xbar_wbp_axi_wlast),
        .s_axi_wuser(xbar_wbp_axi_wuser),
        .s_axi_wvalid(xbar_wbp_axi_wvalid),
        .s_axi_wready(xbar_wbp_axi_wready),
        .s_axi_bid(xbar_wbp_axi_bid),
        .s_axi_bresp(xbar_wbp_axi_bresp),
        .s_axi_buser(xbar_wbp_axi_buser),
        .s_axi_bvalid(converter_bvalid),
        .s_axi_bready(converter_bready),

        // AXI4 read channels (from crossbar)
        .s_axi_arid(xbar_wbp_axi_arid),
        .s_axi_araddr(xbar_wbp_axi_araddr),
        .s_axi_arlen(xbar_wbp_axi_arlen),
        .s_axi_arsize(xbar_wbp_axi_arsize),
        .s_axi_arburst(xbar_wbp_axi_arburst),
        .s_axi_arlock(xbar_wbp_axi_arlock),
        .s_axi_arcache(xbar_wbp_axi_arcache),
        .s_axi_arprot(xbar_wbp_axi_arprot),
        .s_axi_arqos(xbar_wbp_axi_arqos),
        .s_axi_arregion(xbar_wbp_axi_arregion),
        .s_axi_aruser(xbar_wbp_axi_aruser),
        .s_axi_arvalid(xbar_wbp_axi_arvalid && !rd_trk_full),
        .s_axi_arready(w_sub_arready),
        .s_axi_rid(xbar_wbp_axi_rid),
        .s_axi_rdata(xbar_wbp_axi_rdata),
        .s_axi_rresp(xbar_wbp_axi_rresp),
        .s_axi_rlast(converter_rlast),
        .s_axi_ruser(xbar_wbp_axi_ruser),
        .s_axi_rvalid(converter_rvalid),
        .s_axi_rready(converter_rready),

        // Wishbone B4 requester interface (to external completer)
        .m_wb_CYC(wbp_wb_CYC),
        .m_wb_STB(wbp_wb_STB),
        .m_wb_WE(wbp_wb_WE),
        .m_wb_ADR(wbp_wb_ADR),
        .m_wb_DAT_W(wbp_wb_DAT_W),
        .m_wb_SEL(wbp_wb_SEL),
        .m_wb_CTI(wbp_wb_CTI),
        .m_wb_BTE(wbp_wb_BTE),
        .m_wb_STALL(wbp_wb_STALL),
        .m_wb_ACK(wbp_wb_ACK),
        .m_wb_ERR(wbp_wb_ERR),
        .m_wb_RTY(wbp_wb_RTY),
        .m_wb_DAT_R(wbp_wb_DAT_R)
    );

    // Wire converter outputs back to shim's crossbar-facing interface
    assign xbar_wbp_axi_bvalid = converter_bvalid;
    assign converter_bready = xbar_wbp_axi_bready;
    assign xbar_wbp_axi_rvalid = converter_rvalid;
    assign converter_rready = xbar_wbp_axi_rready;
    assign xbar_wbp_axi_rlast = converter_rlast;

endmodule : wbp_adapter
