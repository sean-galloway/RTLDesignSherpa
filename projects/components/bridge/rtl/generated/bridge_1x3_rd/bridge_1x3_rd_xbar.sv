...
#(
    ...
    // AR channel (gated by address re-decode -- see _addr_decode_expr)
    wire cpu_rd_128b_ar_to_subtractive = !(((cpu_rd_128b_ar.addr <= 32'h0fffffff)) || (((cpu_rd_128b_ar.addr >= 32'h10000000) && (cpu_rd_128b_ar.addr <= 32'h4fffffff))) || (((cpu_rd_128b_ar.addr >= 32'h50000000) && (cpu_rd_128b_ar.addr <= 32'h7fffffff))));
...
`ifndef SYNTHESIS
    ...
    always_ff @(posedge aclk) begin
        if (aresetn && $countones({((periph_rd_axi_rid_bridge_id == 0) && periph_rd_axi_rid_valid)}) > 1) begin
            $error("%m: response mux for master cpu_rd (32b) has %0d slaves selected at once; ",
                   ...
                   $countones({((periph_rd_axi_rid_bridge_id == 0) && periph_rd_axi_rid_valid)}));
        end
    end
