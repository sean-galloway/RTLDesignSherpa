...
//
// Documentation: projects/components/bridge_axi4_flat_2x2.sv/PRD.md
// Subsystem: bridge_axi4_flat_2x2.sv
//
...
always_comb begin
    ...
    for (int m = 0; m < NUM_MASTERS; m++) begin
        if (s_axi_awvalid[m]) begin
            ...
            if (s_axi_awaddr[m] >= 32'h0 &&
                s_axi_awaddr[m] < 32'h10000000)
...
always_comb begin
    ...
    for (int m = 0; m < NUM_MASTERS; m++) begin
        if (s_axi_arvalid[m]) begin
            // Slave 0: Slave0 (0x00000000 - 0x0FFFFFFF)
            if (s_axi_araddr[m] >= 32'h0 &&
                s_axi_araddr[m] < 32'h10000000)
            ...
            if (!aresetn) begin
                ...
                if (aw_grant_active[s]) begin
                    ...
                    for (int i = 0; i < NUM_MASTERS; i++) begin
                        int m;
                        m = (aw_last_grant[s] + 1 + i) % NUM_MASTERS;
        ...
        always_ff @(posedge aclk or negedge aresetn) begin
            if (!aresetn) begin
                ...
                if (ar_grant_active[s]) begin
                    ...
                    for (int i = 0; i < NUM_MASTERS; i++) begin
                        int m;
                        m = (ar_last_grant[s] + 1 + i) % NUM_MASTERS;
