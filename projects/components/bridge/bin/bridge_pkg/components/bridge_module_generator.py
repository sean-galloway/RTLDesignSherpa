...
class BridgeModuleGenerator:
    ...
    def _generate_cfg_net_decls(self,
                                wrappers: List[MonitoredWrapper]) -> List[str]:
        """Emit the internal cfg_* wire declarations. Called EARLY in
        the bridge body (before the adapter instantiations) because the
        adapter instances reference these nets by name. The regblock
        instance + hwif_out fan-out is emitted later by
        _generate_cfg_regblock_instance().
        """
        lines: List[str] = []
        lines.append("")
        lines.append("    // ============================================================")
        lines.append("    // Monitor cfg subsystem -- internal cfg net decls (90.3)")
        lines.append("    // ============================================================")
        for w in wrappers:
    ...
    def _generate_cfg_regblock_instance(self,
                                        wrappers: List[MonitoredWrapper]) -> List[str]:
        """Emit the cfg regblock instance + hwif_out -> cfg_* drives.
        Net decls live in _generate_cfg_net_decls() (called earlier).
        Only called when use_cfg_regblock=True (task 90.3).
        """
    ...
    def _generate_monitor_aggregator(self, wrappers: List[MonitoredWrapper]) -> List[str]:
        ...
        if self.internal_axil_group:
            ...
            lines.append("        .mon_compressor_stat_tier1_a        (),")
            lines.append("        .mon_compressor_stat_tier1_b        (),")
            lines.append("        .mon_compressor_stat_tier1_c        (),")
            lines.append("        .mon_compressor_stat_tier0          (),")
            lines.append("        .mon_compressor_stat_cam_miss       (),")
            lines.append("        .mon_compressor_stat_delta_ts_ovf   (),")
            lines.append("        .mon_compressor_stat_event_data_ovf (),")
            lines.append("        .mon_compressor_stat_ed_delta_ovf   ()")
            lines.append("        /* verilator lint_on PINCONNECTEMPTY */")
            lines.append("    );")
            lines.append("")
        else:
    ...
    def _generate_main_bridge(self) -> str:
        ...
        if self.enable_monitoring:
            ...
            if monitored:
                ...
                # Cfg net decls must precede the adapter instantiations
                # which reference them; the regblock instance + assigns
                # come later, after the adapters and the aggregator.
