...
async def cocotb_test_basic_connectivity(dut):
    ...
    tb.log.info("=== Scenario BRIDGE-1x3-02: Write channel routing ===")
...
def test_bridge_1x3_wr_basic_connectivity(request):
    ...
    test_name = f"test_{worker_id}_bridge_1x3_wr_basic_connectivity"
...
def test_bridge_1x3_wr_address_decode(request):
    ...
    # Get worker ID for parallel execution isolation
    worker_id = os.environ.get('PYTEST_XDIST_WORKER', 'gw0')
    test_name = f"test_{worker_id}_bridge_1x3_wr_address_decode"
    log_path = os.path.join(log_dir, f'{test_name}.log')
