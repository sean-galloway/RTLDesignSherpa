...
from projects.components.bridge.dv.tbclasses.bridge_mix_btb import BridgeMixBTB, AxiResponseError
    ...
    for page_idx, page_base in enumerate(pages_0_2):
        for probe_idx, probe_off in enumerate(in_page_0_2):
            ...
            try:
                ...
                # addresses above the seed cap passed the whole suite.
                assert e.is_slverr, (
                    f"M0->S2 probe at 0x{addr:08x} past the "
                    f"seeded model answered {e}; expected SLVERR from the addressed "
                    f"slave. A DECERR here means it decoded to the subtractive "
                    f"catch-all instead.")
                continue
    ...
    for page_idx, page_base in enumerate(pages_2_2):
        for probe_idx, probe_off in enumerate(in_page_2_2):
            ...
            try:
                ...
                # addresses above the seed cap passed the whole suite.
                assert e.is_slverr, (
                    f"M2->S2 probe at 0x{addr:08x} past the "
                    f"seeded model answered {e}; expected SLVERR from the addressed "
                    f"slave. A DECERR here means it decoded to the subtractive "
                    f"catch-all instead.")
                continue
