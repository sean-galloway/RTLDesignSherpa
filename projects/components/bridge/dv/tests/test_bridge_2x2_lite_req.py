...
#
# Test file for bridge_2x2_lite_req
...
from projects.components.bridge.dv.tbclasses.bridge2x2_lite_req_tb import Bridge2x2LiteReqTB, AxiResponseError
...
async def cocotb_test_bridge_2x2_lite_req_basic_connectivity(dut):
    """
    ...
    # ---- Write connectivity --------------------------------------------
    tb.log.info(f"Master 0 (lite4) — writes")
    ...
    for k, test_addr in enumerate(tb.connectivity_addrs(1, master_idx=0)):
        ...
        assert actual == test_data, (
            ...
            f"got 0x{actual:08x}, expected 0x{test_data:08x}")
    tb.log.info(f"Master 1 (lite5) — writes")
    ...
    # ---- Read connectivity ---------------------------------------------
    tb.log.info(f"Master 0 (lite4) — reads")
    # Master 0 → Slave 0 (mem)
    ...
    for test_addr in tb.connectivity_addrs(1, master_idx=0):
        ...
        assert actual == expected, (
            ...
            f"got 0x{actual:08x}, expected 0x{expected:08x} (seeded pattern)")
    tb.log.info(f"Master 1 (lite5) — reads")
    # Master 1 → Slave 0 (mem)
    # Non-base offsets; addr_range is 4 KB-aligned by validator so +0x100
    # is always safely inside the slave's window, and the RNG-drawn extras
    # stay inside the seeded region so every read is data-checked.
...
async def cocotb_test_bridge_2x2_lite_req_boundary_probe(dut):
    """
    ...
    tb.log.info("=" * 80)
@cocotb.test(timeout_time=8000, timeout_unit="ms")
async def cocotb_test_bridge_2x2_lite_req_arbitration(dut):
    """
...
def test_bridge_2x2_lite_req_basic_connectivity(request, test_level):
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/bridge/rtl/filelists/bridge_2x2_lite_req.f'
    )
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_2x2_lite_req_basic_connectivity",
        sim_build=sim_build,
...
@pytest.mark.parametrize("test_level", reg_level_grid())
def test_bridge_2x2_lite_req_boundary_probe(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_2x2_lite_req_boundary_probe",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
...
@pytest.mark.parametrize("test_level", reg_level_grid())
def test_bridge_2x2_lite_req_arbitration(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_2x2_lite_req_arbitration",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
