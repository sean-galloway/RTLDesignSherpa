...
from projects.components.bridge.dv.tbclasses.bridge4x4_rw_tb import Bridge4x4RwTB, AxiResponseError
    ...
    for page_idx, page_base in enumerate(pages_0_3):
        for probe_idx, probe_off in enumerate(in_page_0_3):
            ...
            try:
                ...
                # contract). A DECERR means the subtractive catch-all
                # answered -- the address did not decode to slave
                # 3 at all -- and a bare `except RuntimeError`
                # swallowed that identically, so a decode defect confined to
                # addresses above the seed cap passed the whole suite.
                assert e.is_slverr, (
                    f"M0->S3 probe at 0x{addr:08x} past the "
                    f"seeded model answered {e}; expected SLVERR from the addressed "
                    f"slave. A DECERR here means it decoded to the subtractive "
                    f"catch-all instead.")
                continue
    ...
    for page_idx, page_base in enumerate(pages_2_3):
        for probe_idx, probe_off in enumerate(in_page_2_3):
            ...
            try:
                ...
                # addresses above the seed cap passed the whole suite.
                assert e.is_slverr, (
                    f"M2->S3 probe at 0x{addr:08x} past the "
                    f"seeded model answered {e}; expected SLVERR from the addressed "
                    f"slave. A DECERR here means it decoded to the subtractive "
                    f"catch-all instead.")
                continue
            ...
            # seed pattern shows through instead of d.
            if seeded:
                got = tb.slave_mem_read(3, addr, master_idx=2)
                assert got == d, (
                    f"M2→S3 data mismatch at "
                    f"0x{addr:08x} (page=0x{page_base:08x}, off=0x{probe_off:x}): "
                    f"got 0x{got:08x}, expected 0x{d:08x}")
    ...
    for page_idx, page_base in enumerate(pages_3_3):
        for probe_idx, probe_off in enumerate(in_page_3_3):
            ...
            try:
                ...
                # addresses above the seed cap passed the whole suite.
                assert e.is_slverr, (
                    f"M3->S3 probe at 0x{addr:08x} past the "
                    f"seeded model answered {e}; expected SLVERR from the addressed "
                    f"slave. A DECERR here means it decoded to the subtractive "
                    f"catch-all instead.")
                continue
            ...
            # seed pattern shows through instead of d.
            if seeded:
                got = tb.slave_mem_read(3, addr, master_idx=3)
                assert got == d, (
                    f"M3→S3 data mismatch at "
                    f"0x{addr:08x} (page=0x{page_base:08x}, off=0x{probe_off:x}): "
                    f"got 0x{got:08x}, expected 0x{d:08x}")
...
def test_bridge_4x4_rw_basic_connectivity(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_4x4_rw_basic_connectivity",
        sim_build=sim_build,
...
def test_bridge_4x4_rw_boundary_probe(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_4x4_rw_boundary_probe",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
...
def test_bridge_4x4_rw_arbitration(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_4x4_rw_arbitration",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
