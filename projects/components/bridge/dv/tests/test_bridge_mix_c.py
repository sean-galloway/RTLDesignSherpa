...
from projects.components.bridge.dv.tbclasses.bridge_mix_ctb import BridgeMixCTB, AxiResponseError
...
async def cocotb_test_bridge_mix_c_basic_connectivity(dut):
    """
    ...
    for k, test_addr in enumerate(tb.connectivity_addrs(2, master_idx=0)):
        ...
        assert actual == test_data, (
            ...
            f"got 0x{actual:08x}, expected 0x{test_data:08x}")
    tb.log.info(f"Master 1 (host_axil) — writes")
    # Master 1 → Slave 1 (cfg_regs)
    ...
    for test_addr in tb.connectivity_addrs(2, master_idx=0):
        ...
        assert actual == expected, (
            ...
            f"got 0x{actual:08x}, expected 0x{expected:08x} (seeded pattern)")
    tb.log.info(f"Master 1 (host_axil) — reads")
    # Master 1 → Slave 1 (cfg_regs)
    # Non-base offsets; addr_range is 4 KB-aligned by validator so +0x100
    # is always safely inside the slave's window, and the RNG-drawn extras
    # stay inside the seeded region so every read is data-checked.
...
async def cocotb_test_bridge_mix_c_boundary_probe(dut):
    """
        ...
        for probe_idx, probe_off in enumerate(in_page_0_2):
            ...
            if seeded:
                ...
                assert got == d, (
                    ...
                    f"0x{addr:08x} (page=0x{page_base:08x}, off=0x{probe_off:x}): "
                    f"got 0x{got:08x}, expected 0x{d:08x}")
    tb.log.info(f"Master 1 (host_axil)")
    # Slave 1 (cfg_regs): 0x40000000-0x4000ffff
    ...
    tb.log.info("=" * 80)
@cocotb.test(timeout_time=8000, timeout_unit="ms")
async def cocotb_test_bridge_mix_c_arbitration(dut):
    """
...
def test_bridge_mix_c_basic_connectivity(request, test_level):
    ...
    dut_name = "bridge_mix_c"

    # Get sources from filelist
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/bridge/rtl/filelists/bridge_mix_c.f'
    )
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_mix_c_basic_connectivity",
        sim_build=sim_build,
...
@pytest.mark.parametrize("test_level", reg_level_grid())
def test_bridge_mix_c_boundary_probe(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_mix_c_boundary_probe",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
...
@pytest.mark.parametrize("test_level", reg_level_grid())
def test_bridge_mix_c_arbitration(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_mix_c_arbitration",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
