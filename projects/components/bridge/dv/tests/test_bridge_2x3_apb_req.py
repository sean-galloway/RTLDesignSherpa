...
#
# Test file for bridge_2x3_apb_req
...
from projects.components.bridge.dv.tbclasses.bridge2x3_apb_req_tb import Bridge2x3ApbReqTB, AxiResponseError
...
async def cocotb_test_bridge_2x3_apb_req_basic_connectivity(dut):
    """
    ...
    tb.log.info(f"Configuration: 2M x 3S, RW channels")
    tb.log.info("=" * 80)

    # ---- Write connectivity --------------------------------------------
    tb.log.info(f"Master 0 (apb4m) — writes")
    # Master 0 → Slave 0 (mem)
    ...
    for k, test_addr in enumerate(tb.connectivity_addrs(2, master_idx=0)):
        ...
        assert actual == test_data, (
            ...
            f"got 0x{actual:08x}, expected 0x{test_data:08x}")
    tb.log.info(f"Master 1 (apb5m) — writes")
    # Master 1 → Slave 0 (mem)
    ...
    # ---- Read connectivity ---------------------------------------------
    tb.log.info(f"Master 0 (apb4m) — reads")
    # Master 0 → Slave 0 (mem)
    ...
    for test_addr in tb.connectivity_addrs(2, master_idx=0):
        ...
        assert actual == expected, (
            ...
            f"got 0x{actual:08x}, expected 0x{expected:08x} (seeded pattern)")
    tb.log.info(f"Master 1 (apb5m) — reads")
    # Master 1 → Slave 0 (mem)
    # Non-base offsets; addr_range is 4 KB-aligned by validator so +0x100
    # is always safely inside the slave's window, and the RNG-drawn extras
    # stay inside the seeded region so every read is data-checked.
...
async def cocotb_test_bridge_2x3_apb_req_boundary_probe(dut):
    """
    ...
    for page_idx, page_base in enumerate(pages_1_2):
        for probe_idx, probe_off in enumerate(in_page_1_2):
            ...
            try:
                ...
                # addresses above the seed cap passed the whole suite.
                assert e.is_slverr, (
                    f"M1->S2 probe at 0x{addr:08x} past the "
                    f"seeded model answered {e}; expected SLVERR from the addressed "
                    f"slave. A DECERR here means it decoded to the subtractive "
                    f"catch-all instead.")
                continue
    ...
    tb.log.info("=" * 80)
@cocotb.test(timeout_time=8000, timeout_unit="ms")
async def cocotb_test_bridge_2x3_apb_req_arbitration(dut):
    """
...
def test_bridge_2x3_apb_req_basic_connectivity(request, test_level):
    ...
    verilog_sources, includes = get_sources_from_filelist(
        repo_root=repo_root,
        filelist_path='projects/components/bridge/rtl/filelists/bridge_2x3_apb_req.f'
    )
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_2x3_apb_req_basic_connectivity",
        sim_build=sim_build,
...
@pytest.mark.parametrize("test_level", reg_level_grid())
def test_bridge_2x3_apb_req_boundary_probe(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_2x3_apb_req_boundary_probe",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
...
@pytest.mark.parametrize("test_level", reg_level_grid())
def test_bridge_2x3_apb_req_arbitration(request, test_level):
    ...
    run(
        ...
        toplevel=dut_name,
        module=module,
        testcase="cocotb_test_bridge_2x3_apb_req_arbitration",
        sim_build=sim_build,
        waves=False,
        extra_args=extra_args,
        plus_args=waves['sim_args'],
