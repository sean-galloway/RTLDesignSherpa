...
async def cocotb_test_address_decode(dut):
    ...
    await ClockCycles(tb.clock, 3)
    # Slave 4: axil_periph
    # Range: 0x90000000 - 0x9000ffff
    tb.log.info(f"  Slave 4 (axil_periph): 0x90000000-0x9000ffff")    # Test base address
    await tb.read_transaction(0, 0x90000000, txn_id=0)
    await ClockCycles(tb.clock, 3)
    await tb.slave_respond_read(4, 0x22222226, txn_id=0)
    await ClockCycles(tb.clock, 3)

    # Test end address
    await tb.read_transaction(0, 0x9000fffc, txn_id=0)
    await ClockCycles(tb.clock, 3)
    await tb.slave_respond_read(4, 0x33333337, txn_id=0)
    await ClockCycles(tb.clock, 3)
    await ClockCycles(tb.clock, 20)
    tb.log.info("=" * 80)
...
def test_bridge_1x5_rd_basic_connectivity(request):
    ...
    test_name = f"test_{worker_id}_bridge_1x5_rd_basic_connectivity"
...
def test_bridge_1x5_rd_address_decode(request):
    ...
    # Get worker ID for parallel execution isolation
    worker_id = os.environ.get('PYTEST_XDIST_WORKER', 'gw0')
    test_name = f"test_{worker_id}_bridge_1x5_rd_address_decode"
    log_path = os.path.join(log_dir, f'{test_name}.log')
