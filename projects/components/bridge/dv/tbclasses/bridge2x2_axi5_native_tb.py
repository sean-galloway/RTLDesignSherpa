...
#
# Bridge2x2Axi5NativeTB - Testbench for bridge_2x2_axi5_native
...
# AXI5 ports are driven by the AXI5 BFMs, which declare every AMBA5 sideband
# field as optional (bind-to-nothing reads 0), so one BFM fits any feature
# subset -- and every AXI5 MASTER port gets an AXI5ComplianceChecker on the
# same prefix. Until 2026-09-09 these ports were driven by the AXI4 BFMs with
# the AXI5-only inputs left undriven, and only one hand-written test ever
# attached the checker.
from CocoTBFramework.components.axi5.axi5_interfaces import (
    AXI5MasterRead, AXI5MasterWrite, AXI5SlaveRead, AXI5SlaveWrite,
)
...
class Bridge2x2Axi5NativeTB(TBBase):
    """
    Testbench for bridge_2x2_axi5_native bridge.
    ...
    def __init__(self, dut):
        ...
        self.slave_info = {
            0: ('axi5', 0x00000000, 0x80000000, 128),  # ddr
            1: ('axi5', 0x80000000, 0x80000000, 128),  # sram
        }
        ...
        self._setup_slave_1_sram()
        self._setup_master_0_cpu()
        self._setup_master_1_dma()
    ...
    def _setup_master_0_cpu(self):
        """Set up protocol BFMs for master 0: cpu (protocol: axi5)"""
        ...
        self.master_wr[0] = AXI5MasterWrite(
            self.dut, self.clock,
            prefix="cpu_axi_",
            log=self.log,
            data_width=128,
        ...
        # Protocol checker on the AXI5 boundary: handshake, burst, ID and
        # AMBA5 feature rules (ATOP encoding/response, poison propagation,
        # trace consistency, NSAID/MPAM/MECID). Started in
        # setup_clocks_and_reset; assert_compliance() reads the verdict.
        self.compliance[0] = AXI5ComplianceChecker(
            self.dut, self.clock,
            prefix="cpu_axi_",
            log=self.log,
            data_width=128,
    ...
    def _setup_master_1_dma(self):
        """Set up protocol BFMs for master 1: dma (protocol: axi5)"""
        self.master_rd[1] = AXI5MasterRead(
            self.dut, self.clock,
            prefix="dma_axi_",
            log=self.log,
            data_width=128,
        ...
        self.master_wr[1] = AXI5MasterWrite(
            self.dut, self.clock,
            prefix="dma_axi_",
            log=self.log,
            data_width=128,
        ...
        # trace consistency, NSAID/MPAM/MECID). Started in
        # setup_clocks_and_reset; assert_compliance() reads the verdict.
        self.compliance[1] = AXI5ComplianceChecker(
            self.dut, self.clock,
            prefix="dma_axi_",
            log=self.log,
            data_width=128,
    ...
    def _setup_slave_0_ddr(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 0: ddr (protocol: axi5)"""
        ...
        preset = self._build_preset(0, addr_range, 128)
        ...
        self.slave_wr[0] = AXI5SlaveWrite(
            self.dut, self.clock,
            prefix="ddr_axi_",
            log=self.log,
            data_width=128,
        ...
        self.slave_wr[0].read_return_channel = self.slave_rd[0]
    def _setup_slave_1_sram(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 1: sram (protocol: axi5)"""
        # AXI4 slave — full memory-backed BFM.
        bytes_per_line = 128 // 8
        addr_range = 0x80000000
        # Cap MemoryModel size — see SLAVE_MEM_CAP_BYTES comment.
        mem_bytes = min(addr_range, self.SLAVE_MEM_CAP_BYTES)
        preset = self._build_preset(1, addr_range, 128)
        ...
        self.slave_wr[1] = AXI5SlaveWrite(
            self.dut, self.clock,
            prefix="sram_axi_",
            log=self.log,
            data_width=128,
