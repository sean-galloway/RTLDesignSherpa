...
#
# Bridge2x2Wb4MonTB - Testbench for bridge_2x2_wb4_mon
...
from CocoTBFramework.components.shared.wb4_common import WB4_STATUS_ACK, WB4_STATUS_NAMES
...
class Bridge2x2Wb4MonTB(TBBase):
    """
    Testbench for bridge_2x2_wb4_mon bridge.
    ...
    def __init__(self, dut):
        ...
        self.slave_info = {
            0: ('axi4', 0x00000000, 0x10000000, 64),  # mem
            1: ('wb4', 0x50000000, 0x00010000, 32),  # wbp
        }
        ...
        self._setup_slave_0_mem()
        self._setup_slave_1_wbp()
        self._setup_master_0_wbm()
        self._setup_master_1_cpu()
    ...
    def _setup_master_0_wbm(self):
        """Set up protocol BFMs for master 0: wbm (protocol: wb4)"""
        # Wishbone B4 requester port: the bridge is the completer. One BFM
        # handle for both directions; master_read/master_write unwrap the
        # terminated packet (dat_r, status ERR/RTY -> AxiResponseError).
        # Back-to-back presentation; the bridge's STALL is the only pacing.
        self.master_wb[0] = WB4Master(
            entity=self.dut,
            title="WB4_M0",
            prefix="wbm_wb_",
            clock=self.clock,
            ...
            data_width=32,
            max_outstanding=8,
            randomizer=FlexRandomizer({'stb': ([(0, 0)], [1])}),
            log=self.log,
        )
    def _setup_master_1_cpu(self):
        """Set up protocol BFMs for master 1: cpu (protocol: axi4)"""
    ...
    def _setup_slave_0_mem(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 0: mem (protocol: axi4)"""
    ...
    def _setup_slave_1_wbp(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 1: wbp (protocol: wb4)"""
        # Wishbone B4 completer -- WB4Slave over its own MemoryModel, seeded
        # with the slave pattern and addressed relative to base_addr. The
        # randomizer is pinned to ACK with a one-clock termination and no
        # STALL: the default profile mixes in ERR/RTY, which a bridge test
        # must ask for explicitly (status_hook) rather than meet by chance.
        # set_slave_response_delay() swaps in a slower profile.
        bytes_per_line = 32 // 8
        addr_range = 0x00010000
        mem_bytes = min(addr_range, self.SLAVE_MEM_CAP_BYTES)
        preset = self._build_preset(1, addr_range, 32)
        self.slave_wb[1] = WB4Slave(
            entity=self.dut,
            title="WB4_S1",
            prefix="wbp_wb_",
            clock=self.clock,
            ...
            max_outstanding=16,
            randomizer=FlexRandomizer(self._wb4_slave_profile(1)),
            log=self.log,
        )
    ...
    def set_slave_response_delay(self, slave_idx: int, cycles: int) -> None:
        ...
        wb = self.slave_wb.get(slave_idx)
        if wb is not None:
            # A Wishbone termination is registered: latency 1 is the
            # earliest legal, so `cycles` below that means "as fast as B4 allows".
            wb.set_randomizer(FlexRandomizer(self._wb4_slave_profile(max(1, cycles))))

    @staticmethod
    def _wb4_slave_profile(ack_latency: int) -> dict:
        """WB4Slave randomizer: no STALL, fixed termination latency, always ACK."""
        return {
            'stall': ([(0, 0)], [1]),
            'ack': ([(ack_latency, ack_latency)], [1]),
            'status': ([(0, 0)], [1]),
        }
    ...
    async def master_read(self, master_idx: int, address: int) -> int:
        ...
        if master_idx in self.master_wb:
            # Wishbone has ERR (and RTY), not a response code: the converter
            # folds SLVERR and DECERR into ERR, so report 2 as for APB.
            pkt = await self.master_wb[master_idx].read(address)
            status = int(pkt.fields['status'])
            if status != WB4_STATUS_ACK:
                raise AxiResponseError(address, 2, f'WB4 {WB4_STATUS_NAMES[status]} on read')
            return int(pkt.fields['dat_r'])
    ...
    async def master_write(self, master_idx: int, address: int, data: int,
        ...
        if master_idx in self.master_apb:
            ...
            return
        if master_idx in self.master_wb:
            pkt = await self.master_wb[master_idx].write(address, data)
            status = int(pkt.fields['status'])
            if status != WB4_STATUS_ACK:
                raise AxiResponseError(address, 2, f'WB4 {WB4_STATUS_NAMES[status]} on write')
            return
