...
#
# Bridge2x3ApbReqMonTB - Testbench for bridge_2x3_apb_req_mon
...
from CocoTBFramework.components.axil5.axil5_interfaces import (
    AXIL5SlaveRead, AXIL5SlaveWrite,
)
...
class Bridge2x3ApbReqMonTB(TBBase):
    """
    Testbench for bridge_2x3_apb_req_mon bridge.

    Configuration:
        Masters: 2 (apb4m, apb5m)
        Slaves:  3 (mem, lregs, periph)
        Channels: RW
        Data Width: 32
        Addr Width: 64
        ID Width: 8
    ...
    def __init__(self, dut):
        ...
        self.slave_info = {
            0: ('axi4', 0x00000000, 0x10000000, 32),  # mem
            1: ('axil5', 0x40000000, 0x00010000, 32),  # lregs
            2: ('apb', 0x50000000, 0x00001000, 32),  # periph
        }
        ...
        self._setup_slave_1_lregs()
        self._setup_slave_2_periph()
        self._setup_master_0_apb4m()
        self._setup_master_1_apb5m()
    ...
    def _setup_master_0_apb4m(self):
        """Set up protocol BFMs for master 0: apb4m (protocol: apb)"""
        # APB requester port: the bridge is the completer. One BFM handle
        # covers both directions; master_read/master_write unwrap the
        # completed transaction (PRDATA, PSLVERR -> AxiResponseError).
        self.master_apb[0] = APBMaster(
            entity=self.dut,
            title="APB_M0",
            prefix="apb4m_apb_",
            clock=self.clock,
    ...
    def _setup_master_1_apb5m(self):
        """Set up protocol BFMs for master 1: apb5m (protocol: apb5)"""
        # APB requester port: the bridge is the completer. One BFM handle
        # covers both directions; master_read/master_write unwrap the
        # completed transaction (PRDATA, PSLVERR -> AxiResponseError).
        self.master_apb[1] = APB5Master(
            entity=self.dut,
            title="APB5_M1",
            prefix="apb5m_apb_",
            clock=self.clock,
            bus_width=32,
            addr_width=32,
            # The bridge exposes the APB5 USER pins 1 bit wide (the fabric
            # USER width); PWAKEUP is requester-driven and asserted with
            # PSEL by the BFM.
            auser_width=1, wuser_width=1, ruser_width=1, buser_width=1,
            log=self.log,
        )
    ...
    def _setup_slave_0_mem(self):
        ...
        self.slave_rd[0] = AXI4SlaveRead(
            ...
            data_width=32,
            addr_width=32,
            id_width=2,  # {master index, master id} (BRIDGE-016)
            user_width=1,
            multi_sig=True,
    ...
    def _setup_slave_1_lregs(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 1: lregs (protocol: axil5)"""
        ...
        # This one is an AXI5-Lite port, so it uses the AXIL5 BFMs: the same
        # transaction logic with the optional sideband groups added, which is
        # what lets them bind to the bridge's awuser/awtrace/wpoison/... ports
        # instead of dying at signal resolution.
        bytes_per_line = 32 // 8
        ...
        self.slave_rd[1] = AXIL5SlaveRead(
            self.dut, self.clock,
            prefix="lregs_axil_",
            log=self.log,
            data_width=32,
        ...
        self.slave_wr[1] = AXIL5SlaveWrite(
            self.dut, self.clock,
            prefix="lregs_axil_",
            log=self.log,
            data_width=32,
    ...
    def _setup_slave_2_periph(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 2: periph (protocol: apb)"""
        # APB slave — APBSlave manages its own internal memory; pre-seed
        # it with the slave-specific pattern via direct memory writes.
        preset = self._build_preset(2, 0x00001000, 32)
