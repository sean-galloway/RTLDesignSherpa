...
#
# Bridge2x2RwCdcTB - Testbench for bridge_2x2_rw_cdc
...
class Bridge2x2RwCdcTB(TBBase):
    """
    Testbench for bridge_2x2_rw_cdc bridge.
    ...
    def __init__(self, dut):
        ...
        self.slave_info = {
            ...
            1: ('axi4', 0x80000000, 0x80000000, 32),  # sram
        }
    ...
    def _setup_master_0_cpu(self):
        """Set up protocol BFMs for master 0: cpu (protocol: axi4)"""
    ...
    def _setup_master_1_dma(self):
        """Set up protocol BFMs for master 1: dma (protocol: axi4)"""
    ...
    def _setup_slave_0_ddr(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 0: ddr (protocol: axi4, own clock)"""
    ...
    def _setup_slave_1_sram(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 1: sram (protocol: axi4)"""
        # AXI4 slave — full memory-backed BFM.
        bytes_per_line = 32 // 8
        addr_range = 0x80000000
        # Cap MemoryModel size — see SLAVE_MEM_CAP_BYTES comment.
    ...
    # BRIDGE-017 CDC slave ports run on their own clock. The period comes from
    # BRIDGE_CDC_PERIOD_NS (default 7 ns against the fabric's 10 ns, so the
    # crossing is exercised, not degenerate); a directed test sweeps it.
    CDC_PERIOD_NS = int(os.environ.get('BRIDGE_CDC_PERIOD_NS', '7'))

    async def setup_clocks_and_reset(self):
        await self.start_clock(self.clock_name, freq=10, units='ns')
        await self.start_clock('ddr_aclk', freq=self.CDC_PERIOD_NS, units='ns')
        await self.assert_reset()
        await self.wait_clocks(self.clock_name, 10)
        # Both sides of every crossing see reset (a one-sided reset is not
        # safe on the async FIFOs -- handbook design/cdc.md).
        await self.wait_clocks(self.clock_name, 10)
    ...
    async def assert_reset(self):
        ...
        self.dut.ddr_aresetn.value = 0

    async def deassert_reset(self):
        self.reset_n.value = 1
        self.dut.ddr_aresetn.value = 1
