...
#
# Bridge1x5RdTB - Testbench for bridge_1x5_rd
...
class Bridge1x5RdTB(TBBase):
    """
    Testbench for bridge_1x5_rd bridge.

    Configuration:
        Masters: 1 (cpu_rd)
        Slaves:  5 (periph_rd, ddr_rd, hbm_rd, apb_periph, axil_periph)
        Channels: RD
        Data Width: 128
        Addr Width: 64
        ID Width: 8
    ...
    def __init__(self, dut):
        ...
        self.slave_info = {
            0: ('axi4', 0x00000000, 0x10000000, 32),  # periph_rd
            1: ('axi4', 0x10000000, 0x40000000, 64),  # ddr_rd
            2: ('axi4', 0x50000000, 0x30000000, 128),  # hbm_rd
            3: ('apb', 0x80000000, 0x00010000, 32),  # apb_periph
            4: ('axil', 0x90000000, 0x00010000, 32),  # axil_periph
        }
        ...
        self.observed_ar = {i: [] for i in range(5)}
        self.observed_aw = {i: [] for i in range(5)}
        ...
        self._setup_slave_4_axil_periph()
    ...
    def _setup_slave_0_periph_rd(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 0: periph_rd (protocol: axi4)"""
    ...
    def _setup_slave_1_ddr_rd(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 1: ddr_rd (protocol: axi4)"""
        # AXI4 slave — full memory-backed BFM.
        bytes_per_line = 64 // 8
    ...
    def _setup_slave_2_hbm_rd(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 2: hbm_rd (protocol: axi4)"""
        # AXI4 slave — full memory-backed BFM.
        bytes_per_line = 128 // 8
        addr_range = 0x30000000
        # Cap MemoryModel size — see SLAVE_MEM_CAP_BYTES comment.
        mem_bytes = min(addr_range, self.SLAVE_MEM_CAP_BYTES)
    ...
    def _setup_slave_3_apb_periph(self):
        ...
        preset = self._build_preset(3, 0x00010000, 32)
        self.slave_apb[3] = APBSlave(
            entity=self.dut,
            title="APB_S3",
            prefix="apb_periph_",
            clock=self.clock,
    ...
    def _setup_slave_4_axil_periph(self):
        """Set up protocol BFM and pre-seeded MemoryModel for slave 4: axil_periph (protocol: axil)"""
        ...
        # Cap MemoryModel size — see SLAVE_MEM_CAP_BYTES comment.
        mem_bytes = min(addr_range, self.SLAVE_MEM_CAP_BYTES)
        preset = self._build_preset(4, addr_range, 32)
        ...
        self.slave_rd[4].ar_channel.add_callback(
            lambda pkt, _i=4: self.observed_ar[_i].append(pkt))
