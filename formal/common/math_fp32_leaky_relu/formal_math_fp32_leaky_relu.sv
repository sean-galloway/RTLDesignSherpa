...
//
// Formal wrapper for math_fp32_leaky_relu
// Proves FP32 Leaky ReLU: positive passthrough, NaN propagation, negative scaling

module formal_math_fp32_leaky_relu (
    input logic clk
);
    ...
    always @(posedge clk) begin
        ap_zero: assert (!w_is_zero || (result == 32'h0));
    end

    // P4: Negative input -> sign bit preserved
    always @(posedge clk) begin
        ap_neg_sign: assert (w_is_nan || w_is_zero || !w_sign || result[31]);
    end

    // P5: Negative input -> mantissa preserved
    always @(posedge clk) begin
        ap_neg_mant: assert (w_is_nan || w_is_zero || !w_sign || (result[22:0] == w_mant));
    end

    // P6: Output is one of the valid results (input, scaled, or zero)
    always @(posedge clk) begin
        ap_valid: assert (result == a || result == 32'h0 ||
                         (result[31] == w_sign && result[22:0] == w_mant));
    end
    ...
    always @(posedge clk) begin
        cp_zero: cover (w_is_zero && result == 32'h0);
    end
