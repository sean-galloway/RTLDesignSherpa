...
//
// Formal wrapper for math_fp8_e5m2_leaky_relu
// Proves FP8_E5M2 Leaky ReLU: positive passthrough, NaN propagation, negative scaling

module formal_math_fp8_e5m2_leaky_relu (
    input logic clk
);
    ...
    always @(posedge clk) begin
        ap_neg_mant: assert (w_is_nan || w_is_zero || !w_sign || (result[1:0] == w_mant));
    end

    // P6: Output is one of the valid results (input, scaled, or zero)
    always @(posedge clk) begin
        ap_valid: assert (result == a || result == 8'h0 ||
                         (result[7] == w_sign && result[1:0] == w_mant));
    end
