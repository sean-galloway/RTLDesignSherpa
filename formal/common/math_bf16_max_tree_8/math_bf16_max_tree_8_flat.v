module math_bf16_max_tree_8 (
	...
	function automatic [15:0] fp_max;
		...
		begin
			...
			a_mag = a[14:0];
			b_mag = b[14:0];
			a_is_nan = (a[14:7] == 8'hff) & (a[6:0] != 7'h00);
			b_is_nan = (b[14:7] == 8'hff) & (b[6:0] != 7'h00);
			if (a_is_nan) begin
				fp_max = b;
				_sv2v_jump = 2'b11;
			end
	...
	assign w_level0[0] = fp_max(i_data[112+:16], i_data[96+:16]);
	assign w_level0[1] = fp_max(i_data[80+:16], i_data[64+:16]);
	assign w_level0[2] = fp_max(i_data[48+:16], i_data[32+:16]);
	assign w_level0[3] = fp_max(i_data[16+:16], i_data[0+:16]);
	...
	generate
		for (_gv_gi_1 = 0; _gv_gi_1 < 8; _gv_gi_1 = _gv_gi_1 + 1) begin : gen_idx
			...
			assign ow_max_idx[gi] = i_data[(7 - gi) * 16+:16] == ow_max;
		end
	endgenerate
endmodule
