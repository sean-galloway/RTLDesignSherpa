...
//
// Formal verification wrapper for math_fp32_sigmoid
//
// Verifies:
//   - NaN passthrough
//   - Sign handling
...
module formal_math_fp32_sigmoid (
    input logic clk
);
    ...
    wire a_is_zero     = (exp == 8'd0) && (mant == 23'd0);
    wire a_is_subnormal = (exp == 8'd0) && (mant != 23'd0);
    wire a_is_nan      = (exp == 8'hFF) && (mant != 23'h0);
    wire a_is_inf      = (exp == 8'hFF) && (mant == 23'h0);
    ...
    always @(posedge clk) begin
        if (a == 32'h00000000) begin
            p_sigmoid_zero: assert (result == 32'h3F000000);
        end
    end
    ...
    always @(posedge clk) begin
        if (!sign_a && a_is_inf) begin
            p_pos_inf_gives_one: assert (result == 32'h3F800000);
        end
    end
    ...
    always @(posedge clk) begin
        if (sign_a && a_is_inf) begin
            p_neg_inf_gives_zero: assert (result == 32'h00000000);
        end
    end
    ...
    always @(posedge clk) begin
        if (!a_is_nan) begin
            p_output_non_negative: assert (result[31] == 1'b0);
        end
    end
    ...
    always @(posedge clk) begin
        if (!a_is_nan && !r_is_nan) begin
            p_output_le_one: assert (result <= 32'h3F800000);
        end
    end
