...
//
// Formal verification wrapper for math_fp8_e5m2_to_fp32
//
// Verifies lossless FP8_E5M2 -> FP32 upconversion properties:
//   - Sign preservation
//   - NaN propagation
//   - Infinity preservation (E5M2 has infinity)
//   - Zero preservation (including subnormal flush-to-zero)
//   - Exponent bias adjustment (E5M2 bias=15, FP32 bias=127, diff=112)
//   - Mantissa zero-extension correctness

`timescale 1ns / 1ps

module formal_math_fp8_e5m2_to_fp32 (
    input logic clk
);
    ...
    // Exponent: E5M2 exp + 112 = FP32 exp
    always @(posedge clk) begin
    ...
    // Mantissa: {mant_a[1:0], 21'b0}
    always @(posedge clk) begin
        if (a_is_normal) begin
            p_mant_upper: assert (mant_r[22:21] == mant_a);
            p_mant_lower: assert (mant_r[20:0] == 21'h0);
        end
    end
