...
//
// Formal wrapper for shifter_lfsr_fibonacci (yosys-compatible)
// Instantiates DUT and properties together.
// Run with: sby shifter_lfsr_fibonacci.sby

module formal_shifter_lfsr_fibonacci #(
    ...
    // Constrain seed to non-zero (all-zeros locks up XOR-based Fibonacci LFSR)
    always @(*) begin
        assume (seed_data != '0);
    end
    ...
    // Fibonacci shift behavior: right shift, feedback to MSB
    // Lower bits come from previous upper bits
    always @(posedge clk) begin
        if (f_past_valid > 0 && rst_n && $past(rst_n)) begin
            if ($past(enable) && !$past(seed_load) && $past(|lfsr_out))
                ap_shift_lower: assert (lfsr_out[WIDTH-2:0] == $past(lfsr_out[WIDTH-1:1]));
        end
    end

    // Non-zero state preserved: if register was non-zero and we shift,
    // the Fibonacci LFSR only shifts when |r_lfsr is true, so zero state locks
    always @(posedge clk) begin
        if (f_past_valid > 0 && rst_n && $past(rst_n)) begin
            if ($past(enable) && !$past(seed_load) && !$past(|lfsr_out))
                ap_zero_locks: assert (lfsr_out == '0);
        end
    end
