module math_fp8_e5m2_min_tree_8 (
	...
	output wire [7:0] ow_min;
	output wire [7:0] ow_min_idx;
	function automatic [7:0] fp_min;
		input reg [7:0] a;
		input reg [7:0] b;
		reg a_sign;
		reg b_sign;
	...
	assign w_level0[0] = fp_min(i_data[56+:8], i_data[48+:8]);
	assign w_level0[1] = fp_min(i_data[40+:8], i_data[32+:8]);
	assign w_level0[2] = fp_min(i_data[24+:8], i_data[16+:8]);
	assign w_level0[3] = fp_min(i_data[8+:8], i_data[0+:8]);
	...
	generate
		for (_gv_gi_1 = 0; _gv_gi_1 < 8; _gv_gi_1 = _gv_gi_1 + 1) begin : gen_idx
			...
			assign ow_min_idx[gi] = i_data[(7 - gi) * 8+:8] == ow_min;
		end
	endgenerate
endmodule
