...
//
// Formal verification wrapper for math_fp32_silu
//
// Verifies:
//   - NaN passthrough
//   - Sign handling
//   - Output in valid FP range
//   - Known vector: silu(0) = 0

`timescale 1ns / 1ps

module formal_math_fp32_silu (
    input logic clk
);
    ...
    always @(posedge clk) begin
        if (a == 32'h00000000) begin
            p_silu_zero: assert (result == 32'h00000000);
        end
    end

    // Property: silu(+inf) = +inf
    always @(posedge clk) begin
