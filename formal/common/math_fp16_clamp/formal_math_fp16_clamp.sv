...
//
// Formal verification wrapper for math_fp16_clamp
//
// Verifies:
...
module formal_math_fp16_clamp (
    ...
    // Field extraction
    wire [4:0]  exp_x  = x[14:10];
    wire [9:0]  mant_x = x[9:0];
    wire [4:0]  exp_lo = lo[14:10];
    wire [9:0]  mant_lo = lo[9:0];
    wire [4:0]  exp_hi = hi[14:10];
    wire [9:0]  mant_hi = hi[9:0];

    // NaN detection
    wire x_nan  = (exp_x  == 5'h1F) & (mant_x  != 10'h0);
    wire lo_nan = (exp_lo == 5'h1F) & (mant_lo != 10'h0);
    wire hi_nan = (exp_hi == 5'h1F) & (mant_hi != 10'h0);
