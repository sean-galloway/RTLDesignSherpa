...
//
// Formal verification wrapper for math_fp32_tanh
//
// Verifies:
//   - NaN passthrough
//   - Sign handling
...
module formal_math_fp32_tanh (
    ...
    always @(posedge clk) begin
        if (a == 32'h00000000) begin
            p_tanh_zero: assert (result == 32'h00000000);
        end
    end
    ...
    always @(posedge clk) begin
        if (!sign_a && a_is_inf) begin
            p_pos_inf_gives_pos_one: assert (result == 32'h3F800000);
        end
    end
    ...
    always @(posedge clk) begin
        if (sign_a && a_is_inf) begin
            p_neg_inf_gives_neg_one: assert (result == 32'hBF800000);
        end
    end
    ...
    always @(posedge clk) begin
        if (!a_is_nan && !r_is_nan) begin
            p_output_le_one: assert (result[30:0] <= 31'h3F800000);
        end
    end
