...
//
// Formal verification wrapper for math_fp8_e4m3_clamp
//
// Verifies:
...
//
// Note: FP8 E4M3 has a single NaN encoding: exp=0xF, mant=0x7 (8'hFF)

`timescale 1ns / 1ps

module formal_math_fp8_e4m3_clamp (
    input logic clk
);
    ...
    // Field extraction
    wire [3:0] exp_x  = x[6:3];
    wire [2:0] mant_x = x[2:0];
    wire [3:0] exp_lo = lo[6:3];
    wire [2:0] mant_lo = lo[2:0];
    wire [3:0] exp_hi = hi[6:3];
    wire [2:0] mant_hi = hi[2:0];

    // NaN detection (E4M3: only exp=F, mant=7 is NaN)
    wire x_nan  = (exp_x  == 4'hF) & (mant_x  == 3'h7);
    wire lo_nan = (exp_lo == 4'hF) & (mant_lo == 3'h7);
    wire hi_nan = (exp_hi == 4'hF) & (mant_hi == 3'h7);
