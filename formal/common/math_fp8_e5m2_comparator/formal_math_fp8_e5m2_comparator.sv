...
//
// Formal verification wrapper for math_fp8_e5m2_comparator
//
// Verifies:
...
module formal_math_fp8_e5m2_comparator (
    input logic clk
);
    ...
    math_fp8_e5m2_comparator dut_rev (
        .i_a(b), .i_b(a),
        .ow_eq(rev_eq), .ow_lt(rev_lt), .ow_gt(rev_gt),
        .ow_le(rev_le), .ow_ge(rev_ge), .ow_ne(rev_ne), .ow_unord(rev_unord)
    );

    wire a_nan = (a[6:2] == 5'h1F) & (a[1:0] != 2'h0);
    wire b_nan = (b[6:2] == 5'h1F) & (b[1:0] != 2'h0);
