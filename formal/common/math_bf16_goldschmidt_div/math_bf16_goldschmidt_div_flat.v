...
module math_bf16_goldschmidt_div (
	...
	parameter signed [31:0] LUT_DEPTH = 32;
	parameter [0:0] PIPELINED = 1;
	input wire i_clk;
	input wire i_rst_n;
	input wire [15:0] i_numerator;
	input wire [15:0] i_denominator;
	input wire i_valid;
	output wire [15:0] ow_quotient;
	output wire ow_valid;
	output wire ow_div_by_zero;
	output wire ow_is_inf;
	output wire ow_is_nan;
	...
	wire w_recip_nan;
	wire w_recip_underflow;
	wire [6:0] w_recip_mant_approx;
	wire [6:0] w_recip_mant_interp;
	math_bf16_fast_reciprocal #(.LUT_DEPTH(LUT_DEPTH)) u_initial_recip(
	...
	wire [15:0] w_n0;
	wire [15:0] w_d0;
	wire w_prescale_n_ovf;
	wire w_prescale_n_udf;
	wire w_prescale_n_inv;
	wire w_prescale_d_ovf;
	wire w_prescale_d_udf;
	wire w_prescale_d_inv;
	...
	generate
		if (ITERATIONS >= 2) begin : gen_iter2
			...
			if (PIPELINED) begin : gen_pipe_regs
				...
				always @(posedge i_clk or negedge i_rst_n)
					if (!i_rst_n) begin
						...
						r_special_zero <= ((w_a_is_zero && !w_b_is_zero) || w_b_is_inf) || w_prescale_n_udf;
						r_special_inf <= w_b_is_zero || w_a_is_inf;
						r_special_nan <= ((w_a_is_nan || w_b_is_nan) || (w_a_is_zero && w_b_is_zero)) || (w_a_is_inf && w_b_is_inf);
						r_result_sign <= w_result_sign;
					end
				wire [15:0] w_neg_d1_p;
				assign w_neg_d1_p = {~r_d1[15], r_d1[14:0]};
				wire w_add_f1_p_ovf;
				wire w_add_f1_p_udf;
				wire w_add_f1_p_inv;
				wire w_add_f1_p_valid;
				wire w_mult_n2_p_ovf;
				wire w_mult_n2_p_udf;
				wire w_mult_n2_p_inv;
				math_bf16_adder #(
					.PIPE_STAGE_1(0),
					.PIPE_STAGE_2(0),
					.PIPE_STAGE_3(0),
				...
				wire [15:0] w_neg_d1_c;
				assign w_neg_d1_c = {~w_d1[15], w_d1[14:0]};
				wire w_add_f1_c_ovf;
				wire w_add_f1_c_udf;
				wire w_add_f1_c_inv;
				wire w_add_f1_c_valid;
				wire w_mult_n2_c_ovf;
				wire w_mult_n2_c_udf;
				wire w_mult_n2_c_inv;
				math_bf16_adder #(
					.PIPE_STAGE_1(0),
					.PIPE_STAGE_2(0),
					.PIPE_STAGE_3(0),
				...
				wire w_special_inf;
				wire w_special_zero;
				assign w_special_nan = ((w_a_is_nan || w_b_is_nan) || (w_a_is_zero && w_b_is_zero)) || (w_a_is_inf && w_b_is_inf);
				assign w_special_zero = ((w_a_is_zero && !w_b_is_zero) || w_b_is_inf) || w_prescale_n_udf;
				assign w_special_inf = w_b_is_zero || w_a_is_inf;
				assign ow_quotient = (w_special_nan ? BF16_NAN : (w_b_is_zero ? (w_result_sign ? BF16_NEG_INF : BF16_POS_INF) : (w_special_zero ? BF16_ZERO : {w_result_sign, w_n2[14:0]})));
		...
		else begin : gen_single_iter
			wire w_special_nan;
			wire w_special_inf;
			wire w_special_zero;
			assign w_special_nan = ((w_a_is_nan || w_b_is_nan) || (w_a_is_zero && w_b_is_zero)) || (w_a_is_inf && w_b_is_inf);
			wire signed [9:0] w_true_exp_approx;
			assign w_true_exp_approx = ($signed({2'b00, w_a_exp}) - $signed({2'b00, w_b_exp})) + 10'sd127;
			wire w_true_underflow;
			assign w_true_underflow = w_true_exp_approx < 10'sd1;
			wire w_use_direct_calc;
			assign w_use_direct_calc = ((((w_recip_underflow && !w_true_underflow) && !w_special_nan) && !w_b_is_zero) && !w_a_is_zero) && !w_b_is_inf;
			wire [15:0] w_direct_product;
			assign w_direct_product = (16'd128 + {9'd0, w_a_mant}) * (16'd128 + {9'd0, w_recip_mant_interp});
			wire w_quot_ge_one;
			assign w_quot_ge_one = w_direct_product >= 16'd32768;
			wire signed [9:0] w_direct_exp_raw;
			assign w_direct_exp_raw = w_true_exp_approx - (w_quot_ge_one ? 10'sd0 : 10'sd1);
			wire [7:0] w_direct_exp;
			assign w_direct_exp = (w_direct_exp_raw < 10'sd1 ? 8'd0 : (w_direct_exp_raw > 10'sd254 ? 8'd254 : w_direct_exp_raw[7:0]));
			wire [6:0] w_direct_mant;
			assign w_direct_mant = (w_quot_ge_one ? sv2v_cast_7((w_direct_product - (16'd32768 - 16'd128)) >> 8) : sv2v_cast_7((w_direct_product - (16'd16384 - 16'd64)) >> 7));
			wire [15:0] w_direct_result;
			assign w_direct_result = {w_result_sign, w_direct_exp, w_direct_mant};
			wire w_effective_underflow;
			assign w_effective_underflow = (w_recip_underflow ? w_true_underflow : w_prescale_n_udf);
			assign w_special_zero = ((w_a_is_zero && !w_b_is_zero) || w_b_is_inf) || w_effective_underflow;
			assign w_special_inf = w_b_is_zero || w_a_is_inf;
				...
				always @(posedge i_clk or negedge i_rst_n)
					if (!i_rst_n) begin
						...
						if (w_special_nan)
							...
							r_quotient <= w_direct_result;
						else if (w_special_zero)
							r_quotient <= {w_result_sign, 15'd0};
						else
			...
			else begin : gen_comb_single
				assign ow_quotient = (w_special_nan ? BF16_NAN : (w_b_is_zero ? (w_result_sign ? BF16_NEG_INF : BF16_POS_INF) : (w_use_direct_calc ? w_direct_result : (w_special_zero ? BF16_ZERO : {w_result_sign, w_n1[14:0]}))));
				assign ow_valid = i_valid;
				assign ow_div_by_zero = w_b_is_zero;
