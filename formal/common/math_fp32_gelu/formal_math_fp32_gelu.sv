...
//
// Formal verification wrapper for math_fp32_gelu
//
// Verifies:
//   - NaN passthrough
//   - Sign handling
//   - Output in valid FP range
//   - Known vector: gelu(0) = 0

`timescale 1ns / 1ps

module formal_math_fp32_gelu (
    input logic clk
);
    ...
    always @(posedge clk) begin
        if (a == 32'h00000000) begin
            p_gelu_zero: assert (result == 32'h00000000);
        end
    end

    // Property: gelu(+inf) = +inf
    always @(posedge clk) begin
        if (!sign_a && a_is_inf) begin
            p_pos_inf_passthrough: assert (result == a);
        end
    end
