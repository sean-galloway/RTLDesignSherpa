...
// Note: E4M3 NaN is exp=0xF, mant=0x7 (single encoding)
...
module formal_math_fp8_e4m3_min_tree_8 (
    ...
    // FP8 E4M3 le helper
    function automatic logic fp_le(input logic [7:0] a, input logic [7:0] b);
        logic a_nan, b_nan;
        a_nan = (a[6:3] == 4'hF) & (a[2:0] == 3'h7);
        b_nan = (b[6:3] == 4'hF) & (b[2:0] == 3'h7);
        if (a_nan | b_nan) return 1'b1;
        if (a[7] != b[7]) return a[7];
        if (a[7] == 1'b0) return (a[6:0] <= b[6:0]);
        else               return (a[6:0] >= b[6:0]);
    endfunction
