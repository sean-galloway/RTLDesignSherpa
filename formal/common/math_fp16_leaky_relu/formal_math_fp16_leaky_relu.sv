...
//
// Formal wrapper for math_fp16_leaky_relu
// Proves FP16 Leaky ReLU: positive passthrough, NaN propagation, negative scaling

module formal_math_fp16_leaky_relu (
    input logic clk
);
    ...
    always @(posedge clk) begin
        ap_neg_mant: assert (w_is_nan || w_is_zero || !w_sign || (result[9:0] == w_mant));
    end

    // P6: Output is one of the valid results (input, scaled, or zero)
    always @(posedge clk) begin
        ap_valid: assert (result == a || result == 16'h0 ||
                         (result[15] == w_sign && result[9:0] == w_mant));
    end
