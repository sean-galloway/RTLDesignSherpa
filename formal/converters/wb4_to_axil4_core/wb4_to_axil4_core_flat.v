...
module wb4_to_axil4_core (
	...
	parameter [2:0] AXIL_PROT = 3'b000;
	...
	output wire [AW - 1:0] fub_awaddr;
	output wire [2:0] fub_awprot;
	output wire fub_awvalid;
	input wire fub_awready;
	output wire [DW - 1:0] fub_wdata;
	output wire [SW - 1:0] fub_wstrb;
	output wire fub_wvalid;
	input wire fub_wready;
	input wire [1:0] fub_bresp;
	input wire fub_bvalid;
	output wire fub_bready;
	...
	localparam signed [31:0] CW = $clog2(OUTSTANDING + 1);
	wire w_room;
	wire w_issue;
	wire w_wr_go;
	wire w_rd_go;
	wire w_side_wr_ready;
	wire w_side_rd_valid;
	wire w_side_head_we;
	wire w_side_rd_ready;
	...
	always @(posedge aclk or negedge aresetn)
		if (!aresetn)
			...
			r_open <= (r_open + sv2v_cast_3D2D3(w_issue)) - sv2v_cast_3D2D3(w_retire);
	wire w_head_is_wr;
	wire w_head_ready;
	...
	assign rsp_status = ((w_head_is_wr ? fub_bresp : fub_rresp) == 2'b00 ? sv2v_cast_C2FF6(sv2v_cast_1AA03(2'b00)) : sv2v_cast_C2FF6(sv2v_cast_1AA03(2'b01)));
	...
	always @(posedge aclk)
		if ((f_past_valid && aresetn) && $past(aresetn)) begin
			...
			assert (sv2v_cast_32(r_open) <= OUTSTANDING) ;
		end
endmodule
