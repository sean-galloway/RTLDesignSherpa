...
module apb4_slave_cdc_cg (
	...
	input wire [DW - 1:0] rsp_prdata;
	input wire rsp_pslverr;
	output wire pclk_cg_gating;
	output wire pclk_cg_idle;
	output wire aclk_cg_gating;
	output wire aclk_cg_idle;
	output wire cg_gating;
	output wire cg_idle;
	localparam signed [31:0] APBCmdWidth = (((1 + AW) + DW) + SW) + PW;
	localparam signed [31:0] APBRspWidth = 1 + DW;
	wire gated_pclk;
	wire gated_aclk;
	wire int_cmd_ready;
	wire int_rsp_ready_aclk;
	wire int_rsp_ready_pclk;
	wire int_apb_PREADY;
	wire pclk_user_valid;
	wire pclk_axi_valid;
	wire aclk_user_valid;
	wire aclk_axi_valid;
	reg r_psel_sync1;
	reg r_psel_sync2;
	...
	assign w_rsp_ready = (pclk_cg_gating ? 1'b0 : int_rsp_ready_pclk);
	amba_clock_gate_ctrl #(.CG_IDLE_COUNT_WIDTH(CG_IDLE_COUNT_WIDTH)) pclk_gate_ctrl(
		.clk_in(pclk),
		.aresetn(presetn),
		.cfg_cg_enable(cfg_cg_enable),
		.cfg_cg_idle_count(cfg_cg_idle_count),
	...
	amba_clock_gate_ctrl #(.CG_IDLE_COUNT_WIDTH(CG_IDLE_COUNT_WIDTH)) aclk_gate_ctrl(
		.clk_in(aclk),
		.aresetn(aresetn),
		.cfg_cg_enable(cfg_cg_enable),
		.cfg_cg_idle_count(cfg_cg_idle_count),
	...
	gaxi_fifo_async #(
		...
		.axi_wr_aclk(gated_pclk),
		.axi_wr_aresetn(presetn),
		.axi_rd_aclk(gated_aclk),
	...
	gaxi_fifo_async #(
		...
		.axi_wr_aclk(gated_aclk),
		.axi_wr_aresetn(aresetn),
		.axi_rd_aclk(gated_pclk),
