...
module arbiter_monbus_common #(
    ...
    // Monitor state machine constants
    localparam logic [2:0] MON_IDLE     = 3'b000;
    localparam logic [2:0] MON_ACTIVE   = 3'b001;
    localparam logic [2:0] MON_SAMPLE   = 3'b010;
    localparam logic [2:0] MON_ANALYZE  = 3'b011;
    localparam logic [2:0] MON_FAIRNESS = 3'b100;
    localparam logic [2:0] MON_ERROR    = 3'b101;

    logic [2:0] r_monitor_state;
    ...
    logic [15:0] w_expected_pct;
    logic [15:0] w_actual_pct;
    logic [15:0] w_deviation;
            ...
            if (r_fairness_timer >= 16'(FAIRNESS_REPORT_CYCLES)) begin
                ...
                if (r_total_grants >= 16'(MIN_GRANTS_FOR_FAIRNESS)) begin
                    ...
                    if (r_total_grants >= 16'(MIN_GRANTS_FOR_FAIRNESS)) begin
                        ...
                        // (local vars hoisted out for yosys compat)
                        for (int i = 0; i < CLIENTS; i++) begin
                            if (total_weight > 0 && client_weight[i] > 0) begin

                                // Expected percentage = (client_weight / total_weight) * 100
                                w_expected_pct = (16'(client_weight[i]) * 100) / total_weight;

                                // Actual percentage = (client_grants / total_grants) * 100
                                w_actual_pct = (r_grant_counters[i] * 100) / r_total_grants;

                                // Calculate absolute deviation
                                if (w_actual_pct >= w_expected_pct) begin
                                    w_deviation = w_actual_pct - w_expected_pct;
                                end else begin
                                    w_deviation = w_expected_pct - w_actual_pct;
                                end

                                // Track maximum deviation
                                if (w_deviation > max_deviation_temp) begin
                                    max_deviation_temp = w_deviation;
                                end
                            end
                        end
    ...
    always_comb begin
        // Default enum values
        w_packet_enum_arb_error = 4'(4'h0);
        w_packet_enum_arb_timeout = 4'(4'h0);
        w_packet_enum_arb_completion = 4'(4'h0);
        w_packet_enum_arb_threshold = 4'(4'h0);
        w_packet_enum_arb_performance = 4'(4'h0);

        case (w_packet_pkt_type)
            PktTypeError: begin
                w_packet_enum_arb_error = 4'(w_packet_event_code);
            end
            PktTypeTimeout: begin
                w_packet_enum_arb_timeout = 4'(w_packet_event_code);
            end
            PktTypeCompletion: begin
                w_packet_enum_arb_completion = 4'(w_packet_event_code);
            end
            PktTypeThreshold: begin
                w_packet_enum_arb_threshold = 4'(w_packet_event_code);
            end
            PktTypePerf: begin
                w_packet_enum_arb_performance = 4'(w_packet_event_code);
            end
            default: begin
                // Keep default values
            end
