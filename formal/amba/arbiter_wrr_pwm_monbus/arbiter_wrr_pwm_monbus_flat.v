...
module arbiter_round_robin_weighted (
	...
	parameter signed [31:0] N = $clog2(CLIENTS);
	parameter signed [31:0] C = CLIENTS;
	parameter signed [31:0] MTW = MAX_LEVELS_WIDTH;
	parameter signed [31:0] CXMTW = CLIENTS * MAX_LEVELS_WIDTH;
	input wire clk;
	input wire rst_n;
	input wire block_arb;
	input wire [CXMTW - 1:0] max_thresh;
	input wire [C - 1:0] request;
	input wire [C - 1:0] grant_ack;
	output wire grant_valid;
	output wire [C - 1:0] grant;
	output wire [N - 1:0] grant_id;
	localparam signed [31:0] WEIGHT_STABILIZE_CYCLES = 3;
	localparam signed [31:0] WEIGHT_DRAIN_CYCLES = 2;
	localparam signed [31:0] WEIGHT_TIMEOUT_CYCLES = 15;
	reg [4:0] r_weight_state;
	reg [3:0] r_weight_timer;
	reg [CXMTW - 1:0] r_safe_max_thresh;
	wire w_weight_change_req;
	wire w_pending_grants;
	assign w_weight_change_req = max_thresh != r_safe_max_thresh;
	assign w_pending_grants = (WAIT_GNT_ACK == 1 ? grant_valid && ((grant_ack & grant) == {C {1'sb0}}) : 1'b0);
	always @(posedge clk or negedge rst_n)
		if (!rst_n) begin
			r_weight_state <= 5'b00001;
			...
			casez (r_weight_state)
				5'b00001:
					if (w_weight_change_req) begin
						r_weight_state <= 5'b00010;
						r_weight_timer <= 4'h0;
					end
				5'b00010:
					if (!w_pending_grants) begin
						r_weight_state <= 5'b00100;
						r_weight_timer <= WEIGHT_DRAIN_CYCLES[3:0];
					end
					else if (r_weight_timer < WEIGHT_TIMEOUT_CYCLES[3:0])
				...
				5'b00100:
					if (r_weight_timer == 4'h0)
						r_weight_state <= 5'b01000;
					else
						r_weight_timer <= r_weight_timer - 4'h1;
				5'b01000: begin
					r_safe_max_thresh <= max_thresh;
					r_weight_state <= 5'b10000;
					r_weight_timer <= WEIGHT_STABILIZE_CYCLES[3:0];
				end
				5'b10000:
					if (r_weight_timer == 4'h0)
						r_weight_state <= 5'b00001;
			...
			endcase
	wire [MTW - 1:0] w_client_weight [0:C - 1];
	wire w_normal_operation;
	wire [C - 1:0] w_valid_clients;
	wire [C - 1:0] w_invalid_clients;
	wire [C - 1:0] w_req_post;
	genvar _gv_j_1;
	generate
	...
	endgenerate
	assign w_normal_operation = r_weight_state == 5'b00001;
	assign w_req_post = (block_arb ? {C {1'sb0}} : request);
	reg [MTW - 1:0] r_credit_counter [0:C - 1];
	wire [C - 1:0] w_has_crd;
	wire w_global_replenish;
	reg [MTW - 1:0] w_credit_counter [0:C - 1];
	wire [C - 1:0] w_grant_completed;
	genvar _gv_i_2;
	generate
		for (_gv_i_2 = 0; _gv_i_2 < CLIENTS; _gv_i_2 = _gv_i_2 + 1) begin : gen_grant_completion
			localparam i = _gv_i_2;
			assign w_grant_completed[i] = (WAIT_GNT_ACK == 0 ? grant[i] && grant_valid : (grant[i] && grant_valid) && grant_ack[i]);
		end
	endgenerate
	genvar _gv_i_3;
	function automatic signed [MTW - 1:0] sv2v_cast_C21A8_signed;
		input reg signed [MTW - 1:0] inp;
		sv2v_cast_C21A8_signed = inp;
	endfunction
	generate
		for (_gv_i_3 = 0; _gv_i_3 < CLIENTS; _gv_i_3 = _gv_i_3 + 1) begin : gen_credit_combo_logic
			...
			always @(*) begin
				...
				case (r_weight_state)
					5'b00001:
						if (w_global_replenish)
							w_credit_counter[i] = w_client_weight[i];
						else if (w_grant_completed[i] && (r_credit_counter[i] > 0))
							w_credit_counter[i] = r_credit_counter[i] - sv2v_cast_C21A8_signed(1);
					5'b10000:
						if (w_client_weight[i] > 0)
							w_credit_counter[i] = w_client_weight[i];
						else
							w_credit_counter[i] = sv2v_cast_C21A8_signed(0);
					default: w_credit_counter[i] = r_credit_counter[i];
				endcase
			end
		end
	endgenerate
	genvar _gv_i_4;
	generate
		for (_gv_i_4 = 0; _gv_i_4 < CLIENTS; _gv_i_4 = _gv_i_4 + 1) begin : gen_credit_registers
			localparam i = _gv_i_4;
			assign w_has_crd[i] = ((r_credit_counter[i] > 0) && (w_client_weight[i] > 0)) && w_normal_operation;
			always @(posedge clk or negedge rst_n)
				if (!rst_n)
					r_credit_counter[i] <= sv2v_cast_C21A8_signed(1);
				else
					r_credit_counter[i] <= w_credit_counter[i];
		end
	endgenerate
	reg w_requesting_clients_with_credits;
	always @(*) begin
		if (_sv2v_0)
			;
		...
		begin : sv2v_autoblock_1
			reg signed [31:0] i;
			for (i = 0; i < CLIENTS; i = i + 1)
				if (w_req_post[i] && w_has_crd[i])
					w_requesting_clients_with_credits = 1'b1;
		end
	end
	assign w_global_replenish = ((w_normal_operation && !w_pending_grants) && |w_req_post) && !w_requesting_clients_with_credits;
	wire [C - 1:0] w_mask_req;
	wire [C - 1:0] w_mask_multi_req;
	wire [C - 1:0] w_mask_last_client;
	wire [C - 1:0] w_requesting_eligible;
	wire [C - 1:0] r_last_grant;
	reg [C - 1:0] w_clients_with_credits_count;
	wire w_multiple_eligible;
	always @(*) begin
		if (_sv2v_0)
			;
		w_clients_with_credits_count = 1'sb0;
		begin : sv2v_autoblock_2
			reg signed [31:0] k;
			for (k = 0; k < CLIENTS; k = k + 1)
				if (w_requesting_eligible[k])
	...
	genvar _gv_j_2;
	generate
		for (_gv_j_2 = 0; _gv_j_2 < CLIENTS; _gv_j_2 = _gv_j_2 + 1) begin : gen_request_logic
			localparam j = _gv_j_2;
			assign w_requesting_eligible[j] = w_req_post[j] && (w_has_crd[j] || (w_global_replenish && w_valid_clients[j]));
			assign w_mask_multi_req[j] = w_requesting_eligible[j] && !grant[j];
			assign w_mask_last_client[j] = (!w_multiple_eligible && w_requesting_eligible[j]) && (r_credit_counter[j] > sv2v_cast_C21A8_signed(1));
			assign w_mask_req[j] = w_mask_multi_req[j] || w_mask_last_client[j];
		end
	endgenerate
	wire w_sub_block_arb;
	assign w_sub_block_arb = r_weight_state != 5'b00001;
	arbiter_round_robin #(
		.CLIENTS(CLIENTS),
...
module pwm (
	...
	genvar _gv_i_5;
	generate
		for (_gv_i_5 = 0; _gv_i_5 < CHANNELS; _gv_i_5 = _gv_i_5 + 1) begin : gen_channel
...
module arbiter_monbus_common (
	...
	genvar _gv_j_3;
	generate
		for (_gv_j_3 = 0; _gv_j_3 < CLIENTS; _gv_j_3 = _gv_j_3 + 1) begin : gen_weights
	...
	genvar _gv_i_6;
	generate
		for (_gv_i_6 = 0; _gv_i_6 < CLIENTS; _gv_i_6 = _gv_i_6 + 1) begin : gen_debug_percentages
...
module arbiter_wrr_pwm_monbus (
	...
	input wire clk;
	input wire rst_n;
	input wire [CXMTW - 1:0] cfg_arb_max_thresh;
	input wire [CLIENTS - 1:0] request;
	output wire grant_valid;
	...
	generate
		if (USE_MONITOR) begin : gen_monitor
			...
			assign debug_ack_timeout = 1'sb0;
			assign debug_protocol_violations = 16'h0000;
			assign debug_grant_efficiency = 16'h0000;
			assign debug_client_starvation = 1'sb0;
			assign debug_fairness_deviation = 16'h0000;
