...
module formal_axi_monitor_trans_mgr_banked (
    input logic clk,
    input logic rst_n
);
    ...
    // =========================================================================
    // BANKED WRITE configuration -- the quadrant the default proof never
    // reached. The sibling harness proves IS_READ=1, NUM_BANKS=1, depth 2;
    // both banking defects found in 2026-08 lived on the WRITE path with
    // NUM_BANKS>1, so every property here passed while the design was wrong.
    // Depth 4 over 2 banks is the smallest split that still exercises
    // per-bank allocation and the same-bank age compare.
    ...
    axi_monitor_trans_mgr #(
        ...
        .ID_WIDTH         (ID_WIDTH),
        .IS_READ          (1'b0),      // WRITE path
        .NUM_BANKS        (NUM_BANKS),
        .USE_WDATA_ORDER_Q(1'b1),      // required when banked (elaboration guard)
        .IS_AXI           (1'b1),
