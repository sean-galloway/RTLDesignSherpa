...
module axi_master_wr_splitter (
	...
	output reg [UW - 1:0] m_axi_awuser;
	...
	input wire [IW - 1:0] fub_awid;
	input wire [AW - 1:0] fub_awaddr;
	input wire [7:0] fub_awlen;
	input wire [2:0] fub_awsize;
	input wire [1:0] fub_awburst;
	input wire fub_awlock;
	input wire [3:0] fub_awcache;
	input wire [2:0] fub_awprot;
	input wire [3:0] fub_awqos;
	input wire [3:0] fub_awregion;
	input wire [UW - 1:0] fub_awuser;
	input wire fub_awvalid;
	output reg fub_awready;
	input wire [DW - 1:0] fub_wdata;
	input wire [(DW / 8) - 1:0] fub_wstrb;
	input wire fub_wlast;
	input wire [UW - 1:0] fub_wuser;
	input wire fub_wvalid;
	output wire fub_wready;
	output reg [IW - 1:0] fub_bid;
	output reg [1:0] fub_bresp;
	output reg [UW - 1:0] fub_buser;
	output reg fub_bvalid;
	...
	reg [IW - 1:0] r_orig_awid;
	reg [AW - 1:0] r_orig_awaddr;
	reg [7:0] r_orig_awlen;
	reg [2:0] r_orig_awsize;
	reg [1:0] r_orig_awburst;
	reg r_orig_awlock;
	reg [3:0] r_orig_awcache;
	reg [2:0] r_orig_awprot;
	reg [3:0] r_orig_awqos;
	reg [3:0] r_orig_awregion;
	reg [UW - 1:0] r_orig_awuser;
	reg [AW - 1:0] r_current_addr;
	reg [7:0] r_current_len;
	reg [7:0] r_split_count;
	reg [7:0] r_expected_beats;
	reg [7:0] r_beat_counter;
	reg r_data_splitting;
	reg r_aw_issued;
	reg [7:0] r_expected_response_count;
	reg [7:0] r_received_response_count;
	reg r_waiting_for_responses;
	reg [IW - 1:0] r_original_txn_id;
	reg [1:0] r_consolidated_resp_status;
	reg [UW - 1:0] r_consolidated_buser;
	...
	wire w_split_boundary_reached;
	reg w_generate_wlast;
	...
	always @(posedge aclk or negedge aresetn)
		if (!aresetn) begin
			...
			r_orig_awlen <= 1'sb0;
			r_orig_awsize <= 1'sb0;
			r_orig_awburst <= 1'sb0;
			r_orig_awlock <= 1'sb0;
			r_orig_awcache <= 1'sb0;
			r_orig_awprot <= 1'sb0;
			r_orig_awqos <= 1'sb0;
			r_orig_awregion <= 1'sb0;
			r_orig_awuser <= 1'sb0;
			r_expected_beats <= 1'sb0;
			r_beat_counter <= 1'sb0;
			r_data_splitting <= 1'b0;
			r_aw_issued <= 1'b0;
			r_expected_response_count <= 1'sb0;
			r_received_response_count <= 1'sb0;
			r_waiting_for_responses <= 1'b0;
			r_original_txn_id <= 1'sb0;
			r_consolidated_resp_status <= 2'b00;
			r_consolidated_buser <= 1'sb0;
		end
		else begin
			case (r_split_state)
				2'b01:
					if (((fub_awvalid && m_axi_awready) && !block_ready) && !r_waiting_for_responses) begin
			...
			if (m_axi_awvalid && m_axi_awready)
				r_aw_issued <= 1'b1;
			if ((fub_wvalid && fub_wready) && fub_wlast)
				r_aw_issued <= 1'b0;
	...
	always @(*) begin
		...
		case (r_split_state)
			2'b01: m_axi_awvalid = (fub_awvalid && !block_ready) && !r_waiting_for_responses;
			2'b10: m_axi_awvalid = 1'b1;
		...
		case (r_split_state)
			2'b01:
				if (w_new_split_needed)
					...
					fub_awready = (m_axi_awready && !block_ready) && !r_waiting_for_responses;
			2'b10: fub_awready = (w_is_final_split && m_axi_awready) && !block_ready;
			default: fub_awready = 1'b0;
		endcase
	end
	always @(*) begin
		if (_sv2v_0)
	...
	wire w_is_final_response;
	assign w_is_final_response = (r_received_response_count + 8'd1) >= r_expected_response_count;
	wire [1:0] w_resp_with_current;
	assign w_resp_with_current = (m_axi_bresp > r_consolidated_resp_status ? m_axi_bresp : r_consolidated_resp_status);
	always @(*) begin
		if (_sv2v_0)
			;
	...
	always @(*) begin
		...
		if (r_split_state == 2'b01)
			split_fifo_din = {fub_awaddr, fub_awid, (w_new_split_needed ? 8'd2 : 8'd1)};
		else
	...
	always @(posedge aclk)
		if (aresetn) begin
			if (((r_split_state == 2'b01) && fub_awvalid) && w_new_split_needed)
				;
			...
			if (r_split_state == 2'b10)
				;
			if (r_waiting_for_responses) begin
				if (m_axi_bvalid && !w_is_final_response)
					;
				if (m_axi_bvalid && !w_is_final_response)
					;
			end
			if (w_split_fifo_valid)
				;
			if (r_data_splitting && m_axi_wvalid)
				;
			if (((r_data_splitting && m_axi_wvalid) && m_axi_wready) && m_axi_wlast)
				;
		end
	initial _sv2v_0 = 0;
endmodule
