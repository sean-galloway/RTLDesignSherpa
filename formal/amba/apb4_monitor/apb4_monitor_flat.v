...
module apb4_monitor (
	...
	generate
		if (USE_MONITOR) begin : gen_monitor
			...
			always @(posedge aclk)
				if (!aresetn) begin
					...
					if (w_cmd_handshake && w_has_free_slot) begin
						...
						r_trans_table[w_free_idx][277-:3] <= 3'h1;
					...
					if (w_rsp_handshake && w_has_active_trans) begin
						...
						if (rsp_pslverr)
							r_trans_table[w_active_idx][221] <= 1'b1;
			...
			assign w_fifo_pkt_data = monitor_common_pkg_create_monitor_packet(w_fifo_rd_data[47-:4], 4'h2, {4'h0, w_fifo_rd_data[43-:4]}, 9'h000, UNIT_ID, AGENT_ID, {24'h000000, w_fifo_rd_data[7-:8], w_fifo_rd_data[39-:32]});
