# Suspected Bug: apb4_slave_cg Wakeup Latency

## Summary

The `apb4_slave_cg` module may gate the clock mid-transaction under specific
conditions because the wakeup signal path has **2 cycles of register delay**
before the clock gate ungates.

## The Path

1. `apb4_slave_cg.r_wakeup` is a flop clocked by the ungated `pclk`:
   ```
...
- Cycle 1: `apb4_slave_cg.r_wakeup` goes high (one flop delay)
- Cycle 2: `amba_clock_gate_ctrl.r_wakeup` goes high (second flop delay)
- Cycle 3: `clock_gate_ctrl` ungates the clock

This means PSEL is held for **3 cycles** before the underlying apb4_slave sees
