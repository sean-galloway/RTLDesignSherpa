...
| converters/wb4_to_axil4_core | 2 | PASSING | prove/cover: write takes AW+W together, one direction per command, the consumed response is the head's channel AND is backed by a real response there, open bound, OKAY/err mapping, RTY never produced; 6 covers; 2 mutations FAIL (the backing property was ADDED after a mutation passed the earlier set) |
| wb4_retry          | 2        | PASSING | prove/cover at INFLIGHT=1 (payload equality of every re-issue, 1+retries issues, budget, delay, RTY to the FUB only when spent) and INFLIGHT=2 (bounds, in-order hand-back); covers: retry-then-ACK, exhausted, pass-through, delayed re-issue |
| wb4_master_cg      | 2        | PASSING | glue contract with a clock-enable model of the icg (derived clock not provable in the single-clock flow): reset, no gating with CYC/response visible/enable low, cmd_ready masked, bounded wake; 4 covers; mask and wake-term mutations FAIL |
| wb4_slave_cg       | 2        | PASSING | same model: bounded wake (3rd clock of CYC runs), STALL high while gated, no termination / FUB handshake while gated; 4 covers; STALL-mask and wake-term mutations FAIL |
| wb4_slave_cdc      | 2        | PASSING | single-clock model (both domains on one clock, as apb4_slave_cdc): no over-termination through the crossing, commands bounded by accepts, one-hot, open bound, payload integrity both ways; 5 covers; scrambled-payload mutations FAIL (a fabricated response is absorbed by the base slave's orphan guard, by design) |
