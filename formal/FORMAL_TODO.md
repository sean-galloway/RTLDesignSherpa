...
Generated by running every task, not by hand:

    python3 bin/formal_status.py --areas amba cdc common integ_common --markdown

Scope is repo-root `rtl/` only -- `formal/{amba,cdc,common,integ_common}`. The
project areas (converters, stream, rapids, bridge, apb_xbar, apbx_xbar) are not
counted here.

| Area | PASS | FAIL | ERROR | slow (> 15 min budget) | no .sby | Total |
|------|------|------|-------|------------------------|---------|-------|
...
| | start of day | now |
|---|---|---|
| AMBA tasks that could not elaborate | 16 | 0 |
| tasks proving a hand-copied FORK of the RTL | 30 | 0 |
...
Verilog-2005 portability bug in math_bf16_log2, and -- from retiring the
splitter forks -- a real AXI A3.3.1 violation in axi_master_rd_splitter
(TASK-094). Three of the four `*_mon` covers had never been reachable since
the day they were written (TASK-093).
