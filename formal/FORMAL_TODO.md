...
Generated by running every task, not by hand. Regenerate with:

    python3 bin/formal_status.py --areas amba cdc common integ_common --markdown

Scope is repo-root `rtl/` only -- `formal/{amba,cdc,common,integ_common}`.
The project areas (converters, stream, rapids, bridge, apb_xbar, apbx_xbar)
are not counted here.

| Area | PASS | FAIL | ERROR | slow (>10 min) | no .sby | Total |
|------|------|------|-------|----------------|---------|-------|
| common (non-math) | 50 | 0 | 0 | 2 | 2 | 54 |
| common (math) | 163 | 0 | 0 | 7 | 0 | 170 |
| cdc | 11 | 0 | 0 | 1 | 0 | 12 |
| amba | 51 | 6 | 0 | 0 | 4 | 61 |
| **Total** | **275** | **6** | **0** | **10** | **6** | **297** |

**"slow" is not a failure.** Those ten exceed a 600 s per-task budget: three
FIFO/skid proofs and seven multiplier proofs whose BMC depth makes them
genuinely long. They are listed so a CI budget can be set deliberately rather
than discovered.

**"no .sby" is not all gaps.** Two are deliberate: the SECDED encode and
decode directories hold a README pointing at the combined roundtrip proof in
`dataint_ecc_hamming`. The other four are the `axi4_*_mon_cg` wrappers, whose
blocker was lifted and whose proofs are unwritten -- TASK-090.

### Not passing

```
amba/apb4_master_cg: FAIL
amba/apb5_master_cg: FAIL
amba/axi4_master_rd_mon: FAIL
amba/axi4_master_rd_mon_cg: NOSBY
amba/axi4_master_wr_mon: FAIL
amba/axi4_master_wr_mon_cg: NOSBY
amba/axi4_slave_rd_mon: FAIL
amba/axi4_slave_rd_mon_cg: NOSBY
amba/axi4_slave_wr_mon: FAIL
amba/axi4_slave_wr_mon_cg: NOSBY
cdc/gaxi_skid_buffer_async: TIMEOUT
common/dataint_ecc_hamming_decode_secded: NOSBY
common/dataint_ecc_hamming_encode_secded: NOSBY
common/fifo_sync: TIMEOUT
common/gaxi_fifo_sync: TIMEOUT
common/math_ieee754_2008_fp16_mantissa_mult: TIMEOUT
common/math_multiplier_dadda_4to2_011: TIMEOUT
common/math_multiplier_dadda_tree_016: TIMEOUT
common/math_multiplier_dadda_tree_032: TIMEOUT
common/math_multiplier_wallace_tree_016: TIMEOUT
common/math_multiplier_wallace_tree_csa_016: TIMEOUT
common/math_multiplier_wallace_tree_csa_032: TIMEOUT
```

Every one is filed: TASK-091 (the two `*_master_cg` wake-latency failures) and
TASK-093 (the four `*_mon` covers, unreachable since they were written and
exposed when the area finally got a driver). TASK-092 covers fifteen harnesses
that pass while pinning a DUT input at a constant, which this table cannot
show.

### What the previous version of this table claimed

| | claimed 2026-08-09 | measured 2026-09-11 |
|---|---|---|
| amba errors | 0 | 16 at the start of the day, 0 now |
| common (math) errors | 7 | 0 |
| CI | "make formal-common in coverage.yml" | there is no coverage.yml |

The amba count was not a small drift. Sixteen tasks could not elaborate, and
nothing surfaced it because `make formal` had no amba target at all.
...
- [x] Formal directory structure: formal/{common,cdc,amba,integ_common,bridge,
      stream,converters,apb_xbar,apbx_xbar}/ -- nine areas, not the six this
      list used to name
- [x] Per-module pattern: formal_*.sv (wrapper) + *.sby (config) + Makefile (for sv2v modules)
- [x] Root Makefile targets: make formal, formal-rtl, formal-common, formal-cdc,
      formal-amba, formal-converters, formal-bridge, formal-quick
- [x] .gitignore for sby output directories
- [x] env_python updated with OSS CAD Suite + sv2v paths
- [x] **formal/amba/Makefile (2026-09-11)** -- there was none, and no
      formal-amba target, so sixty AMBA task directories had no entry point.
- [x] **Task sets DISCOVERED, not listed (2026-09-11)** -- both area Makefiles
      derive from `$(wildcard */*.sby)`. common's hand-maintained list ran 34
      of 222 proofs; nothing about the gap looked like a failure.
- [x] **Stripped-copy proofs migrated (2026-09-11)** -- the three arbiter
      monbus tasks each proved a hand-copied fork of the RTL, 251 to 287 lines
      divergent, plus a cut-down package stub. Forks deleted; they flatten the
      real modules now.
- [x] **Status is measured (2026-09-11)** -- `bin/formal_status.py` runs the
      suite and exits non-zero on failure; `bin/formal_audit_stimulus.py`
      finds harnesses pinning a DUT input at a constant.
...
holds filelist-checks.yml and track-clones.yml only. Nothing in CI runs a
proof, which is part of why so much of the suite could sit unrunnable.
