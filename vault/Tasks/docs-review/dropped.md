...
# docs-review — Dropped (ended without completing)

## DOCREV-001 — Integrate the outstanding Kimi accuracy findings — DROPPED 2026-07-28

**Why dropped:** the review corpus was reset on 2026-07-28 (Sean). Seven cdc
rounds in two days (k3 rounds 4-10) had not converged, and the backlog this
task tracked was being re-litigated rather than closed. All prior rounds are
archived at `~/rtl-doc-review/archive-pre-reset-2026-07-28/` (off-repo) and
removed from the tree (`docs/review/kimi/` survives in git history). Fresh
per-area rounds under the tightened reviewer brief + second-model
adjudication supersede backlog integration — see DOCREV-013 for the area
order and per-area startup checklist.

The task's full text is preserved below as historical record (the common,
cdc, and math integrations it documents DID land in the tree — only the
un-integrated remainder is abandoned).
