...
### PUMICE-018: scrub the tests for completeness (pumice)
...
**Scope:** `projects/components/memory-controllers/pumice-ddr2-lpddr2/dv/tests/` -- 31 test files across fub/macro/top.
...
**Area-specific:** pumice has the repo's strongest evidence that a passing
sim can hide a board failure -- the DFI write-timing and x16 column-overlap
bugs both passed in simulation until DFISlavePHY was hardened to model the
real device. Prioritise the tests whose BFM is a model of hardware rather than
a checker of protocol.
