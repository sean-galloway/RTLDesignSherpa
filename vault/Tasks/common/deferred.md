...
# common — Deferred (parked pending a named condition)

---

## COMMON-007 — Additional arbiter types
**Status:** DEFERRED — only the hierarchical (grouped two-level) item
remains. **Un-defers when:** a consumer names its client GROUPS and the
fairness intent (group-fair isolation vs client-fair) — e.g. per-port
channel groups in a multi-port DMA. Moved to this page 2026-08-11 when the
deferred state was added to the convention (this task was its motivating
case). Deferral re-confirmed by Sean 2026-08-11 after
