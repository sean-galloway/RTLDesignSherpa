---
title: RLB/gpio tasks
summary: Task rollup for the GPIO block (retro_legacy_blocks/rtl/gpio).
---

# RLB/gpio — task rollup
...
Block: the GPIO. RTL under
`projects/components/retro_legacy_blocks/rtl/gpio/`; MAS spec under
`projects/components/retro_legacy_blocks/docs/gpio_mas/`.
...
IDs are scoped to THIS sub-area AND its lane: `RLB/gpio BUG-001` and
`RLB/hpet BUG-001` are different bugs. Cite one from outside as "RLB/gpio BUG-001".
