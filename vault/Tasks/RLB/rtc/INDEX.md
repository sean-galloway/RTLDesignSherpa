---
title: RLB/rtc tasks
summary: Task rollup for the real-time clock block (retro_legacy_blocks/rtl/rtc).
---
...
Block: the real-time clock. RTL under
`projects/components/retro_legacy_blocks/rtl/rtc/`; MAS spec under
`projects/components/retro_legacy_blocks/docs/rtc_mas/`.
...
IDs are scoped to THIS sub-area AND its lane: `RLB/rtc BUG-001` and
`RLB/hpet BUG-001` are different bugs. Cite one from outside as "RLB/rtc BUG-001".
