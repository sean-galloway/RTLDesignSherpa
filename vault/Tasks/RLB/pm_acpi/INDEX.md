---
title: RLB/pm_acpi tasks
summary: Task rollup for the ACPI power management block (retro_legacy_blocks/rtl/pm_acpi).
---

# RLB/pm_acpi — task rollup
...
Block: the ACPI power management. RTL under
`projects/components/retro_legacy_blocks/rtl/pm_acpi/`; MAS spec under
`projects/components/retro_legacy_blocks/docs/pm_acpi_mas/`.
...
IDs are scoped to THIS sub-area AND its lane: `RLB/pm_acpi BUG-001` and
`RLB/hpet BUG-001` are different bugs. Cite one from outside as "RLB/pm_acpi BUG-001".
