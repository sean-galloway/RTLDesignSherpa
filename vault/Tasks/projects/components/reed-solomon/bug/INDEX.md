<!-- Managed by the `tasks` convention: see /vault/Tasks/INDEX.md. -->

# projects/components/reed-solomon — bugs

**Next ID: BUG-001** — never recycle a number, even when its entry closed.

A DEFECT with a reproduction: something behaves wrongly and we can say what correct looks like. If you cannot state the expected behaviour, it is an ISSUE, not a bug.

| Page | Count | What |
|---|---|---|
| [open.md](open.md) | 1 | accepted, not started |
| [active.md](active.md) | 0 | in progress right now |
| [closed.md](closed.md) | 0 | done (kept for history) |
| [dropped.md](dropped.md) | 0 | ended without completing |

## Open

- **BUG-000** — reserved template; copy its shape, do not file against it.
