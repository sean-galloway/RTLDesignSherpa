<!-- Managed by the `tasks` convention: see /vault/Tasks/INDEX.md. -->

# projects/components/reed-solomon — tasks

**Next ID: TASK-001** — never recycle a number, even when its entry closed.

Planned work we have decided to do: a feature, a refactor, a migration, a cleanup. It starts from intent, not from a failure.

| Page | Count | What |
|---|---|---|
| [open.md](open.md) | 1 | accepted, not started |
| [active.md](active.md) | 0 | in progress right now |
| [closed.md](closed.md) | 0 | done (kept for history) |
| [dropped.md](dropped.md) | 0 | ended without completing |

## Open

- **TASK-000** — reserved template; copy its shape, do not file against it.
