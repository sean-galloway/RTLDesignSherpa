...
| `stream/stream_core` | RC=0, 344726B | RC=0, 0 AST_AUTOWIRE | blocked -> [[TASK-091]] |
| `stream/datapath_wr_test` | RC=0, 100844B | RC=0, 0 AST_AUTOWIRE | **FAIL** -> [[TASK-092]] |
...
[[TASK-091]] (stream_core's DEPS rotted behind the monitor rework) and
[[TASK-092]] (datapath_wr_test's proof genuinely fails once it can elaborate).

**Not folded in:** the `.sv2v_prep` tracked-artifact problem this task noted is
[[TASK-090]]. The other two "Regen FAILED" entries in `formal/FORMAL_TODO.md`
...
The gate covers 26 entries / 18 distinct RDLs now: the 9 from [[TASK-088]]
