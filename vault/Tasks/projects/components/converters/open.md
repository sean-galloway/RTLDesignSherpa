...
## CONV-005 — dwidth rd RRESP fold uses bitwise OR; SLVERR|EXOKAY inflates to DECERR
**Status:** open 2026-08-24 (qc round_2, verified against RTL)
**Priority:** P2 — reachable but requires exclusive-access traffic

`axi4_dwidth_converter_rd`'s R-path upsize folds RRESP across sub-beats
with `SB_OR_MODE(1)`. With the AXI encoding (OKAY=00, EXOKAY=01,
SLVERR=10, DECERR=11), OR is not worst-case: `SLVERR | EXOKAY = DECERR`.
The write side's B fold uses numeric max, so the two response paths carry
different severity semantics.

Reachable: ARLOCK passes through (`m_axi_arlock = int_arlock`), so
exclusive reads and their EXOKAY responses are legal traffic.

**Work:**
- [ ] RED test first: exclusive read whose sub-beats mix EXOKAY with
      SLVERR; assert the wide RRESP is SLVERR, not DECERR.
- [ ] Fix with a severity-ordered fold (numeric max), not OR — either a
      max mode in `axi_data_upsize`'s sideband handling or RRESP handled
      outside the sideband path.
- [ ] Decide and document the semantics of a wide beat whose sub-beats
      mix EXOKAY and OKAY (spec-wise all beats of an exclusive burst
      should match; say what the fold does anyway).
