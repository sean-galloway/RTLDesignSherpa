...
**"Macro-free fork" is not a reason.** Two trees claimed it and neither
survived contact with evidence. `timing_characterization/rtl/asic_only/` was a
37-file duplicate of `rtl/` differing ONLY in reset spelling; synthesised
through Yosys + slang against ASAP7 it produced a byte-identical mapped netlist
(396 cells, 97 flops, every cell type matching), and the flow reading it still
passed `-DUSE_ASYNC_RESET`, a define meaningful only to the macro tree it was
cloned from. `formal/cdc/cdc_handshake/cdc_handshake_formal.sv` claimed Yosys
could not take the macro, while 92 other .sby units stage `reset_defs.svh` and
read it with `-Iincludes`; its own .sby had simply never staged it, and it now
proves PASS with the macro. The macros exist to serve ASIC and FPGA both --
that is the whole point of them -- so a second copy of a module to spell reset
differently is duplication, not portability. Deleted 2026-09-07.
